# Lumen Bioscience demo seed kit

This kit is intended to help you **seed a demo tenant/client** in your platform for **Lumen Bioscience, Inc.** with:

- A **Client/Tenant** record
- A **primary user** (Brian Finrow, CEO & Co-founder) with a **license assignment**
- A populated **program + study pipeline** (programs, trials, milestones, and external documents)

The included seed data is sourced from **publicly available information** (company website, trial listings, and trial protocol PDFs). Use only in non-production/demo environments.

## Contents

- `seed-data/lumen_bioscience.seed.json` - the seed payload (client, users, pipeline)
...
export PRIMARY_USER_EMAIL="brian.finrow+demo@yourcompany.com"
...
- Upsert by a natural key (client `slug`, user `email`, program `code`, study `registry_id`, document `source_url`)
