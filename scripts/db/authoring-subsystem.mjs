 ...
 * Part 11 evidence those same operations MUST produce; the permissions file
 * makes object-level access durable and seeds each creator as OWNER + AUTHOR.
 * Provisioning any of those pieces alone yields a partially governed surface,
 * which is WORSE than tables plainly absent. The five files therefore apply in
 ...
 *
 * TENANT ISOLATION IS PART OF THE UNIT. The five .sql files create tables but no
 * universal install-fresh guarantee exists for an already-provisioned database.
 * On the install-fresh path, 0021_enable_rls_everywhere runs after this and
 * dynamically policies every tenant table. On apply-c2c, 0021 has already run
 * and will not revisit new tables. This helper therefore installs the SAME
 * `tenant_isolation_policy` 0021 would, making the subsystem tenant-isolated
 * wherever it is provisioned. It is idempotent with migration-local policies.
 *
 ...
 * creates; permissions references documents and sections, so it MUST be last.
...
/**
 * Tables the five files create. This is the provisioning-side readiness
 * contract; server/db/ensureCoreTables.ts must keep the same list so a database
 * cannot report authoring ready while its permission store is absent.
 */
...
/**
 * Tenant-consistent parentage constraints installed by the subsystem. Four bind
 * working-content children to same-tenant parents; two bind permissions to the
 * declared document and optional section. A subsystem whose tables exist
 * without these constraints is not correctly provisioned.
 */
