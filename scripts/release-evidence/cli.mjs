...
if (command === 'generate') {
  ...
  const index=['# Release Evidence Index','',`Commit: \`${commit}\`  `,`Tree: \`${tree}\`  `,`Manifest SHA-256: \`${sha256(body)}\``, '', '## Automated engineering evidence', ...artifacts.map(a=>`- ${a.id}: **${a.status}** — \`${a.sha256}\` (${a.path})`), ...jobs.map(j=>`- ${j.workflow} / ${j.job}: **${j.conclusion}**`),'','## Human decisions (separate governed process)',...manifest.humanApprovals.map(a=>`- ${a.role}: **unapproved**`),'',`> ${DISCLAIMER}`,''].join('\n');
