...
/**
 * Single Batch Import for TrialSage
 *
 * This script imports a single batch of 100 Health Canada trials
 * and updates the tracker file for the next run.
 */
...
// Configuration
const BATCH_SIZE = 100; // Number of reports per batch - one batch at a time
...
// Tracking file
const TRACKING_FILE = 'batch_progress.json';
...
function getTrackingData() {
  ...
  return {
    nextId: 3000, // Start from 3000 to avoid conflicts with existing IDs
    batchesCompleted: 0,
    totalImported: 0,
    lastRunTime: null,
  };
...
// Import a single batch of trials to the database
...
async function runSingleBatch() {
  try {
    console.log('Starting single batch import process...');
...
Target: ${TARGET_TOTAL} Health Canada trials
Progress: ${Math.round((currentHCCount / TARGET_TOTAL) * 100)}%
    `);
    ...
    // Calculate how many more trials we need
...
Starting ID: HC-${trackingData.nextId}
    ...
    const result = await importBatch(trials);
    console.timeEnd('Batch processing');

    // Update tracking data
    trackingData.nextId += batchSize;
    trackingData.batchesCompleted += 1;
    trackingData.totalImported += result.importedCount;
    trackingData.lastRunTime = new Date().toISOString();
...
Target: ${TARGET_TOTAL} Health Canada trials
Progress: ${Math.round((newHCCount / TARGET_TOTAL) * 100)}%
