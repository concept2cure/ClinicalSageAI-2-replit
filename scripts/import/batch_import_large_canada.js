...
/**
 * Large Scale Health Canada Batch Import for TrialSage
 *
 * This script imports large batches of Canadian clinical trials.
 * It uses efficient batch processing and transaction management to
 * handle thousands of trials while maintaining database consistency.
 */
...
// Configuration
const BATCH_SIZE = 1000; // Number of trials per batch - process 1000 at a time
const TRANSACTION_SIZE = 250; // Number of trials per transaction - commit in larger chunks
const TARGET_COUNT = 5000; // Target total number of trials from Health Canada
...
async function importTrialsToDatabase(trials) {
  ...
  try {
    ...
    for (let i = 0; i < trials.length; i += TRANSACTION_SIZE) {
      ...
      logger.info(`Processing batch ${i / TRANSACTION_SIZE + 1} (${i} to ${batchEnd - 1})...`);
      ...
      try {
        ...
        logger.error(`Error during batch ${i / TRANSACTION_SIZE + 1}:`, error.message);
...
async function runLargeBatchImport() {
  logger.info('Starting large-scale Health Canada trial import process...');
