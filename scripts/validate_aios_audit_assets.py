...
from aios_controls_parser import parse_control_ids, parse_exit_gates
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--metrics",
        default="docs/aios/sample_evidence_metrics.json",
...
def main() -> int:
    ...
    sample_metrics_path = root / args.metrics
    sample_pack_path = root / args.evidence_pack

    if not controls_path.exists():
        raise SystemExit(f"Missing controls YAML: {controls_path}")
    if not sample_metrics_path.exists():
        raise SystemExit(f"Missing sample metrics JSON: {sample_metrics_path}")
    if not sample_pack_path.exists():
        raise SystemExit(f"Missing sample evidence pack markdown: {sample_pack_path}")
    ...
    sample_pack = sample_pack_path.read_text(encoding="utf-8")
    for token in EXPECTED_SECTIONS:
        if token not in sample_pack:
