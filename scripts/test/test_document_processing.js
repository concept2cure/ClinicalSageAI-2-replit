...
async function testDocumentProcessing() {
  logger.info('🧪 Testing Enhanced Unified Document Processing System...\n');
  ...
  for (const filename of testFiles) {
    ...
    try {
      ...
      logger.info(`   📋 Document Type: ${result.documentType?.primaryType || 'Unknown'}`);
      logger.info(`   🔍 Confidence: ${(result.documentType?.confidence * 100 || 0).toFixed(1)}%`);
      logger.info(`   📊 Structure Quality: ${result.structure?.structureQuality || 'N/A'}`);
      logger.info(`   📄 Sections Found: ${result.structure?.totalSections || 0}`);
      logger.info(`   🧠 AI Analysis: ${result.aiAnalysis ? 'Complete' : 'Pending'}`);
      logger.info(`   💾 Document ID: ${result.documentId || 'N/A'}`);
      ...
      if (result.structure?.sections?.length > 0) {
        ...
        result.structure.sections.slice(0, 3).forEach(section => {
          logger.info(`      - ${section.type}: "${section.heading}" (Line ${section.lineNumber})`);
        });
        if (result.structure.sections.length > 3) {
          logger.info(`      ... and ${result.structure.sections.length - 3} more sections`);
        }
      }
      ...
      logger.info(`❌ Failed to process: ${error.message}`);
      testResults.push({
        filename,
        success: false,
        error: error.message,
  ...
  if (successful > 0) {
    ...
    testResults
      ...
      .forEach(r => {
        logger.info(`   ${r.filename}: ${r.documentType} (${(r.confidence * 100).toFixed(1)}%)`);
      });
  }
  ...
  if (successful > 0) {
    ...
    testResults
      ...
      .forEach(r => {
        logger.info(`   ${r.filename}: ${r.sectionsFound} sections detected`);
      });
  }
  ...
  try {
    ...
    logger.info('❌ Tesseract OCR not available:', error.message);
  }
