#!/usr/bin/env python3
"""
Fixed parsing script that correctly handles success field
"""
import os
import json
import pandas as pd
...
for fname in os.listdir(csr_dir):
    ...
    try:
        ...
        # Fix for success detection logic
        is_success = int("significant" in outcome or 
                         "met endpoint" in outcome or 
                         "primary endpoint met" in outcome or
                         "endpoint met" in outcome)
        ...
        record = {
            ...
            "success": is_success,
            "failure_reason": csr.get("failure_reason", outcome) if not is_success else "N/A"
        }
