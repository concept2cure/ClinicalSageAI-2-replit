...
logger.info(`Created backup of package.json at ${backupPath}`);
...
if (!currentDevScript.includes('node cleanup-toastify.js')) {
  ...
  logger.info('Updated dev script to include toast cleanup');
}
...
if (!currentBuildScript.includes('node cleanup-toastify.js')) {
  ...
  logger.info('Updated build script to include toast cleanup');
}

// Write the updated package.json
fs.writeFileSync(packageJsonPath, JSON.stringify(packageJson, null, 2));
logger.info('Package.json updated successfully!');
logger.info('Run "npm run dev" to start the application with permanent toast fixes.');
