...
- **2026-05-29** — **Phase 11 Intelligence backend foundation.** Added migration `migrations/20260529_phase11_intelligence.sql` creating the two minor tables the kit specifies — `c2c_tlf_builds` (Biostat TLF queue) and `c2c_forecast_snapshots` (Reports timeline forecast), with the exact DDL from PHASE_11_INSTALL.md §4 (`regulatory_programs.id` confirmed `uuid`, `users.id` confirmed `serial`). Stood up `server/routes/intelligence-cluster.ts` mounted at `/api/intelligence` (flat paths — no collision with the RIM router's `/projects/:projectId/*`): `GET /protocol`, `/cmc`, `/biostat`, `/reports`, each returning the canonical `{ data }` envelope and **omitting any field with no rows** so the surface hooks keep their fixture for that section (no empty-table regressions, no 404s). Biostat serves the TLF queue from `c2c_tlf_builds`; Reports serves the forecast from `c2c_forecast_snapshots` (latest snapshot per project×milestone), both org-scoped via a join to `regulatory_programs`. Uses the `mdx-analytics` graceful-degrade pattern (`safeRows` swallows 42P01/42703). The Intelligence hooks now unwrap the `{ data }` envelope. **Deferred:** the install doc's §4 assumes `c2c_endpoint_library`, `c2c_manufacturing_sites`, `c2c_stability_studies` already exist — they do not, so protocol endpoints/amendments, CMC packages/stability/specs, SAPs, sample-size, interims, and Reports KPIs/bars/precedent-models stay on fixtures until those source tables are designed (a real data-modeling effort, not a port — flagged for the designer/eng).
