...
|15 | **Submission Gateway**  | **Ready to implement** | `ui_kits/submission/`   | Surfaces `mdx-submission-gateway.ts` — `submission` rail item. ACK chain + pre-flight validation |
|16 | **E-signature modal**   | **Ready to implement** | `ui_kits/esign/`        | Promotes `mdx/components/EsignModal.tsx` → `_shared/`; drives Part 11 engine + `/api/c2c/actions/sign` |
|17 | **ISO 14971 Risk Mgmt** | **Ready to implement** | `ui_kits/risk/`         | Surfaces `mdx-risk-management.ts` — severity×probability matrix |
|18 | **Pathway sub-tabs**    | **Ready to implement** | `ui_kits/pathway-tabs/` | Reusable Audit/Correspondence/Approvals strip per program; unblocks `pathwayTabs.ts` |
|19 | **Tasking + Messaging** | **Ready to implement** | `ui_kits/tasking/`      | `tasking` rail item — cross-program board + project messaging |
|20 | **Task tray**           | **Ready to implement** | `ui_kits/task-tray/`    | Persistent top-bar tasking affordance → `_shared/shell/TopBar.tsx` |
|21 | **CMC · Module 3**      | **Ready to implement** | `ui_kits/cmc/`          | Front door to the Module 3 OS (`server/api/cmc/*` + `server/services/cmc/*`) — §3.2.S/P build state, QbD CQA/CPP, ICH compliance, SUPAC, RPI. See `CMC_CODEBASE_FINDINGS.md` |
...
- **2026-08-24 (wave 5)** — **Zero blind surfaces: every routable screen now publishes honest state to AnA — 114 of 120 publish, 6 exempt with recorded reasons, 0 blind (`check-ana-surface-context` ID_BASELINE 68→114 exact, landed across five verified pushes).** The 46 remaining blind ids all gained `usePublishSurfaceContext` grounding. The two kit families publish from their single hosts via one context builder + per-id LITERAL gated calls (the UsageBilling shape — the gate counts literal ids; a dynamic id would ground AnA while the ratchet silently reported her blind): MdxSurfaceHost covers the 16 `device-*` ids (program identity only from `programForContext`'s honesty rule, no leaf counts — absent beats guessed), PdevApp the 8 `pdev*` ids (readiness with its SOURCE named — live report vs snapshot fallback vs an ambiguous zero stated as a fallback default; IND-assembly presence counts with the compile CTA's real disabled causes). The 22 singles keep the sharp edges sharp: the `inconsistency` submission gate publishes never-scanned as NOT clear and fails closed on a read failure with counts only in assessed states; `orchestration`'s dead checkpoint store publishes "NOT a report that zero human-in-the-loop gates are pending"; `ectd-publishing` leads every branch with "read-only; nothing on this screen publishes, transmits, or freezes a sequence"; `precedent-intelligence` publishes `searchRun:false` with nothing from a typed-but-unrun form; `licensing` names the free card's "Current" tag on a failed read as the fallback of the failure, not a finding; `labeling-pi` gates every count on its own `answerable` flag. Admin surfaces publish **aggregates only with hard security exclusions**: no SCIM bearer token, allowlist CIDRs, member emails, per-person MFA/SSO flags, role scopes, grants, key ids, audit chain hashes, setting values, cross-tenant rosters, or per-org refusal paths; `access-requests` is scope-guarded against its master-licensing mount; third-party notes never enter the prompt; `research-admin` (read lifted out of its early-returning child) states the load-bearing negative that four of its five sections are not connected. Also this wave: `ectd-coauthor.open-tab` (view-only where the human buttons auto-run server checks — pinned by a no-POST wiring test — and dirty-guarded because the editor unmounts), a Deep Research stop in the sales demo, and dedicated bus wiring tests backfilled for cmc / submission-center / project-home. Proof: `anaSeesScreens.test.tsx` pins publisher output through `useActiveSurfaceContext` (real counts, the fail-closed register, the identity-console forbidden state with a no-secrets payload scan, the orchestration gates-error words, the program journey identity); `anaDrivesWave5.test.tsx` 6 wiring cases; ~700 tests across the touched suites; `tsc` clean.

- **2026-08-24 (wave 4)** — **The deferred list cleared: AnA operates deep research, the quality module, and both editors' remaining affordances — registry now 34 actions across 15 surfaces.** Everything waves 2–3 recorded as deferred-with-reasons shipped, in the recorded order. **deep-research** publisher-first: its FIRST context publisher (credits, connector inventory, tab, running-job flag; loading and failure published as themselves), then `open-tab` — and a registry test now pins structurally that no `deep-research.*` action may ever carry a query/depth/connector/source param (the credit-spend pre-arm the surface's own header documents); refusals cover the open credential drawer, an in-flight credential save, and switching away from a running metered job. **quality** state lifts: `filter`/`stage`/`openId` and the change-register read lifted into QualityApp (one slot, one state), four actions (`open-tab`, `filter-register`, `filter-changes`, `open-change` — cross-tab moves switch tab first and say so; a filter that would hide the expanded row is cleared, stated; names resolve against the same rows the log renders, ambiguity refused), plus the module's FIRST publisher — no SOP counts (absent beats guessed on a GxP register), sample mode flagged, a failed read never an empty log; approve/revise/retire/advance/training stay conversational. **ectd-coauthor.open-document** unblocked by subscribing the editor's existing `onDirtyChange`: refuses over unsaved edits naming the §, refuses during a running validation/compliance check, holds through the load, resolves title-or-module-number, expands the target module — the same two setters as the human tree click. **authoring.find**: `openFind(query?)` joined `RichSectionEditorHandle` (false in source mode → honest refusal), opening the already-built find bar pre-seeded with plugin highlights; read-only, replace stays human. The wave-3-recorded artifacts-center gap closed: the docx Open button rides the same `authoring.open-document` directive AnA rides instead of navigating empty-handed. Governance regex grew failing-first: `advance` (Part 11 change ceremony) and `launch` (metered-spend start) refused — pinned on `quality.advance-change` / `deep-research.launch-research` before the pattern widened. Training demo gained a quality stop. `check-ana-surface-context` 66→68 exact. Wiring proof: `anaDrivesWave4.test.tsx` (9 cases incl. typed-into-the-real-canvas unsaved-edits refusal, find-bar DOM through the 'authoring' alias, the artifact Open relay, and quality's held-through-the-load apply); ChangeControl suite moved to the controlled-pane harness; affected suites + `tsc` clean. Still deliberately absent with reasons (doc §Wave 4): apps admin-toggle, any deep-research query/depth/source driving, cmc pane-form guards.

- **2026-08-24 (wave 2)** — **AnA now operates ten surfaces end to end; her screen context carries the operable vocabulary; demos act, not just visit.** Handlers + guards + ready signals wired on eight more surfaces against their REAL state (task board incl. the 'tasking'→'tasks' alias path, review board + its first board-aware context via the ONE ReviewThreadsPane publisher, CMC tabs, intelligence browser + its first publisher, document authoring with unsaved-edit/dialog refusals through the human's own requestLeave gate, eCTD tree search, submission center with e-sign/transition busy guards, project-home lifecycle stages). Bus resolves DEEP_LINK_ALIASES; mounted-but-loading surfaces HOLD directives (retry) until their ready signal; `screen_actions` (id+label+param enums) ride every AnA turn via module context and render in the sanitized OBSERVED SCREEN STATE block (a fabricated id still dies at the server registry). Governance hardened failing-first: freeze/dispatch added to the governed-verb refusal; the stale `intelligenceTab` enum replaced with the nine real catalog groups. Demos now open the CMC specifications register, switch intelligence groups, jump the review queue, filter the board, and open a real document mid-tour; third script `training-submission-day` rehearses dispatch day over the new submissions actions. `check-ana-surface-context` baseline 65→66 (global-ri publisher). Wiring proof: `anaDrivesWave2.test.tsx` (alias end-to-end, retry-held apply, honest refusals) + full affected suites + `tsc` clean. Deferred with recorded reasons (recon in `docs/ANA_LIVE_DRIVE_2026-08-23.md` §Expansion): apps admin-toggle (pre-arms a governed switch), deep-research (no publisher; credit-spend pre-arm), ectd-coauthor open-document (no dirty flag to guard), quality/dossier-map/labeling (state lifts or nothing drivable).

- **2026-08-24** — **AnA agentic abilities completed: she operates the screens, not just opens them — plus live product demonstrations (training + sales), interactive and fast.** Live Drive grew from navigation-only to the full agentic surface, with the governance line drawn structurally. **(1) Surface actions.** `shared/navigation/surface-actions.ts` is the governed catalog of UNGOVERNED screen operations (the clicks a person makes: open a program, filter/search, switch views/folders) with a fail-closed resolver mirroring the navigation contract; `GOVERNED_VERB_PATTERN` + `assertUngovernedActionId` refuse sign/approve/submit/lock/delete ids at registration AND resolution (test shown failing on `review.approve-document` et al.) — Part 11 propose-only gates remain the only path for governed work. New tools `list_screen_actions` / `act_on_screen` (instruction truthfully applied-vs-offered per drive context); `drive_action` SSE under a per-mode budget with `agent.ana.screen.act` audit rows; `surface_action` offer-chips on both tool routes. Client: `v2/surfaceActions.ts`, the ONE performer — re-validates against the shared registry, performs only through a handler the mounted surface registered (honest `unavailable`/`failed` otherwise), and bridges the navigate→mount gap with a one-shot TTL stash (navParams idiom). Wave-1 handlers on Projects (`open-program` resolves the real portfolio + publishes the shell project — AnA can now enter a program herself, unlocking every project-scoped screen; `filter`; `set-view`; all refused while the wizard owns the canvas) and Vault (`search`, `open-folder` against the real tree). **(2) Demonstrations.** `shared/navigation/demo-scripts.ts` ships `training-orientation` (13 stops) and `sales-flagship` (9 stops) as registry-validated plans (totality test shown failing on unknown screens/actions); `list_demo_scripts` / `start_product_demo` return the script + presenter instructions (own-voice narration, brisk pace, answer questions mid-demo then resume). `drive_mode:'demo'` rides the same `ana_live_drive` entitlement with budgets from the new SHARED policy table `drive-policy.ts` (assist 3/3 — nav budget still pinned to the chip budget; demo 12/16), raises the round ceiling to 16, and drops the substantive-thinking nudge for pace (high-risk + Thorough still reason). Rail Control menu gained a Demonstrations section; the drive strip gained demo framing, act steps (⚡), a steer field (mid-run interject — interactive without take-over), and the streamed-narration tail on owned surfaces so a demo stop on the editor is not silent. All budgets/caps live in shared and are imported by both halves — no hand-mirrored constants. Tests: registry totality + governance refusal + fail-closed resolution (shared), carriers, executor handlers, reducer modes/budgets, bus stash/TTL/ownership; full v2 client dir + ana server suites green; `tsc --noEmit` 0; `check-ana-surface-context` OK. Full write-up appended to `docs/ANA_LIVE_DRIVE_2026-08-23.md` (§Expansion 2026-08-24).

- **2026-08-23** — **Vault: the capture → classify → file pipeline shipped; uploads are no longer invisible in the tree they were uploaded into.** The platform captured durably (SHA-256, chained audit, versions) but routed nothing: real classifiers existed (`detectCTDSection`, the DIA TMF zone rules, the device tagger) and none ran on the paths customers use, `vault.documents` uploads were filed as `documentType='OTHER'` with no dossier position, and the v2 Vault tree never read the uploads store at all — a file uploaded from `/concept2cure/vault` did not appear in the tree that accepted it. Shipped on `concept2cure-v2`: **(1)** `server/services/vault/vault-filing.service.ts` — ONE deterministic classifier composing the existing CTD/TMF/device knowledge, keyed to the program's client-type vault view (`vault-taxonomy.ts` presets, now actually reachable: CTD modules for pharma/biotech, submission+DHF folders for device/IVD, TMF zones for CRO); fail-closed — an unplaceable file returns `folderId:null` + `needsReview` and lands in a visible Unfiled queue, never a guessed folder. **(2)** Placement columns on `vault.documents` (`migrations/20260823_vault_document_placement.sql`, guarded + additive, in `migration-set.mjs`): the classifier PROPOSES (`suggested`, with confidence + rationale), a person CONFIRMS or moves (`confirmed`, `placed_by/placed_at`) via the new governed `POST /api/c2c/project-vault/:id/file` (placement change + hash-chained Part 11 audit row in one transaction); a re-upload never overwrites a person's confirmed filing. **(3)** `/api/vault/ingest` classifies at ingest (inside the re-entered tenant scope), records the filing in the audit `details`, and returns it so the uploader sees where the file landed; explicit `folderId` in the body is validated against the view taxonomy (400 on invalid, `confirmed` on success). **(4)** `/api/chat/upload` stamps the same classification into `cre_evidence_sources.metadata.dossier` — AnA-paperclip and Project-Home drop-zone captures arrive pre-classified. **(5)** The Vault read-model merges the uploads into the tree as the **filing cabinet** (every preset folder of the view, an always-visible Unfiled queue) and adds the **data room lane**: every captured source with its DERIVED stage — captured → classified → filed, where 'filed' is a checksum join against the program's vault documents, computed not stored — with four honest states (pending store / failed-read-≠-empty / empty / real). **(6)** `Vault.tsx` renders all of it: program badge, cabinet, data room strip + source list, per-upload filing block (folder, kind, CTD section, confidence, rationale) with Confirm / Move / Unfile actions that re-read the tree after the server commits; `useVaultUpload` reports "auto-filed X → Module 3 (suggested until confirmed)" from the server's stored outcome. Registry `vault` entry updated to the real APIs. Tests: classifier totality (every rule folder exists in its view preset) + fail-closed proof, cabinet merge regression, migration proof on pglite (silent no-op without vault schema; 8 columns + 2 indexes land; idempotent), surface tests for error-≠-empty on both the tree and the data room, and hook tests for the filing-aware copy.

- **2026-08-23** — **AnA capture: drafts and "save to vault" now file under the real project — or say plainly that they cannot.** Two capture paths silently lost or mis-filed AnA's work. `save_document_to_vault` omitted `project_id` (integer NOT NULL) from its INSERT, so AnA's explicit "file this" failed on every real call — while its pglite contract test passed because the test's own DDL omitted the column; the table now matches production (`project_id int NOT NULL`), the handler files under `ctx.projectId` and refuses with an honest message when no project is open, and a new REFUSES case pins it (verified by reverting the fix: 4 tests fail against the old code). `persistCollectedDrafts` guarded with `Number.parseInt(streamProjectId)` — the ADR-0011 Class-A coercion: a program UUID beginning with digits parsed to those digits and filed the draft under a valid, WRONG integer project; a UUID beginning with a letter was silently dropped while the rail said "Drafted <title>". Now: fail-closed `parseIntegerProjectId`, program UUIDs resolve through the `projects.regulatory_program_id` anchor (drafts from the live UUID spine are captured), and when no project resolves the client is TOLD per draft ("could not be saved to the version history — no project is linked"), matching the existing write-failure honesty contract. `draft-persistence-honesty.test.ts` re-pins the new contract including the leading-digits-UUID regression. A controlled-document register so clients can build and govern their internal SOPs, reachable at the home rail "Quality and Lifecycle" item (previously dead-ended at the MDX `#validation` tab), over the existing live `qms_documents` backend — no new tables. Ships a 7-type "Quality system" template family (`server/services/qms/sopTemplates.ts`, `GET /api/mdx/qms/templates`: quality manual · policy · SOP · work instruction · form · validation protocol · training curriculum, each with the standard Purpose → Approval structure, also folded into the cross-program `/api/templates` list); the SOP register surface (`client/src/concept2cure/quality/`) with the controlled lifecycle (draft → under review → effective → superseded → retired), approve/revise/retire change control, periodic-review overdue flags, and live read-and-understood training compliance; and the supporting endpoints (`review-due`, `revise`, `retire`, `training/compliance`, template-seeded create metadata). AnA-first, `live ?? fixture`, mirroring `ui_kits/mdx/surfaces/Quality.jsx` + the `intelligence/` module. **Built at the operator's direction without a `ui_kits/` entry, so it inverts the design contract until promoted.** Full handoff, the decisions made without a kit, and the open design questions (a formal kit; rail scope for the 4 unbuilt sub-items — CAPA / post-market surveillance / inspection readiness / compliance monitor; status tokens; the SOP authoring path vs. Phase 9; e-signature wiring; numbering; training matrix; a11y) are in **`QUALITY_SOP_DESIGN_HANDOFF.md`**. On `concept2cure-v2`; deploy + live-URL verification pending.

- **2026-06-03** — **Governance core (tasking · approval gates · RBAC) verified against the established Part 11 / security model; binding contract shipped.** A 3-agent code sweep confirmed the design spec's through-line ("permission is the source of truth, derived not connected") is the right target but **mostly "build," not "connect"**: the 8-role taxonomy does not exist (8 disjoint role vocabularies; the canonical `organization_users.role` has 4; `approver`/`reviewer`/`author` are checked but unassignable); there is no permission-atom store (hard-coded matrices + free-text `permissions json`; the `classification` and CTD-module axes are absent); **author≠approver SoD is hard-enforced only on the artifact-review path, not on approve/sign/lock** (self-sign is possible when no reviewers are assigned; four-eyes is unimplemented); there are **4 disjoint signing substrates** + 2–3 state machines with **no universal `submission_status_history`**; gate-chains-by-type are real (substrate B `signing.*`) but the API is drift-broken and missing labeling/CMC/MDx/510(k)/HAQ; post-sign-edit → invalidate → `change_control` is unwired; and tasking eligibility / approval authority are **carried/stored, not derived**. Real foundations: server-side role resolution (client role distrusted), re-auth on signing, the audit ledger, and the review-path SoD primitive. Confirmed invariant: **SoD author≠approver is un-disableable — Admin may only add approvers, never remove the split.** No enforcement was blind-shipped on the compliance heart (each candidate fix encodes a policy decision tied to the unbuilt model). The grounded real/connect/build map, the refuse-list (Claude Code guardrails), and the operator decisions that gate the build are in **`GOVERNANCE_BINDING_CONTRACT.md`**.

- **2026-06-03** — **Tasking module: P0 cross-tenant IDOR fixed; Claude Design specs shipped.** `/api/regulatory/tasks` (the API the tasking UI binds to) authenticated requests but trusted a client-supplied `organizationId`, leaking/mutating tasks across tenants; every handler now derives org from the verified JWT (`getSecureOrgId`), with by-id org-ownership checks and a 5-case regression test (`31cf35b`, `8c0a1b2`, `tsc` clean). A 3-agent verification documented the backend reality — **7 task tables and three uncoordinated canonical stores** (board=`unified_tasks`, MDX workbench=`c2c_project_work_items`, AnA=`project_tasks`), no task audit / soft-delete / notifications, an orphaned regulatory blueprint catalog — plus the built-UI inventory and the full cross-service map. Follow-up pass added **immutable task data lineage** (`065b5e3`) — task create / status / link now write the SHA-256-chained `audit_logs` + `c2c_ana_actions` ledger via the shared `recordGovernedAction` primitive (best-effort, graceful, unit-tested) — plus an **assurance + decision-lineage + client-use-case-coverage** report. **For the tasking UI, Claude Code starts at `TASKING_HANDOFF.md` → `TASKING_UI_WORK_ORDER.md`; Design surfacing in `docs/reports/tasking-module-surfacing-report-2026-06-03.md`; assurance/decision lineage in `docs/reports/tasking-module-assurance-2026-06-03.md`.**

- **2026-06-03** — **Projects module backend → GA-grade; UI handoff + work order shipped.** Closed the Projects spec backend gaps that are safely buildable from the sandbox: project instructions now steer chat on both paths (A1.1); A2 two-mode capacity (`projects.retrieval_mode` + `knowledge_token_estimate`, surfaced on `GET /:id/knowledge`; in-context full-corpus injection dark-launched behind `PROJECT_INCONTEXT_INJECTION_ENABLED`); a `project_knowledge_search` model tool; real PDF/DOCX text extraction at upload; and A3 contextual-embeddings ingest dark-launched behind `PROJECT_CONTEXTUAL_INGEST_ENABLED`. 44 unit tests, `tsc --noEmit` clean, migration `20260603_project_retrieval_mode.sql`. Remaining backend (gateway unification, conversations↔AI wiring, flag enablement, citation version) needs the live preview-DB + models — see `docs/reports/projects-module-ga-status-2026-06-03.md`. **For the Projects UI, Claude Code starts at `PROJECTS_HANDOFF.md` → `PROJECTS_UI_WORK_ORDER.md`.**

- **2026-06-02** — **Dead-code purge: disconnected legacy island removed from `client/src/concept2cure/` (256 files).** ZenApp had already been cut over to lazy-load only the new design-system surfaces (mdx, pdev, biopharma, cmc, intelligence cluster, authoring, labeling, risk, tasking, submission, projects), leaving a large island of already-replaced code reachable from nothing in the live entry graph (`main.tsx` → `App.jsx` → `ZenRouter` → `ZenApp`). Removed it after proving zero reachable importers via a full client reachability analysis and a whole-repo reference grep. Deleted: `components/{editor,regulatory,intelligentDocs,intelligence,workspace,readiness,proof,provenance,traceability,artifacts,builder,knowledge,quality,reports,submission,templates,workflow,control-plane,compliance,home,precedent,sidebar,calendar,clinical,biostats,audit,common}/` dead members, the superseded `mdx/surfaces/*` prototypes (Vault/Audit/Ivd/Ivdr/Cdx/Ldt/Quality/Search/Onboarding/Conversations/AnaReview/Notifications/QSub/Samd/Clinical/Templates/InDesign) + their `mdx/{hooks,data}/*`, `mdx/components/{EsignModal,GovernedActionButton,DataState}`, `mdx/_stubs/ComingSoon`, `mdx/lib/dataMode`, `ZenAppWithSession.tsx`, `App.tsx`, the orphaned `hooks/*` (41), `services/{documentIntelligenceService,medicalDeviceService,regulatoryIntelligenceService}`, `config/*`, `contexts/{EnablementContext,IndustryContext,ZenWorkspaceContext}`, `models/*`, `types/*`, dead barrels, and `client/src/components/legacy-esign/` (only importer was the dead GovernedActionButton). The live `mdx/surfaces/{Vault,Templates}Surface` come from `workbench/Workbench`, not the deleted `surfaces/` duplicates. Updated three tests (deleted the dead-only `phase2-fabric-assimilation`, trimmed the dead `useEnterprise` block from `project-module-route-policy`, and dropped superseded-surface cases from `design-system-surfaces-smoke`). `tsc --noEmit` → 0 errors; affected suites green. Server-side and pre-concept2cure client legacy were left untouched this pass (server routes load via dynamic `import(c.mod)` that static analysis can't safely resolve).
- **2026-05-31** — **Phase 11 Intelligence backend + Phase 9 authoring live data merged (#630).** Phase 11 backend: migration `migrations/20260529_phase11_intelligence.sql` (`c2c_tlf_builds` + `c2c_forecast_snapshots`, FK-free, validated on real Postgres) + `server/routes/intelligence-cluster.ts` (`/api/intelligence/{protocol,cmc,biostat,reports}`, `{ data }` envelope, `safeRows` degrade, omit-empty) + mount + 10 unit tests. Shared fix: `useFetchJson` now sends `Authorization: Bearer` + `x-organization-id` (the global `/api` gate is Bearer-only), so c2c/mdx live fetches load instead of silently 401ing to fixtures. Phase 9 authoring wired to the live `/api/c2c/documents/*` API — live document list, outline, section content, and a governed Part-11 "send for review" save (reason-for-change → version snapshot + `c2c_ana_actions` ledger) — all `live ?? fixture`.
- **2026-05-31** — **AnA Document Formatting & Template Engine — backend foundation shipped.** New brief `DOCUMENT_FORMATTING_TEMPLATE_BRIEF.md`. Adds AnA's ability to recreate the formatting of an uploaded document (page size, margins, body/heading fonts, brand colours, embedded logo, running header/footer) as a reusable, renderer-neutral `TemplateSpec`, and to produce any document in that format as both `.docx` and PDF. New service `server/services/templates/` (spec model + sanitiser, OOXML extractor, render adapter, print-CSS HTML, logo size reader, audited store). Made the existing `server/services/docx/docxFactory.ts` **template-driven** via an optional `generateRegulatory(input, style)` argument — default output preserves the original regulatory look exactly (verified by round-trip test), so no existing caller changes; with a style it renders in the client's format and can place an embedded logo. PDF reuses the existing `renderHtmlToPdf()` and `documentExportService` options — **no second document engine introduced** (per the "don't recreate what we already have" directive). New REST API `/api/c2c/templates/*` (org-scoped, audited; mounted in `register-inline-routes.ts`), schema `migrations/20260531_template_specs.sql` (`c2c_template_specs`, idempotent + FK-free), two AI Action handlers (`extract_template_from_upload`, `render_document_with_template`) so AnA can drive it conversationally, and a `formatting` capability group in `ana-capability-registry.ts` so AnA is aware. 25 unit tests green; touched files typecheck + lint clean. **Follow-up (in brief §6/§7):** fillable form-field recreation, PDF cover-logo parity on the project exporter, and any template-management UI — which must be designed in `ui_kits/` first per `CLAUDE.md`, not invented here.
- **2026-05-30** — **PR #624 merged.** Phase 4 surface wiring + Phase 11 Intelligence cluster (frontend) + Phase 9 Universal Authoring (frontend) + legacy MDX editor removal landed on `concept2cure-v2`.
...
- **2026-05-28** — **Labeling/IFU kit + shared platform rail + CMC workstream staging.** Shipped `ui_kits/labeling/` (surfaces `mdx-labeling.ts` — IFU/package-insert/symbols/translations/coverage). Built `ui_kits/_shared/rail.jsx` — the ONE `PlatformRail` (15-item nav, persistent collapse via `localStorage('c2c.railCollapsed')`); threaded into cmc, submission, risk, tasking, pathway-tabs, labeling so the rail is on every standalone surface (lands as `_shared/shell/Rail.tsx` in v2, mounted once). **CMC status: overview surface shipped; sub-surface data staged in `ui_kits/cmc/data.jsx` (CMC_NAV + specs/stability/batch/blueprint/global) but the sub-nav surfaces are NOT yet wired — Specifications, Stability, Batch records, Change simulator, Blueprint generator, Global compliance, Copilot still to build.** Home rail `cmc` → `../cmc/`.

- **2026-05-28** — **CMC · Module 3 kit shipped** to `ui_kits/cmc/`, built against the real backend (`server/api/cmc/*` — 30 files incl. `module3OperatingSystemRoutes`, `cmc-copilot`, `change-impact-simulator`; `server/services/cmc/*` — QbD analyzer, control-strategy generator, ICH-compliance rules, SUPAC classifier, aiInsights, readiness). Recommendation: **UI over an already-rich backend** — a front door, not new logic. Surfaces §3.2.S/§3.2.P build state, QbD CQA/CPP with Q2/DoE gap flags, ICH Q1A–Q10 compliance chips, SUPAC change simulator, RPI portfolio metrics (`reg_submissions`/`reg_questions`/`reg_obligations`/`reg_rpi_snapshots`). AnA-first with `deriveInsights` Today queue. Full architecture in `CMC_CODEBASE_FINDINGS.md`. Home rail `cmc` repointed `intelligence/?tab=cmc` → `../cmc/`.

- **2026-05-28** — **Session: backed-but-blind kits + tasking ubiquity.** Shipped to `ui_kits/`: **submission** (Submission Gateway — FDA ESG/EMA CESP/EUDAMED/PMDA, ACK 1/2/3 chain, pre-flight validation), **esign** (21 CFR Part 11 signature modal — §11.50 meaning / §11.100 re-auth / audit chain; promotes `mdx/components/EsignModal.tsx` to `_shared/`), **risk** (ISO 14971 — severity×probability matrix, pre/post-control, risk controls), **pathway-tabs** (reusable Audit/Correspondence/Approvals program strip, now AnA-first with composer + file drop), **tasking** (cross-program board + project messaging), **task-tray** (persistent top-bar tasking affordance for every shell). Authoring selection toolbar gained **Send for review** + **Request approval** (highlight→route). Home rail wired: `mdx`, `submission`, `tasking` hrefs. New brief: `FILE_UPLOAD_MEMORY_BRIEF.md` — every AnA paperclip saves to the active project vault + writes a `c2c_memory_atoms` row so AnA grounds on uploads thereafter. All composers AnA-first (composer + file drop → data). Model label is **AnA 1.0** everywhere (no Opus branding).
