...
- **17 `@ts-nocheck` files** including `server/middleware/auth.ts:1` and `server/auth/index.ts` — auth code sits outside the "0 type errors" guarantee. `tsconfig.json:35-36` excludes `AuthContext.tsx` from typechecking; lint ignores all of `client/src`. **Partially FIXED (2026-05-29):** `@ts-nocheck` removed from both auth files — the "conflict with tenantContext.ts" rationale was stale (identical declaration-merged `Express.Request` augmentations coexist at 0 errors). Removal surfaced and fixed a real masked bug: `auth/index.ts` re-exported `isPublicRoute`/`hasPermission` from a module that exports neither (`hasPermission` lives in `tenantIsolation.ts`; `isPublicRoute` exists nowhere). Full `tsc` held at 0; 39 auth tests pass. **`tenant-traceability.ts`: FIXED (2026-05-29)** — `@ts-nocheck` removed and all 17 genuine type errors resolved. Behavior was locked first with a 16-case supertest integration test (`tests/routes/tenant-traceability-routes.test.ts`), then fixed: (a) `req.tenantContext` org/user ids narrowed via `tenantOrgId()`/`tenantUserId()` helpers (organizationId is a runtime string compared against integer columns — coercing to number is wire-identical in node-postgres, so behavior-preserving); (b) `validationResult.data` and the insert `.values()` typed via `qmpTraceabilityMatrix.$inferInsert` — note the zod-derived `InsertQmpTraceabilityMatrix` infers as `{}` (a drizzle-zod `createInsertSchemaOmit` quirk), so the table's own `$inferInsert` is the correct type source; (c) verify-batch `req.body` destructure typed. Full `tsc` held at 0; all 16 integration tests pass (behavior unchanged). **`cerGenerator.ts`: FIXED (2026-05-29)** — `@ts-nocheck` removed; the 4 surfaced errors were genuine Bull v4 API misuse and are now corrected (behavior-improving): `job.progress(p, name)` → `job.progress(p)` (Bull takes one arg — the section name was silently dropped, and is still conveyed as the `step` field); the WS emit now sends `job.progress()` (public getter) instead of the **internal `job._progress`** field (a real latent bug — it was emitting Bull's private state); and `job.id` (`JobId`) coerced to `String` where `string` is required. The pragma's "optional deps (puppeteer-cluster/bull/OpenAI)" rationale was stale — only the 4 Bull errors existed. Full `tsc` held at 0. `universal-packager`: **IMPROVED (2026-05-29)** — its blanket `@ts-nocheck` (which blinded ~850 lines) is replaced with 2 targeted `@ts-expect-error` lines at the only genuine `@types/pdfkit` gaps (`fillOpacity` at :490, `moveTo`/`doc.y` at :518). The rest of the file is now fully type-checked, and `@ts-expect-error` self-validates (it errors if the gap ever closes). Full `tsc` held at 0. **`documentAuthoring.routes.ts` no longer exists** — the path the audit listed was stale (the file was created in `eb1751d` and later deleted).

### NEW — broken live endpoint: `POST /api/document-authoring/documents` (MEDIUM, surfaced 2026-05-29)
While clearing the `@ts-nocheck` noise, the deleted `documentAuthoring.routes.ts` turned out to leave a **live 404**: `client/src/concept2cure/hooks/useDocumentActions.ts:95` calls `POST /api/document-authoring/documents` (document creation on the 21 CFR Part 11 path), but the only mounts of that route — `bootstrap/register-document-routes.ts:40` (`/api/document-authoring`) and a now-removed block in `documents-unified.ts` — both dynamically import the deleted file and fall into their `catch`, so the endpoint is unserved. `authoring.router.ts` (live at `/api/authoring`) has `/documents/:id/*` sub-routes but **no `POST /documents` create**, so the function is not covered elsewhere. **Cleaned (2026-05-29):** removed the dead redundant `/authoring` mount + stale catalog entry from `documents-unified.ts` (it only ever logged a failure; authoring is served at `/api/authoring`). `register-document-routes.ts:40` left intact as an honest boot-time error breadcrumb. **FIXED (2026-05-29):** restored a focused, fully type-checked `documentAuthoring.routes.ts` (no `@ts-nocheck`) serving `POST /documents` (create + initial version + Part 11 audit via `auditService.logAction`) and `GET /documents/:id` (tenant-scoped). Re-wired the `documents-unified.ts` `/authoring` mount + catalog entry. The existing `register-document-routes.ts:40` mount now resolves instead of failing closed, so `POST /api/document-authoring/documents` (the client call) is live again. Scope is deliberately minimal — signatures/locks/MFA/review remain in `authoring.router.ts` (`/api/authoring`); the deleted 2,000-line file was NOT wholesale-resurrected. Covered by `tests/routes/document-authoring-routes.test.ts` (9 cases incl. auth/workspace gates, validation, create-with/without-content, and a cross-org tenant-isolation 404). Full `tsc` = 0.
