...
> **Update (2026-05-29):** CR-1, CR-2, and CR-3 addressed on branch `claude/concept2cure-v2-audit-5zufv` (PR #621). CR-1/CR-2 fail-closed; CR-3 all three gaps fixed (org-scoped `resolveTarget`, real DB errors propagate, and a `verifyAuditChain` + `GET /api/c2c/actions/verify-chain` verifier added). CR-4 (route flip, Phase-3 gated) and CR-5 (`SimpleApp.tsx` deletion) remain open.
