...
--
-- WHAT THIS CLOSES
-- The compile, validate and readiness gates now derive what an IND must file
-- from this pack's mandatory flags (server/services/ectd/required-sections.ts)
-- instead of a hard-coded table. That made two flags in ich-m4-v2.1 into
-- blockers every IND client would hit:
--
--   1.3.1  Contact / sponsor / applicant information. In the FDA eCTD Module 1
--          Specification v2.3 this heading holds CHANGES — change of address,
--          change in contact agent, change in sponsor, transfer of obligation,
--          change in ownership. An initial IND carries none; its sponsor and
--          contact details travel on Form FDA 1571 under 1.1. Mandatory → optional.
--   5.2    Tabular listing of all clinical studies. ICH M4E content that a
--          first-in-human IND with no prior studies has nothing to put in.
--          21 CFR 312.23(a)(9) asks for previous human experience where it
--          exists, not for an empty table. Mandatory → optional.
--
-- Everything else — headings, order, labels, the other flags — is identical to
-- ich-m4-v2.1. tests/regulatory/fda-module1-numbering.test.ts holds the pack to
-- the FDA heading list; server/services/ectd/__tests__/required-sections.test.ts
-- reads this file and pins what the gates now require of an IND.
--
...
-- Same reason as 20260901: c2c_rule_packs is keyed (doc_type, agency, version)
-- and c2c_documents carries a composite FK to it. Changing required_sections in
-- place would change what already-scaffolded documents claim to have been
-- built against — in a Part 11 table that is a falsified record. New version,
-- then supersede.
--
...
BEGIN
  IF to_regclass('public.c2c_rule_packs') IS NULL THEN
    RAISE NOTICE 'c2c_rule_packs not present - skipping ind:fda ich-m4-v2.2 outline';
    RETURN;
  END IF;
  ...
  IF EXISTS (SELECT 1 FROM information_schema.columns
    ...
    UPDATE c2c_rule_packs SET
      ...
      uncertainties  = 'Module 1 headings transcribed from the FDA Module 1 v2.3 heading list; mandatory flags are 21 CFR 312.23 defaults for an initial IND (1.3.1 and 5.2 optional — post-initial changes and prior-study listings). Not reviewed by a regulatory professional; confirm against current FDA IND eCTD guidance before filing.'
    WHERE doc_type = 'ind' AND agency = 'fda' AND version = 'ich-m4-v2.2';
  END IF;
