...
export const BatchSchema = z.object({
  ...
  storageConditions: z.string().optional(),
  distributionStatus: z.enum(['in_stock', 'shipped', 'delivered', 'consumed']),
...
export class MemSupplyChainStorage implements ISupplyChainStorage {
  ...
  async createSupplier(data: CreateSupplier): Promise<Supplier> {
    ...
    const supplier: Supplier = { ...data, id: this.generateId(), createdAt: now, updatedAt: now };
  ...
  async getSupplier(id: number, organizationId: number, clientWorkspaceId: number): Promise<Supplier | null> {
    ...
    if (!supplier || !this.checkTenantAccess(supplier, organizationId, clientWorkspaceId)) return null;
    return supplier;
  }
  ...
  async updateSupplier(
    ...
    const updated: Supplier = { ...supplier, ...data, updatedAt: new Date().toISOString() };
  ...
  async listSuppliers(
    organizationId: number,
    clientWorkspaceId: number,
    filters: { supplierType?: string; qualificationStatus?: string; riskLevel?: string; search?: string } = {}
  ): Promise<Supplier[]> {
    let suppliers = Array.from(this.suppliers.values()).filter(s => this.checkTenantAccess(s, organizationId, clientWorkspaceId));

    if (filters.supplierType) suppliers = suppliers.filter(s => s.supplierType === filters.supplierType);
    if (filters.qualificationStatus) suppliers = suppliers.filter(s => s.qualificationStatus === filters.qualificationStatus);
    if (filters.riskLevel) suppliers = suppliers.filter(s => s.riskLevel === filters.riskLevel);

    if (filters.search) {
      const search = filters.search.toLowerCase();
      suppliers = suppliers.filter(s => s.name.toLowerCase().includes(search) || s.supplierCode.toLowerCase().includes(search));
    }
  ...
  async createMaterial(data: CreateMaterial): Promise<Material> {
    const now = new Date().toISOString();
    const material: Material = { ...data, id: this.generateId(), createdAt: now, updatedAt: now };
  ...
  async getMaterial(id: number, organizationId: number, clientWorkspaceId: number): Promise<Material | null> {
    ...
    if (!material || !this.checkTenantAccess(material, organizationId, clientWorkspaceId)) return null;
    return material;
  }
  ...
  async updateMaterial(
    ...
    const updated: Material = { ...material, ...data, updatedAt: new Date().toISOString() };
  ...
  async listMaterials(
    organizationId: number,
    clientWorkspaceId: number,
    filters: { materialType?: string; materialCategory?: string; regulatoryStatus?: string; search?: string } = {}
  ): Promise<Material[]> {
    let materials = Array.from(this.materials.values()).filter(m => this.checkTenantAccess(m, organizationId, clientWorkspaceId));

    if (filters.materialType) materials = materials.filter(m => m.materialType === filters.materialType);
    if (filters.materialCategory) materials = materials.filter(m => m.materialCategory === filters.materialCategory);
    if (filters.regulatoryStatus) materials = materials.filter(m => m.regulatoryStatus === filters.regulatoryStatus);

    if (filters.search) {
      const search = filters.search.toLowerCase();
      materials = materials.filter(m => m.name.toLowerCase().includes(search) || m.materialCode.toLowerCase().includes(search));
    }
  ...
  async createBatch(data: CreateBatch): Promise<Batch> {
    const now = new Date().toISOString();
    const batch: Batch = { ...data, id: this.generateId(), createdAt: now, updatedAt: now };
  ...
  async getBatch(id: number, organizationId: number, clientWorkspaceId: number): Promise<Batch | null> {
    ...
    if (!batch || !this.checkTenantAccess(batch, organizationId, clientWorkspaceId)) return null;
    return batch;
  }
  ...
  async updateBatch(
    ...
    const updated: Batch = { ...batch, ...data, updatedAt: new Date().toISOString() };
  ...
  async listBatches(
    ...
    let batches = Array.from(this.batches.values()).filter(b => this.checkTenantAccess(b, organizationId, clientWorkspaceId));

    if (filters.batchStatus) batches = batches.filter(b => b.batchStatus === filters.batchStatus);
    if (filters.manufacturingStatus) batches = batches.filter(b => b.manufacturingStatus === filters.manufacturingStatus);
    if (filters.qcStatus) batches = batches.filter(b => b.qcStatus === filters.qcStatus);
    if (filters.releaseStatus) batches = batches.filter(b => b.releaseStatus === filters.releaseStatus);
    if (typeof filters.materialId == 'number') batches = batches.filter(b => b.materialId === filters.materialId);

    if (filters.search) {
      const search = filters.search.toLowerCase();
      batches = batches.filter(b => b.batchNumber.toLowerCase().includes(search) || (b.lotNumber ? b.lotNumber.toLowerCase().includes(search) : false));
    }
  ...
  async qualifySupplier(
    ...
    return this.updateSupplier(
      supplierId,
      { qualificationStatus: 'qualified', qualificationDate: new Date().toISOString(), auditScore },
      organizationId,
      clientWorkspaceId
    );
  }
  ...
  async suspendSupplier(
    ...
    return this.updateSupplier(
      supplierId,
      { qualificationStatus: 'suspended', notes: reason },
      organizationId,
      clientWorkspaceId
    );
  }
