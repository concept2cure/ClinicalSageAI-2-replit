 ...
 *
 * Offline only: scores tasks that carry a captured candidate
 * (`candidateContent` / `candidateExtraction`). No DB or LLM required, so it is
 * runnable in CI and regression-gates captured outputs.
 *
 * It had a `--live` mode until 2026-09-22, removed because it could not produce
 * a result anyone should rely on: it scored a task's CAPTURED candidate in
 * preference to the live generation, routed through the gateway without pinning
 * a model (so a fallback answered unrecorded), and a run that scored nothing
 * exited 0. Scoring a model live is performance qualification, and is now
 * server/eval/pq/run-pq.ts (`npm run pq:run -- --model <id>`), which pins one
 * model, checks the model the provider reports serving, and never scores a
 * captured candidate.
 *
 * Usage:
...
function parseArgs(argv: string[]): CliOptions {
  const opts: CliOptions = { minCoverage: null, minF1: null };
  for (let i = 0; i < argv.length; i++) {
    if (argv[i] === '--min-coverage') opts.minCoverage = Number(argv[++i]);
    else if (argv[i] === '--min-f1') opts.minF1 = Number(argv[++i]);
    else if (argv[i] === '--live') {
      throw new Error('--live was removed: live scoring is performance qualification — npm run pq:run -- --model <id>');
    }
  }
  return opts;
}
...
async function main(): Promise<void> {
  ...
  for (const task of tasks) {
    if (task.taskType === 'generation') {
      const content = typeof task.candidateContent === 'string' ? task.candidateContent : null;
      if (content === null) {
        skipped += 1;
        console.info(`  ${task.id.padEnd(34)} [${task.docType}] skipped (no captured candidate)`);
        continue;
      }
  ...
  if (skipped > 0) console.info(`  skipped (no captured candidate) ${skipped}`);
  ...
  /* A threshold that was asked for and had nothing to measure is a failed gate,
     not a passed one. Until 2026-09-22 both checks below were skipped when
     nothing was scored, so an empty run exited 0. */
  if (opts.minCoverage !== null && generationScored === 0) {
    console.error('FAIL: --min-coverage was set and no generation task was scored.');
    failed = true;
  }
  if (opts.minF1 !== null && extractionScored === 0) {
    console.error('FAIL: --min-f1 was set and no extraction task was scored.');
    failed = true;
  }
