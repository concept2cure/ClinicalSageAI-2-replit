 ...
 * writes it: `audit_logs` with the columns that function inserts beyond
 * AUDIT_LOGS_PGLITE_DDL, and `c2c_ana_actions`. Apply AFTER AUDIT_LOGS_PGLITE_DDL.
 * Only the columns the writer touches; no chain_seq, so under vitest the chain
 * writer uses its pre-fix order (it says so, once).
 */
