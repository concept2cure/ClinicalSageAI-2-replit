...
const MODULE_1: INDSection[] = [
  // Codes follow the FDA eCTD Module 1 Specification v2.3 (US regional M1): forms
  // sit under 1.1, the cover letter is 1.2, and the general investigational plan
  // for an initial IND is 1.20. The eCTD packager places every Module 1 leaf by
  // this code (server/services/ectd/controlled-vocab/fda-regional-sections.ts),
  // so an ICH-generic numbering here would misfile the content in the backbone.
  // Pinned by tests/regulatory/fda-module1-numbering.test.ts.
  {
    code: '1.1.1',
    ...
    required: true,
    contentType: 'form',
    guidance: '21 CFR 312.23(a)(1); FDA eCTD M1 v2.3 §1.1 (forms)',
  ...
  },
  {
    ...
    required: true,
    contentType: 'form',
    guidance: '21 CFR 312.53; FDA eCTD M1 v2.3 §1.1 (forms)',
  ...
  },
  {
    ...
    required: true,
    contentType: 'form',
    guidance: '42 U.S.C. 282(j)(5)(B); FDA eCTD M1 v2.3 §1.1 (forms)',
  ...
  },
  {
    ...
    guidance: '21 CFR 312.23(a)(1); FDA eCTD M1 v2.3 §1.2 (cover letters)',
  ...
  },
  {
    ...
    required: true,
    contentType: 'narrative',
    guidance: '21 CFR 312.23(a)(3); FDA eCTD M1 v2.3 §1.20 (general investigational plan for initial IND)',
    generationPrompt: 'Draft the Introductory Statement and General Investigational Plan for the initial IND of {{PRODUCT_NAME}} ({{INDICATION}}). Include: drug name, active ingredients, pharmacological class, structural formula, dosage form and route, broad objectives and planned duration of the investigation; then the rationale, indication(s), general approach, the studies planned for the first year with expected subject numbers, and the anticipated risks. Do not fabricate data; use placeholders for unknown values.',
