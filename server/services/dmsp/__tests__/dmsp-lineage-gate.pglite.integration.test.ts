/**
 * Ledger L158 — Data Management & Sharing Plan prose carries lineage through the shared gate.
 *
 * `updateElementTx` used to write `dms_plan_elements.content` with no lineage at
...
import { updateElementTx } from '../dmsp-service';
...
async function spans(elementId: number) {
  const r = await exec.query(
    `SELECT provenance_kind, reference_id FROM document_span_lineage
      WHERE document_table = 'dms_plan_elements' AND document_id = $1 AND deleted_at IS NULL`,
    [String(elementId)],
  );
  return r.rows as Array<{ provenance_kind: string; reference_id: string | null }>;
}
async function seed(): Promise<number> {
  await exec.query(`INSERT INTO dms_plans (id, organization_id, status) VALUES (1, $1, 'draft') ON CONFLICT DO NOTHING`, [ORG]);
  const r = await exec.query(
    `INSERT INTO dms_plan_elements (organization_id, plan_id, content) VALUES ($1, 1, '') RETURNING id`,
    [ORG],
  );
...
beforeAll(async () => {
  ...
  await pglite.exec(`
    CREATE TABLE dms_plans (id SERIAL PRIMARY KEY, organization_id INTEGER NOT NULL, status TEXT NOT NULL DEFAULT 'draft', updated_at TIMESTAMPTZ DEFAULT now());
    CREATE TABLE dms_plan_elements (id SERIAL PRIMARY KEY, organization_id INTEGER NOT NULL, plan_id INTEGER NOT NULL, content TEXT, addressed BOOLEAN DEFAULT false, updated_at TIMESTAMPTZ DEFAULT now());
  `);
}, 60_000);
afterAll(async () => {
  await pglite?.close();
...
describe('dmsp-service updateElementTx', () => {
  ...
  it('with sources, the verbatim clause lands on the source and the rest on the author (ledger L154)', async () => {
    ...
    const src = await exec.query(
      `INSERT INTO cre_evidence_sources (organization_id, visibility_class, source_type, title, checksum, ingestion_status, extraction_status)
       VALUES ($1, 'tenant_private', 'client_document', 'csr.pdf', 'sha-dmsp-service', 'ingested', 'extracted') RETURNING id`,
      [ORG],
    );
  ...
  it('a flag-only edit records no lineage (nothing authored)', async () => {
    const id = await seed();
    await updateElementTx(exec as any, ORG, id, { addressed: true }, USER);
