...
describe('conversation-os retrieval', () => {
  it('chunks and retrieves scoped evidence', async () => {
    await ingestKnowledgeChunks({ conversationId: 'conv-retrieval', sourceId: 'src-1', text: 'IND Module 3 contains CMC controls. Diagnostics evidence confirms analytical validity.', tags: ['cmc', 'approved'], projectId: 'p1', userId: 'u1' });
    const result = await retrieveRelevantChunks({ conversationId: 'conv-retrieval', query: 'cmc controls', approvedOnly: true, projectId: 'p1', userId: 'u1' });
...
describe('conversation-os tool gate and orchestration', () => {
  it('blocks tool execution in off mode and runs hard-task plan/execute loop', async () => {
    ...
    await ingestKnowledgeChunks({ conversationId: 'conv-tools', sourceId: 'ind-ectd', text: 'eCTD sequence assembly requires controlled source mapping and objective execution evidence.', tags: ['ectd', 'approved'], projectId: 'p2', userId: 'u2' });
    ...
    const result = await planAndExecute({ conversationId: 'conv-tools', task: 'Prepare IND/eCTD submission objective and execution plan with sources', projectId: 'p2', userId: 'u2' });
  ...
  it('returns explicit persisted-no-governance state on invalid acceptance context', async () => {
    await upsertToolManifest({ conversationId: 'conv-proposal', mode: 'on-demand', projectId: '55', userId: '77' });
    const proposal = await createProposal({ conversationId: 'conv-proposal', artifactId: 'artifact-conv-proposal', content: 'Draft proposal content', projectId: '55', userId: '77' });

    const accepted = await acceptProposal({ conversationId: 'conv-proposal', proposalId: proposal.id, projectId: '55', userId: '77', organizationId: 'bad-org' });
