...
class AdvisoryCommitteeService {
  ...
  async searchPatterns(input: ACSearchInput): Promise<AdvisoryCommitteePattern[]> {
    log.info('Searching AC patterns', { committee: input.committeeType, category: input.riskCategory });
   ...
   */
  async createPattern(pattern: Omit<AdvisoryCommitteePattern, 'id' | 'createdAt' | 'updatedAt'>): Promise<AdvisoryCommitteePattern> {
    log.info('Creating AC pattern', { code: pattern.patternCode, committee: pattern.committeeType });
    ...
    const result = await pool!.query(`
      ...
      RETURNING *
    `, [
      pattern.organizationId, pattern.patternCode, pattern.patternName, pattern.committeeType,
      pattern.riskCategory, pattern.typicalQuestions, pattern.votingQuestionTemplates,
      pattern.historicalFrequency, pattern.negativeVoteCorrelation, pattern.fdaOverrideRate,
      JSON.stringify(pattern.panelistSensitivity), pattern.consumerRepImpact, pattern.statisticianFocusAreas,
      pattern.mitigationStrategies, pattern.recommendedPresentations, JSON.stringify(pattern.counterargumentFrameworks)
    ]);
  ...
  async assessRisk(input: {
    ...
    const riskFactors: ACRiskFactor[] = relevant.map(p => ({
      ...
      impact: p.negativeVoteCorrelation > 0.6 ? 'critical'
        : p.negativeVoteCorrelation > 0.4 ? 'high'
        : p.negativeVoteCorrelation > 0.2 ? 'medium'
        : 'low',
      mitigation: p.mitigationStrategies[0] ?? 'Prepare comprehensive response addressing this concern.',
    }));

    // Estimate favorable vote probability
    const avgNegCorrelation = relevant.length > 0
    ...
    const fdaOverrideLikelihood = relevant.length > 0
      ? relevant.reduce((s, p) => s + p.fdaOverrideRate, 0) / relevant.length
      : 0;

    // Collect key questions
    const keyQuestions = relevant
      .flatMap(p => p.typicalQuestions.slice(0, 2))
      .slice(0, 10);

    // Generate preparation plan
    const preparationPlan: ACPreparationItem[] = relevant.map(p => ({
      area: p.patternName,
      recommendation: p.recommendedPresentations[0] ?? `Prepare response for ${p.riskCategory} concerns`,
      priority: p.negativeVoteCorrelation > 0.5 ? 'critical'
        : p.negativeVoteCorrelation > 0.3 ? 'high'
        : p.negativeVoteCorrelation > 0.15 ? 'medium'
        : 'low',
    ...
    const overallRisk = avgNegCorrelation > 0.5 ? 'critical'
      : avgNegCorrelation > 0.35 ? 'high'
      : avgNegCorrelation > 0.2 ? 'medium'
      : 'low';
   ...
   */
  async getCommitteeTypes(organizationId: string): Promise<{ type: string; patternCount: number }[]> {
    const result = await pool!.query(`
  ...
  private mapPattern(row: Record<string, unknown>): AdvisoryCommitteePattern {
    return {
      ...
      counterargumentFrameworks: (row.counterargument_frameworks as CounterargumentFramework[]) ?? [],
