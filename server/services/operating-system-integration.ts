...
import { AssumptionRegistryService, type CreateAssumptionInput } from './assumption-registry-service';
...
export class OperatingSystemIntegration {
  ...
  async captureFromSapGeneration(ctx: SapAssumptionContext) {
    ...
    assumptions.push(await this.assumptionService.createAssumption({
    ...
    assumptions.push(await this.assumptionService.createAssumption({
      ...
      unit: 'proportion',
      rationale: ctx.dropoutRate === 0.15
    ...
    const decision = await this.decisionService.createDecision({
      ...
      recommendationSummary: `Generate SAP with N=${ctx.sampleSize}, dropout=${(ctx.dropoutRate * 100).toFixed(0)}%, endpoint=${ctx.primaryEndpoint}`,
      recommendationDetail: `Statistical Analysis Plan generated using ${assumptions.length} structured assumptions. ` +
  ...
  async captureFromDefensibilityAssessment(ctx: DefensibilityDecisionContext) {
    const confidence = ctx.overallRating === 'strong' ? 'strong' as const
      : ctx.overallRating === 'adequate' ? 'moderate' as const
      : ctx.overallRating === 'weak' ? 'provisional' as const
      : 'uncertain' as const;

    const actionState = ctx.criticalIssueCount > 0
      ? 'recommended_only' as const
      : ctx.majorIssueCount > 0
        ? 'recommended_only' as const
        : 'prepared' as const;

    const approvalState = ctx.criticalIssueCount > 0
    ...
    const decision = await this.decisionService.createDecision({
      ...
      recommendationSummary: `Study design rated as "${ctx.overallRating}" (${ctx.overallScore}/100). ` +
        `${ctx.criticalIssueCount} critical, ${ctx.majorIssueCount} major issues. ` +
        `Reviewer risk: ${ctx.reviewerRiskLevel}.`,
      recommendationDetail: ctx.recommendations.length > 0
  ...
  async captureFromDocumentGeneration(ctx: DocumentGenerationDecisionContext) {
    const decision = await this.decisionService.createDecision({
      ...
      recommendationDetail: `Document generated as advisory output. ` +
    ...
    for (const assumptionId of ctx.usedAssumptionIds) {
      try {
        await this.assumptionService.linkToArtifact(
          assumptionId,
          ctx.organizationId,
          ctx.generatedArtifactId,
          ctx.generatedArtifactVersionId
        );
