/**
  ...
  async calculateOptimalDoseEscalation(studyId: string) {
    try {
      ...
      const cohorts = await db!.select().from(doseCohorts).where(eq(doseCohorts.studyId, studyId));

      // Fetch all DLT events
      const dlts = await db!.select().from(dltEvents).where(eq(dltEvents.studyId, studyId));
      ...
      switch (study.escalationMethod) {
        ...
        case 'fibonacci':
          recommendation = await this.calculateModifiedFibonacci(
            doseLevelData,
            cohortsWithDLTs,
            study
          );
        ...
        messages: [
          ...
          },
          {
            ...
            content: JSON.stringify({
              study,
              recommendation,
              doseLevels: doseLevelData,
              cohortData: cohortsWithDLTs,
            }),
          },
        ],
        temperature: 0.2,
  ...
  private async calculateModifiedFibonacci(doseLevels: any[], cohorts: any[], study: any) {
    ...
    if (dltRate > 0.33) {
      ...
      const percentIncrease =
        fibonacciSequence[Math.min(currentLevelIndex, fibonacciSequence.length - 1)];
  ...
  private async calculateBOIN(doseLevels: any[], cohorts: any[], study: any) {
    const targetToxicity = study.targetToxicityRate || 0.3;
  ...
  private async calculateCRM(doseLevels: any[], cohorts: any[], study: any) {
    ...
    return {
      ...
      confidenceInterval: this.calculateCRMConfidenceInterval(
        posteriorProbs,
        doseLevels,
        optimalDoseIndex
      ),
  ...
  private calculateIsotonicRegression(doseLevels: any[], cohorts: any[], target: number) {
    ...
    return {
      ...
      confidenceInterval: {
        lower:
          doseLevels[Math.max(0, mtdIndex - 1)]?.doseAmount ||
          doseLevels[mtdIndex].doseAmount * 0.75,
        upper:
          doseLevels[Math.min(doseLevels.length - 1, mtdIndex + 1)]?.doseAmount ||
          doseLevels[mtdIndex].doseAmount * 1.25,
      },
    };
  }
  ...
  private calculateCRMConfidenceInterval(
    ...
    return {
      lower:
        doseLevels[Math.max(0, optimalIndex - 1)]?.doseAmount ||
        doseLevels[optimalIndex].doseAmount * 0.8,
      upper:
        doseLevels[Math.min(doseLevels.length - 1, optimalIndex + 1)]?.doseAmount ||
        doseLevels[optimalIndex].doseAmount * 1.2,
    };
  }
  ...
  /**
   * Cross-Species Intelligence Translator
   * Advanced allometric scaling with AI pattern recognition
   */
  async performCrossSpeciesAnalysis(params: {
    ...
    const { organizationId, compoundId, speciesData } = params;
    ...
    const analysisResponse = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: `Perform advanced cross-species PK/PD analysis using:
                   1. Allometric scaling (body weight and surface area)
                   2. Physiologically-based scaling
                   3. In vitro-in vivo extrapolation (IVIVE)
                   4. Safety margin calculations (NOAEL, MABEL)
                   5. First-in-human dose prediction

                   Consider species-specific metabolism, protein binding, and tissue distribution.
                   Apply FDA guidance on estimating MRSD for first-in-human trials.
                   Include uncertainty factors and safety margins.`,
        },
        {
          role: 'user',
          content: JSON.stringify(speciesData),
        },
      ],
      temperature: 0.2,
      max_tokens: 2000,
    ...
    // Store cross-species analysis
    const [pkpdAnalysis] = await db!
      .insert(crossSpeciesPkpd)
      .values({
        organizationId: organizationId,
        compoundId: compoundId,
        analysisType: 'allometric_comprehensive',
        scalingMethod: analysis.method || 'mixed',
        humanDosePrediction: analysis.humanDose || 0,
        safetyMargin: analysis.safetyMargin || 10,
        confidenceInterval: analysis.confidenceInterval || {},
        noaelBasedDose: analysis.noaelDose || 0,
        mabelBasedDose: analysis.mabelDose || 0,
        recommendedStartingDose: analysis.recommendedDose || 0,
        metadata: {
          modelUsed: AI_MODELS.PREDICTION,
    ...
    for (const species of speciesData) {
      const scaling = analysis.speciesScaling?.[species.species] || {};
      await db!.insert(speciesComparisons).values({
        pkpdAnalysisId: pkpdAnalysis.id,
        species: species.species,
        bodyWeight: scaling.bodyWeight || 0,
        doseAdministered: species.dose,
        cmax: species.cmax,
        auc: species.auc,
        clearance: species.clearance,
        volumeDistribution: species.volume,
        halfLife: species.halfLife,
        bioavailability: scaling.bioavailability || 1,
        scalingFactor: scaling.factor || 1,
        metadata: scaling.metadata || {},
      });
    }

    return {
      analysis: pkpdAnalysis,
      recommendations: this.generateDosingRecommendations(analysis),
      regulatoryPackage: await this.generateRegulatoryBridgingReport(pkpdAnalysis),
    };
  }
  ...
  private async calculateBayesianMTD(cohorts: any[], dlts: any[]) {
    ...
    const dltRates = cohorts.map(cohort => {
      ...
      return {
        ...
        dltRate: cohort.patients_enrolled > 0 ? cohortDLTs.length / cohort.patients_enrolled : 0,
      };
    });
  ...
  async performCrossSpeciesAnalysis(params: {
    ...
    const [analysis] = await db!
      ...
      .values({
        ...
        organizationId,
        createdBy: 'system',
      } as typeof crossSpeciesPkpd.$inferInsert)
      .returning();
    ...
    for (const species of speciesData) {
      ...
      } as typeof speciesComparisons.$inferInsert);
    }
  ...
  async calculatePredictiveSuccessScore(params: {
    ...
    const targetValidationScore = await this.calculateTargetValidationScore(
      indication,
      biomarkerData
    );
    ...
    const [prediction] = await db!
      ...
      .values({
        ...
        riskFactors,
        recommendations: [
          { type: 'go_no_go', value: predictedOutcome.goNoGo },
          { type: 'key_drivers', value: predictedOutcome.keyDrivers },
        ],
        modelVersion: AI_MODELS.PREDICTION,
        organizationId,
      } as typeof foresightPredictions.$inferInsert)
      .returning();
    ...
    return {
      ...
      goNoGoRecommendation: predictedOutcome.goNoGo,
      keyDrivers: predictedOutcome.keyDrivers,
      riskFactors,
      recommendations,
      visualizationData: {
        radarChart: {
  ...
  private async identifyRiskFactors(params: any) {
    ...
    if (
      params.phase === 'III' &&
      (!params.preclinicalData || params.preclinicalData.safetyScore < 0.7)
    ) {
      riskFactors.push({
        category: 'safety',
        severity: 'medium',
    ...
    if (
      ...
      params.competitorData.filter((c: any) => c.successStatus === 'failed').length > 3
    ) {
      riskFactors.push({
        category: 'target',
        severity: 'high',
