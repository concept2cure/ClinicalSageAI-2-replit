 ...
 * missing — and what this service owns — is the comprehension record and the
 * discipline around it:
 *
 *   • recordExtractionOutcome — the extraction tier, written in the ingest
 *     transaction. An extraction failure is recorded AS a failure with its
 *     reason; it is never allowed to look like an empty-but-fine document.
 *   • recordReadReceipt / getReadCoverage — every read serves an exact
 *     character span and records it against the content hash it came from.
 *     Coverage is integer span arithmetic, not an estimate.
 *   • assertCatalogWriteAllowed / completeCatalog — the gate this module
 *     exists for: a catalog entry (kind / purpose / summary / key_data) can
 *     only be written once the receipts cover the ENTIRE extracted text. A
 *     page-sample "review" is refused with the exact uncovered ranges, so the
 *     agent knows precisely what is left to read.
 *   • listProjectDocuments / getCatalogBootstrapDigest — discovery and recall:
 *     what files exist, where each is filed, what each is for, and which ones
 *     have not been studied yet (said honestly, never hidden).
 *
 * Pure logic (span merging, coverage, gating, digest formatting) is exported
 * for unit tests; only the thin wrappers below touch the database. Tenancy on
 * vault.documents follows the established idiom: an EXISTS join to
 * regulatory_programs.organization_id, because the table itself carries no
 * org column (see vault-ingest.ts).
 */
...
// ─────────────────────────────────────────────────────────────────────────────
// Pure core — spans, coverage, gating (no DB, no I/O)
// ─────────────────────────────────────────────────────────────────────────────
...
/**
 * The org-checked document row every tool call resolves through. Same EXISTS
 * idiom as project-vault.ts — the org predicate on regulatory_programs is the
 * only thing standing between two tenants on this table.
 */
