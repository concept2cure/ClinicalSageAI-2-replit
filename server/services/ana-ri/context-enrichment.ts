...
export async function enrichContextForChat(params: {
  ...
  if (slash) {
    ...
    const commandDescriptions: Record<string, string> = {
      ...
      sap: slash.args ? `Generate a Statistical Analysis Plan for: ${slash.args}. For any sample-size or power figure in the SAP, call the compute_sample_size tool — never hand-calculate.` : 'Generate a Statistical Analysis Plan. Gather study parameters: indication, phase, primary endpoint, effect size, alpha/power, allocation, dropout, and missing data strategy. For any sample-size or power figure, call the compute_sample_size tool (the validated deterministic engine) — never estimate these numbers yourself.',
      ...
      defensibility: 'Run a statistical defensibility assessment on this project. Score across 7 dimensions: evidence sufficiency, defensibility, reviewer sensitivity, claim risk, cross-section consistency, submission risk. Generate reviewer risk annotations. When the assessment depends on sample size or power adequacy, call the compute_sample_size tool to ground the judgment in the validated engine rather than estimating.',
