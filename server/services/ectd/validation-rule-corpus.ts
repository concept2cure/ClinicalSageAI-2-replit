...
export type RuleRegion = 'ich' | 'fda' | 'eu' | 'jp';
...
export const RULE_CORPUS: ValidationRule[] = [
  ...
  },
  {
    ...
    regions: ['fda', 'eu', 'jp'],
    severity: 'high',
    rationale: 'Each region requires its Module 1 backbone (us-regional.xml / eu-regional.xml / jp-regional.xml) validating against the regional schema.',
    source: FDA_CRIT,
    enforcement: 'packager',
  },
  {
...
export function rulesForRegion(region: 'fda' | 'eu' | 'jp'): ValidationRule[] {
...
export function corpusSummary(): CorpusSummary {
  const byRegion: Record<RuleRegion, number> = { ich: 0, fda: 0, eu: 0, jp: 0 };
...
export default { RULE_CORPUS, getRule, rulesForRegion, rulesByEnforcement, dispatchFindingCodes, corpusSummary };
