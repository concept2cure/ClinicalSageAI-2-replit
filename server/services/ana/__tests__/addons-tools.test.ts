...
describe('Effort + COI tools — context + validation guards', () => {
  ...
  it('add_effort_line requires the numeric percentages', async () => {
    const out = JSON.parse(await getToolHandler('add_effort_line')!({ certification_id: 1, activity_label: 'R01' }, { organizationId: 1, userId: 1, humanConfirmed: true } as any));
  ...
  it('create_coi_disclosure rejects an invalid disclosure_type', async () => {
    const out = JSON.parse(await getToolHandler('create_coi_disclosure')!({ personnel_id: 1, disclosure_type: 'bribe', entity_name: 'X' }, { organizationId: 1, userId: 1, humanConfirmed: true } as any));
...
describe('Sponsored-programs connector tools — context + input guards', () => {
  it('search_grants_gov requires tenant context', async () => {
    const out = JSON.parse(await getToolHandler('search_grants_gov')!({ query: 'cancer' }, { humanConfirmed: true } as any));
    expect(out.error).toMatch(/tenant context/);
  });
  it('search_grants_gov requires a non-empty query', async () => {
    const out = JSON.parse(await getToolHandler('search_grants_gov')!({ query: '  ' }, { organizationId: 1, humanConfirmed: true } as any));
    expect(out.error).toMatch(/query is required/);
  });
  it('screen_restricted_party requires tenant context', async () => {
    const out = JSON.parse(await getToolHandler('screen_restricted_party')!({ party_name: 'Acme' }, { humanConfirmed: true } as any));
    expect(out.error).toMatch(/tenant context/);
  });
  it('screen_restricted_party requires a party_name', async () => {
    const out = JSON.parse(await getToolHandler('screen_restricted_party')!({}, { organizationId: 1, humanConfirmed: true } as any));
...
describe('Briefing + coverage-gap tools — context + input guards', () => {
  it('research_compliance_briefing requires tenant context', async () => {
    const out = JSON.parse(await getToolHandler('research_compliance_briefing')!({}, { humanConfirmed: true } as any));
    expect(out.error).toMatch(/tenant context/);
  });
  ...
  it('review_ha_interaction requires an interaction_id', async () => {
    const out = JSON.parse(await getToolHandler('review_ha_interaction')!({}, { organizationId: 1, humanConfirmed: true } as any));
    expect(out.error).toMatch(/interaction_id is required/);
  });
  it('register_controlled_substance rejects an invalid schedule', async () => {
    const out = JSON.parse(await getToolHandler('register_controlled_substance')!({ substance_name: 'X', dea_schedule: 'VII' }, { organizationId: 1, userId: 1, humanConfirmed: true } as any));
    expect(out.error).toMatch(/valid dea_schedule/);
  });
  it('prepare_meeting_package requires an interaction_id', async () => {
    const out = JSON.parse(await getToolHandler('prepare_meeting_package')!({}, { organizationId: 1, humanConfirmed: true } as any));
    expect(out.error).toMatch(/interaction_id is required/);
  });
});
