...
describe('submission AI tasks — tenant + input guards', () => {
  it('plan_submission refuses without org/user context', async () => {
    ...
    const out = JSON.parse(await handler({ application_type: 'ind', client_type: 'biotech', regions: ['fda'] }, { humanConfirmed: true } as ToolContext));
    expect(out.error).toMatch(/tenant context/);
  });
  it('plan_submission validates required inputs', async () => {
    const handler = getToolHandler('plan_submission')!;
    const out = JSON.parse(await handler({ client_type: 'biotech', regions: ['fda'] }, { organizationId: 1, userId: 2, humanConfirmed: true } as ToolContext));
  ...
  it('dispatch_qc_check refuses without org/user context', async () => {
    ...
    const out = JSON.parse(await handler({ region: 'fda', validation_errors: 0, unresolved_shadow_criticals: 0, leaves: [] }, { humanConfirmed: true } as ToolContext));
    expect(out.error).toMatch(/tenant context/);
  });
  it('trace_provenance refuses without org/user context', async () => {
    const handler = getToolHandler('trace_provenance')!;
    const out = JSON.parse(await handler({ submission_id: 1, target_section_code: '2.7' }, { humanConfirmed: true } as ToolContext));
    expect(out.error).toMatch(/tenant context/);
  });
  ...
  it('place_into_sequence validates required inputs before touching the core', async () => {
    ...
    const noSeq = JSON.parse(
      await handler({ section_code: '2.7.3', title: 'Summary' }, { organizationId: 1, userId: 2, humanConfirmed: true } as ToolContext),
    );
    expect(noSeq.error).toMatch(/sequence_id/);
    const noSection = JSON.parse(
      await handler({ sequence_id: 1, title: 'Summary' }, { organizationId: 1, userId: 2, humanConfirmed: true } as ToolContext),
    );
    expect(noSection.error).toMatch(/section_code/);
    const noTitle = JSON.parse(
      await handler({ sequence_id: 1, section_code: '2.7.3' }, { organizationId: 1, userId: 2, humanConfirmed: true } as ToolContext),
    );
  ...
  it('check_consistency validates required inputs', async () => {
    ...
    const out = JSON.parse(await handler({ submission_id: 1 }, { organizationId: 1, userId: 2, humanConfirmed: true } as ToolContext));
  ...
  it('assess_pathway_readiness refuses without org/user context', async () => {
    ...
    const out = JSON.parse(await handler({ sequence_id: 1, pathway: 'ctis' }, { humanConfirmed: true } as ToolContext));
    expect(out.error).toMatch(/tenant context/);
  });
  it('assess_pathway_readiness validates the pathway enum', async () => {
    const handler = getToolHandler('assess_pathway_readiness')!;
    const out = JSON.parse(await handler({ sequence_id: 1, pathway: 'bogus' }, { organizationId: 1, userId: 2, humanConfirmed: true } as ToolContext));
  ...
  it('build_pathway_manifest refuses without org/user context', async () => {
    ...
    const out = JSON.parse(await handler({ sequence_id: 1, pathway: 'mdr' }, { humanConfirmed: true } as ToolContext));
    expect(out.error).toMatch(/tenant context/);
  });
  ...
  it('list_validation_rules returns the corpus without tenant context', async () => {
    ...
    const out = JSON.parse(await handler({}, { humanConfirmed: true } as ToolContext));
    expect(out.ok).toBe(true);
  ...
  it('list_validation_rules validates the region enum', async () => {
    const handler = getToolHandler('list_validation_rules')!;
    const out = JSON.parse(await handler({ region: 'zz' }, { humanConfirmed: true } as ToolContext));
  ...
  it('get_market_submission_spec returns a single spec by id', async () => {
    ...
    const out = JSON.parse(await handler({ spec_id: 'us-ectd' }, { humanConfirmed: true } as ToolContext));
    expect(out.ok).toBe(true);
  ...
  it('get_market_submission_spec filters by market without tenant context', async () => {
    const handler = getToolHandler('get_market_submission_spec')!;
    const out = JSON.parse(await handler({ market: 'eu' }, { humanConfirmed: true } as ToolContext));
    expect(out.ok).toBe(true);
  ...
  it('get_market_submission_spec validates the family enum', async () => {
    const handler = getToolHandler('get_market_submission_spec')!;
    const out = JSON.parse(await handler({ family: 'bogus' }, { humanConfirmed: true } as ToolContext));
  ...
  it('get_document_template returns a document spine by id', async () => {
    ...
    const out = JSON.parse(await handler({ template_id: 'clinical_overview' }, { humanConfirmed: true } as ToolContext));
    expect(out.ok).toBe(true);
  ...
  it('get_document_template lists a family without tenant context', async () => {
    const handler = getToolHandler('get_document_template')!;
    const out = JSON.parse(await handler({ family: 'estar' }, { humanConfirmed: true } as ToolContext));
    expect(out.ok).toBe(true);
  ...
  it('validate_market_formatting enforces a spec and flags a bad name', async () => {
    ...
    const out = JSON.parse(await handler({ spec_id: 'us-ectd', leaves: [{ file_name: 'Bad Name.PDF' }] }, { humanConfirmed: true } as ToolContext));
    expect(out.ok).toBe(true);
  ...
  it('validate_market_formatting errors on an unknown spec', async () => {
    const handler = getToolHandler('validate_market_formatting')!;
    const out = JSON.parse(await handler({ spec_id: 'nope', leaves: [] }, { humanConfirmed: true } as ToolContext));
  ...
  it('get_submission_requirements returns and assesses a type', async () => {
    ...
    const out = JSON.parse(await handler({ submission_type: 'nda', present_template_ids: ['quality_overall_summary'] }, { humanConfirmed: true } as ToolContext));
    expect(out.ok).toBe(true);
  ...
  it('assess_pathway_eligibility checks all-criteria-met', async () => {
    ...
    const out = JSON.parse(await handler({ designation: 'fda_breakthrough', answers: { serious: true, preliminary_substantial: true } }, { humanConfirmed: true } as ToolContext));
    expect(out.ok).toBe(true);
    expect(out.assessment.eligible).toBe(true);
  });
  it('assess_pathway_eligibility errors on an unknown designation', async () => {
    const handler = getToolHandler('assess_pathway_eligibility')!;
    const out = JSON.parse(await handler({ designation: 'nope' }, { humanConfirmed: true } as ToolContext));
  ...
  it('classify_post_submission_change recommends a category from flags', async () => {
    ...
    const out = JSON.parse(await handler({ market: 'us', flags: { major_impact: true } }, { humanConfirmed: true } as ToolContext));
    expect(out.ok).toBe(true);
  ...
  it('assess_device_evidence_structure gap-checks a CER', async () => {
    ...
    const out = JSON.parse(await handler({ document: 'cer', present_section_ids: ['summary', 'scope'] }, { humanConfirmed: true } as ToolContext));
    expect(out.ok).toBe(true);
    expect(out.assessment.ready).toBe(false);
  ...
  it('classify_device applies the MDR rules', async () => {
    ...
    const out = JSON.parse(await handler({ framework: 'mdr', facts: { implantable: true, contactsCnsOrCentralCirculation: true } }, { humanConfirmed: true } as ToolContext));
    expect(out.ok).toBe(true);
  ...
  it('get_device_reviewer_checklist returns the 510(k) reviewer questions', async () => {
    ...
    const out = JSON.parse(await handler({ submission_type: '510k' }, { humanConfirmed: true } as ToolContext));
    expect(out.ok).toBe(true);
  ...
  it('classify_device validates the framework', async () => {
    const handler = getToolHandler('classify_device')!;
    const out = JSON.parse(await handler({ framework: 'nope', facts: {} }, { humanConfirmed: true } as ToolContext));
  ...
  it('get_biocompatibility_endpoints returns endpoints for a permanent implant', async () => {
    ...
    const out = JSON.parse(await handler({ nature: 'implant_tissue_bone', duration: 'long_term' }, { humanConfirmed: true } as ToolContext));
    expect(out.ok).toBe(true);
  ...
  it('assess_device_evidence_structure now handles rmf', async () => {
    const handler = getToolHandler('assess_device_evidence_structure')!;
    const out = JSON.parse(await handler({ document: 'rmf', present_section_ids: ['plan'] }, { humanConfirmed: true } as ToolContext));
    expect(out.ok).toBe(true);
    expect(out.assessment.ready).toBe(false);
  ...
  it('build_device_blueprint validates the submission type', async () => {
    const handler = getToolHandler('build_device_blueprint')!;
    const out = JSON.parse(await handler({ submission_type: 'nope' }, { humanConfirmed: true } as ToolContext));
  ...
  it('assess_stored_cer refuses without tenant context', async () => {
    ...
    const out = JSON.parse(await handler({ report_id: 'CER-1' }, { humanConfirmed: true } as ToolContext));
    expect(out.error).toMatch(/tenant context/);
  });
  it('assess_stored_cer requires a report_id', async () => {
    const handler = getToolHandler('assess_stored_cer')!;
    const out = JSON.parse(await handler({}, { organizationId: 1, userId: 2, humanConfirmed: true } as ToolContext));
  ...
  it('build_global_device_strategy maps device evidence across regions', async () => {
    ...
    const out = JSON.parse(await handler({ kind: 'device' }, { humanConfirmed: true } as ToolContext));
    expect(out.ok).toBe(true);
  ...
  it('build_global_device_strategy validates the kind', async () => {
    const handler = getToolHandler('build_global_device_strategy')!;
    const out = JSON.parse(await handler({ kind: 'nope' }, { humanConfirmed: true } as ToolContext));
  ...
  it('get_regulatory_timeline returns the 510(k) RTA + decision goal', async () => {
    ...
    const out = JSON.parse(await handler({ pathway: '510k' }, { humanConfirmed: true } as ToolContext));
    expect(out.ok).toBe(true);
  ...
  it('build_device_blueprint now includes a readiness scorecard', async () => {
    const handler = getToolHandler('build_device_blueprint')!;
    const out = JSON.parse(await handler({ submission_type: '510k', classification: { framework: 'fda', facts: { fdaClass: 'II', predicateAvailable: true } } }, { humanConfirmed: true } as ToolContext));
    expect(out.ok).toBe(true);
  ...
  it('validate_udi computes the GS1 check digit', async () => {
    ...
    const ok = JSON.parse(await handler({ udi: '(01)00012345678905(10)LOT1' }, { humanConfirmed: true } as ToolContext));
    expect(ok.ok).toBe(true);
    expect(ok.udiDiOk).toBe(true);
    const bad = JSON.parse(await handler({ udi: '(01)00012345678900' }, { humanConfirmed: true } as ToolContext));
  ...
  it('get_electrical_standards adds collaterals from facts', async () => {
    ...
    const out = JSON.parse(await handler({ electricallyPowered: true, hasAlarms: true }, { humanConfirmed: true } as ToolContext));
    expect(out.ok).toBe(true);
  ...
  it('get_sterilization_requirements resolves the method standard', async () => {
    ...
    const out = JSON.parse(await handler({ sterile: true, method: 'eo' }, { humanConfirmed: true } as ToolContext));
    expect(out.ok).toBe(true);
  ...
  it('assess_combination_product maps PMOA to the lead center', async () => {
    ...
    const out = JSON.parse(await handler({ components: ['drug', 'device'], primary_mode_of_action: 'device' }, { humanConfirmed: true } as ToolContext));
    expect(out.ok).toBe(true);
  ...
  it('assess_combination_product validates components', async () => {
    const handler = getToolHandler('assess_combination_product')!;
    const out = JSON.parse(await handler({ components: ['nope'] }, { humanConfirmed: true } as ToolContext));
  ...
  it('assess_qms returns the clause structure and gap-checks', async () => {
    ...
    const structure = JSON.parse(await handler({}, { humanConfirmed: true } as ToolContext));
    expect(structure.ok).toBe(true);
    expect(structure.clauses.some((c: { id: string }) => c.id === 'capa')).toBe(true);
    const assessed = JSON.parse(await handler({ present_clause_ids: ['qms_general'] }, { humanConfirmed: true } as ToolContext));
  ...
  it('get_device_labeling adds sterile elements for a sterile device', async () => {
    ...
    const out = JSON.parse(await handler({ sterile: true }, { humanConfirmed: true } as ToolContext));
    expect(out.ok).toBe(true);
...
describe('ingestion tools — tenant + input guards', () => {
  ...
  it('extract_submission_document validates required inputs', async () => {
    const handler = getToolHandler('extract_submission_document')!;
    const out = JSON.parse(await handler({ document_id: 1 }, { organizationId: 1, userId: 2, humanConfirmed: true } as ToolContext));
