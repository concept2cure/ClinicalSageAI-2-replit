 ...
 *
 * Both tools are tenant-scoped via `orgId` from model input (the handlers in
 * AnaToolExecutor further restrict via ToolContext). They are pure queries over
 * governed internal data (no LLM, no network) → `rim_learned` pedigree.
 *
...
export const SUMMARIZE_RIM_INTELLIGENCE: AnaTool = {
  ...
  description:
    "Get a high-level summary of accumulated Regulatory Intelligence Model (RIM) intelligence for an organization — total pattern count, domain breakdown with counts, top patterns by occurrence, and the date range of observations. Use to understand the breadth and depth of what the platform has learned for this org. Tenant-scoped. DETERMINISTIC: pure query over governed internal data, no LLM, no network.",
  input_schema: {
    type: 'object',
    properties: {
      orgId: {
        type: 'number',
        description: 'Organization ID to summarize intelligence for.',
      },
    },
    required: ['orgId'],
  },
