...
const traceDownstreamMock = vi.fn(async (..._args: any[]) => ({ nodes: [], edges: [] } as any));
const traceUpstreamMock = vi.fn(async (..._args: any[]) => ({ nodes: [], edges: [] } as any));

vi.mock('../../data-lineage-service', () => ({
  recordLineageBatch: (e: unknown[]) => recordLineageBatchMock(e),
  traceDownstream: (...args: any[]) => traceDownstreamMock(...args),
  traceUpstream: (...args: any[]) => traceUpstreamMock(...args),
}));
