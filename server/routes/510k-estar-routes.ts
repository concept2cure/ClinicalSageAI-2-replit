      ...
      ...(attachments?.length
        ? {
            ...
            /* The resolver is built per request and used once. It carries the
               ...
               `DeviceContentClient` is a raw pg-style query surface, not the
               Drizzle relational builder `requestDb` returns, so this takes
               `requestPgClient` — the same request-scoped, RLS-pinned raw
               client `part11-compliance.ts` and `cerv2-document-routes.ts`
               already use for this exact shape of call.
               `RequestSqlClient.query` is declared non-generic (rows typed as
               `Record<string, unknown>`), while `DeviceContentClient.query` is
               generic in the row type — the same widening `part11-compliance.ts`
               casts through for the same reason, confined to this one call. */
            attachmentResolver: createDeviceAttachmentResolver({
              organizationId: getOrganizationId(req),
              programUuid: anchor.programUuid,
              client: requestPgClient(req) as unknown as DeviceContentClient,
            }),
          }
        : {}),
    });
