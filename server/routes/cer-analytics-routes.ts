...
router.get('/:ndc_code', async (req: Request, res: Response) => {
  try {
    const ndcCodeRaw = req.params.ndc_code; const ndc_code = Array.isArray(ndcCodeRaw) ? ndcCodeRaw[0] : (ndcCodeRaw ?? "");
...
router.get('/forecast/:ndc_code/:event', async (req: Request, res: Response) => {
  try {
    const ndcCodeRaw = req.params.ndc_code; const ndc_code = Array.isArray(ndcCodeRaw) ? ndcCodeRaw[0] : (ndcCodeRaw ?? ""); const eventRaw = req.params.event; const event = Array.isArray(eventRaw) ? eventRaw[0] : (eventRaw ?? "");
