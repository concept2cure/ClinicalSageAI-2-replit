...
import { authenticate as authenticateJWT, requireRole, requirePermission } from '../middleware/authAdapter';
...
const createAuditLog = async (
  ...
  try {
    ...
    const dataIntegrityCheck = crypto
      .createHash('sha256')
      .update(recordString)
      .digest('hex');
...
router.get('/documents/:id', authenticateJWT, checkDocumentPermission('read'), async (req: Request, res: Response) => {
  try {
...
router.post('/documents', authenticateJWT, documentRateLimiter, async (req: Request, res: Response) => {
  try {
    const userId = Number(req.user?.id || req.userId || 1);
    const organizationId = Number(req.user?.organizationId || req.tenantContext?.organizationId || 1);
...
router.put('/documents/:id', authenticateJWT, checkDocumentPermission('write'), documentRateLimiter, async (req: Request, res: Response) => {
  try {
    const documentId = parseInt(req.params.id);
    const userId = Number(req.user?.id || req.userId || 1);
...
router.post('/documents/:id/versions', authenticateJWT, checkDocumentPermission('write'), async (req: Request, res: Response) => {
  try {
    const documentId = parseInt(req.params.id);
    const userId = Number(req.user?.id || req.userId || 1);
    ...
    const versionData = insertDocumentVersionSchema.parse({
      ...
      createdById: userId,
      checksum: crypto.createHash('sha256').update(req.body.content || '').digest('hex')
    });
...
router.get('/documents/:id/versions', authenticateJWT, checkDocumentPermission('read'), async (req: Request, res: Response) => {
  try {
    const documentId = parseInt(req.params.id);
...
router.get('/audit-trail', authenticateJWT, requireRole('admin'), async (req: Request, res: Response) => {
  try {
    const { 
      documentId, 
      userId, 
...
router.get('/audit-trail/:id', authenticateJWT, requireRole('admin'), async (req: Request, res: Response) => {
  try {
...
router.post('/documents/:id/locks', authenticateJWT, validateSession, async (req: Request, res: Response) => {
  try {
    const documentId = parseInt(req.params.id);
    const userId = Number(req.user?.id || req.userId || 1);
    ...
    const lock = await db!.insert(documentLocks).values({
      documentId,
      sectionId,
      lockType,
      lockedById: userId,
...
router.delete('/documents/:id/locks/:lockToken', authenticateJWT, async (req: Request, res: Response) => {
  try {
    const documentId = parseInt(req.params.id);
...
router.post('/documents/:id/export', authenticateJWT, checkDocumentPermission('read'), async (req: Request, res: Response) => {
  try {
    const documentId = parseInt(req.params.id);
    ...
    switch (format.toLowerCase()) {
      ...
      case 'docx':
        ...
        exportData.documentFormat = 'application/vnd.openxmlformats-officedocument.wordprocessingml.document';
        break;
      case 'pdf':
...
router.post('/integrations/lims/sync', authenticateJWT, requireRole('admin'), async (req: Request, res: Response) => {
  try {
...
router.get('/documents/:id/permissions', authenticateJWT, checkDocumentPermission('admin'), async (req: Request, res: Response) => {
  try {
    const documentId = parseInt(req.params.id);
...
router.put('/documents/:id/permissions', authenticateJWT, checkDocumentPermission('admin'), async (req: Request, res: Response) => {
  try {
    const documentId = parseInt(req.params.id);
...
router.post('/documents/:id/comments', authenticateJWT, validateSession, async (req: Request, res: Response) => {
  try {
    const documentId = parseInt(req.params.id);
    const userId = Number(req.user?.id || req.userId || 1);
...
router.get('/documents/:id/comments', authenticateJWT, checkDocumentPermission('read'), async (req: Request, res: Response) => {
  try {
    const documentId = parseInt(req.params.id);
