...
const C2C = 'server/routes/c2c/documents.ts';

describe('AI draft — retrieval outcome is reported, not swallowed', () => {
  it.each([AUTHORING, C2C])('%s distinguishes failed retrieval from an empty corpus', (file) => {
    const src = read(file);
  ...
  it.each([AUTHORING, C2C])('%s records the failure reason rather than only logging it', (file) => {
    const src = read(file);
  ...
  it.each([AUTHORING, C2C])('%s reports the status to the caller', (file) => {
    const src = read(file);
  ...
  it('the failure reason is bounded, so a driver error cannot flood the response', () => {
    for (const file of [AUTHORING, C2C]) {
