...
router.post('/', async (req, res) => { const p = certSchema.safeParse(req.body ?? {}); if (!p.success) return res.status(400).json({ error: { code: 'VALIDATION', details: p.error.flatten() } }); await governed(req, res, 'create', p.data.reason, async (c, o, u) => { const { id } = await createCertificationTx(c, o, u, p.data); recordEffortStatementCreated(); return { target: `effort-certification:${id}`, body: { id } }; }); });
