...
/*
 * The organization UUID — the tenant key every NON-public schema, lumen_data_atoms
 * and the vault are stored against, and NOT interchangeable with the integer
 * getSecureOrgId returns (a uuid column raises on an integer, which reads as an
 * outage) — comes from server/db/currentTenant.ts (currentTenantOrgUuid): the
 * verified tenant scope's, never a request header. A request-based accessor
 * lived here with no production caller while nine routes read the client's
 * x-org-uuid header instead; it was folded into that one, 2026-09-24.
 */
