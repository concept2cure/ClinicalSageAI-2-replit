...
function makeFakeDb() {
  // Two documents in two different tenancies, and one attachment on DOC_A.
  ...
  const selectBuilder = () => {
    ...
    const q: any = {
      ...
      then(res: any, rej: any) {
        return Promise.resolve((rows[table] ?? []).filter(r => matches(r, filter))).then(res, rej);
      },
    };
    return q;
  };

  const insertBuilder = (t: unknown) => {
  ...
  const deleteBuilder = (t: unknown) => {
    const table = tableName(t);
    let filter: Record<string, unknown> = {};
    const settle = () => {
      const target = rows[table] ?? [];
      const removed = target.filter(r => matches(r, filter));
      // Actually remove them, so a second delete finds nothing.
      rows[table] = target.filter(r => !matches(r, filter));
      return removed;
    };
    const q: any = {
      where(c: any) { filter = readEqualities(c); return q; },
  ...
  return {
    ...
    auditRows: () => rows[AUDIT],
    auditActions: () => rows[AUDIT].map(r => r.action),
  };
}
...
describe('Attachments — tenant scoping', () => {
  it('refuses to attach to another organization\'s document, and writes nothing', async () => {
    ...
    expect(fake.attachments()).toHaveLength(1); // the pre-existing one only
    expect(fake.auditActions()).toEqual([]);
  });
