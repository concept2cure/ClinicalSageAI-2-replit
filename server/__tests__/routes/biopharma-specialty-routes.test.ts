...
import { pediatricRouter, orphanRouter, supplementsRouter } from '../../routes/biopharma-specialty.routes';

function appWith(router: Router, prefix: string, org: number | null) {
  const app = express();
...
const CASES = [
  {
    name: 'pediatric',
    router: pediatricRouter,
    prefix: '/api/biopharma/pediatric',
  ...
  {
    name: 'supplements',
    router: supplementsRouter,
    prefix: '/api/biopharma/supplements',
...
for (const c of CASES) {
  describe(`biopharma ${c.name} route`, () => {
    it('403 without org context', async () => {
      const res = await request(appWith(c.router, c.prefix, null)).get(c.prefix);
      expect(res.status).toBe(403);
    });

    it('returns org-scoped rows shaped to the display contract', async () => {
      query.mockResolvedValueOnce({ rows: [c.row] });
      const res = await request(appWith(c.router, c.prefix, 7)).get(c.prefix);
    ...
    it('fails closed to an empty envelope when the store is not provisioned', async () => {
      ...
      const res = await request(appWith(c.router, c.prefix, 1)).get(c.prefix);
      expect(res.status).toBe(200);
