...
router.get('/sites', async (req: Request, res: Response) => {
  try {
    const { 
      programId, projectId, country, status, 
      minScore, sortBy = 'composite_score', sortDir = 'DESC',
      limit = 50, offset = 0
    } = req.query;
    ...
    const validSortFields = ['composite_score', 'enrollment_score', 'quality_score', 'operational_score', 'created_at'];
...
router.post('/sites', async (req: Request, res: Response) => {
  try {
    const { 
      programId, projectId, siteNumber, siteName,
      investigatorName, address, city, state, country,
      zipCode, phone, email, status = 'candidate',
      targetEnrollment, createdBy
    } = req.body;
    ...
    `, [
      programId, projectId, siteNumber, siteName,
      investigatorName, address, city, state, country,
      zipCode, phone, email, status, targetEnrollment, createdBy
    ]);
...
router.get('/sites/:id', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const { includeMetrics = 'true', includeRisks = 'true', includePredictions = 'true' } = req.query;
...
router.post('/sites/:id/metrics', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const { 
      metricDate, enrollmentActual, enrollmentTarget, screeningRate,
      screenFailRate, dropoutRate, queryRate, queryResolutionDays,
      protocolDeviations, saeCount, averageDataEntryDays, source
    } = req.body;
    ...
    const result = await pool.query(`
      ...
      RETURNING *
    `, [
      id, metricDate || new Date(), enrollmentActual, enrollmentTarget,
      screeningRate, screenFailRate, dropoutRate, queryRate,
      queryResolutionDays, protocolDeviations, saeCount,
      averageDataEntryDays, source || 'manual'
    ]);
...
router.post('/sites/:id/risks', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const { 
      riskCategory, riskDescription, likelihood, impact,
      mitigation, owner, dueDate, createdBy
    } = req.body;
    ...
    const result = await pool.query(`
      ...
      RETURNING *
    `, [
      id, riskCategory, riskDescription, likelihood, impact,
      riskScore, mitigation, owner, dueDate, createdBy
    ]);
...
router.post('/sites/:id/predictions', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const { 
      predictionDate, predictedEnrollment, confidenceInterval,
      modelVersion, factors
    } = req.body;
    ...
    const result = await pool.query(`
      ...
      RETURNING *
    `, [
      id, predictionDate, predictedEnrollment,
      JSON.stringify(confidenceInterval), modelVersion, JSON.stringify(factors)
    ]);
