...
router.get('/documents', async (req: Request, res: Response) => {
  try {
    const { programId, productId, status, documentType, limit = 50, offset = 0 } = req.query;
...
router.post('/simulations', async (req: Request, res: Response) => {
  try {
    const { documentId, simulationName, proposedChanges, createdBy } = req.body;
    ...
    const result = await pool.query(
      ...
      RETURNING *
    `,
      [documentId, simulationName, JSON.stringify(proposedChanges), overallImpactScore, createdBy]
    );
