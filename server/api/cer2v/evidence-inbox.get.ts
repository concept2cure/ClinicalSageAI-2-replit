...
export default async function handler(req: Request, res: Response) {
  try {
    ...
    const inbox = evidenceRows
      .filter(row => !linkedEvidenceIds.has(String(row.evidenceId)))
      .map(row => ({
    ...
    return res.json({ success: true, evidence: inbox });
  } catch (error: any) {
    return res.status(500).json({
      success: false,
      error: error?.message || 'Failed to load evidence inbox',
    });
  }
}
