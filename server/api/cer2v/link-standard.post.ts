...
export default async function handler(req: Request, res: Response) {
  try {
    ...
    if (!standardId || !evidenceId) {
      return res.status(400).json({ success: false, error: 'standard id and evidenceId are required' });
    }
    if (!db || !programId) {
    ...
    return res.status(500).json({
      success: false,
      error: error?.message || 'Failed to link evidence to standard',
    });
  }
}
