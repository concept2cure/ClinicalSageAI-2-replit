...
// Paths exempt from CSRF (they have their own verification). Matched as exact
// path or as a directory prefix ending in '/'. Bare prefix matching (e.g.
// '/healthz' allowing '/healthzevil') is intentionally avoided to prevent
// crafted-path bypass of the exempt list.
const EXEMPT_PATHS = [
...
function tokensMatch(a: unknown, b: unknown): boolean {
  ...
  // Reject anything that isn't a fixed-length hex token so timingSafeEqual
  // can't be probed with attacker-controlled lengths.
