...
import { isPublicApiPath } from './public-api-allowlist';

const defaultLogger = createScopedLogger('auth-boundary');

export type AuthBoundaryMode = 'enforce' | 'warn';

// The allowlist is data the launch-scope verdict reads too (a path the boundary
// serves unauthenticated has a caller no screen shows), so it lives in a module
// with no imports. Moved unchanged; re-exported here for existing callers.
export { PUBLIC_API_ALLOWLIST, isPublicApiPath } from './public-api-allowlist';
