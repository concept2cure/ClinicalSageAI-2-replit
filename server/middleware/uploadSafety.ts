 ...
 *
 * ── Origin ──
 * The AV half is an INGRESS control: it exists because the bytes in hand were
 * chosen by whoever made the request. Bytes the PLATFORM produced in-process —
 * the official FDA eSTAR retained into the vault — are not ingress, and tying
 * them to clamd would make a working export depend on a daemon this repository
 * configures nowhere and which, unconfigured, is a 503 in production by design
 * (AV-01, docs/audit-2026-07/12-findings-register.md). `origin` says which case
 * a caller is in. It defaults to `upload`, so every existing call keeps the
 * fail-closed posture, and it reaches no HTTP schema — only a server-side call
 * site can name `platform-generated`. The signature check runs either way.
 *
...
/** Where the bytes came from — see the module header's Origin note. */
export type UploadOrigin = 'upload' | 'platform-generated';

export interface UploadSafetyOptions {
  /** Defaults to 'upload'. Never derived from request input. */
  origin?: UploadOrigin;
}
...
export async function assertUploadSafe(
  ...
  // 2) Antivirus scan — skipped for bytes this process produced (module
  //    header, Origin). Logged, so a skip is never invisible.
  if ((options.origin ?? 'upload') === 'platform-generated') {
    logger.info('Content scan skipped for platform-generated bytes', { filename, declaredMime });
    return;
  }
