...
// `clientSecret`, `accessToken`, `Authorization` are all caught.
 ...
 * path like '/api/healthEvil' can't slip past the exclude list.
 */
...
function createAuditLogger(options = {}) {
  ...
  return function auditLogger(req, res, next) {
    ...
    // Wrap res.end so we observe the response without altering its signature.
    // Express/Node's res.end has several overloads ((), (chunk), (chunk, cb),
    // (chunk, encoding), (chunk, encoding, cb)). We forward ALL arguments via
    // rest-spread so callbacks and encoding hints are preserved.
    ...
    res.on('finish', () => {
      ...
      if (logResponseData && capturedChunk) {
        try {
          const text =
            typeof capturedChunk === 'string' ? capturedChunk : capturedChunk.toString('utf8');
          ...
          // Non-JSON response: skip the body entirely rather than log raw bytes
          // that may contain binary or sensitive content.
        }
      }
