...
/**
 * Platform-generated bytes.
 *
 * The AV scan is an INGRESS control: it exists because multer just wrote
 * attacker-chosen bytes into memory. The official FDA eSTAR the platform
 * retains into the vault is not ingress — it is produced in-process from a
 * template that ships in the image, filled with text from the tenant's own
 * governed records, and hashed on the way out.
 *
 * Routing it through the upload gate unchanged would tie a working export to
 * clamd: `CLAMAV_HOST` is configured nowhere in this repository, and in
 * production an unconfigured scanner is a 503 by design (AV-01 in
 * docs/audit-2026-07/12-findings-register.md — a deploy without it already
 * kills every governed upload). An export that only works where an unrelated
 * daemon is reachable is a worse outcome than a scan that was never the right
 * control for these bytes.
 *
 * So the origin is explicit, it defaults to `upload`, and it is not settable
 * from any request body — only a server-side call site can name it. The
 * signature check still runs: it costs nothing and catches a corrupt or
 * mistyped buffer.
 */
describe('assertUploadSafe — platform-generated bytes', () => {
  beforeEach(() => {
    delete process.env.CLAMAV_HOST;
    delete process.env.CLAMAV_PORT;
  });

  it('does not reject in production when no scanner is configured', async () => {
    process.env.NODE_ENV = 'production';
    await expect(
      assertUploadSafe(PDF_BUF, 'application/pdf', 'K-1_eSTAR.pdf', { origin: 'platform-generated' }),
    ).resolves.toBeUndefined();
  });

  it('still rejects bytes that do not match the declared type (400)', async () => {
    process.env.NODE_ENV = 'production';
    await expect(
      assertUploadSafe(Buffer.from('not a pdf at all'), 'application/pdf', 'K-1_eSTAR.pdf', {
        origin: 'platform-generated',
      }),
    ).rejects.toMatchObject({ status: 400, code: 'FILE_SIGNATURE_MISMATCH' });
  });

  it('leaves an upload with no stated origin fail-closed in production', async () => {
    process.env.NODE_ENV = 'production';
    await expect(assertUploadSafe(PDF_BUF, 'application/pdf', 'user.pdf')).rejects.toMatchObject({
      status: 503,
      code: 'FILE_SCAN_UNAVAILABLE',
    });
  });
