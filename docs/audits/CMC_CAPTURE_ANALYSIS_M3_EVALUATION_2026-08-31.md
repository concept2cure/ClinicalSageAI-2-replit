...
| 5. Persist the guided flow | **landing (2026-09-06)** | `cmc_interview_sessions` (org-scoped, purge-listed) holds the FlowState per step; the AnA flow tools start/advance/resume by `session_id`; a commit projector turns a completed CMC interview into register writes through each register's one canonical create — the seven creates were extracted from their route handlers into `services/cmc/register-writes.ts`, which the routes and the projector both call (plan is pure and pinned; a refused entry stops the commit with the refusal and what landed is reported, never hidden). The `visibleWhen` fix passes the prior answers into validation, which had judged predicates over a prior node's field against `undefined`. Thirty dead suggested actionTypes across the flow categories now name registered tools. §3.2.P.2's two absent producers now exist: the manufacturing register's process development → `manufacturingProcessDev`, and a new `formulation_development` column on the formulation record → `formulationDevelopment`. |
| 6. Analysis on the spine | **done for what the registers can support** | Over the canonical tables, refusing rather than guessing: ICH Q3A/Q3B thresholds (unit- and dose-keyed, "greater than" not "at"); ICH Q1E shelf life with the `min(2×observed, observed+12)` extrapolation cap and a two-sided criterion parser; ICH Q3C(R8) residual solvents and Q3D(R2) elemental impurities on one shared catalog with numeric limits (the private copy in cmc-quality deleted, its fail-open default removed); ICH M7(R2) mutagenic impurities — class from the recorded Ames result and structural alert, TTC staged by treatment duration, cohort-of-concern limits — with five new register inputs, none defaulted (found and fixed: the M7 engine's ppm limit was AI/MDD×1000, a thousandfold too high); PhRMA regression-control-chart out-of-trend detection over recorded stability series with slope CI and projected time to the limit, rendered per series in §3.2.S.7/§3.2.P.8; computed conformance for stability and batch results (`qcResultVerdict`); the second `estimateShelfLife` (EP25 scan) deleted onto the canonical one so the diagnostics route applies the Q1E cap. Process capability (ICH Q6A specification; ISO 22514 Pp/Ppk on the overall sd, Cp/Cpk on the moving-range sigma) per test across recorded batches, rendered in §3.2.S.4.4/§3.2.P.5.4 once the QC register carries the batch a sample represents (`qc_testing.batch_number`, `project_id`), refusing under six batches, without a criterion, or across batches recorded against different criteria; the IVD calculator is an adapter over the same engine. Both analyses are now askable without compiling: `POST /api/cmc/stability-studies/:id/trending` and the `assess_recorded_stability_trend` tool over a recorded study, `GET /api/cmc/projects/:id/process-capability` and `assess_recorded_process_capability` over a project's QC register — the capability pair calling the same assembly the section renders (`services/cmc/recorded-capability`) and the trending pair the same one §3.2.S.7/§3.2.P.8 render (`services/cmc/recorded-stability`), so in each case the route, the tool and the document cannot disagree, and a series that cannot be assessed is returned with its reason rather than omitted. f2 dissolution similarity is computed by the dissolution comparison engine (`services/cmc/dissolution-comparison.ts`) from recorded profiles with its eligibility conditions, and §3.2.P.2 defers to it rather than asserting similarity. |
| 7. Plumbing guards | **done** | `writeThroughToCanonicalSource` returns a discriminated outcome instead of null, so a propagation failure is observable; every register POST/PUT (the thirteen older fire-and-forget sites included) awaits one `linkToModule3` path and reports `module3Linked`/`module3Warning`, metered on failure. The OS source-object enum is derived from the single `CMC_SOURCE_TYPES` list and accepts every composer-demanded type; `impactedSectionsForSourceType` unions the appendix rules so an approved 3.2.A.* goes stale. Completeness has one definition (the composer's): the key-presence copy in the convergence service and the build-state route's fabricated 100 are deleted; the export gate, readiness read, board and Overview all read it, the gate and the approve route refuse an approved-but-incomplete section by name, and a failed board read renders as a failed read. |
