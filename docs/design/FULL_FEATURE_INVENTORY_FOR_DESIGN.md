...
> - `design-system/colors_and_type.css` (repo root, 21,059 bytes) — **the declared canonical source of truth**, imported first by `client/src/index.css` (`@import "../../design-system/colors_and_type.css"`) and by `main.tsx`. Also mirrored to `client/public/design-system/colors_and_type.css`.
...
- **Controls & actions:**
  - Header: **"Open §11 in editor"** (→ `onOpenEditor(11)`), **"Export 510(k) package"** (AnA: assemble Module 6 PDF + Form FDA 3514 + attachments draft ZIP — explicitly *not* the official eSTAR), **"Generate official eSTAR (PDF)"** — disabled-with-reason gated by `useEstarReadiness('510k','device')`; title explains blockers; POSTs to governed `/api/510k/estar/official`.
...
- **Data:** `useK510EstarSections`, `useK510Predicates`, `useK510SeMatrix`, `useEstarReadiness`; fixtures `K510_STAGES/PREDICATES/SE_ROWS/ESTAR`. 20 canonical eSTAR sections (e.g. §11 Substantial Equivalence Discussion, §19 Performance Testing — Clinical [blocker]).
...
- **Sample-size calculator** inputs are uncontrolled with no compute endpoint yet (audit-only server compute planned per docs/legacy/PHASE_11_INSTALL.md §3); the displayed sample size is the fixture value, not recomputed.
