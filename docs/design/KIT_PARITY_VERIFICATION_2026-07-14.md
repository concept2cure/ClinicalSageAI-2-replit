...
| Three-zone shell — nav rail · AnA conversation first · work pane | ✅ | `client/src/concept2cure/v2/Shell.tsx` (`rail` nav + persistent `AnaRail` co-author + work pane; the `SURFACE_VIEWS` `ownsConversation` flag — since renamed from `hideAna`, and now the discriminating member of the `SurfaceView` union — yields the rail's column to editor surfaces that carry their own right pane, and drops `onAsk` from their props so the question cannot be sent to a rail that is not there) |
