...
- **eCTD validation & publishing** — `ectd-export.ts`(/validate) · ✅ · Completeness + cross-module consistency · ops: `POST /api/ectd/export/:submissionId/validate` · UI: ✓/⚠/✗ per section, "proceed to submission" gate.
  - `document-understanding.ts` was cited here too. Removed 2026-09-10: it performs no eCTD validation. It is rule-based text-structure extraction whose document-type classifier happens to carry an `ectd_module` regex, and it described itself as a LayoutLMv3 service until that date.
