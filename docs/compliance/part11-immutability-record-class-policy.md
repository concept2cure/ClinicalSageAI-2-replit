...
| Orchestrator step event (`submission_orchestrator_steps`) | Append-only     | New status = new event row. The FK from runs is **`NO ACTION`**, not the `ON DELETE CASCADE` this row asserted until 2026-09-18: deleting a run that has step events is REFUSED by the database (`violates foreign key constraint … is still referenced`), so the events survive. That is the protection the old note called for as "a future change" — it is already in force, and the note understated it in the one direction that matters, telling a reader the events were cascade-deletable. The shape is drizzle's default: push creates this table, so the `ON DELETE CASCADE` both SQL definitions declare never runs, and `db/migrations/20260730_orchestrator_run_ledger_hardening.sql`'s swap to RESTRICT silently no-ops because its `DO` block matches `confdeltype = 'c'` only. NO ACTION and RESTRICT both refuse the delete here (they differ only in when the check fires), so the append-only guarantee holds either way. |
