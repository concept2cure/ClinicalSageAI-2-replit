...
| Transmit path (packages) | `server/routes/submission-ops.ts` (identifiers / assemble / preflight), `server/services/ectd/section-to-ctd.ts`, `server/services/ectd/{package-content-fingerprint,package-content-change}.ts`, `server/services/submission-gateways/governed-transmit.ts`, `client/src/concept2cure/v2/surfaces/GatewayTransmittals.tsx` | Record identifiers → assemble (one leaf per artifact, placeable headings only, findings persisted, content fingerprint recorded) → transmit, on one package id, from the Dispatch surface or the API. Governed transmit recomputes the fingerprint from the database and refuses a bundle the package has moved past (`BUNDLE_CONTENT_DRIFT`; no fingerprint = `BUNDLE_CONTENT_UNPROVEN`). `package-content-change.ts` is the single implementation of the invalidation rule (package row lock, content revision, stale-bundle + preflight clear, governed mapping), used by the mapping routes, the assemble store, the artifact editor and the biostatistics workflow alike. Operator runbook: `docs/runbooks/ectd-transmit-path.md`. |
