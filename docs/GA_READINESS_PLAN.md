...
|---|---|---|---|
| 0.1 | **Complete production RLS evidence** (`RLS_ENFORCE=on`) | Production boot now requires canonical `on`, request/pool helpers fail closed, and connection poisoning is unit-tested. Adoption and database proof remain incomplete: 25 raw-SQL candidates, 77 shared-pool route files, and live per-policy tests are still open. See `docs/RLS_ENFORCEMENT_BURNDOWN.md`. | Live two-tenant reads/writes and connection-failure reuse show no leak; every route/job/tool path is scoped or explicitly justified; boot fails closed if enforcement is missing in prod. |
