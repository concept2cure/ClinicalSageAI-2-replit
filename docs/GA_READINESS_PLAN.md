...
|---|---|---|---|
| 0.1 | **Complete production RLS evidence** (`RLS_ENFORCE=on`) | Production boot requires canonical `on`; protected requests, alternative-auth entry points, schedulers, and workers are scoped; all 25 raw-SQL findings and 77 shared-pool routes are dispositioned by passing parity/coverage gates. Live PostgreSQL policy and connection-reuse proof remains open. See `docs/RLS_ENFORCEMENT_BURNDOWN.md`. | Live two-tenant reads/writes and connection-failure reuse show no leak; every route/job/tool path is scoped or explicitly justified; boot fails closed if enforcement is missing in prod. |
