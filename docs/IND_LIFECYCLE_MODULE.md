...
| Cross-reference register (every external file the IND depends on, with LOA coverage; a reference without an LOA on file is a blocking error — the RA management view of m1.4) | 21 CFR 314.420 | `ind-lifecycle/ind-cross-reference-service` (`buildCrossReferenceRegister`) | `POST /api/ind-lifecycle/cross-reference-register` |
