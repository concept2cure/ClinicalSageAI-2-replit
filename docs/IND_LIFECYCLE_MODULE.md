...
| ICSR composer (intake case → ICH E2B(R3) data elements + escaped XML projection + completeness/gaps; H.1 narrative from the shared safety-narrative writer) | ICH E2B(R3) / 21 CFR 312.32 | `ind-lifecycle/e2b-icsr-composer` | `POST /api/ind-lifecycle/safety-report/icsr` (JSON; `?format=xml` for the E2B(R3) XML) |
