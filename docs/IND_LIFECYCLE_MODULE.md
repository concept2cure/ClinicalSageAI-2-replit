...
| Letter of Authorization (cross-reference to a DMF / IND / NDA / BLA; model + gaps + m1.4.1 placement intent + PDF) | 21 CFR 314.420 | `ind-lifecycle/ind-loa-service` | `POST /api/ind-lifecycle/loa`, `/loa/pdf` |
| Statement of Right of Reference (m1.4.2) + List of Authorized Persons (m1.4.3) — models + gaps + placement intents + PDFs | 21 CFR 314.420 | `ind-lifecycle/ind-cross-reference-service` | `POST /api/ind-lifecycle/right-of-reference(/pdf)`, `/authorized-persons(/pdf)` |
| Cross-reference register (every external file the IND depends on, with LOA coverage; a reference without an LOA on file is a blocking error — the RA management view of m1.4) | 21 CFR 314.420 | `ind-lifecycle/ind-cross-reference-service` (`buildCrossReferenceRegister`) | `POST /api/ind-lifecycle/cross-reference-register` (pure compute) |
| **Persisted** cross-references (durable, tenant-scoped, audited per-submission register; record each DMF/IND/NDA/BLA dependency once, mark the LOA on file as it arrives, register recomputed from stored rows) | 21 CFR 314.420 / Part 11 | `ind-lifecycle/ind-cross-reference-persistence`, schema `shared/schema/ind-cross-references`, migration `20260614_ind_cross_references.sql` | `POST/GET /api/ind-lifecycle/submission/:id/cross-references`, `GET …/cross-reference-register`, `PATCH`/`DELETE /api/ind-lifecycle/cross-references/:id` |
| File the LOA for a tracked cross-reference (renders the LOA from the stored dependency, creates an amendment sequence with the checksummed m1.4.1 leaf, and flips loaOnFile → register becomes ready) | 21 CFR 314.420 / Part 11 | `ind-lifecycle/ind-lifecycle-persistence` (`persistCrossReferenceFiling`) | `POST /api/ind-lifecycle/submission/:id/cross-references/:crossRefId/file-loa` |
...
| **Persisted** IND Safety Reports (durable, tenant-scoped, audited drafts tracked draft→filed; obligation/deadline/document stored; overdue feed; filing marks the draft filed + links its sequence) | 21 CFR 312.32 / Part 11 | `ind-lifecycle/ind-safety-report-persistence`, schema `shared/schema/ind-safety-reports`, migration `20260614_ind_safety_reports.sql` | `POST/GET /api/ind-lifecycle/submission/:id/safety-reports`, `GET …/safety-reports/overdue`; `/safety-report/file` accepts an optional `draftId` |
...
| ICSR transmission (wraps the composed elements in the ICH ICSR batch + message header N.1.*/M.1.* → full transmittable message; transmit-readiness gate on mandatory gaps; FAERS/EudraVigilance receiver defaults; ACK parsing AA/AE/AR) | ICH E2B(R3) | `ind-lifecycle/e2b-icsr-message` | `POST /api/ind-lifecycle/safety-report/icsr/message` (JSON; `?format=xml` for the message). The byte-level AS2 transport is a thin env-dependent adapter over this message. |
| **Persisted** ICSR transmissions (durable, tenant-scoped, audited; tracked prepared→transmitted→acknowledged/rejected; transmit gated on readiness; agency ACK AA/AE/AR recorded) | ICH E2B(R3) / Part 11 | `ind-lifecycle/ind-icsr-transmission-persistence`, schema `shared/schema/ind-icsr-transmissions`, migration `20260615_ind_icsr_transmissions.sql` | `POST/GET /api/ind-lifecycle/submission/:id/icsr-transmissions`, `POST /api/ind-lifecycle/icsr-transmissions/:id/transmit`, `…/acknowledge` |
...
| **Persisted** IND Annual Reports (durable, tenant-scoped, audited drafts tracked draft→filed; open-gap count + 60-day due date stored; overdue feed; filing marks the draft filed + links its `annual` sequence) | 21 CFR 312.33 / Part 11 | `ind-lifecycle/ind-annual-report-persistence`, schema `shared/schema/ind-annual-reports`, migration `20260615_ind_annual_reports.sql` | `POST/GET /api/ind-lifecycle/submission/:id/annual-reports`, `GET …/annual-reports/overdue`; `/annual-report/file` accepts an optional `draftId` |
...
| **Persisted** IND amendments (durable, tenant-scoped, audited amendment-plan drafts tracked draft→filed; amendment classes + leaf/warning counts stored; event-driven so no overdue feed; filing marks the draft filed + links its sequence) | 21 CFR 312.30/.31 / Part 11 | `ind-lifecycle/ind-amendment-persistence`, schema `shared/schema/ind-amendments`, migration `20260615_ind_amendments.sql` | `POST/GET /api/ind-lifecycle/submission/:id/amendments`; `/amendment/file` accepts an optional `draftId` |
