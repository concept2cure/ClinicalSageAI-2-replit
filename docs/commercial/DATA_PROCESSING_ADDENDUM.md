...
| Claude on Amazon Bedrock | Frontier model in Provider's (or Customer's) AWS account | `us` by default; other regions when deployed (`AI_BEDROCK_RESIDENCY`) | Yes by default (no-retention, no-training posture of the private substrate) | The placement for residency- or BAA-constrained tenants. Requires an Order Form line; not provisioned by default. |
| Claude on Google Vertex AI | Frontier model in a GCP project | `us`, `eu` | Yes by default | Available in code; not deployed for launch. |
| OpenAI models on Azure | Frontier model in an Azure tenant | `us`, `eu` | Yes by default | Available in code; not deployed for launch; not approved for high-risk regulatory drafting. |
