...
|---|---|---|---|---|
| E1 | Re-order project landing so the conversation panel leads (title → "Ask AnA about this project / Resume / New chat" → recent chats/artifacts/files → capability grid → readiness)? | §10.3: no KPI hero, conversation-first | **Done 2026-09-07.** `ProjectHome.tsx`: the composer leads the main column; "Conversations" lists the program's real threads from `GET /api/chat/threads?program_id=` and clicking one resumes it in `conversation-thread`; the readiness ring sits in the aside. Threads are minted with the program key in `chat_threads.metadata.programId` (`chat-thread-helpers.ts`, no migration). Walked in a browser against a seeded program on 2026-09-07, which found that resume restored nothing: `GET /threads/:id/messages` gated on `ai_threads` while serving `chat_messages`, so every AnA thread was "not found". The owning store is now resolved once and each handler acts on it. | — |
| E2 | Consolidate the five composers onto one `Composer` component? | §10.1 one composer | **Measured 2026-09-07, not extracted.** The shared composer *behaviour* is now two hooks every host uses: `useChatUpload` (attach) and `useAppMentions` (`@app`). The chip markup (`.ana-files/.ana-file`) exists only in `Shell.tsx`; the thread, home and eCTD composers each render their own chip and send controls over the same hooks, and the project-home composer seeds-and-navigates rather than streams. There is no identical markup left to lift, so a single `Composer` would be a new abstraction over four different chromes, not a de-duplication. | Remaining scope, if the owner wants one chrome: a `Composer` in `v2/composer.tsx` taking `{draft, onSend, upload, mentions, slot}`; adopt in Shell → thread → home; eCTD keeps its editor-bound send. Not done here. |
| E3 | Build inline `@app` / slash autocomplete in the composer? | §10.1 required | **Done 2026-09-07.** `shared/navigation/callable-apps.ts` derives the `@app` vocabulary from `NAVIGATION_TARGETS`; `v2/appMentions.tsx` is the one hook + menu wired into the rail, thread and home composers (↑/↓/Enter/Tab/Escape, `role="listbox"`). The server parses the sent text against the same vocabulary (`ana-ri/invoked-apps-block.ts`): prompt block, tool-selection hints and the self-drive pins — so a hand-typed mention means the same as a chosen one. Second pass, same day: the server already had its own `@app` path (`message-intent.ts`: short ids such as `@biostats` at message start) — the two were merged onto the one vocabulary, which now carries those short handles as aliases, so `@biostats`, `@biostat-workbench` and `@Biostatistics workbench` resolve to the same app anywhere in the message; the prompt block is the shared one and context enrichment runs for every invoked app. Slash autocomplete is built on the same footing: `SUPPORTED_SLASH_COMMANDS` moved to `shared/ana/slash-commands.ts` with one line of copy per command, the parser imports it, and typing `/` as the first character of a draft offers exactly that list. The `+` menu's "Slash commands" entry now starts a `/` in the composer instead of sending a message. | — |
...
**Behavioural** — new general chat in one click ✓ (Home composer, rail composer) · new project chat in one click ✓ (`ProjectHome` composer) · resume recent chat ✓ (thread history; project threads listed and resumable from the project landing since 2026-09-07) · file attach from composer ✓ · `@app` invocation ✓ (E3, 2026-09-07) · project context visible ✓ (TopBar breadcrumb, `readShellProject`) · review/task/submission routing via Communication Center ✗ — those live in `review`, `tasks`, `submission-center`; the Communication Center is the agency loop (E1/E3-adjacent product question, recorded).

**Visual** — clean sans shell typography ✓ with four serif chrome classes retained by decision · neutral palette ✗ warm-cream brand palette retained by decision · border/radius/spacing consistent ✓ (token-authority gate) · no dashboard-card hero on project landing ✓ (E1, 2026-09-07: composer leads, readiness ring in the aside).
