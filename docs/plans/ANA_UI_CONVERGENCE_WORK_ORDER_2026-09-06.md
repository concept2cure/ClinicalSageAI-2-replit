...
| E3 | Build inline `@app` / slash autocomplete in the composer? | §10.1 required | **Done 2026-09-07.** `shared/navigation/callable-apps.ts` derives the `@app` vocabulary from `NAVIGATION_TARGETS`; `v2/appMentions.tsx` is the one hook + menu wired into the rail, thread and home composers (↑/↓/Enter/Tab/Escape, `role="listbox"`). The server parses the sent text against the same vocabulary (`ana-ri/invoked-apps-block.ts`): prompt block, tool-selection hints and the self-drive pins — so a hand-typed mention means the same as a chosen one. Slash commands remain the `+` menu's message; a `/` parser was not built. | — |
