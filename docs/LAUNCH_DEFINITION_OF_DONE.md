...
| **D4** Validation package — *drafted and executed locally: VMP, six URS, RA, IQ, six OQ protocols, TM-001 generated from the runs, VSR-001 §1–§12 (`docs/validation/`, `docs/evidence/W3/`, `WA`–`WF`). Latest local run 2026-09-23, in the production posture *and* with production authentication — RLS enforcing, non-owner runtime role, dev-login refused, every identity signing in and signing with password + TOTP (`docs/evidence/W3/2026-09-23/`): IQ 12/0/3; OQ 95 pass / 0 fail / 1 deviation of 96; TM-001 66 of 68 requirements pass (URS-AUTH-012 partial; URS-PROJ-010, the sign-in audit trail, added after the run and so far executed only on its own, VSR-001 §13.8). It found F-18 (governed signatures accepted without an enrolled second factor; fixed) and F-19 (under RLS no sign-in reached the audit trail; fixed, proven on real PostgreSQL as the non-superuser role) (VSR-001 §13). F-6 closed (VSR-001 §11). Owed: staging execution with the production image and a real second account, the F-15 decision on the development RLS posture (VSR-001 §12), a PQ-passed provider for the model step (2026-09-22: only registry-approved models may now serve drafting or review — enforced at every gateway selection point, where before nothing read the approval; 0 of 4 approved models have a PQ, shown as a blocker in `ga-readiness-report.mjs`; `docs/evidence/MODEL-GOVERNANCE/`), signatures* | CSA-aligned: validation plan, URS per launch app, risk assessment, IQ, OQ with executed Playwright evidence, traceability matrix generated from tests, summary report. Signed by the founder and one qualified contractor. | Signed PDFs under `docs/evidence/validation/` | Claude drafts and executes; humans sign |
