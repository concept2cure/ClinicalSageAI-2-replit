...
| P11-11 | ALCOA+ | Diagnostic/analytical computations are deterministic and attributable | `stats/diagnostic-design.ts` (clinical sizing); `stats/analytical-performance.ts` (CLSI EP05/EP06/EP07/EP09/EP17/EP28 + Bland–Altman); `stats/clinical-performance.ts` (2×2 accuracy + ROC/AUC); `routes/diagnostics-performance.ts` (`/api/diagnostics-performance`) | `stats/__tests__/{analytical,clinical}-performance.test.ts`, `routes/__tests__/diagnostics-performance.test.ts` (closed-form references) |
