...
| 0.7 | 2026-09-23 | W3 | OQ-PROJ-18 (URS-PROJ-012): a platform administrator suspends a dedicated account through the product's own route; the account's session is then refused by the API and the session check, its sign-in is refused after the password with no challenge issued, the refusal is read back on the ledger, and the account is restored in every outcome. Credentialed (`OQ_PLATFORM_ADMIN_*`, `OQ_STANDING_*`, §1); not executed without them. Until F-28 and F-29 nothing read `users.status` but the release signature (VSR-001 §16). The platform administrator holds the platform role only; the step's first execution was refused 403 before Master Administration's own gate ran, by an organisation-admin gate every `/api/admin` request passed through (F-31, fixed). |
...
OQ-PROJ-18 (v0.7) needs two further identities, supplied only through the environment like every credential: a platform administrator, `OQ_PLATFORM_ADMIN_EMAIL` / `_PASSWORD` / `_TOTP_SECRET` (an active `platform_role_grants` row, and a member, not an administrator, of its organisation, so the step shows that Master Administration admits the platform role alone; the record states whether it is an organisation admin), and the account it takes out of use, `OQ_STANDING_EMAIL` / `_PASSWORD` / `_TOTP_SECRET`, a member of the run identity's organisation that no other step uses. Without them the step is recorded *not executed — credential not supplied*. The step restores the account whatever its outcome. Every sign-in counts against the server's per-address sign-in limit of ten in fifteen minutes; with this step the OQ run makes ten, so the IQ, which makes one more, runs on its own server process or more than fifteen minutes before the OQ run.
...
| OQ-PROJ-18 | URS-PROJ-012 | scripted | Credentialed. Sign in as the standing subject (`OQ_STANDING_EMAIL`: password and authenticator code) and read projects with its session. As the platform administrator (`OQ_PLATFORM_ADMIN_EMAIL`), `PATCH /api/admin/master/users/:subjectId/status {status:"suspended", reason}`. With the subject's session: `GET /api/c2c/projects`, `GET /api/auth/session`; then `POST /api/auth/login` as the subject. The ledger is read with the run session before and after. Then `PATCH … {status:"active", reason}` and read projects with the subject's session again (v0.7) | Projects 200 before; the suspension 200. Suspended: projects 401 `ACCOUNT_INACTIVE`; the session check `authenticated: false` with `AUTH_ACCOUNT_INACTIVE`; the password step 403 `AUTH_ACCOUNT_INACTIVE` with no challenge issued; the newest ledger entry the step added for the subject reads "Sign-in refused: the account is not active (suspended or deprovisioned)", hash-chained. Restored: the same session reads projects 200 |
