...
| 0.6 | 2026-09-25 | WO-16C / D2 | Launch scope is enforced at the API as well as in the navigation (`server/services/entitlements/launch-scope-api.ts`; evidence `docs/evidence/D2-API-SCOPE/2026-09-25/`). Until then a signed-in tenant could call the locked boards' routes directly. OQ-SRDY-08 now also requires the boards' own APIs, the contradiction scan and the execution history, to answer 403 `LAUNCH_SCOPE`. OQ-SRDY-06: with launch scope enforced, the posture this protocol runs in, both scans are refused 403 `LAUNCH_SCOPE`; neither may be answered as clean, which was the step's point (F-25). OQ-SRDY-06b: the scan is locked in this posture, so its positive half is a deviation that says so; it stays executable with launch scope off. OQ-SRDY-05b no longer reads `GET /executions/:id`, which is the Orchestration board's history. It reads the status and steps from the execution the response returns. **Not yet executed at this version.** |
...
| OQ-SRDY-05b | URS-SRDY-005 | scripted | When intake anchored the program (`meta.projectAnchorId`): execute the review for the anchored project (v0.4; the `GET /executions/:id` read-back removed in v0.6, it is the locked Orchestration board's) | the run completes, and the `inspect_project_state` step of the execution the response returns names the program. No anchor: deviation naming the reason intake gave |
| OQ-SRDY-06 | URS-SRDY-006 | scripted | `POST /api/governed-intelligence/contradictions/scan/<the program id>`; the same for a project id the organisation does not hold (v0.5) | neither 200 with findings. Launch scope enforced: both 403 `LAUNCH_SCOPE` (v0.6). Launch scope off: 400 and 404 respectively |
| OQ-SRDY-06b | URS-SRDY-006 | scripted | When intake anchored the program: scan the anchored project (v0.5) | launch scope off: 200 with a deterministic result. Launch scope enforced: deviation, the scan is locked (v0.6). No anchor: deviation naming the reason intake gave |
...
| OQ-SRDY-08 | URS-SRDY-008 | scripted (browser) | `GET /api/module-subscriptions/navigation`; open `/concept2cure/orchestration`, `/concept2cure/inconsistency` (v0.5); `POST /api/governed-intelligence/contradictions/scan/1`, `GET /api/orchestration/executions/<any id>` (v0.6) | both not entitled, source `launch-scope`; `dispatch-readiness` entitled; each deep link shows "not in this release"; both APIs answer 403 `LAUNCH_SCOPE` |
