...
The dossier store no longer loads fictional evidence on import or retains it after empty live hydration. Dossier files now render explicit loading, empty, and unavailable states; sample dossier content requires explicit sample mode and live hydration replaces rather than merges with it.
