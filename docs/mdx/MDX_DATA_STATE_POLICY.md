...
The in-memory dossier store no longer seeds fictional 510(k), PMA, or CER evidence at module import. Empty backend hydration actively clears prior content; loading, empty, unavailable, and permission-denied file panels are distinct; live sections replace rather than merge with sample evidence. Kit dossier content is installed only through the explicit sample-mode boundary. The dossier hook uses the shared typed HTTP error contract, preserves 401/403 responses (including section-content requests), and maps them to `permission-denied` rather than empty or unavailable.
