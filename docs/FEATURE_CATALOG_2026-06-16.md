...
- **Event bus** — ~~pub-sub with DB persistence (`ind_events`)~~. Never ran: zero importers, and no migration creates `ind_events`. `server/events/eventBus.js` deleted 2026-09-22.
...
- **ORM & infra** ✅ — Drizzle (primary, drizzle-zod validation) over PostgreSQL + pgvector; 122 migrations; RLS policies in `database/policies/`; ~60 pgEnums. *(Correction: `server/prisma/client.js` is a **live** Prisma-compatible wrapper over Drizzle — tenant-guarded, with tests (its former consumers `semanticSearch.js` and `bulk_import.js` were dead and deleted 2026-09-22) — NOT a deprecated ORM to remove. Only the orphaned `schema.prisma` descriptor is vestigial.)*
