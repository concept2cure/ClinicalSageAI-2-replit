...
|---|---|---|---|
| **G1-1** | **A from-scratch install silently half-works.** `install-fresh.mjs` prints `✅ Application schema install complete` while skipping 10 migrations it calls "safe to skip" and never creating **15 Postgres schemas** — 9 of which a follow-on psql loop supplies, one self-provisions, and **5 have no provisioning path at all** (Ch 03, C2). Four **mounted** route families (`innovation`, `clinical_ops`, `intelligent_docs`, `manufacturing`) cannot serve a request. `ai_threads` / `chat_messages` — queried by the mounted chat routes — do not exist. | Executed: `evidence/00-live-proof-log.md` §LP-02 | days |
...
| **G1-4** | **Stored XSS on a reachable surface.** `BatchDraft.tsx:490` renders unsanitized HTML built from a persisted document title/preview, and `derivePreview` (`batch-draft-routes.ts:104-111`) **HTML-entity-decodes** markup the editor had escaped, re-animating it. *(Corrected — the original finding blamed streamed model output, which is dead code. Ch 03, C7.)* Two-part fix: sanitize the sink **and** stop the decode. | Chapter 04 §4.2 | hours |
...
| **G1-7** | **The product has no front door.** As of HEAD, `RAIL_PRIMARY` has **5** destinations and `NAV_HIDDEN` has **40** (42 literal entries, 2 duplicated), leaving ~85 of **96** registered surfaces reachable only by typed URL or a command palette that matches labels only and caps at 8 results. At least one complete offering (translation) has a live, mounted backend and **zero** reachable UI. | Chapter 09 | days (nav), weeks (coherence) |
