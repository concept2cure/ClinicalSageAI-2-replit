...
(`stats/signal-disproportionality.ts:11-13`). Of the measures the panel
computes, a `b`↔`c` transpose changes **only PRR**:

| Measure | Under a `b`↔`c` swap | Why |
| --- | --- | --- |
| `PRR = [a/(a+b)]/[c/(c+d)]` | **changes** | not symmetric in `b`/`c` |
| `ROR = ad/bc` | invariant | `bc` is symmetric |
| Yates χ² | invariant | numerator keys off `\|ad−bc\|`; denominator is the same four marginals reordered |
| IC, EBGM | invariant | both key off `E = (a+b)(a+c)/N`, symmetric in `b`/`c` |

Verified by execution (`a=20, b=10, c=5, d=1000` vs transposed): PRR 134 → 80.8,
while ROR, χ², IC and EBGM are identical to the digit. That partial agreement is
exactly what makes such a bug hard to spot — a transposed table still returns
plausible-looking output on four of five measures.

PRR alone is enough to matter: it gates `frequentist.signal`
(`prr >= 2 && chi2 >= 4 && a >= 3`) and hence the consolidated tier. Worked
example of a flip: `a=10, b=79, c=298, d=5000` gives a true PRR of 1.998
(`signal=false`, tier `weak`) but a transposed PRR of 2.087 (`signal=true`,
tier `moderate`).
