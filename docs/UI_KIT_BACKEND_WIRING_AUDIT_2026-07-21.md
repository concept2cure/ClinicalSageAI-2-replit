...
**Remaining ANA reachability (corrected from the sub-audit's over-claim; the first two items are since RESOLVED — kept with their resolutions so the reasoning survives):**
- **The device surfaces are NOT blind — and are no longer MDX aliases.** ~~`DeviceWorkstream` → `MdxRoute` mounts the MDX app, which carries its own real `useAnaChat` ANA rail (`mdx/App.tsx:188,326`), so rail-hiding is correct for them — un-hiding would render two ANA rails.~~ **RESOLVED.** There are now 16 device entries in `SURFACE_VIEWS`, one per surface, each rendering only its own canvas rather than five aliases for the whole MDX application. The second Rail / TopBar / AnA composer that made the flag necessary is gone, so **none of the device entries claims the rail's column** — they are registered `{ component: DeviceSurfaces['device-…'], full: true }` and the shell owns both the chrome and the conversation.
- **`document-authoring` + `ectd-coauthor` — rail-visibility bug.** ~~Both call the real assistant via `onAsk` (e.g. "Draft with AnA", "Lock … for review"), but the shell rail isn't rendered on them, so the response streams invisibly. Fix: render the rail as a non-squeezing overlay, or mount the grounded `components/ana/Ana.tsx` in-editor.~~ **RESOLVED, and not by rendering the shell rail.** The flag was renamed `hideAna` → **`ownsConversation`** and reworked so it carries the obligation as well as the layout: a surface that takes the rail's column is responsible for where a question goes. Both editors now answer **in place** — `document-authoring` in a fourth right-rail mode beside history / comments / sources, `ectd-coauthor` in its middle intelligence pane — each running its own `useAnaChat`.

  The mechanism is a **discriminated union**, not a boolean read at render time. `SurfaceView` (`client/src/concept2cure/v2/surfaceViews.ts`) has two arms: the plain arm takes `ComponentType<SurfaceViewProps>`, and `ownsConversation: true` narrows `component` to `ComponentType<OwnedSurfaceViewProps>`, where `OwnedSurfaceViewProps = Omit<SurfaceViewProps, 'onAsk'>`. So an owning surface **cannot be handed the shell's `onAsk` at all** — the prop is absent from its type, and the old failure ("hide the rail, keep calling `onAsk`, watch the answer stream into a column nobody drew, and `ask()` persists `anaOpen:true` so it reappears on the next surface that does draw one") is a compile error at the registration site rather than a silence. `V2App` matches: `{!ownsConversation && <AnaRail …/>}` for the column, and `ask()` routes to `startShellConversation` instead of the rail when the active surface owns the conversation. Six surfaces hold the flag today — `client-portal`, `conversation-thread`, `document-authoring`, `ectd-coauthor`, `insights`, `rbm` — and each discharges it: answer in its own dock, hand the question to `conversation-thread` and navigate there, or offer no assistant at all and say so (`client-portal`). `admin-console` and `csr-workflow` **dropped** it rather than discharge it — on the admin console the §11.50 sign-off prompt is drawn *by* the rail, so hiding the rail hid the signature gate.
