...
1. Additional artifact creation/update entry points not yet routed through canonical harness:
   - `server/routes/concept2cure.ts`:
     - source upload convergence insert around line ~2718
     - audit report export artifact insert around line ~6778
     - HAQ session insert/update around lines ~8726/~8744
   - `server/routes/knowledge-base.ts`:
     - direct artifact inserts around lines ~957, ~1156, ~1938
   - `server/routes/authoring-actions.ts`:
     - direct artifact updates around lines ~348, ~521, ~626, ~756

2. Typecheck green baseline cannot be proven due to broad pre-existing repository type errors.
