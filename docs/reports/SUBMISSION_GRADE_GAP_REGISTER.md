...
| **Non-US Module-1 cockpits stopped at EMA/PMDA** — the requirements engine, route, and surface modeled/served all 7 markets but only EMA/PMDA were seeded; **MHRA / Health Canada / TGA / NMPA** now open with live assembled-vs-required readiness | High | UK/CA/AU/CN regional Module 1 | #1034 |
| **Post-approval CMC change control was a static fixture** — now computed live: a governed proposed-change store projected through the deterministic SUPAC/variations classifier (FDA reporting category AR/CBE-0/CBE-30/PAS → risk band, EMA type, SUPAC tier, citation) | Medium | 21 CFR 314.70; SUPAC; ICH Q12; EU Reg 1234/2008 | #1035 |
| **Last static specialty cards** — pediatric PREA/PIP milestones + orphan RPD vouchers/grants + orphan patient-advocacy now read from governed org-scoped stores, fail-closed to the fixture | Low | PREA/PIP; orphan programs | #1036 |
...
| **Marketing-application cockpits are US-first** — NDA/BLA anchored (356h/PDUFA/RTF). **Largely closed:** **all seven markets'** regional **Module-1 readiness** cockpits are now live (FDA/EMA/PMDA #1029 + MHRA/HC/TGA/NMPA #1034) and the BLA biologics workbench is surfaced (#1031); what remains is **no fully executable** MAA/J-NDA **end-to-end submission workflow** (dossier assembly → validate → dispatch) beyond Module-1 readiness | High | Non-US marketing applications | region CTD Module 1 | **Design/build**: executable per-region submission workflow beyond Module-1 readiness (assembly + validator + dispatch), gated on the same licensed-validator asset as the US path. |
...
- Post-approval lifecycle: **fully surfaced live** — renewal/periodic-report cycles (#1032), the CMC change-control card now computed through the deterministic SUPAC/variations classifier (FDA reporting category → risk band, #1035), and the pediatric PREA/PIP milestones + orphan RPD/advocacy cards (governed stores, #1036). No static specialty fixtures remain.
...
3. **The feature-level slices** — now landed: IB surface, PV awareness-date clock, M2.6/M4/SEND projection (#1027), signing-authority convergence (#1028), Module-1 cockpit for all 7 markets (#1029 + #1034), EU SmPC (#1030), BLA biologics-workbench convergence (#1031), and the full post-approval lifecycle surface — renewal cycles (#1032), computed CMC change control (#1035), pediatric PREA + orphan cards (#1036). **Remaining feature-level work:** the executable **end-to-end non-US submission workflow** (assembly → validate → dispatch) beyond Module-1 readiness — gated on the same licensed eCTD-validator asset as the US path (a Critical row above), so it's an asset-unblock, not just a build.
