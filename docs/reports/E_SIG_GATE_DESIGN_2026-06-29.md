...
1. **The assembled leaf manifest's content digest.** `outputs.assembly.leaves` is a deterministic list (`buildLeafManifestFromSections`); each leaf already carries an MD5 of its rendered content. We compute `sha256(JSON.stringify(leaves))` (canonical key order — the manifest is already built in deterministic order from `composeFullModule3`).
...
**Why not the eCTD backbone XML.** The backbone XML (`index.xml` / `us-regional.xml`) is generated *during* `package.transmit` today (or by the ZIP builder, when wired). Until the ZIP builder is in the orchestrator, the backbone XML is not a stable artifact at `package.sign` time. The leaf manifest + validator outcome + identity tuple is the stable, post-`package.validate` snapshot — exactly the surface that §11.70 needs to bind to the record. When the ZIP builder lands, the design extends naturally: prepend `sha256(backboneXml)` to the digest concatenation. **Open question OQ-5** captures this transition.

### B.6 Verification on resume

When `runOrchestrator(inputs, { resumeRunId })` is invoked and finds `package.sign` in `awaiting-signature`:
