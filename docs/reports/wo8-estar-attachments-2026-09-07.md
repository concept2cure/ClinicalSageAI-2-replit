# Roadmap item 4 — attachments in the official eSTAR (slice 1 of 5: the writer)
...
This closes the first of five slices. The other four are specified in §4 so the next
session starts from a plan rather than a diagnosis.
...
## 4. The remaining four slices

2. **Encrypted object construction.** `encryptObjectData` already derives the per-object key
   (Algorithm 2 + the `sAlT` suffix). Needed beside it: string encryption for `/F` and `/UF`,
   and a builder producing the `/EmbeddedFile` + `/Filespec` pair for a named file. Prove it
   by decrypting the result back with the module's own reader AND with pypdf.
...
Nothing is attached to anything yet. The eSTAR the platform produces today still populates
0 attachment slots — this slice removes the reason it could not.
