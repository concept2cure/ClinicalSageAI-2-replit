...
## 14. Deep Dead Code Audit (Agent Findings)

A comprehensive dead code scan revealed significant additional cleanup opportunities:

### 34 Orphaned Client Components

These `.tsx` files in `client/src/components/` are never imported anywhere in the codebase:

| Component | Safe to Delete |
|-----------|---------------|
| AdvancedDashboard.tsx | YES |
| BenchmarkDetailsModal.tsx | YES |
| CERAPIDemo.tsx | YES |
| CERDashboard.tsx | YES |
| CERGenerator.tsx | YES |
| CSRDirectUploader.tsx | YES |
| CSRExtractorDashboard.tsx | YES |
| CsrIntelligenceStats.tsx | YES |
| DashboardLayout.tsx | YES |
| DeepSearchPanel.tsx | YES |
| DocumentDiffDialog.tsx | YES |
| EmailPreferenceCenter.tsx | YES |
| EndpointRecommender.tsx | YES |
| ExportLogDashboard.tsx | YES |
| ExportMenu.tsx | YES |
| FullAuditDashboard.tsx | YES |
| Module32Tour.tsx | YES |
| NLPQuery.tsx | YES |
| NeuroSymbolicDashboard.tsx | YES |
| OnboardingTour.tsx | YES |
| ProductAuditQuestionnaire.tsx | YES |
| ProtectedComponent.tsx | YES |
| ProtocolAnalyzer.tsx | YES |
| ProtocolPlanningDashboard.tsx | YES |
| SafeHtml.tsx | YES |
| SortableItem.tsx | YES |
| SourceLinkPanel.tsx | YES |
| SubmissionBuilderWebSocket.tsx | YES |
| TagWatchlistManager.tsx | YES |
| TrialSuccessPredictor.tsx | YES |
| TrialSuccessPredictorV2.tsx | YES |
| VersionDiff.tsx | YES |
| VersionsTour.tsx | YES |
| WeeklyDigestViewer.tsx | YES |
| WisdomTraceExporter.tsx | YES |

### 60 Unused Server Route Files

These route files in `server/routes/` are NOT imported in `server/index.ts`. Some may be imported by other route files or dynamically loaded — **review before deleting**. Notable large files:

- `authoring.router.ts` (162KB) — likely imported by another route, not directly by index.ts
- `academic_protocol_assessment.ts` (52KB)
- `export_routes.ts` (42KB)
- `support-admin.ts` (42KB)
- `submission-ops.ts` (41KB)

### 2 Explicitly Deprecated Routes (Past Sunset Review)

| File | Deprecated Since | Sunset Date | Status |
|------|-----------------|-------------|--------|
| `server/routes/510kRoutes.ts` (27KB) | 2026-01-26 | 2026-06-30 | Not imported, safe to delete |
| `server/routes/510kEstarRoutes.ts` (15KB) | 2026-01-26 | 2026-06-30 | Not imported, safe to delete |

### 4 Stale ESLint Ignore Patterns (FIXED)

Removed references to nonexistent paths from `package.json` lint scripts:
- `server/middleware/_archived` (deleted in wave 1)
- `client/src/components/_deprecated` (never existed)
- `client/src/pages/clean_temp.jsx` (never existed)
- `client/src/pages/full_backup.jsx` (never existed)

### Recommended Next Session

1. **Delete 34 orphaned client components** (~200KB dead code, zero risk)
2. **Delete 2 deprecated 510k routes** (~42KB, explicitly marked for removal)
3. **Audit 60 unused route files** — verify which are truly dead vs. imported by other routes
4. **Consolidate duplicate route families** (CER: 5 files, 510k: 7 files, CSR: 3 files)
