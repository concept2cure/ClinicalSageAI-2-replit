...
| W2 / D1 — Terraform to a booting task: B1 (`DATABASE_URL` is a JSON credential), B2 (illegal RDS name), B3 (task definition lacks the preflight's variables), B5 (health check calls `wget`). `terraform/environments/production/*`, `terraform/modules/{rds,secrets,ecs-fargate}/*`. Brief: `docs/evidence/W2/2026-09-23/README.md`. B4/B6/B7 go to the founder, not defaulted | `…session_013CtPf8pjozina2nVvDYkyB` | **claimed** 2026-09-23 — evidence `docs/evidence/W2/2026-09-23b/`. Extended 2026-09-24, same row and lane ("to a booting task"): the image cannot boot at all — `dist/index.js` statically imports `vite`, a devDependency the image prunes — so `server/vite.ts` plus a new bundle-imports gate; and minting `app_service` as the RDS master (`scripts/db/provision-app-role.mjs`). For B9 (`…01GSjEDJ`): this change rewrites production's `module "ecs"` block (secrets and environment now come from `local.boot_secrets` / `local.boot_environment`) and adds outputs to `modules/ecs-fargate`; your `trust_proxy_hops` line lands on top of it |
| W2 / D1 + D6 — B9: the ALB answers CloudFront alone (origin-facing prefix list + origin secret header), so `TRUST_PROXY_HOPS=2` records the user, and the CloudFront→ALB path works: origin certificate check, API 403/404 no longer rewritten to `200 index.html`, `/readyz` `/healthz` `/collab` `/scim/v2` `/mcp` `/.well-known` routed to the ALB, deploy smoke test through the public URL and failing closed. `terraform/modules/{alb,cloudfront}/*`, the `smoke-test` job of `.github/workflows/deploy-aws.yml`. In the B1–B5 lane's `environments/production/*`: only the `alb` and `cdn` blocks, one `trust_proxy_hops` line in `ecs`, one appended variable — the claimant is told | `…session_01GSjEDJLuZsEzPa9PnVg1yF` | **claimed** 2026-09-24 — evidence `docs/evidence/W2/2026-09-24-b9/` |
| **Launch-catalog review follow-through (D2/D5/D6)** — the 2026-09-22 periodic review's OPEN findings (`docs/evidence/reviews/2026-09-22/`): status each at HEAD, then fix the open, unclaimed ones failing-first (Part 11 #2 QMP audit, #3 contradiction-resolution audit, #4 release-signature manifestation, #5 server-side reason-for-change, T2 task-write authority). T1 (discarded task-ledger outcome) is WO-16C's and is not touched. Also: an adversarial pass over the 54 governed files added 09-19→09-24; findings inside another lane's files are written up here for that lane, not edited. Files another lane touched in the last 24h are skipped. Evidence: `docs/evidence/reviews/2026-09-22/follow-through/` | `…session_01WcyqbqWn6LszBqUWUSNnqA` | **claimed** 2026-09-24 |
