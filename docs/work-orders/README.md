...
| **W1 / D2 — AnA's progress and output record, one in every host.** `client/src/concept2cure/v2/{AnaWorkPanel,AnaWorkSections,anaWorkModel,AnaActivity,AnaOutputs,workDock}.tsx?`, `components/ana/{anaProgress,useAnaChat*}`, `server/services/ana/{turn-plan,turn-context-used}.ts`; next: the Authoring section "AI draft" onto the same record (`surfaces/AuthoringAiDraft.tsx`) and the declared plan persisted with the turn (`tool-trace.ts` metadata, `loadThread`). Evidence: `docs/evidence/W1/2026-09-24-ana-progress/` | `…session_01T2wooCZu46W7msw4TJuuzr` | **claimed** 2026-09-24 — first half pushed `c4953cf4`; uploads-by-id (turns on PDF bytes to the model) left to the founder |
