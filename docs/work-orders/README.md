...
| Lane | Session | State |
|---|---|---|
| **D3 — the cortex query route's client-supplied tenant key.** `server/routes/cortexQueryRoutes.ts` (`x-org-uuid` fallback; the search branch that runs with no org predicate), measured end to end on the two-tenant fixture as its own file (`tests/db/cortex-query-tenant-header.dbtest.ts`). Evidence: `docs/evidence/D3/2026-09-24-cortex-tenant-header/`. Not `send-message.ts`'s identical fallback (the chat lane's), which is reported there, not edited | `…session_01J935DZwfFEardJCv85SJds` | **claimed** 2026-09-24 — row **D3** |
...
**→ D3, unclaimed**
- The cortex vault Q&A route takes its tenant key from the client's `x-org-uuid` header, and both its SQL predicate and RLS trust it. **Claimed 2026-09-24 by `…session_01J935DZwfFEardJCv85SJds` (row D3)** — see the lane table.
