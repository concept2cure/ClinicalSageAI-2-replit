...
| Lane | Session | State |
|---|---|---|
| **D6 — P0 closures from the 2026-09-24 security audit, this session's tranche** (`docs/security/REMEDIATION_AND_ENHANCEMENT_PLAN_2026-09-24.md` §1, cold files only): P0-1 `server/services/ana/ana-realtime.ts`; P0-13a `server/startup/env.ts`, `server/middleware/authBoundary.ts`; P0-5 `server/routes/scim.ts`; P0-7 `server/services/part11/signature-persistence.ts` (+ the 20260730 esign trigger migration in place); P0-2 `server/mcp/auth/*`, `server/mcp/index.ts`, `server/middleware/auth.ts`, `server/utils/jwtVerify.ts`; P0-4a `server/services/token-revocation.ts`, `server/routes/authEnterprise.ts`, `client/src/services/portal/authService.tsx`; P0-3 `server/routes/sso.ts`, `server/services/saml-provider.ts`, `server/middleware/requirePlatformAdmin.ts`; P0-6 `server/routes/c2c/exports.ts`, `server/routes/document-understanding.ts`, `server/utils/document-file-roots.ts`; then P0-18, P0-8a, P0-9a, P0-11, P0-12, P0-16a as time allows. Not touched: `routes/auth.ts`, `environment.ts`, `provision-app-role.mjs`, `terraform/**`, `.github/workflows/**`, `c2c/artifacts.ts`, `mdx-qms.ts`, `AnaToolExecutor.ts`, `audit/chain.ts` (other lanes' 24 h windows) | `…session_0194UQPxy9Er2ibRAjog8Ven` | **claimed** 2026-09-24 21:20 UTC — row **D6** (D5 for P0-7/P0-18/P0-8a); evidence `docs/evidence/D6/2026-09-24-p0/<item>/{red,green}/`, one commit per item |
...
| **D5 — AnA's QMS change-control and governed-fact tools report their audit row.** WO-16C hand-on item 2: `AnaToolExecutor.ts` `qms_change_create` / `_transition` / `_link` (`void auditService.logAction`), and the two discarded writes in `server/services/living-record/fact-change-orchestrator.ts` that `establish_governed_fact` / `apply_fact_change` and `routes/change-propagation.ts` answer. Those tool handlers only, not the rest of `AnaToolExecutor.ts` | `…session_01AiwZKGaEFjD9AfVvkYExci` | **claimed** 2026-09-24 — row **D5** |
