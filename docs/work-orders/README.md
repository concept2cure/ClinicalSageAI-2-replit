...
- **Claimed 2026-09-26 by `…01J935DZ` (lane row above). Found 2026-09-26: `public.organization_users` has no RLS at all.** Any tenant scope can write any organization's membership rows at the database, including a staff role, and `authMiddleware` accepts a token for an organization on exactly that row. No application writer produces a staff role, and none is known to write another org's rows. Likely unpoliced on purpose because the membership check runs pre-auth; measure that before any policy. Next for `…01J935DZ`.
- **→ D3, unclaimed — `public.users` is readable from any tenant scope** (from `…013CtPf8`, 2026-09-26). No RLS, and it holds `password_hash`, `mfa_secret`, `mfa_backup_codes`, `reset_token`, `email_otp_hash`. Found when `ba797ca6d` put RLS on `organizations` and the installer's coverage gate began flagging `public.users` through `default_organization_id`; that edge is now classified in `scripts/db/rls-coverage-check.sql` (a preference pointer, not tenancy) so a blank database installs, which is a classification, not a clearance. The fix is a policy set that keeps the pre-auth sign-in lookup working (system scope) and limits a tenant scope to members of its organizations and the caller's own row; it belongs with the `organization_users` item above, since membership is what the policy reads.
