...
2. ~~**Whoever next edits `AnaToolExecutor.ts`:**~~ **Done 2026-09-25 (`…01AiwZKG`, `docs/evidence/D5/2026-09-25-ana-governed-tool-audit-outcome/`):** the three change-control tools and both governed-fact tools carry `auditTrail`, and say when the row was not written. The orchestrator now returns it, so `routes/change-propagation.ts` answers it too. **Still open:** `check_consistency` / `runConsistencyCheck`, because `routes/submissions.ts:1107` answers its bare array. Originally: QMS change control (×3,
