...
| L41 | **`authoring-engine` is a fixture screen sold as a subscribable capability.** It renders entirely from inline fixture data (`AE_PIPE` / `AE_SYSTEMS` / `AE_AUTOMATIONS`, commented "kit window globals") and calls no endpoint. It carries no sample-mode label, so nothing on screen tells a customer the pipeline stages, guarantees and checks they are reading are hard-coded rather than their configuration. It is in the Apps catalog as a purchasable module whose description asserts "no hallucinations, every required element" and validation that "runs as intended 100% of the time", and the biopharma journey routes to it as "Author with AnA Engine". The real authoring surface is `document-authoring` (1,387 ln over eight endpoint families). Decision needed: label it illustrative, fold it into `document-authoring`, or delete it — but it cannot keep asserting an uncheckable compliance claim over fixtures | prod | open | `client/src/concept2cure/v2/surfaces/AuthoringEngine.tsx` |
| L42 | **Surface count has never had a consolidation pass.** 118 registry ids over 86 modules. The D1–D11 doctrine ("never 3 of anything") was applied to services, routes and tables and never to surfaces, so the UI layer still carries the clusters the backend shed: three reporting surfaces over three unrelated backends (`insights`→insights-canvas/report-os, `report-engine`→analytics, `report-governance`→intelligent-reports), six submission surfaces, four authoring, three labeling, three eCTD, two biostat, two quality, two lineage. Some splits are real regulatory distinctions (`labeling-pi` is a US PI, `labeling-smpc` an EU SmPC); others are the same capability built twice. Needs a per-cluster survivor decision, not a bulk merge | prod | open | `client/src/concept2cure/v2/surfaceViews.ts` |
