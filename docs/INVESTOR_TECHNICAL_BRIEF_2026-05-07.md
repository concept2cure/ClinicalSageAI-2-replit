...
| ALCOA+ coverage estimate (Feb 2026: 58%) | **On track to 75%+** | The structural blockers above are now closed. The remaining lift is one env flag: provision `audit.tamper_proof_log` schema, set `AUDIT_HMAC_SECRET`, flip `AUDIT_TRAIL_ENABLED=true`. The platform self-verifies the chain in-process every 5 minutes after that — `chainIntegrityMonitor` starts at boot under the same flag, no operator cron required. The `audit:verify:24h` and `audit:verify:full` npm scripts remain available for ad-hoc verification but are no longer on the critical path. |
