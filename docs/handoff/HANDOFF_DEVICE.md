...
| 4 | **Embed attachments in the official eSTAR** (repo checklist item B4). A slot map (section → one of the 112/140 `AddAttachment` slots → its `/CHAPTER n/CHn.nn/` manifest token → its indicator); a multi-object incremental-update writer that allocates `/EmbeddedFile` + `/Filespec` objects under the template's AESV2 key and rewrites the catalog's `/Names /EmbeddedFiles`; the `form`-packet occurrence + `AttachmentName`; bytes sourced from authored sections (rendered) and vault evidence. | The keystone. Without it nothing the platform authors reaches the filed form. Largest PDF engineering left; keep it on `node:crypto` + `pdf-lib`. | **slices 1–2 of 5 done 2026-09-07** — `docs/reports/wo8-estar-attachments-2026-09-07.md`. The incremental-update writer takes a LIST of objects, mixed stream/non-stream, allocates new numbers and emits real multi-subsection `/Index`; and `pdf-embedded-files.ts` builds the `/EmbeddedFile` + `/Filespec` pair with stream AND strings enciphered under each object's own key. Both cross-checked with pypdf against the real encrypted template. **Slices 3–5 are specified in that report §4** (the catalog name tree; the slot map measured off the `form` packet; the `form`-packet occurrence) |
