...
> **PAUSED 2026-09-22 under CLAUDE.md RULE 2 — read this before starting any item below.**
>
> RULE 2 (binding from 2026-09-20) allows no new capability outside the launch catalog until
> rows D1–D10 of `docs/LAUNCH_DEFINITION_OF_DONE.md` are green. **This whole stream is outside
> it.** `shared/constants/launch-scope.ts` — the one canonical list — contains no device,
> 510(k), eSTAR, MDx, IVD or regulatory-correspondence id; its Submission Center is eCTD-only
> (`ectd-compile`, `ectd-coauthor`, `ectd-publishing`, `gateway-transmittals`, `dossier-map`).
> Every surface this roadmap builds on is therefore behind `LAUNCH_SCOPE_ENFORCE`, off in
> production.
>
> So item 4's surface, item 5's amendment package and item 6 are **not to be started**. The
> engine work already on the branch stays — it is tested, and it is what the stream resumes
> from — but it is not launch work and no session should spend launch time extending it.
>
> Said plainly, because it happened: the 2026-09-20 device work (the CDRH issue taxonomy, and
> wiring it into correspondence intake) extended out-of-scope surfaces on the day RULE 2
> landed; the rule file appeared in that session's final merge. The defect fixes made the same
> day repaired code already shipped and are not in question — the `/format/` substring bug
> that tagged every letter containing "information" as an eCTD issue, and the response
> package that compiled an empty issue matrix. The taxonomy itself was new capability.
>
> **For the founder, one calibration note on the launch dashboard.** Three of the 18 blockers
> `node scripts/ops/ga-readiness-report.mjs` reports are device items — the three PreSTAR
> templates, their field maps, and `ESTAR_REQUIRE_TEMPLATE`. They are accurate (the six
> 510(k)/De Novo/PMA descriptors ARE vendored and mapped; only the PreSTARs are missing), but
> the dashboard counts them against a launch whose catalog excludes device entirely. Either the
> dashboard or the catalog is wrong about device's scope, and that is a scope decision, not a
> fix this stream should make. Separately, that probe's label reads "AcroForm field maps" and
> tells the reader to verify keys "against the real AcroForm" — on a dynamic-XFA eSTAR the
> AcroForm `/Fields` array is empty, so anyone following that instruction concludes the
> template cannot be mapped. The maps use `xfaSomPath`.
...
produces the real FDA form with 20 administrative boxes filled, 0 of 113 attachment slots
populated, no transmission, no letter loop, and four fixture leaks on the demo surface.
(The slot count read 112 until 2026-09-08; see item 4. As of 2026-09-08 the form fills
those slots — what is missing is the surface that chooses which document goes in which.)
...
| 2026-09-08 | E | Roadmap item 4 (slice 5 of 5) — documents in named CDRH attachment slots | **The official eSTAR files documents into named slots now**; it populated 0 of 113/145 before. `estar-attachment-plan.ts` plans against the same integrity-verified template bytes the fill uses (planning outside would resolve chapters from a different copy of the file), and REFUSES six ways — a slot the template does not declare, an unresolvable chapter, a second file into a slot the form holds one row for, the template's own acceptance rules, content the resolver could not produce, and a section still marked draft. Every refusal blocks the WHOLE export (422 with the reasons and the report): a submission missing a document the operator asked to file, handed back with a 200 and a note beside it, is the failure this codebase refuses everywhere else. Verified cold by pypdf 6.16.2 on a real 5.29 MB artifact: two attachments with matching sha256, `/Params /Size` and `/CheckSum` describing the plaintext, FDA's own `/Desc` on each, `/JavaScript` at 3 entries, 10 XFA packet names, and `***Start***<<Doc 0.pdf|/CHAPTER 1/CH1.01/>><<Doc 1.pdf|/CHAPTER 1/CH1.03/>>` with `indexOf("<<") = 11 > 0`. The blank-form check had to be NARROWED or attachments would have cleared it: the manifest is a value the fill writes, so a request with attachments and no administrative data would have produced a blank FDA form with files stapled to it — `ESTAR_ATTACHMENT_MANIFEST_KEY` is deliberately outside `ESTAR_FIELD_MAPS` so the check counts administrative fields only, and a manifest planned-then-not-written is its own blocker. Two corrections: the one-attachment rule is FIVE controls per template (`CLAddAttachment110`, `ADAddAttachment660`, `RMAddAttachment100`, `BRAddAttachment110`, `ADAddAttachment803`), not the two the 09-07 report recorded as measured — read from the template now, not transcribed — and the slot module's own docblock still carried the withdrawn 112/140 while the tests beside it pinned 113/145. `ci:governed-export-consequence-shape` was run and SEEN FAILING on the real change, then narrowed to pin the property rather than the argument list, and given the self-test the 09-07 report said it lacked — writing which found two cases the gate accepted. 20 mutations each seen to fail the test that names them; 79 new tests; 26 files / 475 across the estar engine, forms and both official-route suites; tsc clean. **What remains of item 4 is the surface**, and it is blocked on the section→slot map being a persisted governed artifact — a picker over 113 slots with nowhere to save to would be worse than none | `docs/reports/wo8-estar-attachments-slice5-2026-09-08.md` |
| 2026-09-08 | E | Four defects in slice 5, found after it was pushed | Found by re-reading my own diff adversarially and by one `tsc` run — not by a failing test. (1) The governed-section resolver was BROKEN AT RUNTIME: the route built it with `client: requestDb(req)`, a Drizzle instance whose `.query` is the relational-query namespace rather than the `(text, params)` function `DeviceContentClient` calls, so every `authored_section` attachment would have thrown `client.query is not a function`. Invisible by construction — the route's tests mock the resolver FACTORY, the right seam for testing the wiring and the reason nothing ever executed the resolver, which shipped with no test of its own. It has five now, and the one that matters was seen failing with the exact Drizzle-shaped client that shipped. (2) `createAuditedUnplacedExport` — the path a program-uuid anchor takes, which is most of them — had NO size ceiling while its sibling has refused over 25 MiB since it was written; harmless while every export was a fixed ~5.3 MB eSTAR, and this slice removed exactly that. Measured: 60 MiB in, 83,886,080 base64 characters delivered in one JSON body, on a route any editor can call. (3) An oversized submission threw at DELIVERY — after the renders, the vault reads, the encryption and the retention write — and the route called it "failed before consequence persistence", which is false once retention has written a vault row; the fill now takes the deliverer's ceiling and refuses in the same 422-with-reasons channel, naming the documents and the numbers. (4) The filer-facing "N of M administrative fields filled" count was kept honest by a docblock and no test. Plus two from earlier the same day: the jurisdiction radio the User Fee Form's chapter branches on, and chained object numbering (`nextFreeObjectNumber` was tested only on a PRISTINE PDF, where the newest and first trailers are the same trailer — so nine tests could not see a stale read that would collide with the datasets fill's own xref stream and silently destroy the form fill). **Three of the six share one shape: a property that was true, load-bearing, and asserted only in prose.** Every fix seen failing first; 30 files / 513; tsc clean | `docs/reports/wo8-estar-attachments-slice5-2026-09-08.md` §6b |
| 2026-09-08 | E | Slice 6 DESIGNED (not built), and three defects it found in slice 5 | A judged, adversarially-verified panel — 6 surveys, 3 designs, 6 judges, 5 refuters, 1 synthesis; scores operator 87 / governed 81 / minimal 77; 34 attacks, 23 high severity — specified `c2c_estar_slot_map` and the attachment surface, and re-measured the templates itself. Saved whole at `docs/reports/estar-slot-map-design-2026-09-08.md`. It corrects an assumption three earlier designs shared: **zero slots on either template fail to resolve** once the template's own datasets values are supplied, which `planEstarAttachments` already does — so the User Fee Form is placeable and was never the obstacle. Three of its findings were defects in code already pushed, all fixed: (1) the attachment resolver read the newest governed document of ANY device class, and the rule-pack keys it files by COLLIDE — k510 D5 is "Shelf life and packaging" and denovo D5 is "Cybersecurity", k510 E1 is "Biocompatibility" and denovo E1 is "Proposed labeling" — so a CER or a De Novo scaffold silently changed which document a named CDRH slot was filled from, with a clean 200; (2) `attachmentReport` was seeded after the plan, so two refusals produced a 422 with no report at all; (3) it independently rediscovered the `client: requestDb(req)` runtime crash fixed hours earlier and warns any design reinstating it re-breaks every authored-section attachment. **The design names a BLOCKING precondition:** the §4c Acrobat artifact is not a companion risk to this slice but a prerequisite — the map is keyed on slot identity and stores a chapter per row, so if the LiveCycle runtime does not preserve a datasets-only `AttachmentManifest` across open-and-save, every stored row is a claim about a mechanism that was never real, and RULE 1 makes amend-in-place the only correction path for a shipped migration. Nothing built; the branch carries the fixes only | `docs/reports/estar-slot-map-design-2026-09-08.md` |
| 2026-09-20 | E | Roadmap item 5, first half — the letter loop reads CDRH letters | Item 5 is not a new loop: `c2c_correspondence` / `_issues` / `_response_packages` is the canonical correspondence OS and it is MOUNTED (register-inline-routes.ts:1228), so this extends it. Measured first, on the letters CDRH sends: a **Refuse to Accept** parsed as `other_unclassified`, severity **low**, **blocker false** — the most severe device outcome short of NSE, classified as nothing, because the shipped rule reads `refuse to file` (the DRUG term) and `rtf` is one character from `rta`. An **Additional Information** letter with four numbered deficiencies (SE, biocompatibility, cybersecurity, labeling) recognised NONE of them and returned one issue mapped to CTD 2.5 / 2.7.4, matched on the word "risk". Every section the shipped taxonomy can name is a drug CTD number; a 510(k) has none. The device CATEGORIES already existed in the union and none was reachable — the type was designed for device and the rules were never written. Sections are per PATHWAY because the rule-pack keys collide hard (510(k) D5 = Shelf life, De Novo D5 = Cybersecurity; D4 = Sterilization vs Software; E1 = Biocompatibility vs Labeling), the same defect class that forced the document-class narrowing in the attachment resolver; PMA/IDE/Q-Sub/513(g) classify but name no section rather than borrowing 510(k)'s. Keys transcribed and asserted against the seeding migration, including that the key is the RIGHT one (a pathway mix-up passes every existence check). Wired into intake from `c2c_submissions.submission_type` — the row, tenant-scoped, from the lookup intake already did, and the parse moved after it so a 404 no longer burns one. **Also fixed, drug-side:** the taxonomy's unanchored `/format/` matched "in-FORMAT-ion", so every letter containing the word "information" — including every Request for Additional Information — carried a fabricated eCTD issue mapped to `module_index`; three more accidents beside it (`risk` in asterisk, `quality` in equality). Nine mutations seen failing; three of my own new tests were found asserting nothing behind an `if (status !== 201) return` and fixed; tsc clean | `server/services/regulatory-correspondence/device-issue-taxonomy.ts` |
| 2026-09-20 | E | Roadmap item 5, second half — the response package was built from almost nothing | The response package is the letter loop's deliverable. Three defects, all behind one `as any`, all measured first. (1) `POST /response-packages` passed `SELECT * FROM c2c_correspondence_issues` into the compiler UNMAPPED — snake_case rows into a camelCase contract — so `mappedCtdSections` read undefined and **every package ever compiled had an empty issue matrix**, while the data to fill it sat in the row being passed. (2) Nothing persisted `structured_extraction` (no column, not in the INSERT) or `subcategory` (column existed, never written), so the parser's section candidates and the regulator's actual evidence ask died in the intake response and every checklist fell back to the generic 'Issue evidence attachment'. Column added by amending the creating migration IN PLACE with a dated note per RULE 1 — CREATE for fresh, guarded ALTER for provisioned, no DROP. (3) `status: 'missing' as const` made readiness a constant: always `evidence_gap`, three of five states unreachable, and resolving the issue could not move it — except the inverted case, where ZERO issues meant zero gaps and a package with **nothing assessed reported `review_ready`**. Readiness now reads signals that exist (nothing selected → draft; unconfirmed extraction → draft; unresolved or unrevised → evidence_gap; confirmed, closed and revised → review_ready); `approval_ready`/`send_ready` are deliberately not produced because compiling is not approving, and that nothing sets them is left visible. **Three existing tests changed and two pinned the defect** — one asserted `evidence_gap` under the name `review_ready` with a comment explaining the constant; one asserted `review_ready` for zero issues. Seven mutations seen failing; one of my own new tests was asserting the wrong insert and was fixed; four migration gates pass; tsc clean | `server/services/regulatory-correspondence/issue-row-mapper.ts` |
| 2026-09-22 | E | Stream paused under RULE 2 — reported BLOCKED, not done | RULE 2 landed while this stream was mid-roadmap. Checked the stream against `shared/constants/launch-scope.ts` before continuing: no device, 510(k), eSTAR, MDx, IVD or correspondence id is in the launch catalog, so item 5's amendment package (the planned next step) is the new-capability-outside-the-catalog the rule forbids. Did not start it. Then looked for a launch row this session could move instead, using the DoD's own dashboards: `ga-readiness-report` 5/40 with 18 blockers, **every one owned by Procurement, Ops, Regulatory Ops or Security**, or by Engineering only after Procurement vendors a file; `submission-preflight` 2/15, the two present being this stream's own eSTAR templates, the 13 missing all licensed downloads or agency validators. Nothing is engineering-closable from this environment, and the launch playbook that assigns W1–W7 does not exist in the repo, which the W4 session already showed produces collisions (two D4 packages built the same day). Reported blocked per RULE 2 rather than inventing work. Recorded above: 3 of the 18 GA blockers are device items counted against a device-free launch, and the probe's 'AcroForm' wording is wrong for an XFA form. One overdue in-scope obligation noted and NOT taken unilaterally: the DoD's weekly periodic review of the launch catalog (`docs/evidence/reviews/`, which does not exist yet) — it is a control-tower duty | `docs/LAUNCH_DEFINITION_OF_DONE.md`; `shared/constants/launch-scope.ts` |
| 2026-09-22 | E | The weekly launch-catalog review, run at the founder's choice (not device work) | First run of the DoD's weekly review: Part 11 UX, honest-state and design-system auditors, plus the security and tenant gates by hand, all on the six launch apps. Every critical and high finding was re-read on the server path by this session. 4 critical (protocol finalize and review disposition ledgered as `sign` with no ceremony; QMP create/activate/delete unaudited; contradiction resolve unaudited with an invented resolver; every task ledger write best-effort with its outcome discarded, including PIN-signed completion) and 2 high (release-signature panel shows no signer; task writes have no role gate). Honest state is clean across all six apps. CI on the branch was red, with 11 Lint steps failing: four fixed here (sandbox absolute import, stale generated ramp, two tenant-gate false positives), the rest attributed to their commits. **Device-relevant:** the review found `mdx/surfaces/VaultSurface.tsx:367-379` renders hardcoded "Recent audit" rows, ungated in production. It sits outside the launch catalog, so it is recorded, not fixed; fix it first when this stream resumes. Correction to this session's own earlier statement: the launch `vault` and `tasks` surfaces are `v2/surfaces/Vault.tsx` and `TaskBoard`, not the device components | `docs/evidence/reviews/2026-09-22/` |
| 2026-09-23 | E | D5 governed path: P1 protocol signatures, under the founder's delegation (not device work) | Protocol finalize and review disposition now run the canonical Part 11 ceremony: re-authentication, the meaning checked against authorship, and the ledger plus the `electronic_signatures` row on one transaction. Three adversarial review rounds; every finding was re-verified in code. The second round found the authorship check running after the finalize write, which made every finalizer an author (proven on PGlite, red then green). Also landed: the signature row states the decision; a reviewer account picker, so a reviewer signs their own review; `ci:sign-ceremony`, which now sees ternary commands (4 more unceremonied sign routes baselined: IRB, IBC, IACUC, RIM); AnA refuses its 9 signing tools; and the request DB client no longer pools a connection that is inside a transaction. Open: T (task ledger atomicity), P2 (QMP), P3 (contradiction resolve) are being repaired after review, uncommitted. **Session incident, recovered:** a merge helper emptied the working copy of `TaskBoard.tsx` while an agent owned it. It was rebuilt from the agent's own backup and re-merged, and the helper no longer writes conflicted merges into agent files. For D2: non-launch module APIs are live unless production sets `MODULE_ENFORCEMENT` | `docs/evidence/D5-GOVERNED-PATH/2026-09-22/README.md` |
| | | | | |
