...
| 2026-09-24 | A | Thirty-third — W2 B9 under Rule 2 (row D1, D6's open item): the ALB answers CloudFront alone | The ALB admitted 0.0.0.0/0 and forwarded everything; it now admits the CloudFront origin-facing prefix list on 443 and forwards only with an origin secret header. Closing it exposed the CloudFront path's own defects, fixed with it: an origin certificate check every `/api/*` request would fail, a distribution-wide 403/404 → `200 index.html` mapping that also rewrote API refusals (and would have read `/readyz` green for any deployment), no route for `/readyz` `/collab` SCIM or the connector, a three-header list. Production trusts two hops (F-24); the smoke test reads `/readyz` through the public URL and fails closed (audit SMOKE-01). Offline proof: mocked-provider `terraform test`, failing on the old modules, all 13 fixes caught when reverted one by one; nothing applied | `docs/evidence/W2/2026-09-24-b9/` |
| 2026-09-24 | A | Thirty-fourth — TRIVY-01 under Rule 2 (row D1): the deploy's security gate | The blocking IaC scan ahead of build-push failed on 16 HIGH/CRITICAL findings, so no tag could deploy; CI's copy was advisory and hid it. Fixed where there was a control to add (k8s securityContext, state-bucket KMS key + write policy, ALB invalid-header drop), excepted at the resource with a reason where exposure is deliberate (public ALB, public frontend bucket, task egress), WAF put to the founder; the scan exited 0, then went red again on merge from the vault-storage lane's new bucket (handed to them), so CI's copy stays advisory until that lands. Trivy built from source (CI's version); all 9 changes caught when removed one by one; exceptions shown to be per resource | `docs/evidence/W2/2026-09-24-trivy/` |
| 2026-09-24 | A | Thirty-fifth — SMTP in the stack (D1, D6) | Login OTP is mandatory and the server logs that no one can sign in without SMTP; the stack had no SMTP and the preflight required none, so a deploy would read ready and admit no one. The stack now carries SMTP to both containers (credentials as secrets, port 465 only because the mailer requires TLS nowhere else) and the preflight refuses a task definition without it. Stack tests red first (2), then 21/21; each change reverted is caught; the deploy's own preflight accepts the rendered definition and refuses it with SMTP stripped. Also: TRIVY-01 closed, the vault lane fixed its bucket and ci.yml's config scan now blocks | `docs/evidence/W2/2026-09-24-smtp/` |
| | | | | |
