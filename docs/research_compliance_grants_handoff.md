...
- **RLS tenant-isolation — gap found + closed + verified.** `0021_enable_rls_everywhere.sql` is a one-time DO-block; it ran before any research-admin table existed, so **60 org-scoped tables had no `tenant_isolation_policy`** (no DB-level backstop — isolation rested solely on explicit `WHERE organization_id` predicates). `migrations/20260612_rls_research_admin.sql` backfills the identical policy (shadow-mode `app.rls_enforce` kill switch, `app.current_tenant_id`/`app.current_org_id` match, super-admin bypass) + ENABLE + FORCE. `scripts/db-verify/verify-rls.ts` proves it **10/10** against the real policy: coverage + FORCE, shadow-mode-no-breakage, isolation both directions, WITH CHECK on cross-tenant writes, fail-closed with no context, super-admin bypass — and the main harness still passes 82/82 with RLS on in shadow mode.
