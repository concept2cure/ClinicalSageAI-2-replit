...
- **RLS tenant-isolation — gap found + closed + verified + made enforceable.**
  ...
  3. *Enforceability:* the governed mutation path never set the tenant session var, so flipping enforcement on would fail-closed the new domains. `server/services/tenant/governed-tenant-context.ts` (`setTenantContextTx`) sets it transaction-locally; it's now called right after `BEGIN` in **all 13 governed routers** (effort, research-security, grants, research-compliance, iacuc, irb, ibc, nonclinical, rim, inspection, controlled-substances, lifecycle, etmf). No-op in shadow mode.
  4. *Verified:* `scripts/db-verify/verify-rls.ts` proves it **12/12** against the real policy — coverage + FORCE, shadow-mode-no-breakage, isolation both directions, WITH CHECK on cross-tenant writes, fail-closed with no context, super-admin bypass, **and the governed path (`setTenantContextTx`) under enforcement writes for its tenant + is blocked cross-tenant**. Main harness still 82/82 with RLS on in shadow mode.
  - *Remaining rollout (documented):* the AnA conversational handlers (`AnaToolExecutor.ts`) follow the same `BEGIN → domain Tx → recordGovernedAction → COMMIT` pattern and need the identical one-line `setTenantContextTx(client, ctx.organizationId)` after their `BEGIN` before enforcement is globally enabled.
