...
- **64/64 governed-path assertions pass** (`scripts/db-verify/verify-research-compliance.ts`): FCOI derive→certify→provenance→audit-hash-chain→c2c_ana_actions ledger→signature-invalidation; controlled-substances perpetual ledger rejects negative inventory; HA commitment threads 2 provenance links; IACUC approval stamps the 3-year expiration + Module 4 provenance; training gate ("no index until trained") blocks then clears; effort certification (certify + content hash + over-commit rejection); research-security COI (foreign-nexus flag + review); preclinical bridge (digested → governed registry + Module 4 provenance); grant closeout (2 CFR 200.344 gate + award close), subaward eligibility (2 CFR 200.214 screen gate), budget-vs-actual (2 CFR 200.308 over-allocation gate + over-budget risk), cost share (2 CFR 200.306 shortfall→met), and no-cost extensions (2 CFR 200.308 grantee-authority gate + period_end move); Report-OS domain providers compute real numbers; deadline events flow to central `unified_tasks`.
- Reproducible via `scripts/db-verify/README.md`.
- **Audit hash-chain — tamper-evidence verified.** `scripts/db-verify/verify-audit-chain.ts` runs the real verifier (`audit/chain.ts:verifyAuditChain`) over the governed actions the new domains write: **6/6** — the sha256 chain across all governed-action rows re-derives correctly (intact); tampering a row's content breaks the chain and the verifier pinpoints the exact row; tampering the stored hash is detected; tamper tests roll back so the real chain is untouched. This is the Part 11 §11.10(e) integrity pillar alongside RLS.
- **RLS tenant-isolation — gap found + closed + verified + made enforceable.**
  1. *Gap:* `0021_enable_rls_everywhere.sql` is a one-time DO-block that ran before any research-admin table existed, so **60 org-scoped tables had no `tenant_isolation_policy`** (no DB-level backstop — isolation rested solely on explicit `WHERE organization_id` predicates).
  2. *Policy backfill:* `migrations/20260612_rls_research_admin.sql` re-applies the identical policy (shadow-mode `app.rls_enforce` kill switch, `app.current_tenant_id`/`app.current_org_id` match, super-admin bypass) + ENABLE + FORCE, scoped to uncovered tables.
  3. *Enforceability:* the governed mutation path never set the tenant session var, so flipping enforcement on would fail-closed the new domains. `server/services/tenant/governed-tenant-context.ts` (`setTenantContextTx`) sets it transaction-locally; it's now called right after `BEGIN` across the **entire governed surface** — all 13 governed routers (effort, research-security, grants, research-compliance, iacuc, irb, ibc, nonclinical, rim, inspection, controlled-substances, lifecycle, etmf) **and all 43 governed AnA tool handlers** in `AnaToolExecutor.ts`. No-op in shadow mode.
  4. *Verified:* `scripts/db-verify/verify-rls.ts` proves it **12/12** against the real policy — coverage + FORCE, shadow-mode-no-breakage, isolation both directions, WITH CHECK on cross-tenant writes, fail-closed with no context, super-admin bypass, **and the governed path (`setTenantContextTx`) under enforcement writes for its tenant + is blocked cross-tenant**. Main harness 82/82 + 328 AnA tests green with the wiring in place; RLS stays in shadow mode until `app.rls_enforce='on'`.
...
| **PATCH** | **`/awards/:id/cost-share`** (commit) · **POST `/awards/:id/cost-share/contributions`** · **GET `/awards/:id/cost-share`** | `update`/`create` |
| **POST** | **`/awards/:id/nce`** (request) · **`/nce/:id/approve`** (grantee-authority gated) · **GET `/nce`** | `create`/`sign` |

### AnA tools (same governed path, surface `ana`)
`create_grant_proposal`, `record_grant_award` (threads provenance), `review_grant_reporting` (read-only federal obligations), **`set_grant_milestone_status`**, **`open_grant_closeout`**, **`update_grant_closeout`**, **`finalize_grant_closeout`** (gated sign), **`record_subaward`**, **`screen_subaward`**, **`execute_subaward`** (gated sign), **`add_grant_budget_line`** (over-allocation gated), **`record_grant_expenditure`**, **`review_grant_budget`**, **`record_cost_share_contribution`**, **`review_cost_share`**, **`request_no_cost_extension`**, **`approve_no_cost_extension`** (grantee-authority gated sign). The subaward screen pairs with the read-only `screen_restricted_party` tool (live SAM.gov lookup).
...
### Cost share + no-cost extensions (2 CFR 200.306 / 200.308) — added
**Cost share:** `grant_awards.cost_share_committed` + `grant_cost_share_contributions` ledger; `costShareStatus` reconciles committed vs contributed (met %, shortfall). **NCE:** `grant_nce_records` — `evaluateNce` classifies a request as within grantee authority (first, ≤12 months, 2 CFR 200.308(d)(2)) or requiring sponsor prior approval; `approveNceTx` is **gated** (grantee cannot self-approve a sponsor-approval-required extension) and on approval moves the award's `period_end` (which recomputes the closeout window). Report adds `costShareCommitted`/`costShareContributed`/`costShareShortfall`/`ncesByStatus`; metrics add `grants_cost_share_contributions_total`, `grants_nce_requested_total`, `grants_nce_approved_total`. DB-verified (shortfall→met; grantee first-extension auto, second-extension grantee-blocked then sponsor-approved with period_end moved).
...
### Tests landed
`grants-logic.test.ts` (25 — incl. `evaluateCloseout`, `evaluateSubawardEligibility`, `budgetVsActual`, `computeIndirectCost`, `costShareStatus`, `evaluateNce` gates), `ana/__tests__/grants-tools.test.ts` (19 — milestone-status + closeout + subaward + budget + cost-share + NCE tools). DB harness covers the full grant lifecycle through closeout, subaward eligibility, budget-vs-actual, cost share, and no-cost extensions (`scripts/db-verify/verify-research-compliance.ts`, now **64/64**). Typecheck clean.
...
- **Briefing → tasking (actionable):** `triage_compliance_attention` (governed) → `compliance-triage.ts` runs the briefing and dispatches each CRITICAL item into the platform's central `unified_tasks` (high/critical priority, via the existing tasking bridge), so the work lands in the same task list as everything else — no parallel tracker. **Idempotent** (stable `domain|signal` source key → re-running doesn't duplicate). DB-verified (over-spent award → critical task in unified_tasks; re-run is a no-op).
- **Briefing → centralized reporting:** `research_admin.scorecard` report type (taxonomy + Report-OS provider) renders the cross-domain briefing as a first-class report in the centralized reporting/analytics module — severity counts, by-domain rollup, and the ranked top attention items. Lazy-imports the briefing to avoid an import cycle. DB-verified (`computeDomainReport('research_admin.scorecard', orgId)`).
- **Coverage-gap fills (governed, same audited path):** `record_grant_opportunity` (pre-award NOFO → governed `grant_opportunities` row, threaded to grants.gov via `external_id` — the conversational pair for `search_grants_gov`), `fulfill_regulatory_commitment` (HA commitment lifecycle close), `review_ha_interaction` (meeting-readiness, read-only, FDA Formal Meetings/PDUFA), `register_controlled_substance` (creates the substance the perpetual-inventory `log_cs_transaction` books against, 21 CFR 1304).
- **Orchestration — study onboarding:** `assess_study_onboarding` (read-only) → `assessStudyOnboarding` (in `compliance-checklist.ts`) resolves required approvals + training for an activity profile, runs the training gate over the named team, and **cross-references each training gap to the specific committee approval it blocks** (a PI missing CITI human-subjects blocks IRB, not IACUC). One turn answers "what do I need to stand up this study and is my team ready?" — genuinely more than the two underlying tools because of the per-committee attribution. Pure assessor unit-tested (`research-compliance/__tests__/study-onboarding.test.ts`); DB-verified.
- **Orchestration — closeout readiness:** `prepare_award_closeout` (read-only) → `prepareAwardCloseout` (service) loads the four 2 CFR 200.344 closeout items, outstanding/overdue milestones, federal reporting obligations (final RPPR/FFR), cost-share status, and budget posture, then `assembleCloseoutReadiness` (pure) produces a single `readyToClose` verdict + prioritized blockers/warnings — stricter than the finalize gate (also wants milestones current, cost share met, spend within award). Pure assembler unit-tested (`grants/__tests__/closeout-readiness.test.ts`); DB-verified (finalized award → ready; award with no closeout opened → blocked + surfaces obligations).
- **Orchestration — HA meeting package:** `prepare_meeting_package` (read-only) → `prepareMeetingPackage` (service) loads the interaction readiness, the open questions, and the commitments that originated from that interaction (new `listQuestionsByInteraction` / `listCommitmentsByInteraction` reads); `assembleMeetingPackage` (pure, in `ha-logic.ts`) folds them into a readiness verdict + prioritized pre-meeting action list. Pure assembler unit-tested (`ha-interactions/__tests__/meeting-package.test.ts`); DB-verified.
- **Routing eval (description-quality gate):** `ana/__tests__/tool-selection-routing.test.ts` runs representative domain prompts through the deterministic `selectToolsForTurn` pre-filter and asserts the right tool survives the top-50 cut (a tool whose description lacks its own use-case terms would silently never be offered). Covers every tool added this session.
- **Handler situational context:** `ToolContext` now carries `surface` / `projectType` / `documentType` (threaded from the chat route's `tool_context`, the same signal the tool selector already uses). The instrumented handler wrapper records the originating `surface` into tool telemetry (`recordToolSurface` / `getToolSurfaceUsage` in `tool-telemetry.ts`), and `describe_capabilities` exposes per-surface usage — so the new context reaches a real consumer (input to per-surface tool tiering later), not dead plumbing. Tested in `ana/__tests__/tool-surface-context.test.ts`.
- **Tests:** input-guards in `addons-tools.test.ts` / `grants-tools.test.ts`; DB harness exercises the briefing aggregation, CS register→receipt, and the grants.gov-opportunity → linked-proposal pre-award flow (harness now **71/71**).
