...
## Related fix (already landed): snapshot hydration on resume

The `useAI: true` resume-drift gap this runbook originally warned about is now
**fixed**. Runs signed on/after the snapshot-persistence change persist the exact
signed package (leaf manifest + validator outcome + backbone) on the
`package.sign` step, and resume **hydrates** that frozen record instead of
re-deriving from source. The signature therefore binds the record that was
actually approved (§11.70), and `useAI: true` runs — whose AI narratives cannot
be regenerated byte-for-byte — resume cleanly. See
`server/services/__tests__/sign-payload-snapshot.test.ts`.

This does **not** change the deploy-boundary action above: runs suspended in
`awaiting-signature` **before** this change carry no snapshot, so they take the
legacy re-derive + drift path and still need the enumerate + re-sign procedure at
the deploy. Only runs suspended after the change hydrate.

Transmission of the actual leaf **bytes** from the frozen snapshot is still
future work — the snapshot stores the leaf manifest (with per-leaf md5
checksums) but not the PDF bytes. For deterministically-rendered leaves the bytes
reproduce; for `useAI` leaves the rendered PDFs must be persisted to blob storage
before the transmit step can ship exactly what was signed. Track this with the
transmit work.
