...
| IAM-06 | High (regulated) | No server-enforced inactivity timeout, absolute lifetime or concurrent-session limit; `sessionTimeout: 30min` never read; `sessionTimeoutMinutes` stored, not enforced; tokens are bearer JWTs in web storage with a 1-day lifetime. | `server/routes/auth.ts:79,1220-1228`; `server/middleware/enterprise-security.ts:108`; `server/routes/tenant-config.ts:52,272`; `client/src/utils/authToken.ts`; `server/config/environment.ts:331` | Annex 11 §12; 11.10(d); HIPAA 164.312(a)(2)(iii) | new; **server half closed 2026-09-26** (P1-1 commit: `services/session-inactivity.ts`; idle window from `sessionTimeoutMinutes`, 15 min default, enforced by both authenticators, `verifyLiveToken` and the refresh; 12-hour lifetime; `docs/evidence/D6/2026-09-25-p1/P1-1/`); the client timer, the OQ step and the concurrent-session limit open |
