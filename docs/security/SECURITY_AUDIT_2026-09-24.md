...
| DP-32 | Low–Medium | An effective controlled document is retired without a ceremony from chat or HTTP; the AnA door's reason is optional. | `AnaToolExecutor.ts:13649-13690`; `mdx-qms.ts:680-700` | 11.10(d)(e), 11.50; Annex 11 §9, §14 | new; **HTTP and legacy doors closed 2026-09-26** (P1-29 commit: retirement is the approve ceremony with its own digest, signature row and chained ledger pair in one transaction; the legacy transition door refuses `retired`; `docs/evidence/D6/2026-09-25-p1/P1-29/`); the AnA door waits for its window (2026-09-27 03:15 UTC) and is a refusal in P1-34's registry |
...
| DP-36 | High | The propose-only / confirmation partition (P0-12) covers only the ~110 commands routed through `execute_platform_command`; the ~40 directly registered AnA write tools (`save_document_to_vault`, `update_vault_document`, `qms_change_transition`, `retire_qms_document`, …) run with no person confirming, by the gate module's own design note. Re-opens DP-08 in part; DP-31 and DP-32 unchanged. | `server/services/ana/governed-tool-gate.ts:80-89`; `server/services/ana/AnaToolExecutor.ts:13668-13690, 13795-13820, 19631-19757`; `command-rbac.ts` keys against `registerToolHandler` names: zero overlap | 11.10(d); AI Act 14/50; NIST AI RMF MANAGE 2.4 | open (2026-09-26 lens) → P1-34; **registry half landed 2026-09-26 (P1-34 commit)**: `propose-only-tools.ts` classifies 195 directly registered tools (161 proposals, 22 refusals, 12 read-only exclusions; the population is ~170 writers, not ~40), `classifyDirectToolCall` beside `classifyToolCall`, the MCP door refuses (`mcp/tools/runtime.ts`); the wrapper gate, the `/governed-action` tool branch and the stream hold-and-ask wait for their windows (2026-09-27 03:15 / 03:16 / 04:52 UTC); DP-08 stays re-opened in part and DP-31/DP-32 unchanged until the wrapper lands |
