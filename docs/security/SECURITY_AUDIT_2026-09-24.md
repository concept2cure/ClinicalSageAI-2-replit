...
| IAM-06 | High (regulated) | No server-enforced inactivity timeout, absolute lifetime or concurrent-session limit; `sessionTimeout: 30min` never read; `sessionTimeoutMinutes` stored, not enforced; tokens are bearer JWTs in web storage with a 1-day lifetime. | `server/routes/auth.ts:79,1220-1228`; `server/middleware/enterprise-security.ts:108`; `server/routes/tenant-config.ts:52,272`; `client/src/utils/authToken.ts`; `server/config/environment.ts:331` | Annex 11 §12; 11.10(d); HIPAA 164.312(a)(2)(iii) | new; **closed 2026-09-26** (P1-1 commits: `services/session-inactivity.ts`; idle window from `sessionTimeoutMinutes`, 15 min default, enforced by both authenticators, `verifyLiveToken` and the refresh; 12-hour lifetime; the client's idle warning and sign-out reason; OQ-PROJ-19 written, execution pending; a concurrent-session limit of `settings.security.maxConcurrentSessions`, five by default, every sign-in registered through `openSession`, the oldest session superseded and refused as `SESSION_SUPERSEDED`; the same-day follow-up made the Redis registration one atomic script and Redis's decision the only one, gave SAML sessions the tenant's settings, made `optionalAuth` verify like the other authenticators and `/select-organization` rotate; `docs/evidence/D6/2026-09-25-p1/P1-1/` and `P1-1-concurrency/`). Residual: the `tenant-config.ts` schema key for the limit (file inside another lane's window until 02:42 UTC 09-27) and the OQ step for it; without Redis the limit and the idle store hold per task (P1-3, founder) ; **P1-38 commit of 2026-09-26**: the two mints outside `openSession` named by the 2026-09-26 lens (`routes/setup.ts`, `mcp/auth/platform-token.ts`) and a third the extended contract test found (the uncalled `server/auth.ts` `login()`, deleted) are closed; every `type: 'access'` mint under `server/` spreads a registered session, enforced by `session-open-contract.test.ts` scanning every `jwt.sign` (`docs/evidence/D6/2026-09-25-p1/P1-38/`) |
...
| IAM-08 | Medium | `/mfa/verify` tries the emailed code first for every account; `/mfa/resend` mints an emailed code for TOTP challenges; recovery codes are issued and never accepted; no org policy to require an authenticator app. | `server/routes/auth.ts:1427-1438,1565-1606`; `server/routes/authEnterprise.ts:468-479`; `server/services/mfaService.ts:11,443-458,512-535` | 11.300; NIST 800-63B | open (D6 owner item); **recovery codes closed by the P1-2 commit of 2026-09-25** (redeemable once each at the login challenge via `verifyLoginSecondFactor`; the enterprise challenge wired, `routes/auth.ts`'s `/mfa/verify` waits on its window; the emailed-code fallback and the org policy remain); **routes half closed by the P1-2 part-2 commit of 2026-09-26** (recovery codes at `/mfa/verify`; no emailed code for an authenticator account; the org policy remains) ; **enterprise door closed by the P1-38 commit of 2026-09-26**: `/verify-mfa` reads the enrolment first and never tries an emailed code for an authenticator account (`docs/evidence/D6/2026-09-25-p1/P1-38/`); the org policy remains (`tenant-config.ts` window) |
| IAM-09 | Medium | Lockout counter is read-then-write (races; fails open on write failure); `/mfa/resend` resets the emailed-code attempt counter; limiters are per-task and per-IP; `/api/v1/auth` gets the auth router without the failures-only limiter. | `server/services/auth-security-service.ts:186-222`; `server/services/emailOtpService.ts:67-80`; `server/bootstrap/register-platform-routes.ts:91`; `server/middleware/enterprise-security.ts:1070` | 11.300(d) | open (AUTH-19) + new; **closed by `1219a144`** (P1-3, 2026-09-25: one conditional UPDATE, fails closed; a code re-issue keeps spent guesses; `/api/v1/auth` limited); **resend cap closed 2026-09-26 (IAM-18-8 commit)**: `users.email_otp_resends`, `reissueEmailOtp` one conditional UPDATE, at most 3 re-issued codes per challenge, 429 `MFA_RESEND_LIMIT` audited as `resend_limit`, a fresh challenge costs the password (`docs/evidence/D6/2026-09-25-p1/IAM-18-8/`); Redis in production remains the founder's |
...
| IAM-11 | Medium | `/api/v1/drafting/task_status/:task_id` has no tenant predicate; `start_task` no project ownership; `/api/stability/results/:resultId` PATCH/DELETE by id alone, no audit row. | `server/routes/misc-inline-routes.ts:417-476`; `server/src/routes/stability.router.ts:2054-2078` | 11.10(e) | **closed 2026-09-26** (`83849bfd`: drafting tasks written and read through the organisation's own program, no in-memory fallback; stability result PATCH/DELETE on the tenant's own row with `stab_audit` rows; `docs/evidence/D6/2026-09-25-p1/P1-8/`) |
...
| IAM-15 | Medium | Nine `x-org-uuid` header fallbacks remain as the tenant key when the session UUID is null; RLS contains them only while enforcement is on. | `server/routes/ana-features.ts:2360,2597,4879,4952,5037`; `server/routes/chat/send-message.ts:190,292`; `server/routes/c2c/ai-editing.ts:149,1073` | tenant key client-controlled | **closed 2026-09-26** — the `x-org-uuid` half by `b1618c69` (lane `…01W5zW66`, 2026-09-25; `docs/evidence/D3/2026-09-24-atom-search-tenant-key/`: the nine fallbacks read the session's tenant key); the `x-client-id` half by the P1-7b commit (`docs/evidence/D6/2026-09-25-p1/P1-7b/`: the workspace header is a claim verified against the session's organisation by one predicate, `FeatureToggleService.workspaceInOrganization`, at the feature gate and in the cerv2 save and regulatory-submissions routes, 403 on a foreign workspace; `workspace-trust-header` rule in `check:security-patterns`, 4 → 0). Residual: `cortex-unified.ts:128` deferred until its window closes (2026-09-27 04:43 UTC); the two-tenant dbtest executed 2026-09-26 on the provisioned local database, 7/7 |
...
| IAM-17 | Low–Medium | Signup returns a 24-hour admin token with no email verification and no MFA; no breached-password screening; password change writes a log line, not an audit event; `mustChangePassword` is advisory. | `server/routes/auth.ts:991-1029,2285`; `server/services/auth-security-service.ts:45-88` | 11.100(b), 11.300(b), 11.10(e) | open (AUTH-18) + new; **sign-up verification closed 2026-09-26** (P1-2 third part: the account is created `pending_verification`, a 24-hour single-purpose link in the URL fragment moves it to `active` once, sign-in for a pending account is refused as `AUTH_EMAIL_UNVERIFIED` at both doors, a deployment that cannot mail the link refuses sign-up; `docs/evidence/D6/2026-09-25-p1/P1-2-signup/`); the login-timing item closed by P1-2 part 2; **common-password and context checks closed 2026-09-26** (P1-2 fourth part: a bundled list of 11,197 commonly used passwords compared bare and behind decorations, and the account's own words refused, at sign-up, setup, change and reset; `docs/evidence/D6/2026-09-25-p1/P1-2-passwords/`); the breach-corpus lookup is the founder's decision; **the password-change audit event closed 2026-09-26** (`user_password_changed` from `POST /password/change`, in the account's tenant) |
| IAM-18 | Low | Grouped: `ci:server-error-leaks` red (147 vs 146 baseline, `server/routes/c2c/commitments.ts:156`); public `/api/ai-gateway/health` lists providers and echoes `err.message`; `/api/metrics` readable by any tenant's user; the `validateTenantContext` impersonation detector is mounted before auth and never runs; enterprise verify-password/verify-mfa record `user_login success` and mint a token for suspended accounts; SAML token returned in a query string and `InResponseTo` `ifPresent`; 50 MB JSON parsed before auth on `/api/concept2cure`; login timing reveals which emails exist; AI rate-limit bucket misses the main AI prefixes; `/api/ai-assistance` has no mount-level auth. | `server/startup/inline-endpoints.ts:53-69,693-717`; `server/middleware/enterprise-security.ts:541-563`; `server/index.ts:121,137`; `server/routes/authEnterprise.ts:245-563`; `server/routes/sso.ts:396-399`; `server/services/saml-provider.ts:100-109`; `server/startup/middleware.ts:95`; `server/routes/auth.ts:376-396`; `server/middleware/redisRateLimiter.ts:349-367`; `server/bootstrap/register-core-routes.ts:128` | 11.10(e); GDPR 32 | mixed; **(2) and (3) closed by the P1-17 commit of 2026-09-25** (`/api/metrics`, `/api/health/full`, `/api/health/jobs` need the scrape token or a platform administrator; the gateway health answers a status word, detail at `/detail` behind the same guard, no exception echo); **(1) closed on trunk by `3625a205`; the leak baseline ratcheted 146 → 145 → 120 (P1-17 paydown of 2026-09-26: 25 cold sites in 12 files onto `serverError()`, the two coded 503s in `ana-features.ts` static with the detail logged; 120 sites / 77 files remain, 7 in 4 hot files; `docs/evidence/D6/2026-09-25-p1/P1-17-paydown/`); **(4), (5), (9), (10) closed by the IAM-18 residuals commit `50b04d99` of 2026-09-25** (detector behind the boundary; **(4) re-checked and polished 2026-09-26**: the dead public-path skip list deleted in favour of the boundary's one allowlist, the impersonation audit row names the full `/api` path through the shared `requestFullPath`, a real-token end-to-end test through `applyAuthBoundary` shows 403 `TENANT_MISMATCH` with the audit row; the unmounted second detector `middleware/tenantIsolation.ts`, which only logged, deleted; enterprise steps refuse an inactive account; AI prefixes metered as AI; AI-assistance mount gated); **(6) closed 2026-09-26** (both callbacks hand the session over in the URL fragment to the sign-in page, which adopts it only when `GET /session` confirms it; SAML `InResponseTo` defaults to `always`; `docs/evidence/D6/2026-09-25-p1/IAM-18-6/`); **(8) closed: the main door by the P1-2 part-2 commit `e93c7878`, the enterprise door and both null-hash branches by the IAM-18-8 commit of 2026-09-26** (one shared pad, `server/services/login-timing-pad.ts`, cost 12 at both doors; `docs/evidence/D6/2026-09-25-p1/IAM-18-8/`); **(7) closed 2026-09-26** (the Concept2Cure 50 MB parser mounts after the auth boundary; anonymous bodies unread; `docs/evidence/D6/2026-09-25-p1/IAM-18-7/`) |
| IAM-19 | Medium | The `/ana` duplex socket has no periodic session re-check: a session ended by logout, password change, revocation, membership removal or tenant suspension keeps an open `/ana` socket and its tool loop alive, while the main namespace gained `startSessionRecheck` under P1-9. | `server/services/ana/ana-realtime.ts:197-311`; compare `server/socketServer.ts:199-221,377-379` | 11.10(d); HIPAA §164.312(a)(2)(iii) | open (2026-09-26 lens, `docs/evidence/reviews/2026-09-26/security.md`; residual of P0-1) → P1-33 |
...
| DP-11 | Medium | Two disjoint stores: launch apps write `audit_logs`; the inspector-facing signed export reads only `audit_events`; the export key falls back to `JWT_SECRET`; a symmetric seal cannot be verified offline by an inspector. | `server/services/auditService.ts`; `server/services/audit/signedAuditExport.ts:115-125,269-297`; `server/routes/audit-trail-routes.ts:619-624` | 11.10(b)(e); Annex 11 §8.1, §9 | open + new; **store half closed by `3c7fb27c` + `5eb52e22` (VR-02, lane `…01KnUGoX`, 2026-09-25; `docs/evidence/D5-SIGNED-EXPORT-AUDIT-LOGS/2026-09-25/`): the export reads both stores in one signed stream.** The `JWT_SECRET` fallback for the export key (`signedAuditExport.ts`) and the symmetric signature remain (P1-19b) |
...
| DP-25 | Low | Mixed clocks: application time for `audit_logs`/signatures/`tamper_proof_log`, database `now()` for `audit_events`; no declared time source. | `server/lib/tamper-proof-audit.ts`; `db/migrations/20260222_audit_events_hash_chain.sql` | 11.10(e) | new; **closed by declaration 2026-09-26** (VMP-001 v0.2 §7a: Amazon Time Sync through Fargate and RDS; every audit and signature timestamp writer-generated in UTC, never client-supplied (`docs/evidence/D6/2026-09-25-p1/P1-26/writers.txt`); the chain ordered by `chain_seq`, not by clock). The VMP's approval remains the founder's |
| DP-26 | Low | Browser Sentry session replay with no `beforeSend` scrubber (dormant, no DSN); server logger does not redact email/IP keys. | `client/src/utils/sentry.ts:6-15` | GDPR 25; HIPAA 164.312(b) | new; **browser half closed 2026-09-26** (P1-27 commit: `sentryScrub.ts` as `beforeSend`/`beforeBreadcrumb`, `sendDefaultPii: false`, replay masking stated; `docs/evidence/D6/2026-09-25-p1/P1-27/`); **server half closed 2026-09-26** (the logger masks an e-mail address to its first character and domain and an IP address to its network part wherever either appears as a log value; sensitive keys stay redacted whole; `server/utils/logger.ts`, same evidence folder) |
...
| INF-04 | High | The SPA is served from S3 through CloudFront with no response-headers policy and no meta CSP: no HSTS, CSP, `frame-ancestors` or `nosniff` on the document; Helmet covers only ALB paths; signing and approval screens can be framed. | `terraform/modules/cloudfront/main.tf:151-173`; `client/index.html:6-8`; `server/middleware/enterprise-security.ts:166` | NIST SC-8/SI-10; SOC 2 CC6.6; HIPAA 164.312(e) | new (June "cleared" held for Express pages only); **closed by the W2 lane by 2026-09-26** (a response-headers policy with HSTS, CSP, nosniff and frame DENY on the default CloudFront behaviour, `terraform/modules/cloudfront/main.tf`; per the 2026-09-26 lens, not verified against a live distribution). INF-05/06 remain absent |
...
| Id | Sev | Finding | Evidence | Hook | Status |
|---|---|---|---|---|---|
| DP-31 | Medium | A QMS change-control record is approved with no ceremony on either door: the AnA tool `qms_change_transition` is ungoverned, its "reason" is three characters of model output and its audit write is fire-and-forget after the stamp; the HTTP twin's only control on `approved` is segregation of duties. | `server/services/ana/AnaToolExecutor.ts:13765-13790`; `server/routes/mdx-qms.ts:1351-1398`; `server/services/qms/changeControl.service.ts:203-215`; `server/services/ana/governed-tool-gate.ts:96-117` | 11.10(d), 11.50, 11.200(a)(1); Annex 11 §14; ICH Q10 3.2.3 | new; **closed on the AnA door by P1-34 (2026-09-26)**: approval refused by the change-control service (the signed Approve button), closing refused by the tool register, every other transition a person's confirmation; the HTTP `closed` transition is unsigned (P1-34 hand-on 6) |
| DP-32 | Low–Medium | An effective controlled document is retired without a ceremony from chat or HTTP; the AnA door's reason is optional. | `AnaToolExecutor.ts:13649-13690`; `mdx-qms.ts:680-700` | 11.10(d)(e), 11.50; Annex 11 §9, §14 | new; **partial by P1-34 (2026-09-26)**: the AnA door needs a person's confirmation and a reason of eight characters; the retire act is unsigned on both doors (P1-34 hand-on 6); **HTTP and legacy doors closed 2026-09-26** (P1-29 commit: retirement is the approve ceremony with its own digest, signature row and chained ledger pair in one transaction; the legacy transition door refuses `retired`; `docs/evidence/D6/2026-09-25-p1/P1-29/`); the AnA door's confirm-plus-reason is weaker than the signed HTTP door beside it (cross-check filed for the register's lane) |
...
| DP-36 | High | The propose-only / confirmation partition (P0-12) covers only the ~110 commands routed through `execute_platform_command`; the ~40 directly registered AnA write tools (`save_document_to_vault`, `update_vault_document`, `qms_change_transition`, `retire_qms_document`, …) run with no person confirming, by the gate module's own design note. Re-opens DP-08 in part; DP-31 and DP-32 unchanged. | `server/services/ana/governed-tool-gate.ts:80-89`; `server/services/ana/AnaToolExecutor.ts:13668-13690, 13795-13820, 19631-19757`; `command-rbac.ts` keys against `registerToolHandler` names: zero overlap | 11.10(d); AI Act 14/50; NIST AI RMF MANAGE 2.4 | open (2026-09-26 lens) → P1-34; **closed 2026-09-26 by P1-34**: a register of all 763 tools (`server/services/ana/tool-authorization.register.json`), 159 writes proposed, 16 refused, 21 by rule, an unknown tool proposed; the scan missed 43 writes, found by tracing all 573 presumed reads (`docs/evidence/D6/2026-09-26-p1-34/`) |
| DP-37 | Medium | A raw SQL leaf-program lookup for vault documents carries no tenant predicate; safe today only because `verifyLeafSource` runs first at the single call site. `ci:tenant-isolation:no-regression` fails on it (1 above the baseline of 9). | `server/services/submission-service/submission-service.ts:1843-1845` (its siblings at `:1846-1861` filter on the organisation) | GDPR 32; 11.10(d) | **closed 2026-09-26 by lane `…01DiJJAk`, `6ce7c30b`** (the leaf-program read goes through the program, as the tenancy verifier scopes it; `ci:tenant-isolation:no-regression` green again, 8 current against a baseline of 9) |
| DP-38 | Low–Medium | `POST /api/audit/signatures` writes a non-authoritative `audit_events` marker (`regulatory_significant`, `gxp_relevant`, body-supplied entity, meaning, reason, metadata) on `requireAuthedOrgId` alone; every sibling write in the file gates on `requireAuditRecorder` since P1-20. | `server/routes/audit-trail-routes.ts:469-513`; `server/services/audit/audit-api-authority.ts:23,75-81` | 11.10(e); GDPR 5(1)(f) | **closed 2026-09-26** (P1-36 commit: `requireAuditRecorder` on `POST /api/audit/signatures` as on its siblings; a viewer or member is 403 before the body is read; `docs/evidence/D6/2026-09-25-p1/P1-36/`; follow-up the same day: the two chain-monitor routes, which had no gate at all, refuse anyone but a platform administrator and the check's 500 no longer carries the error text) |
| DP-39 | Low | The server logger's personal-data mask (P1-27) runs over the `context` object's string values only: the message string and array elements are not scanned, so a message-only call interpolating an address logs it raw. | `server/utils/logger.ts:142,187-196` | GDPR 5(1)(c), 25; HIPAA §164.312(b) | **closed 2026-09-26** (P1-37 commit: the message string and array elements are masked too, in both logger mirrors; `docs/evidence/D6/2026-09-25-p1/P1-37/`; follow-up the same day: the console bridge masks string arguments) |
...
| Append-only audit trail with chain and HMAC seal; chain verifier; signed exports with offline-verifiable attestation | partial; "offline-verifiable" **refuted** | **store half no longer refuted:** since `3c7fb27c` (VR-02, 2026-09-25) the signed export covers both `audit_events` and `audit_logs`; "offline-verifiable" still refuted: the export uses a symmetric key an inspector cannot hold (DP-11); DP-04 |
