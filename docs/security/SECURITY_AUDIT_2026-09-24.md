...
| DP-38 | Low–Medium | `POST /api/audit/signatures` writes a non-authoritative `audit_events` marker (`regulatory_significant`, `gxp_relevant`, body-supplied entity, meaning, reason, metadata) on `requireAuthedOrgId` alone; every sibling write in the file gates on `requireAuditRecorder` since P1-20. | `server/routes/audit-trail-routes.ts:469-513`; `server/services/audit/audit-api-authority.ts:23,75-81` | 11.10(e); GDPR 5(1)(f) | **closed 2026-09-26** (P1-36 commit: `requireAuditRecorder` on `POST /api/audit/signatures` as on its siblings; a viewer or member is 403 before the body is read; `docs/evidence/D6/2026-09-25-p1/P1-36/`) |
| DP-39 | Low | The server logger's personal-data mask (P1-27) runs over the `context` object's string values only: the message string and array elements are not scanned, so a message-only call interpolating an address logs it raw. | `server/utils/logger.ts:142,187-196` | GDPR 5(1)(c), 25; HIPAA §164.312(b) | **closed 2026-09-26** (P1-37 commit: the message string and array elements are masked too, in both logger mirrors; `docs/evidence/D6/2026-09-25-p1/P1-37/`) |
