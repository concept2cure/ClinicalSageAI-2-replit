...
| Sentence-level source traceability missing (the headline differentiator) | **Backend + API DONE; only UI missing.** `sentenceTraceabilityService.ts` (≈795 lines) does sentence splitting, per-sentence source mapping, click-through resolution, coverage reporting, persistence to `evidence_links` — and it is already exposed via `POST /traceability/map`, `/traceability/click-through`, `/traceability/report` in `audit-services.ts`. The only remaining work is the client click-through UI | `server/services/sentenceTraceabilityService.ts`, `server/routes/audit-services.ts` |
...
2. **Regulatory corpus is data-starved — pipeline now runnable (2026-06-10).** The ingestion pipeline, CSR intelligence library, and precedent benchmarking are production-grade, and now have a runnable entrypoint (`scripts/ingest-corpus.ts`) + ops runbook (`docs/runbooks/corpus-ingestion.md`), proven end-to-end over real trial data. What remains is purely operational: run the ingestion sweep against a DB with `clinicaltrials.gov` egress allowed (the corpus tables are still near-empty until someone runs it). *Owner: data-ops — run the sweep; no engineering blocker.* `server/services/corpus/*`.
3. **Veeva Vault integration — engineering CLOSED (2026-06-10); credentials/UAT remain.** Protocol client (`server/integrations/veeva-vault/client.ts`: session auth, upload/fetch/download, VQL; fail-closed) plus the coexistence workflow layer (`field-mapping.ts` with per-org type overrides, `veeva_vault_links` migration + idempotent link store, `VaultSyncService` push/pull with mandatory audit). 18 offline tests. Remaining: real Vault credentials + a customer-vault UAT pass, and route/UI surfacing. *Owner: ops/customer onboarding + design.*
...
6. **E-signature §11.50 manifestation — CLOSED (2026-06-10).** All three elements (printed name, date/time, meaning) are captured and stored; `GET /signatures/:signatureId/manifest` now returns a pre-formatted manifestation block (pure `buildSignatureManifest`, tested) for PDF/print embedding. `server/routes/part11-compliance.ts`.
