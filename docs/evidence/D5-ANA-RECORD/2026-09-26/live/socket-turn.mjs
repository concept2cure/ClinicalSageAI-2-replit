...
const timer = setTimeout(() => { console.info(JSON.stringify({ event: 'timeout' })); process.exit(1); }, 60000);
socket.on('connect_error', (e) => { console.info(JSON.stringify({ event: 'connect_error', error: e.message })); process.exit(1); });
socket.on('connect', () => socket.emit('ana:message', { turnId: 'live-1', message }));
for (const ev of ['ana:tool', 'ana:done', 'ana:error']) {
  socket.on(ev, (p) => {
    console.info(JSON.stringify({ event: ev, ...p, text: p.text ? String(p.text).slice(0, 80) : undefined }));
