...
| B16 *(new)* | Sign-in would 403 on any domain outside a hard-coded list: `csrfProtection` compares `Origin` to `ALLOWED_ORIGINS`, which was not set | `ALLOWED_ORIGINS = app_url`; `app_url` validated to be exactly the string a browser sends as `Origin` (lowercase host, no path, no trailing slash) | `terraform-test-must-fail.txt` |
