...
| `/api/mdx/software` | IEC 62304 software lifecycle | **surfaced** — Engineering documents zone (SBOM/SRS/etc.) |
| `/api/mdx/ldt` | LDT inventory | screenless; engine retired, `legalStatus` returned |
| `/api/mdx/clinical-studies` | device clinical studies | screenless |
| `/api/mdx/rbm-board` | risk-based monitoring board | screenless |
| `/api/mdx/notifications` | notifications | screenless |
| `/api/mdx/search` | cross-surface search | screenless |
| `/api/mdx/ana-drafts` | AnA draft persistence | screenless |

The remaining screenless routes are reachable **conversationally through AnA**
but not by clicking. `qms` (a tenth, consumed only by the separate `quality/*`
app) now also has an MDX-shell readiness view.

Placement note for the three surfaced so far: each went to the surface the
platform review named for it — CDx and CLIA into the IVD programme workspace,
software lifecycle into device Engineering. `clinical-studies`, `rbm-board`,
`notifications` and `search` do not have an obvious single home, so their
placement is left as a product decision rather than an engineering guess.
