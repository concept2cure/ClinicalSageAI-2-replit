...
| AUD-008 | Medium | The baseline captures tracked and untracked non-ignored files. This is useful during preparation but can accidentally include an uncommitted contract in an approval candidate. | **Open:** release evidence generation should require a clean Git worktree; retain an explicit override only for local draft capture. |
...
`pg`. Promise/callback APIs and PostgreSQL transaction aliases are covered.
...
**Not yet proven:** clean-tree enforcement, generated-contract freshness,
database schema dump/restore, repository settings export, performance baseline,
