...
| c2c_dossier_map | DossierMap (mounted, fixture-free) | **CONVERGE** (verified) | **[CONVERGED]** onto project_sections + parent projects (the real CTD tracking store the authoring workflow writes): dossier-map-view-assembler rolls per-section state to module grain (pct = complete/total with the project-sections completion set, tone derived from pct only, most-advanced-status dedup across projects, archived projects excluded); route real-store-only; reseed writes real project_sections; blob deprecated. Agent died at the session limit at 95% — finished + tests written by the integrator |
| c2c_research_admin | ResearchAdmin (mounted, fixture-free) | **CONVERGE** (verified) | **[CONVERGED]** onto research_personnel + personnel_training (real writers: roster-service via /api/research-compliance, citi-service via /api/citi-training, AnA executors): research-admin-view-assembler derives each training cell's status LIVE via the landed pure citi-logic trainingStatus (60-day window); no record → honest 'n/a' (the store has no requiredness matrix — absence is never asserted as a compliance failure); route real-store-only; reseed writes real dated CITI-cycle records; blob deprecated |
| c2c_evidence_asks | Evidence — "Ask the corpus" (mounted) | **CONVERGE** (verified) | **[CONVERGED]** onto the REAL AI trace chain — ai_retrieval_runs + ai_retrieval_chunks + ai_generation_runs (20260224_ai_trace_chain), the exact provenance tables POST /api/evidence/ask persists on every live corpus ask (evidence-ask.ts:201/242/338): evidence-asks-view-assembler maps each retrieval run → one ask (question, rank-ordered source chunks, model_assisted pedigree when a generation run is linked else deterministic_query); GET /api/evidence-asks real-store-only, returns the org's latest real ask or honest null. HONEST by construction — the chain retains the answer's HASH not its prose (answer:null, answerRetained:false; the traced sources ARE the grounded evidence) and no cite/suggestion rows are written (→ [], never fabricated); reseed writes the real chain (thread+run+3 chunks+gen; answer hashed not stored, exactly as production); blob deprecated |
| c2c_evidence_objects | PDEV Evidence Picker (mounted, fixture-free) | **CONVERGE** (verified) | **[CONVERGED]** onto the REAL `evidence_objects` graph table (drizzle pgTable, shared/schema/programs.ts:122-193 — uuid PK, org-scoped; the canonical evidence inventory read by 10+ live services and resolved by the PDEV attach POST). GET /api/evidence-objects now SELECTs `FROM evidence_objects`, mapping to the picker contract {id::text, title, evidence_type→type, evidence_category→category}; `source` is an HONEST derivation `COALESCE(source_reference, source_type, '')` (the real table has no `source`/`program_code` columns — nothing fabricated, documented in the route header); `q` ILIKE spans only real columns; programId accepted-but-not-filtered (real program_id is an unpopulated link). meta.source='evidence_objects'; 42P01→honest empty. EvidencePicker.tsx was already fixture-free (live fetch + honest empty state). Seed 90 inverted to write REAL evidence_objects rows (blob write dropped); blob deprecated. Route + pglite integration tests — 10 green. |
| c2c_investigator_brochure | InvestigatorBrochure (mounted, now fixture-free) | **CONVERGE** (verified) | **[CONVERGED]** — the IB has NO authored instance table (BUILD rejected: correct, none exists), so GET /api/investigator-brochure is *assembled* from the REAL upstream evidence the CTD composers already produce: nonclinical_studies (§4), clinical_studies/clinical-ops studies (§5.1), adverse_events + risk_management_plans (§5.2/5.3), cmc_module3_sections (§2/§3). investigator-brochure-view-assembler runs the deterministic ib-builder gap engine (resolveAvailability + detectSectionGaps + sectionStatus) for each section's rendered/partial/missing verdict — to_regclass-guarded per store, soft-delete aware, strictly tenant-scoped. HONEST states: a domain the org has no evidence for renders `missing` (never fabricated); an org with no upstream evidence at all gets the deterministic ICH E6(R2) §7 skeleton with provisioned=false; any store error fails closed to that skeleton (never 500, never fabricates IB content). Client migrated to useLiveRows + EmptyState (fixtures deleted). Seed 124 writes the real cmc_module3_sections product-domain rows (nonclinical/safety already seeded); blob deprecated (retained). Assembler pglite + route tests — 12 green. |
| c2c_labeling_pi | LabelingPi (mounted, now fixture-free) | **BUILD** (verified; spl_sections rejected — program-scoped/drifted; labeling_documents is device-IFU) | **[BUILT]** `labeling_pi_sections` (20260801, org-scoped, one live row per (org, section) via partial unique index, USPI document order DERIVED from section_no) + write path (labeling-pi-service upsert: section/status/flag vocab validated, content/negotiation payload-shape validated so malformed JSONB can never break the render loop; POST /api/labeling-pi audited); GET real-store-only; client LP_ROWS fixture deleted → honest states + the two fabricated KPIs (hard-coded '17 sections'/'1 boxed warning') now derived from real rows; reseed preserves the BX-204 USPI narrative; blob deprecated. Agent died at the session limit after migration+service — route/client/seed/tests finished by the integrator |
| c2c_program_journey | BiopharmaJourney (mounted, fixture-free) | **BUILD** (verified) | **[BUILT]** `program_journeys` + `program_journey_stages` (20260801, org-scoped, soft-delete; the per-stage overlay is one REAL row per lifecycle stage, not a blob) + write path (program-journey-service: create validates seg/stage/status vocab + display-payload shapes; POST /api/program-journey + POST /:id/stages/:stageId both audited); GET assembles the overlay from real stage rows; reseed writes parent + 9 stage rows (BX-204 NDA narrative preserved); blob deprecated. Agent died at the session limit after the migration — service/route/seed/tests finished by the integrator |
...
| c2c_blockers | submission-ops / MDX Validation / AnA risk-watch (mounted) | **ALREADY_REAL** (verified first-hand) | NOT a blob — real DDL (`migrations/20260524_c2c_blockers.sql`) + drizzle schema (`c2cBlockers`) with LIVE non-seed writers: `db.insert(c2cBlockers)` at submission-ops/automation-runner.ts:379 (dispatch gates) and regulatory-correspondence/operating-layer.ts:112, `.update(c2cBlockers)` at routes/submission-ops.ts:824; real readers (readiness-engine, risk-watch, submission-twin, mdx-analytics). No convergence needed. **Fixed a latent GA bug found in the trace:** the org column is `org_id` (INTEGER), but the `blocker` case in resolveTargetOwnerId (separation-of-duties.ts) and the AnA action existence-resolver (routes/c2c/actions.ts) filtered on the non-existent `organization_id` → every such query 42703'd. In SoD the catch treats 42703 as "not migrated → degrade to ALLOW", so the un-disableable author≠signer control was silently disabled for blocker targets. Both queries corrected to `org_id`; added a SoD blocker-case test asserting the real column so it can't regress. |
