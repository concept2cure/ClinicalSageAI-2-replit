...
2. **Backend lands a capability with no face.** Measured against the code at
   `2a5b46d`: **365** distinct `/api` prefixes are mounted from the canonical
   entry points; **134 of them (36%) have no discoverable reference in
   `client/src`** — not by exact path, not by last path segment. The repository's
   own CI auditor (`scripts/ci/audit-route-mounts.mjs`) counts **323 mount
   events**, with 8 errors and 7 warnings.

   This includes the capabilities the strategy material calls the moat:
   `/api/operating-system` (assumptions + decisions), `/api/resolution`
   (correction bundles), `/api/study-design`, `/api/c2c/study-twin`,
   `/api/evidence-fabric`, `/api/data-lineage`. Capability no user can reach is
   not capability.

   *(Method note, and a warning for anyone re-measuring: this codebase mounts
   routes through **three** idioms — `app.use('/api/x')`, a declarative
   `{ path, mod, name }` table, and `['/api/x', mod, name]` tuple arrays. Scanning
   only the first understates mounts by ~33% and produces false "unmounted"
   findings. Prefer the repo's own auditor over ad-hoc greps.)*
...
1. **The left navigation rail is frozen. Do not change it at all.**
   *(Owner instruction, 24 July 2026.)* No additions, removals, reordering,
   re-tiering, relabeling, restyling, hiding, or collapsing — in any work order in
   this document. Every UI need must be met inside a surface, in ⌘K, or via an
   in-context entry point. If a task appears to require a rail change, stop and
   raise it with the owner; do not design around it.
2. **No new top-level destination.** If an existing project, document, dossier,
   review, submission, Vault, or AnA surface can own the workflow, it does. The
   Apps page must not become a catalog of backend services. Every work order below
   names its host surface.
3. **Four honest states, always.** Every surface renders exactly one of: **live
...
4. **Never invent a number.** Scores, percentages, coverage figures and readiness
   ...
   choice.
5. **The UI may not soften a governance boundary.** Freeze, approval, e-signature,
...
6. **AI output is labeled and never authoritative.** Anything generated by AnA is
...
7. **Build only against a published contract.** See §3. A surface at readiness
   `routes-ready` may be *designed* but must not be *wired* until its typed
   contract lands.
8. **Compliance rails are gates, not suggestions.** Each surface's `compliance`
...
### 🚫 Hard constraint — the left navigation rail is OUT OF SCOPE

**Owner instruction, 24 July 2026: do not change the left navigation rail at all.**

No reordering, no re-tiering, no collapsing, no hiding, no adding, no removing, no
relabeling, no restyling. The rail is frozen. This overrides anything else in this
document that implies otherwise, and it is not negotiable by design rationale — if
a restructure seems necessary, that is a question for the owner, not a decision
for this work order.

### Objective
A buyer must see one submission operating system rather than a catalog of
surfaces — achieved **without touching the rail**. The lever is the journey spine
and the project home, not navigation chrome.
...
2. Design the **project home as the journey entry point**: current stage, what is
   blocked and why, the next governed action, and who owns it. This is where the
   narrative collapses — a user who lands here should know what to do next without
   consulting the rail at all.
3. Add **in-context continuation** between journey stages, so a user moves
   forward from the surface they are on rather than navigating back out.
4. Produce a **reachability matrix**: for every surface, the shortest path to it
   from a journey surface or ⌘K, using the rail exactly as it exists today. Where
   something is hard to reach, the fix is an in-context entry point or a ⌘K
   affordance — **never a rail change**.
5. Where two surfaces appear to own the same workflow, record it as a finding
   with a proposed owner. Do not act on it in this work order.

### Non-goals
No rail changes of any kind. No new surfaces. No renaming a surface without a
registry change request. No hiding a governed action behind progressive
disclosure — blocked work stays visible.

### Gate
- A first-time user reaches the correct journey without training, **with the rail
  unchanged**.
- `git diff` touching any rail component is empty.
- Reachability matrix shows no capability became harder to reach.
- **Stop and wait.**
...
2. Coverage metrics **always** render their denominator and inclusion rules.
   An unexplained percentage is prohibited (§1 rule 4).
...
| 365 distinct mounted `/api` prefixes; 134 (36%) with no `client/src` reference | all three mount idioms, canonical entry points, vs. `client/src/**` |
| 323 mount events, 8 errors, 7 warnings | `scripts/ci/audit-route-mounts.mjs` run at this SHA |
| Only 3 registry-declared prefixes have no mount coverage | registry vs. mounts, containment-tested both directions |
