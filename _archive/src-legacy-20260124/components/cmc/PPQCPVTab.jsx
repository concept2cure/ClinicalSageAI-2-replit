...
export default function PPQCPVTab({ processId }) {
  ...
  async function loadPPQ() {
    const r = await fetch(`/api/cmc-process/processes/${processId}/ppq`);
    setPPQ(await r.json());
  }
  ...
  async function loadCPV() {
    const r = await fetch(
      `/api/cmc-process/processes/${processId}/cpv?param=${encodeURIComponent(param)}`
    );
    setCPV(await r.json());
  }
  ...
  async function importCSV() {
    ...
    fd.append('file', file);
    const r = await fetch(`/api/cmc-process/processes/${processId}/cpv/import`, {
      method: 'POST',
      body: fd,
    });
    if (r.ok) {
