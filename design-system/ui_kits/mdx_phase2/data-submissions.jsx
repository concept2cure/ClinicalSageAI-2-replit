/* global window */
/* ═════════════════════════════════════════════════════════════════
   Submission Center — platform data
   ─────────────────────────────────────────────────────────────────
   The Submission Center is workstream-agnostic. Every regulatory
   filing across every Concept2Cure.RI workstream funnels through one
   surface — MDX (510(k), PMA, CER), Biotech (IND, BLA), Pharma (NDA,
   MAA, supplements), CRO (IDE, study filings) — to every agency
   gateway (FDA ESG, EMA CESP, EUDAMED, Health Canada CESG, plus a
   roadmap of PMDA, MHRA, NMPA, MFDS, TGA, ANVISA, WHO).

   Three data structures form the platform contract:
     1. SUBMISSION_TYPES   — what kind of dossier (510(k), IND, MAA…)
   ...
   *contents* of each stage (build tree, validator profile, package
   format, gateway protocol, receipt fields, AIC log entries).

   Adding a new submission type or agency in the future is a data
   change here, not a UI change.
   ═════════════════════════════════════════════════════════════════ */

/* ─────────────────────────────────────────────────────────────
   Workstreams — top-level filter pinned at the topbar.
   ───────────────────────────────────────────────────────────── */
window.SUBMISSION_WORKSTREAMS = [
...
/* ─────────────────────────────────────────────────────────────
   Submission types — what kind of regulatory artifact.
   Each carries a "shape" key that the Build tab uses to render
   the right outline (eSTAR sections vs eCTD modules vs EUDAMED
   actor file vs CER section list).
   ───────────────────────────────────────────────────────────── */
window.SUBMISSION_TYPES = [
  /* MDX — shipped */
...
/* ─────────────────────────────────────────────────────────────
   Gateways — agency endpoints. v1 ships full receipt detail for
   four (FDA ESG, EMA CESP, EUDAMED, Health Canada CESG); the
   remainder render as available targets with a "Receipt format
   pending" placeholder so the roadmap is visible.
   ───────────────────────────────────────────────────────────── */
window.SUBMISSION_GATEWAYS = [
  /* Full v1 */
  ...
  { id: 'hc-cesg',     agency: 'Health Canada',region: 'CA',      label: 'Health Canada CESG',v1: true,
    shape: 'cesg', desc: 'Common Electronic Submissions Gateway',
    receiptFields: ['Submission ID', 'Acknowledgement letter'],
    accepts: ['nda','anda','ind','510k','denovo','ctd-cta'] },

  /* v2 — Receipt format pending */
  { id: 'pmda-gw',     agency: 'PMDA',         region: 'JP',      label: 'PMDA Gateway',      v1: false,
    shape: 'pmda',     desc: 'Pharmaceuticals & Medical Devices Agency · Japan',
    accepts: ['jnda','510k','pma','ide'] },
  { id: 'mhra-gw',     agency: 'MHRA',         region: 'UK',      label: 'MHRA Gateway',      v1: false,
    shape: 'mhra',     desc: 'Medicines & Healthcare products Regulatory Agency · UK',
    accepts: ['maa','510k','ind','bla','nda'] },
  { id: 'nmpa-portal', agency: 'NMPA',         region: 'CN',      label: 'NMPA Portal',       v1: false,
    shape: 'nmpa',     desc: 'National Medical Products Administration · China',
    accepts: ['nda','510k','pma','ind','bla'] },
  { id: 'mfds-portal', agency: 'MFDS',         region: 'KR',      label: 'MFDS Portal',       v1: false,
    shape: 'mfds',     desc: 'Ministry of Food & Drug Safety · Korea',
    accepts: ['nda','510k','pma'] },
  { id: 'tga-trams',   agency: 'TGA',          region: 'AU',      label: 'TGA TRAMS',         v1: false,
    shape: 'tga',      desc: 'Therapeutic Goods Administration · Australia',
    accepts: ['nda','510k','pma','ind'] },
  { id: 'anvisa-peticionamento', agency: 'ANVISA', region: 'BR',  label: 'ANVISA Peticionamento', v1: false,
    shape: 'anvisa',   desc: 'Agência Nacional de Vigilância Sanitária · Brazil',
    accepts: ['nda','510k','pma'] },
  { id: 'who-prequal', agency: 'WHO',          region: 'GLOBAL',  label: 'WHO Prequalification', v1: false,
    shape: 'who',      desc: 'WHO Prequalification programme',
    accepts: ['bla','nda'] },
];
...
/* ─────────────────────────────────────────────────────────────
   The submissions in flight. MDX-rich + a few greyed Biotech /
   Pharma rows visible to telegraph the platform story.
   ───────────────────────────────────────────────────────────── */
window.SUBMISSIONS_V2 = [
  /* ───── MDX · 510(k) ───── */
  {
    id:        'sub-or902-510k',
    workstream:'mdx',
    ...
    gateway:   'fda-esg',
    prog:      'OR-902',
    progTitle: 'OR-902 Spinal Implant',
    title:     'OR-902 Spinal Implant — final eSTAR',
    stage:     'transmit',
    status:    'active',
    targetAt:  '4 days · 2026-04-29',
    sentAt:    null,
    files:     48,
    bytes:     '142 MB',
  ...
  /* ───── MDX · PMA ───── */
  {
    ...
    gateway:   'fda-esg',
    prog:      'CV-330',
    progTitle: 'CV-330 Implantable Cardiac Monitor',
    title:     'CV-330 PMA · Module 5 clinical (CSR + ISS + ISE)',
    stage:     'sign',
    status:    'active',
    targetAt:  '67 days · 2026-06-30',
    sentAt:    null,
    files:     312,
    bytes:     '4.8 GB',
  ...
  },
  {
    ...
    gateway:   'fda-esg',
    prog:      'CV-410',
    progTitle: 'CV-410 Catheter — Lead extension',
    title:     'CV-410 PMA-S · 180-day labeling change',
    stage:     'package',
    status:    'active',
    targetAt:  '95 days · 2026-07-28',
    sentAt:    null,
    files:     38,
    bytes:     '218 MB',
  ...
  /* ───── MDX · CER (EUDAMED + Notified Body) ───── */
  {
    ...
    gateway:   'eu-eudamed',
    prog:      'IV-415',
    progTitle: 'IV-415 Companion Diagnostic',
  ...
  },
  {
    ...
    gateway:   'eu-eudamed',
    prog:      'IV-208',
    progTitle: 'IV-208 Tumor Marker Assay',
  ...
  {
    id:        'sub-cv117-510k-done',
    workstream:'mdx',
    type:      '510k',
    gateway:   'fda-esg',
    prog:      'CV-117',
    progTitle: 'CV-117 ECG Patch',
    title:     'CV-117 ECG Patch · K254481 (cleared)',
    stage:     'aic',
    status:    'complete',
    targetAt:  '—',
    sentAt:    'Feb 03 · ACK3 Feb 03',
    files:     37,
    bytes:     '76 MB',
    gate:      { errs: 0, warns: 0, ok: 20 },
    cover:     'signed',
  ...
  /* ───── BIOTECH (preview) ───── */
  {
    ...
    gateway:   'fda-esg',
    prog:      'MDB-521',
    progTitle: 'MDB-521 — anti-CD19 CAR-T',
  ...
  },
  {
    ...
    gateway:   'fda-esg',
    prog:      'MDB-340',
    progTitle: 'MDB-340 — recombinant Factor IX',
  ...
  /* ───── PHARMA (preview) ───── */
  {
    ...
    gateway:   'ema-cesp',
    prog:      'PX-118',
    progTitle: 'PX-118 — JAK1 selective',
...
/* ─────────────────────────────────────────────────────────────
   Per-submission detail. Build outline (type-shaped), validation
   summary by section, signers, packaging, transmit + receipt
   (gateway-shaped), and AIC log entries with hash chain.
   Keyed by submission id.
   ───────────────────────────────────────────────────────────── */
window.SUBMISSIONS_DETAIL = {
  ...
  'sub-or902-510k': {
    ...
    /* Build — eSTAR section list. Counts roll up from the editor + vault. */
    build: {
      shape: 'estar',
    ...
    /* Validation — agency-shaped rule profile. */
    validation: {
    ...
    /* Sign — Part 11 e-signatures (renders inline; standalone Approvals
       surface follows the same shape). */
    signatures: {
    ...
    /* Package — what gets generated for the gateway. */
    package: {
      shape:       'eSTAR ZIP',
    ...
    /* Transmit — gateway shape (FDA ESG = AS2/WebTrader, returns 3 ACKs). */
    transmit: {
    ...
    /* Receipt — gateway-shaped acknowledgements. ESG = Core ID + ACK1/2/3. */
    receipt: {
      shape:       'esg',
      coreId:      null,
      ack1:        null,
    ...
    /* AIC log — hash-chained immutable record. Same schema for every
       submission regardless of agency or workstream. */
    aic: [],

    /* Activity — surface-level event log (separate from AIC). */
    activity: [
  ...
  'sub-cv117-510k-done': {
    ...
    transmit: {
      ...
      readyToSend:  false, /* already sent */
    ...
    /* AIC chain — hash-linked immutable entries. Real chain runs
       sha-256(prev_hash || event_json). Truncated to 6 chars for
       inline display; full hash visible in audit drawer. */
    aic: [
  ...
  /* ─────────────────────────────────────────────────────────
     MDB-521 · IND · biotech preview (eCTD-shape, CESP-style)
     ───────────────────────────────────────────────────────── */
  'sub-mdb521-ind': {
...
Object.assign(window, {
  SUBMISSION_WORKSTREAMS:  window.SUBMISSION_WORKSTREAMS,
  SUBMISSION_TYPES:        window.SUBMISSION_TYPES,
  SUBMISSION_GATEWAYS:     window.SUBMISSION_GATEWAYS,
  SUBMISSION_PIPELINE_V2:  window.SUBMISSION_PIPELINE_V2,
  SUBMISSIONS_V2:          window.SUBMISSIONS_V2,
  SUBMISSIONS_DETAIL:      window.SUBMISSIONS_DETAIL,
});
