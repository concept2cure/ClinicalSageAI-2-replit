/**
 * Q-Sub briefing-document workspace — Phase 7 · doc-first + correspondence rail.
 *
...
const { QSUB_KPIS, QSUB_TYPES, QSUB_SECTIONS, QSUB_OPEN, QSUB_CORRESPONDENCE, QSUB_DOCUMENTS, QSUB_DOC_FRAMEWORKS } = window;

function QSubSurface({ program, onAskAna, onOpenEditor }) {
  ...
  const [selected, setSelected] = React.useState((programQsubs[0] || QSUB_OPEN[0]).id);
  ...
  return (
    <>
      ...
      <div className="qsub-grid">
        ...
        {/* Selected Q-Sub: briefing-document sections + correspondence */}
          ...
          <section className="section">
            ...
            <div className="qsub-sections">
              {QSUB_SECTIONS.map(s => (
                ...
                >
                  <div className="qsub-section-num mono tiny">§{QSUB_SECTIONS.indexOf(s) + 1}</div>
                  <div className="qsub-section-body">
                    <div className="qsub-section-label">{s.label}{s.required && <span style={{ color: 'var(--error)' }}> *</span>}</div>
...
window.QSubSurface = QSubSurface;
