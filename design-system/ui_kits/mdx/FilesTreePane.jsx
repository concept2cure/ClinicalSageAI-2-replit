...
function buildTree(pathway) {
  ...
  const apprChildren = (data.approvals || []).map((a) => {
    const fname = `${a.id} — ${slug(a.label)}.json`;
    return {
      name: fname,
      kind: 'file',
      fileKind: 'approval',
...
/* ─── Previews ───────────────────────────────────────────── */
function PreviewBody({ node, pathway, onOpenSection }) {
...
function PreviewApproval({ node }) {
  ...
  const obj = {
    ...
    requestor: a.requestor, signer: a.signer,
...
function PreviewAudit({ node }) {
  ...
  const lines = events.slice(0, 200).map((e) => JSON.stringify({
    ts: e.when, kind: e.kind, who: e.who, summary: e.summary,
    section: e.section?.id, hash: e.hash?.slice(0, 12),
  }));
  return (
