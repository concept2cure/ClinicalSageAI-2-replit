...
| 4 | ✅ **DONE — estimand framework threaded into the SAP.** The `EstimandEngineService` already captured + validated the five ICH E9(R1) attributes and the five intercurrent-event strategies (define/validate/recommendMethods/multiplicity/regulatory-examples, wired at `/api/biostat/estimand/*`), and the defensibility layer already scores `estimandClarity`. The unmet half of the GA bar — "SAP renders them" — is now closed: a pure, deterministic renderer (`server/services/estimand-sap-section.ts`) lays out each estimand's five attributes + intercurrent-event strategies as a first-class SAP section (Section 2, immediately after Objectives per ICH E9(R1)), with the analysis-method and missing-data sections cross-referencing it. No estimand supplied ⇒ an explicit deferral that names the five attributes the statistician must specify, never a fabricated estimand. `SapGeneratorService.generateSAP` accepts `estimands` and returns per-estimand completeness metadata. 15 tests (reproducibility + attribute correctness + honesty/deferral + SAP-splice/renumber) in `tests/services/estimand-sap-section.test.ts`. | M | — | done |
...
| 6 | ✅ **DONE — enrollment + dropout forecast models.** Pure `server/services/stats/enrollment-forecast.ts`: Poisson–Gamma (Anisimov–Fedorov) accrual — per-site Poisson recruitment with Gamma-distributed rates (negative-binomial over-dispersion) and staggered site activation. Exact closed forms for expected enrollment-by-time and time-to-target; seeded Monte Carlo predictive distributions for completion time and count-by-time with 80% prediction intervals (calibrated by construction). Exponential dropout/retention with an interval. `accrualHoldoutError` reports MAE/RMSE of the expected-accrual curve vs an observed series. Reachable at `POST /api/biostat/enrollment/forecast`. 15 tests: closed forms (incl. staggered activation), MC mean = N/rate, **80% interval coverage ≈ nominal across fresh draws**, over-dispersion widens intervals, dropout = e^{−ρt}, reproducibility, holdout error. Evaluation against the real CT.gov corpus (calibrated holdout error at scale) remains gated on #1. | M | #1 | done |
| 7 | ✅ **DONE — region-specific design rules engine.** Pure `server/services/region-design-rules.ts`: a codified, cited ruleset across PMDA / MHRA / NMPA / Swissmedic / ANVISA / FDA / EMA — Japanese bridging + ethnic sensitivity (ICH E5), MRCT Japanese participation + regional consistency (ICH E17), Chinese local-data requirement (NMPA overseas-data guidance), QT in the regional population (ICH E14), post-Brexit UK separate jurisdiction, Switzerland outside EU/EEA, ANVISA local representation (RDC 9/2015), Project Orbis eligibility for oncology with ≥2 partner agencies, and the FDA diversity action plan for phase 3. `evaluateRegionRules(design, agencies)` emits ValidationResult-style findings (error/warning/info + status + recommendation + retrievable guidance citation); `listRegionRules` / `getRegionGuidance` make the guidance retrievable. Complements `cross-jurisdictional-intelligence.ts` (divergence mapping) by checking one concrete design. Reachable at `POST /api/biostat/region-rules/evaluate` and `GET /api/biostat/region-rules/catalog`. 17 tests (per-agency rule firing, severity mapping, MRCT/Orbis/diversity, honesty/not-applicable, determinism, guidance retrieval). Full-text guidance corpus ingestion remains the separate data-ops dependency (#1). | M | — | done |
| 8 | ✅ **DONE — diagnostic / device performance pack (MDX), proportion + MRMC paths.** Proportion paths: `stats/special.ts` (regularized incomplete beta via Lentz continued fraction → exact binomial tails + Clopper–Pearson) and `stats/diagnostic-design.ts` — single-proportion sizing against a performance goal by **both** normal-approximation **and** exact binomial search, co-primary sensitivity-AND-specificity sizing through prevalence to total enrollment with joint power, exact performance intervals (`POST /api/biostat/diagnostic/sizing`; 16 tests: Γ, I_x(2,3)=0.6875, binomial tails, CP upper for 0/10, normal sizing n=108, exact α-control). **MRMC reader studies:** `stats/f-distribution.ts` (central + noncentral F + quantile + F-test power, built on the incomplete beta) and `stats/mrmc.ts` — Obuchowski–Rockette two-modality figure-of-merit power and reader-count solver, with the noncentrality derived from the OR variance of the reader-averaged modality difference `Var = (2/J)[σ²_TR + σ²_ε − Cov1 + (J−1)(Cov2 − Cov3)]` (`POST /api/biostat/diagnostic/mrmc`). 19 tests: F quantiles vs tabulated criticals (F.95(1,10)=4.9646, etc.), F_{1,∞}=z², noncentral-F = normal power in the large-df limit, OR variance vs hand calc, Δ=0 ⇒ power=α, monotonicities, reader solver. F primitives are reference-validated; the MRMC noncentrality is derived from first principles and checked against the normal limit + statistical properties (variance components come from a pilot OR analysis). **Bayesian device-pivotal templates:** `stats/bayesian-device.ts` — exact Beta–Binomial single-arm pivotal design (FDA 2010 Bayesian device guidance): posterior exceedance `P(θ>goal|data)≥threshold` rule, posterior mean + credible interval, optional informative power prior borrowing historical successes/failures, exact operating characteristics (type I at the goal, power at the expected rate via the binomial tail), sample-size search, and interim **predictive probability of success** via the Beta–Binomial predictive (`POST /api/biostat/diagnostic/bayesian-device`; 14 tests: exact tails vs Beta(1,n+1), monotone exceedance, OC = binomial tail, type-I control, PPoS endpoints). | M | #3 | done |
...
| 10 | ✅ **DONE — external-control rigor pack.** Pure `server/services/stats/external-control.ts`: covariate-balance diagnostics (Austin standardized mean differences + variance ratios, optional IPTW weights + effective sample size, balance verdict at the 0.1 bar); principled borrowing — normal **power prior** (discount historical precision by a0 ∈ [0,1]) and normal **commensurate prior** (borrow through a τ² link so prior-data conflict auto-attenuates); treatment-effect estimation against the borrowed control; and two **tipping-point** sensitivities (hypothetical historical bias δ, and borrowing strength a0) that report the value at which the conclusion flips. Deterministic regulatory-framing **memo** renderer (sentence case, numbers over adjectives, sections omitted when inputs absent — no fabrication). Reachable at `POST /api/biostat/external-control/sensitivity`. 16 tests: SMD definition + binary/weighted cases, borrowing analytic limits (a0=0 ⇒ concurrent-only, a0=1 ⇒ pooled midpoint; τ²→∞ ⇒ no borrowing), reproducible tipping points with stable provenance, robust-vs-flip detection, honest memo. | M | #3 | done |
| 11 | ✅ **DONE (extension) — event-time projection for event-driven trials.** Pure `server/services/stats/event-projection.ts`: forecasts the calendar time at which a survival trial reaches its target number of events — the timing that schedules every interim/DMC and final analysis (complements the fixed-design log-rank sizing already in `power-sample-size-service`, which gives the event count but not the calendar timing). Uniform accrual + exponential time-to-event (control median + hazard ratio) + exponential dropout ⇒ **exact** expected cumulative-events curve `E[D(t)]`, its inversion to the expected time-to-D, and a seeded Monte Carlo 80% prediction interval. Reachable at `POST /api/biostat/adaptive/event-projection`. 11 tests: closed form vs an independent numerical integral, analytic special case `r·[t−(1−e^{−λt})/λ]`, exact inversion, MC-vs-closed-form agreement, dropout/HR/time monotonicities, reproducibility. | M | #3, #6 | done |
| 12 | ✅ **DONE (extension) — win ratio + Finkelstein–Schoenfeld for hierarchical composite endpoints.** Pure `server/services/stats/win-ratio.ts`: pairwise treatment-vs-control comparison down a prioritized outcome hierarchy (e.g. death → HF hospitalization → biomarker) with the Pocock right-censoring win rule for time-to-event levels and direct comparison for continuous levels; an indeterminate (censored) pair is a tie, never a guess. Reports win ratio + delta-method CI, win odds, Buyse net benefit, and the Finkelstein–Schoenfeld test via the two-sample U-statistic variance (DeLong-style projection) with a two-sided p-value. Reachable at `POST /api/biostat/win-ratio`. 10 tests: fully hand-computed wins/losses + FS z=−0.70711/p=0.47950, TTE censoring tie rule, hierarchy fall-through, arm-swap symmetry (WR inverts, NB flips, p preserved), CI narrows + result becomes significant with more data, reproducibility. | M | — | done |
| 13 | ✅ **DONE (extension) — BOIN phase 1 dose finding.** Pure `server/services/stats/dose-finding-boin.ts`: the **correct** Bayesian Optimal Interval design (Liu & Yuan 2015) — optimal escalation/de-escalation boundaries `λ_e = ln((1−φ1)/(1−φ))/ln(φ(1−φ1)/(φ1(1−φ)))`, `λ_d` symmetric — replacing the simplified fixed 0.6×/1.4×-target heuristic in `foresight-ai-engine`. Decision rule (escalate/stay/de-escalate), Beta–Binomial safety **elimination**, the published decision-boundary table, isotonic regression (PAVA), and isotonic **MTD selection**. Reachable at `POST /api/biostat/dose-finding/boin`. 13 tests: boundaries reproduce the published values exactly (φ=0.30 ⇒ 0.236491/0.358519; φ=0.25 ⇒ 0.196801/0.298392), decision + elimination cases, the φ=0.30 decision table (n=3,6,9,12), PAVA pooling, and MTD selection with isotonic smoothing + eliminated/untried skipping. | M | — | done |
| 14 | ✅ **DONE (extension) — restricted mean survival time (RMST).** Pure `server/services/stats/rmst.ts`: Kaplan–Meier estimator, RMST(τ) as the area under the KM curve to horizon τ, the Greenwood-type RMST variance `Σ A_i²·d_i/(n_i(n_i−d_i))` with `A_i=∫_{t_i}^{τ}Ŝ`, and the two-arm RMST **difference** with a z test + CI — the model-free effect measure preferred when proportional hazards fails (Uno 2014; Royston–Parmar 2013). Reachable at `POST /api/biostat/survival/rmst`. 8 tests: hand-computed KM curve with censoring, the exact `RMST = mean(min(T,τ))` identity for uncensored data, the **hand-computed Greenwood variance = 0.3125**, τ-beyond-data flag, no-event ⇒ RMST=τ/var=0, difference antisymmetry + reproducibility, p-value shrinks with more data. | M | — | done |
