...
| 8 | 🟡 **PARTIAL — diagnostic performance pack (MDX), proportion paths shipped; MRMC pending.** `stats/special.ts` (regularized incomplete beta via Lentz continued fraction → exact binomial tails + Clopper–Pearson) and `stats/diagnostic-design.ts`: single-proportion sizing against a performance goal by **both** normal-approximation **and** exact binomial search (the exact path governs the small disease-positive counts of rare-marker IVDs), co-primary sensitivity-AND-specificity sizing translated through prevalence into total enrollment with joint power, and exact performance intervals — the IVDR clinical-performance path. Reachable at `POST /api/biostat/diagnostic/sizing`. 16 tests: special functions vs analytic values (Γ, I_x(2,3)=0.6875, binomial tails, CP upper for 0/10), normal sizing matches the closed-form (n=108), exact test controls α and hits power. **Remaining (follow-up):** MRMC reader-study sizing (Obuchowski–Rockette / Hillis noncentral-F) and Bayesian device-pivotal templates — deferred rather than ship unvalidated reader-study math. | M | #3 | partial |
