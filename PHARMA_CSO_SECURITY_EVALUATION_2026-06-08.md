...
### 3.3 ✅ HIGH — Hard-delete of regulated records (Part 11 §11.10(e)) — **REMEDIATED on `concept2cure-v2`**
...
**Fix (landed):** every regulated-table delete now runs **inside a single transaction** that also writes the deletion to the hash-chained, append-only `audit_events` table — atomic and **fail-closed**, so an audit-write failure rolls the delete back and a regulated record is never removed unaudited (§11.10(e): attributed, reason-stamped, time-stamped). The `check-regulated-delete-audit` CI gate now runs with an **empty allowlist** and is a pure positive-coverage guard — any new unaudited regulated delete is a hard failure.

> Note: this hardening converged independently on `concept2cure-v2`. This branch originally carried a parallel implementation (an audit-before-delete `logRegulatedDeletion` helper writing a pre-image snapshot to `audit_logs`); on merge it was dropped in favour of the canonical `concept2cure-v2` version to avoid divergence. Either satisfies §11.10(e); the pre-image-snapshot variant additionally addressed §11.10(c) retrievability and remains an available enhancement.
...
| ~~P1~~ ✅ | Audit-in-transaction (fail-closed) for `ind_applications`/`coauthor_documents`/`authoring_documents` (§3.3) — **present on `concept2cure-v2`**; soft-delete read-filtering is the deferred follow-up | Eng | done | `check-regulated-delete-audit` allowlist = 0 ✅ |
