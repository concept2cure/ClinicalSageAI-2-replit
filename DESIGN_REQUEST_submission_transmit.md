...
2. **Pre-flight / validation** — `POST /api/submission-ops/packages/:packageId/preflight` (tenant-scoped, read-only). Runs the validator layer over the assembled bundle and returns `{ packageId, bundle: { sha256, sizeBytes, format }, validators: [{ id, label, configured, ran, errorCount, warningCount, error? }], findings: [...], blocking, errorCount, warningCount }`. `blocking` is true iff any error-severity finding exists — use it to gate the UI's pre-flight step (no transmit with open errors). 409 `{ gate:'not_assembled' }` if the package has no `metadata.bundle` (assemble first). The existing `Validation` surface continues to show transmittal-scoped findings post-transmit. **Agency validators are configuration-gated and currently UNCONFIGURED** (`fda_evalidator`, `ema_validator`, `pmda_precheck` each report `configured:false, ran:false` unless their `*_VALIDATOR_URL` env is set), so pre-flight today reflects **internal structural validation only** — the agency validators contribute nothing and never a false pass until vendor tooling is wired.
...
- **An extensible validator layer + pre-flight endpoint exist** (`POST .../packages/:packageId/preflight`, see §2). It aggregates the stored internal structural findings with any **configured** external agency validators and returns per-validator status plus a `blocking` flag. **The agency validators (`fda_evalidator`, `ema_validator`, `pmda_precheck`) are configuration-gated and currently UNCONFIGURED** — they only run when `FDA_VALIDATOR_URL` / `EMA_VALIDATOR_URL` / `PMDA_VALIDATOR_URL` (optional `*_VALIDATOR_TOKEN`) are set, and when unset report `configured:false, ran:false` and contribute no findings (never a false green). The HTTP adapter used when configured is a **GENERIC documented contract (POST octet-stream → `{ findings: [...] }`), NOT a real vendor protocol** — wiring real FDA eValidator/Lorenz/EMA/PMDA validation requires the vendor's tooling/credentials and conforming the adapter to their actual contract. Until then, pre-flight reflects internal structural validation only.
