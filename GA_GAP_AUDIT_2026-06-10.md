...
3. **Veeva Vault integration — protocol layer CLOSED (2026-06-10); workflow layer open.** A credentials-gated Vault REST client now exists (`server/integrations/veeva-vault/client.ts`: session auth, document upload/fetch/download, VQL query; fail-closed, tested offline). Remaining: the coexistence workflow on top — document-type field mapping, bi-directional sync rules, org-level credential storage. *Owner: engineering, on these primitives.*
