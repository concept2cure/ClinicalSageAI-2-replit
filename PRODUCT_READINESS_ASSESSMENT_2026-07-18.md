...
6. **P0 — AI prompt library unreadable in the deployed bundle.** A follow-on to the boot fix: the six prompt-loading sites resolved `server/services/ai-gateway/prompts` from their own module location, which works unbundled but points at a non-existent path once esbuild collapses everything into `dist/index.js` — so AI generation, consistency checks, section authoring, shadow review, ingestion classification, and FCoI review would each `ENOENT` on first prompt load in production, invisibly to the build and unit suites. Replaced with a single working-directory-anchored `PROMPTS_DIR` (`server/services/ai-gateway/prompts-dir.ts`) that is correct for source, bundle, and test runner; added `tests/ci/prompts-dir.test.ts` (`94f8eeb`). **Verified:** the resolved path reads a real prompt; the former bundle paths confirmed non-existent; dev boot clean.
