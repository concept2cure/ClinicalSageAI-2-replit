...
- **The dock reaches the owned panes.** Document authoring and the eCTD co-author, which run their own AnA conversation without the shell rail, mount the same dock with an "At work" toggle; one per-browser show/hide choice is shared by every host. Accessibility pass: section titles are headings wrapping their disclosure buttons, collapsed bodies stay in the DOM so `aria-controls` always resolves, the inputs disclosure is a named region, the host toggle is the dock's one control (no second close inside it, so focus never falls to the page body), and the live region speaks the same placeholder the list shows.
- **Review pass.** A turn that timed out or lost its connection reads "Did not finish · 1m 30s", never "Finished"; steps still in flight when a turn is stopped, times out or fails are closed as not finished with the reason, instead of pulsing "running" forever; the Outputs section opens itself once when the first output lands and can be collapsed again; a steer is cleared from the waiting list by position (the server trims and caps its echo, so a long steer never matched by text); a settled phase keeps the live clock's format above ten seconds; the dock memoises what it derives from the turns; one `useNow`, `byRound` and status glyph serve the dock and the transcript record; the transcript record keeps its collapsed body mounted so its disclosure's `aria-controls` resolves; the conversation composer's focus border meets the 3:1 non-text floor.
- **Design and copy pass.** Tools starts closed (its rows are the Work queue's rows in forensic form) so the dock never outgrows the rail; the conversation surface leaves drafts to its artifact cards instead of listing them twice; round two reads "Went back · round 2" as in the transcript; the Context row says the lens as a phrase ("a risk question") through the one table the transcript uses, never the classifier code; the pane toggles say "AnA at work" like their accessible names; client-side phase labels carry the same trailing ellipsis as the server's.
- **The background queue fails closed.** `GET /api/ana-ri/agent-activity` now returns a 500 when the investigations read throws, so the dock shows a failed read with a retry; the greeting path keeps its fail-soft reader. The RBM co-monitor pane mounts the dock as well.
