...
- AnA: began decomposing the AnA tool mega-files — `agentic-workflow-tools.ts` (agentic workflow tools), `bla-biologics-tool-defs.ts` (BLA 351(a) biologics + CTD nonclinical/clinical definitions), `document-surface-tool-defs.ts` (document view/operations), `discovery-cheminformatics-tool-defs.ts` (discovery-stage compound search), `notifications-study-memory-tool-defs.ts` (notification, clinical-study and working-memory tools), `evidence-literature-tool-defs.ts` (evidence/literature search and citation), `submission-center-tool-defs.ts`, and `legacy-import-tool-defs.ts`, all extracted verbatim with tool wiring unchanged. `AnaToolDefinitions.ts` is down from ~9.6k to ~3.7k lines.
