// Split from skill-validation.test.ts (Tier 1 static validation) to keep both
// files reviewable: that file validates the generated-doc pipeline (commands,
// registry, usage strings, freshness, preambles, cross-skill paths); this one
// validates the structure and content of the individual skills (office-hours,
// investigate, review checklist, planted-bug fixtures, CEO review, gstack-slug,
// test bootstrap, regression/coverage/retro sections, Codex, trigger phrases,
// repo mode, ship triage). Both run under `bun test test/` (see package.json).
