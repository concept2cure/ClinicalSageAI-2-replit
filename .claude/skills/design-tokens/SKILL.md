...
>
> This project already has a complete, locked token system. **Do NOT generate a new one.** Do NOT introduce new CSS custom properties, a new Tailwind theme extension, or a new palette.
>
> The authoritative sources in this repo are:
>
> - `tailwind.config.ts` / `tailwind.config.js` — the canonical Tailwind theme
> - `client/src/index.css` and any `globals.css` / `:root` declarations — canonical CSS variables
> - `.claude/skills/concept2cure-v2-design-system.md` — the governed stone palette, type ramp, and spacing
> - `.claude/skills/claude-ui-design-principles.md` — the 12 Anthropic-style principles that gate every visual decision
>
> **Allowed use of this skill:**
>
> 1. **Audit only.** Use the "Check what already exists" checklist (step 1 below) to survey token coverage. Report gaps to the user.
> 2. **Surgical extension, with explicit approval.** If a gap is found (e.g., missing a specific motion token), propose the extension and wait for user approval before editing token files. Extend existing tokens — never replace.
> 3. **No aesthetic selection.** The upstream step that asks the user to pick a named aesthetic (Rams, Swiss, Brutalist, etc.) is disabled. The philosophy is fixed: Anthropic Claude — calm, intelligent, restrained.
>
> Replace-or-Delete Law applies: if this skill identifies a competing / orphaned token file, flag it for consolidation per `CLAUDE.md` → "UI Convergence and Legacy Surface Deletion".
