...
> **C2C OVERRIDE (NON-NEGOTIABLE — read before anything else in this file).**
>
> This project already has a locked-in aesthetic philosophy and component system. The upstream skill's menu of 8 aesthetic philosophies (Rams, Swiss, Japanese Minimalism, Brutalist, Scandinavian, Art Deco, Neo-Memphis, Editorial) is **not applicable here**.
>
> Before using this skill, you MUST:
>
> 1. **Use the Anthropic Claude design philosophy only** — calm, intelligent, restrained. See `.claude/skills/claude-ui-design-principles.md` for the 12 principles. Do NOT pick a different named aesthetic.
> 2. **Use only components from the governed registry** — `client/src/component-registry.ts` (28 mapped components). See `.claude/skills/figma-component-contract.md` and `.claude/skills/concept2cure-v2-component-registry.md`. No raw `<button>` / `<input>`; no custom status pills; no local empty states.
> 3. **Use the existing stone palette and Tailwind config** — do NOT generate a new token system. See `.claude/skills/concept2cure-v2-design-system.md` and the `design-tokens` override block.
> 4. **Defer to chat-first design and UI state standards** — see `.claude/skills/chat-first-design.md` and `.claude/skills/ui-standards.md`. No new dashboards, no new empty-state components, no raw loading spinners.
> 5. **Convergence rules apply** — before adding a new shell-level surface, read the "UI Convergence and Legacy Surface Deletion" section of `CLAUDE.md`. Replace-or-Delete is mandatory.
>
> The sections below are retained for their codebase-scanning checklists and implementation hygiene (mobile-first, accessibility, component-first composition). Ignore anything about choosing or naming an aesthetic.
