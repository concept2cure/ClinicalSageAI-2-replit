...
export interface UiSurface {
  ...
  /** Left-rail tier. */
  navTier: NavTier;
  /** Real app-shell key (zen-app-constants LayoutMode). The renderer hangs here. */
  layoutMode: string;
...
export const UI_SURFACES: UiSurface[] = [
  ...
  },
  {
    ...
    group: 'collaboration',
    uiKit: 'tasking',
    apiPrefixes: ['/api/tasks', '/api/regulatory/tasks', '/api/collaboration'],
    anaToolFamilies: [],
    sharedContract: '@shared/types/communication-center',
    discoveryCatalog: null,
    readiness: 'routes-ready',
    compliance: [A11Y, TONE, 'motion-discipline'],
    notes: 'Channels, messages, activity feed, presence, mentions, task board, due dates. See ui_kits/tasking + ui_kits/task-tray.',
  },
  {
    id: 'dossier-map',
    label: 'Dossier map',
