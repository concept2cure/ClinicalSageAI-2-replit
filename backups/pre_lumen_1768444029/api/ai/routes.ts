...
import { extractCommitmentsRulesV1 } from './commitmentsExtractor.js';

const router = Router();

router.post('/commitments/extract', async (req: Request, res: Response) => {
  try {
    const documentText = (req.body?.documentText ?? req.body?.text ?? '').toString();
    const submissionType = req.body?.submissionType?.toString();
    const documentType = req.body?.documentType?.toString();

    if (!documentText || documentText.trim().length < 20) {
      return res.status(400).json({
        success: false,
        error: 'documentText is required (min 20 chars)',
      });
    }

    const data = extractCommitmentsRulesV1({
      documentText,
      submissionType,
      documentType,
    });
    ...
    console.error('Commitments extraction error:', error);
...
- Use bullet points and emojis for better readability`;
    ...
    // The client currently sends `model` as a provider selector ('gemini'|'openai').
    // Fall back to whichever provider is configured.
    const requestedProvider = String(model || '').toLowerCase();
    const providerToUse =
      (requestedProvider === 'gemini' && geminiApiKey && 'gemini') ||
      (requestedProvider === 'openai' && openaiApiKey && 'openai') ||
      (geminiApiKey && 'gemini') ||
      (openaiApiKey && 'openai') ||
      'none';
    ...
    let providerUsed: 'gemini' | 'openai' | 'fallback' = 'fallback';
    ...
    if (providerToUse === 'gemini') {
      ...
      throw new Error('No AI provider configured (set GEMINI_API_KEY/GOOGLE_API_KEY or OPENAI_API_KEY)');
    }
    ...
    const fallbackResponse = `I'm experiencing technical difficulties connecting to the AI service (or it is not configured). However, I can still help with your regulatory question.
