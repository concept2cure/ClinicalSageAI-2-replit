...
const extractOrgContext = (req, res, next) => {
  ...
  // In dev/demo, allow a safe fallback org to avoid SPA boot failures.
  if (!orgIdHeader && !isProduction) {
    ...
    if (!isNaN(parsedFallback) && parsedFallback > 0) {
      req.organizationId = parsedFallback;
...
// Apply organization context middleware only to Phase 3 AI endpoints.
// This router is mounted at /api; keeping this unscoped would intercept unrelated
// endpoints like /api/projects and break browser loads.
...
router.get('/ai/status', (req, res) => {
  ...
  const health = globalThis.__aiProviderHealth || {};
  const geminiHealth = health?.gemini || { ok: null, lastErrorAt: null, lastError: null };
  const openaiHealth = health?.openai || { ok: null, lastErrorAt: null, lastError: null };
  ...
  const geminiEnabled = Boolean(geminiApiKey) && !disableGemini;
  const openaiEnabled = Boolean(openaiApiKey);

  // If Gemini is configured but recently failing at runtime, treat it as effectively disabled.
  const GEMINI_FAIL_SOFT_WINDOW_MS = 10 * 60 * 1000; // 10 minutes
  const geminiRecentlyFailed =
    geminiHealth?.ok === false &&
    typeof geminiHealth?.lastErrorAt === 'number' &&
    Date.now() - geminiHealth.lastErrorAt < GEMINI_FAIL_SOFT_WINDOW_MS;
  const effectiveGeminiEnabled = geminiEnabled && !geminiRecentlyFailed;

  const OPENAI_FAIL_SOFT_WINDOW_MS = 10 * 60 * 1000; // 10 minutes
  const openaiRecentlyFailed =
    openaiHealth?.ok === false &&
    typeof openaiHealth?.lastErrorAt === 'number' &&
    Date.now() - openaiHealth.lastErrorAt < OPENAI_FAIL_SOFT_WINDOW_MS;
  const effectiveOpenaiEnabled = openaiEnabled && !openaiRecentlyFailed;

  // Prefer OpenAI by default only if it's healthy; otherwise use Gemini if available.
  const defaultProvider = effectiveOpenaiEnabled ? 'openai' : effectiveGeminiEnabled ? 'gemini' : 'none';
  const geminiDefaultModel = process.env.GEMINI_MODEL || process.env.GEMINI_DEFAULT_MODEL || 'gemini-3-flash-preview';
  const geminiFallbackModel = 'gemini-2.0-flash-exp';
  const openaiDefaultModel = 'gpt-4o';
  const geminiKeyName = process.env.GEMINI_KEY_NAME || null;
  const geminiProjectId = process.env.GEMINI_PROJECT_ID || null;
  const geminiProjectNumber = process.env.GEMINI_PROJECT_NUMBER || null;
  ...
  res.json({
    ...
    providers: {
      gemini: {
        enabled: effectiveGeminiEnabled,
        ...
        runtime: {
          ok: geminiHealth?.ok,
          lastErrorAt: geminiHealth?.lastErrorAt,
          lastError: geminiHealth?.lastError,
          suppressed: geminiRecentlyFailed,
        },
      },
      openai: {
        enabled: effectiveOpenaiEnabled,
        defaultModel: openaiDefaultModel,
        runtime: {
          ok: openaiHealth?.ok,
          lastErrorAt: openaiHealth?.lastErrorAt,
          lastError: openaiHealth?.lastError,
          suppressed: openaiRecentlyFailed,
        },
      },
    },
    models_available: [
      ...(geminiEnabled ? [geminiDefaultModel, geminiFallbackModel] : []),
      'gpt-4o',
      'gpt-4-turbo',
      'gpt-3.5-turbo',
        'local (Xenova/flan-t5-small)',
    ],
