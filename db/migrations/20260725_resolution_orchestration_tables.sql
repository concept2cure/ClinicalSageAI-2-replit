-- ═══════════════════════════════════════════════════════════════════════════════
-- Migration: Resolution orchestration storage (canonical lineage port)
-- Date: 2026-07-25
-- Conflict: C-10 (docs/architecture/C2C_SCHEMA_AND_ENUM_CONFLICT_LEDGER.md)
--
-- WHY THIS EXISTS
-- resolution_plans, resolution_bundles, resolution_bundle_items and
-- supersession_records were previously defined ONLY in
-- migrations/0010_resolution_orchestration.sql — a file with no execution path
-- (not journaled, not in shared/schema.ts's drizzle-kit push surface, not
-- applied by apply-c2c-migrations.mjs). In deployed environments the tables do
-- not exist, so executeBundle() throws on its FIRST query and the entire
-- correction layer — plans, bundles, supersession lineage, receipts — has never
-- been executable in production.
--
-- This is a VERBATIM port of the dead file's DDL (below the marker) into the
-- manifest-managed lineage, so the Drizzle services in
-- server/services/resolution/* work unchanged. Proven by
-- tests/schema-contract/resolution-receipts.contract.test.ts, which drives the
-- real executor against this DDL.
--
-- ROLLBACK: all four tables are new here and start empty.
--   DROP TABLE IF EXISTS supersession_records, resolution_bundle_items,
--     resolution_bundles, resolution_plans;
--   DROP TYPE IF EXISTS supersession_state, bundle_state, resolution_state,
--     resolution_confidence, resolution_path, resolution_trigger_type;
-- ═══════════════════════════════════════════════════════════════════════════════
-- ── verbatim from migrations/0010_resolution_orchestration.sql ──
