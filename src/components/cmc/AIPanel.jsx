...
const AIPanel = ({ processId, onClose, onRefresh }) => {
  ...
  const fetchSuggestions = async () => {
    setLoading(true);
    try {
      const response = await fetch(`/api/cmc-process/processes/${processId}/ai/suggest`, {
        method: 'POST',
      });
  ...
  const handleAccept = async suggestionId => {
    ...
    try {
      const response = await fetch(`/api/cmc-process/ai/${suggestionId}/accept`, {
        method: 'POST',
      });
      if (response.ok) {
        setSuggestions(
  ...
  const handleReject = async suggestionId => {
    setProcessingId(suggestionId);
    try {
      const response = await fetch(`/api/cmc-process/ai/${suggestionId}/reject`, {
        method: 'POST',
      });
      if (response.ok) {
        setSuggestions(
