...
const MOCK_REFS = [
  { 
    ...
    author: 'Smith, J. et al.', year: '2025', journal: 'N Engl J Med', 
    type: 'Clinical Trial', status: 'OK', 
    usageCount: 3, // <--- NEW: Track usage
    abstract: 'In this phase 3 trial, the novel inhibitor demonstrated a statistically significant improvement...'
  },
  { 
    id: 'PMID:382911', 
    title: 'Safety Profiles of Immunotherapy Combinations', 
    author: 'Doe, A. et al.', year: '2024', journal: 'Lancet Oncol', 
    type: 'Review', status: 'RETRACTED', 
    usageCount: 0,
  ...
  { 
    ...
    author: 'Chen, L. et al.', year: '2025', journal: 'J Clin Oncol', 
    type: 'Epidemiology', status: 'OK', 
    usageCount: 1,
...
export function ReferencePanel() {
  ...
  const handleDragStart = (e, ref) => {
    ...
    const payload = JSON.stringify({
      ...
      value: citationValue, // <--- Injects the formatted text
  ...
  return (
    <div className="flex flex-col h-full bg-white">
      ...
      <div className="p-3 border-b border-gray-200 bg-purple-50 shrink-0 space-y-2">
        <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-1">
               {/* STYLE TOGGLE */}
               <button 
                 onClick={() => setCitationStyle(prev => prev === 'HARVARD' ? 'VANCOUVER' : 'HARVARD')}
        ...
        {results.map(ref => (
          <div 
            ...
            className={`group p-3 border rounded cursor-pointer transition-all relative
                ${selectedRef?.id === ref.id ? 'border-purple-500 shadow-sm ring-1 ring-purple-100' : 'border-gray-200 hover:border-purple-300'}
                ${ref.status === 'RETRACTED' ? 'bg-red-50/50 opacity-75' : 'bg-white'}
            `}
          >
            {/* RETRACTION BANNER */}
            {ref.status === 'RETRACTED' && (
                <div className="absolute top-0 right-0 bg-red-100 text-red-700 text-[9px] font-bold px-1.5 py-0.5 rounded-bl">RETRACTED</div>
            )}
            ...
            <div className="flex items-center gap-2 mb-1 pr-14">
              <span className={`text-[10px] font-bold px-1.5 rounded ${ref.status === 'RETRACTED' ? 'text-red-700 bg-red-100' : 'text-purple-700 bg-purple-50'}`}>
                {ref.type}
              </span>
            </div>
            
            <div className={`text-xs font-semibold leading-tight mb-1 pr-2 ${ref.status === 'RETRACTED' ? 'text-red-900 line-through' : 'text-gray-800'}`}>
                {ref.title}
            </div>
      ...
      <div className="shrink-0 border-t border-gray-200 bg-white">
        {selectedRef ? (
          <div className={`p-3 text-xs ${selectedRef.status === 'RETRACTED' ? 'bg-red-50' : 'bg-gray-50'}`}>
