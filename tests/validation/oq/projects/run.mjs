...
await step(
  ...
  async ({ api, expect, deviation, baseUrl }) => {
    ...
    const headersOf = (token) => ({ Authorization: `Bearer ${token}`, Origin: baseUrl, 'Content-Type': 'application/json' });
    const call = async (token, method, route, body) => {
      const r = await fetch(`${baseUrl}${route}`, { method, headers: headersOf(token), body: body ? JSON.stringify(body) : undefined });
      return { status: r.status, json: await r.json().catch(() => ({})) };
    };
    ...
    const openBefore = (await call(subject.accessToken, 'GET', '/api/c2c/projects')).status;
    expect(openBefore === 200, `the subject's session could not read projects before the suspension (${openBefore})`);

    const status = (to) =>
      call(admin.accessToken, 'PATCH', `/api/admin/master/users/${subjectId}/status`, {
        status: to,
        reason: `OQ-PROJ-18: account standing (${to})`,
      });
    let findings;
    const suspended = await status('suspended');
    try {
      expect(suspended.status === 200, `the platform administrator could not suspend the subject (${suspended.status})`, suspended.json);
      const read = await call(subject.accessToken, 'GET', '/api/c2c/projects');
      ...
      findings = { read, probe, signIn: { status: signIn.status, code: codeOf(signInBody), challenge: Boolean(signInBody.challengeId) } };
    } finally {
      const restored = await status('active');
      expect(restored.status === 200, `the subject could not be restored (${restored.status}); restore it before the next run`, restored.json);
    }

    const { read, probe, signIn } = findings;
    ...
    expect(probe.status === 401 && probe.json?.authenticated === false && codeOf(probe.json) === 'AUTH_ACCOUNT_INACTIVE', `the session check did not report the suspended account's session ended: ${probe.status} ${codeOf(probe.json)}`);
    expect(signIn.status === 403 && signIn.code === 'AUTH_ACCOUNT_INACTIVE' && !signIn.challenge, `a suspended account's password step answered ${signIn.status} ${signIn.code}${signIn.challenge ? ' and issued a challenge' : ''}`);
    ...
    const reopened = (await call(subject.accessToken, 'GET', '/api/c2c/projects')).status;
    expect(reopened === 200, `the restored subject's session read projects ${reopened}`);
    return `subject user:${subjectId} (${subject.method}): projects ${openBefore}; suspended by the platform administrator; its session read projects ${read.status} ${codeOf(read.json)}, session check ${probe.status} ${codeOf(probe.json)}, password step ${signIn.status} ${signIn.code}, no challenge; newest new ledger entry "${added[0].event}", chained; restored, the same session read projects ${reopened}`;
  },
);
