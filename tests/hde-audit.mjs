    ...
    ({ token, projectId }) => {
      ...
      localStorage.setItem(
        ...
        JSON.stringify([
          {
            id: projectId,
            deviceName: 'HDE Test Device',
            deviceType: '510k',
            manufacturer: 'TestCo',
            deviceClass: 'III',
            intendedUse: 'HDE audit',
            status: 'draft',
            createdAt: new Date().toISOString(),
            attachedDocuments: [],
            state: { documentType: 'hde' },
          },
        ])
      );
...
const FORBIDDEN = [
  ...
  { pat: /\bDe\s+Novo\b/g, label: 'De Novo' },
];
...
async function run() {
  ...
  await seedAuth(page);
  await page.goto(BASE + '/concept2cure/project/' + PROJECT_ID + '/510k?mode=hde', {
  ...
  console.log('=== HDE DEEP TAB AUDIT (' + tabCount + ' tabs) ===\n');
  ...
  // HDE expected content check
  const bodyFull = await page.evaluate(() => document.body.innerText);
  const expected = ['HDE', 'Humanitarian', 'Probable Benefit', 'HDE Workspace'];
  console.log('\n--- HDE CONTENT CHECK ---');
