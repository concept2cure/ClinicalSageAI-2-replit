...
class LoadingStateManager {
  ...
  setLoading(key, isLoading) {
    ...
    window.dispatchEvent(new CustomEvent('aiLoadingStateChange', {
...
export const generateContent = async (section, context = {}) => {
  ...
  try {
    const response = await apiCall(SERVICE_CONFIG.endpoints.assist, {
      content: context.currentContent || `Generate comprehensive content for eCTD section ${section}`,
...
export const suggestImprovements = async (content, type = 'regulatory') => {
  ...
  try {
    ...
    const suggestions = recommendation.split('\n')
...
export const generateContentSuggestions = async (documentId, sectionCode, currentContent, userQuery) => {
...
export const checkComplianceAI = async (documentId, content, regions = ['FDA', 'EMA']) => {
  return generateContentSuggestions(documentId, 'compliance', content, `Check compliance for: ${regions.join(', ')}`);
};

export const analyzeFormattingAI = async (documentId, content, documentType) => {
  return generateContentSuggestions(documentId, 'formatting', content, `Analyze formatting for ${documentType}`);
};
...
export const validateAtom = async (atom) => {
  ...
  try {
    const response = await apiCall(SERVICE_CONFIG.endpoints.verify, {
