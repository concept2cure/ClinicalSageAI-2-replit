 ...
 */
export function useSectionSync({ projectId, leafId, onSectionUpdate, onConflict, enabled = true }) {
  ...
  const connectToSSE = useCallback(() => {
    ...
    setSyncStatus('connecting');
    const url = `/api/cerv2-sections/subscribe/${projectId}`;
  ...
  const handleSectionUpdate = useCallback(
    data => {
      const { sectionId, leafId: sourceLeafId, affectedDocuments, patch, author, timestamp } = data;
      ...
      if (affectedDocuments.includes(leafId)) {
        ...
        queryClient.invalidateQueries(['/api/cerv2-sections', sectionId]);
  ...
  const processUpdateQueue = useCallback(() => {
    ...
    queue.forEach(update => {
      // Re-send the update
      fetch('/api/cerv2-sections/patch', {
        method: 'POST',
        headers: {
  ...
  const sendUpdate = useCallback(async update => {
    try {
      const response = await fetch('/api/cerv2-sections/patch', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const resolveConflict = useCallback((conflictId, resolution) => {
    ...
    return fetch('/api/cerv2-sections/resolve-conflict', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
