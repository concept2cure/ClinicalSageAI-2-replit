...
const DocumentAuthoringComponent = () => {
 ...
 regions: Array.isArray(doc.template?.regions)
 ? doc.template.regions
 ...
 } = useAISuggestionEngine(currentDocument?.id || 'temp-doc', documentContent, showEditorView);
 ...
 (tokenKey, tokenType, tokenLabel) => {
 if (!editor) return;

 const value =
 tokenValues[tokenKey] || customTokens.find(t => t.key === tokenKey)?.value || '';
 ...
 if (exportHistoryFilter.exportType !== 'all')
 params.append('export_type', exportHistoryFilter.exportType);
 params.append('limit', '100');

 const response = await fetch(`/api/authoring/docs/${currentDocument.id}/exports?${params}`, {
 headers: {
 ...
 try {
 const response = await fetch(`/api/authoring/export-history/${exportId}`, {
 method: 'DELETE',
 headers: {
 'x-tenant-id': getCurrentTenantId(),
 ...
 try {
 const response = await fetch(`/api/authoring/documents/${currentDocument.id}/reviews`, {
 headers: {
 'x-tenant-id': getCurrentTenantId(),
 },
 });
 ...
 try {
 const response = await fetch(
 `/api/authoring/documents/${currentDocument.id}/request-review`,
 {
 method: 'POST',
 headers: {
 'Content-Type': 'application/json',
 ...
 setIsExporting(true);
 try {
 const response = await fetch(`/api/authoring/docs/${currentDocument.id}/export`, {
 method: 'POST',
 headers: {
 'Content-Type': 'application/json',
 'x-tenant-id': getCurrentTenantId(),
 ...
 try {
 const response = await fetch(`/api/authoring/docs/${currentDocument.id}/submit`, {
 method: 'POST',
 headers: {
 'Content-Type': 'application/json',
 'x-tenant-id': getCurrentTenantId(),
 ...
 setIsSigning(true);
 try {
 const response = await fetch(`/api/authoring/docs/${currentDocument.id}/sign`, {
 method: 'POST',
 headers: {
 'Content-Type': 'application/json',
 'x-tenant-id': getCurrentTenantId(),
 ...
 setIsFreezing(true);
 try {
 const response = await fetch(`/api/authoring/docs/${currentDocument.id}/freeze`, {
 method: 'POST',
 headers: {
 'Content-Type': 'application/json',
 'x-tenant-id': getCurrentTenantId(),
 ...
 try {
 const response = await fetch(`/api/authoring/docs/${currentDocument.id}/signatures`, {
 headers: {
 'x-tenant-id': getCurrentTenantId(),
 },
 });
 ...
 'This will replace the current document content with the template. Continue?'
 );
 if (!confirmed) return;
 ...
 toast({
 title: `✅ Document recomputed with fresh LIMS data (${result.dataPoints.linearity + result.dataPoints.accuracy + result.dataPoints.precision} data points)`,
 });
 } else {
 ...
 const templateName =
 currentDocument?.template?.name || currentDocument?.name || 'Default Template';
 ...
 description: 'Complete drug substance documentation template',
 version: 'v1.0',
 aiGuidance: true,
 },
 {
 ...
 category: 'eCTD Module 3',
 regions: ['FDA', 'EMA'],
 description: 'Drug product manufacturing and control documentation',
 version: 'v1.0',
 aiGuidance: true,
 },
 {
 ...
 description: 'Comprehensive clinical study report template',
 version: 'v2.0',
 aiGuidance: true,
 },
 {
 ...
 category: 'Quality',
 regions: ['FDA', 'EMA', 'PMDA'],
 description: 'Stability study protocol and reporting template',
 version: 'v1.2',
 aiGuidance: true,
 },
 {
 ...
 regions: ['FDA', 'EMA'],
 description: 'Manufacturing process validation documentation',
 version: 'v1.0',
 aiGuidance: true,
 },
 {
 ...
 .map(doc => (doc.id === normalizedCurrent.id ? normalizedCurrent : normalizeDocument(doc)))
 .filter(Boolean);
 ...
 const categories = [
 ...new Set((documentTemplates || []).filter(t => t?.category).map(t => t.category)),
 ];
 const regions = [
 ...new Set(
 (documentTemplates || []).filter(t => Array.isArray(t?.regions)).flatMap(t => t.regions || [])
 ),
 ];
 ...
 disabled={
 documentIsFrozen ||
 (currentDocument?.status !== 'APPROVED' && currentDocument?.status !== 'approved')
 }
 ...
 <FileCheck className="h-4 w-4 mt-0.5 text-stone-500" />
 ...
 <div className="flex items-start space-x-2">
 {template.icon ? (
 <template.icon className="h-4 w-4 mt-0.5 text-stone-500" />
 ) : (
 <FileText className="h-4 w-4 mt-0.5 text-stone-500" />
 )}
 <div className="flex-1">
 <div className="text-sm font-medium">{template.name}</div>
 <div className="text-xs text-gray-500">{template.description}</div>
 ...
 className={guidanceDrawerOpen ? 'bg-stone-800 hover:bg-stone-900' : ''}
 >
 ...
 <FolderOpen className="h-4 w-4 mr-2 text-stone-500" />
 Dossier Structure
 </h3>
 </div>
 ...
 className="bg-stone-50 hover:bg-stone-100 border-stone-200"
 >
 {recomputeLoading ? (
 <Clock className="h-4 w-4 mr-1 animate-spin" />
 ) : (
 ...
 >
 {/* Document Header */}
 <div className="mb-6 p-4 bg-gray-50 border-l-4 border-stone-500">
 ...
 <div className="mb-6 p-4 bg-stone-50 rounded border">
 ...
 className="w-full h-64 p-3 border border-gray-200 rounded resize-none focus:ring-2 focus:ring-stone-400"
 style={{
 ...
 <BookOpen className="h-4 w-4 mr-2 text-stone-500" />
 ICH Guidelines
 </h4>
 {regulatoryGuidanceMap[selectedSection] && (
 <div className="space-y-2 text-sm">
 {regulatoryGuidanceMap[selectedSection].ichReferences.map((ref, index) => (
 <div key={index} className="p-2 bg-stone-50 rounded border">
 <div className="font-medium text-stone-800">{ref}</div>
 ...
 {Array.isArray(citationsDatabase) &&
 citationsDatabase.map(citation => (
 ...
 <Badge className="text-xs bg-stone-700">
 ...
 Version: {entry.metadata.version || 'N/A'} • Sections:{' '}
 {entry.metadata.sections || 'N/A'}
 </div>
 )}
 </div>
 ...
 return (
 <div className="p-6 space-y-6 bg-gradient-to-br from-stone-50 to-indigo-100 min-h-screen">
 {/* Header */}
 ...
 <TabsTrigger
 value="comments"
 className="data-[state=active]:bg-stone-800 data-[state=active]:text-white relative"
 >
 ...
 <CardDescription>
 Choose from {Array.isArray(documentTemplates) ? documentTemplates.length : 0}{' '}
 professional regulatory templates
 </CardDescription>
 </div>
 <div className="flex items-center space-x-2">
 ...
 {templatesLoading ? (
 <div className="text-center py-8">
 <div className="animate-spin h-8 w-8 border-4 border-stone-700 border-t-transparent rounded-full mx-auto mb-4"></div>
 ...
 className="h-8 px-3 bg-stone-800 hover:bg-stone-900"
 >
 ...
 <div
 key={comment.id}
 className="border-l-2 border-gray-200 pl-4 hover:border-stone-400 transition-colors"
 >
 <div className="flex items-start space-x-3">
 <div
 className={`w-10 h-10 rounded-full flex items-center justify-center text-white font-semibold ${getUserAvatarColor(comment.author_name)}`}
 >
 ...
 <DropdownMenuItem
 onClick={() =>
 updateCommentStatus(comment.id, 'archived')
 }
 >
 Archive
 </DropdownMenuItem>
 ...
 <div
 className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-semibold ${getUserAvatarColor(reply.author_name)}`}
 >
 ...
 <div className="flex items-start space-x-2">
 <div
 className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-semibold ${getUserAvatarColor('Current User')}`}
 >
 CU
 </div>
 <div className="flex-1">
 ...
 style={{
 width: `${reviewStats.total > 0 ? (reviewStats.approved / reviewStats.total) * 100 : 0}%`,
 }}
 />
 </div>
 </div>
 ...
 <div className="animate-spin h-6 w-6 border-3 border-stone-700 border-t-transparent rounded-full mx-auto"></div>
 </div>
 ) : reviews.length > 0 ? (
 reviews.map(review => (
 ...
 <div className="flex items-center space-x-2">
 <div
 className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-semibold ${getUserAvatarColor(review.reviewer_name)}`}
 >
 ...
 <div
 className={`w-6 h-6 rounded-full flex items-center justify-center text-white text-xs ${getUserAvatarColor(activity.actor_name)}`}
 >
 ...
 {Array.isArray(documentTemplates) &&
 documentTemplates
 .filter(t => t)
 .slice(0, 12)
 .map(template => (
 ...
 <div className="flex flex-wrap gap-1">
 {Array.isArray(previewTemplate?.regions) &&
 previewTemplate.regions.map(region => (
 ...
 <div className="p-3 bg-stone-50 rounded border-l-4 border-stone-500">
 <div className="flex items-center space-x-2">
 ...
 By signing this document, you certify that you have reviewed and approve its
 content.
 </p>
 </div>
 <div className="space-y-4">
 ...
 Freezing this document will permanently lock it from any further edits. This
...
export function GuidanceDrawer({ open, onClose, sectionCode, locale }) {
 ...
 fetch(
 `/api/authoring/guidance/compose?section=${encodeURIComponent(sectionCode)}&region=${region}`
 )
 .then(r => r.json())
...
export function TemplatesDrawer({ open, onClose, docId, locale, onApply }) {
 ...
 if (!r.ok) {
 toast({ title: x?.error || 'Apply failed.' });
 return;
 }
 onApply?.(x);
 onClose();
...
export function ReviewerChecklistDrawer({ open, onClose, docId, sectionId, sectionCode, locale }) {
 ...
 async function load() {
 const j = await fetch(
 `/api/authoring/docs/${docId}/checklist?section_id=${encodeURIComponent(sectionId)}&region=${region}`
 ).then(r => r.json());
 setData(j.checklist || null);
 setItems(j.items || []);
 }
 ...
 const x = await r.json();
 if (!r.ok) {
 toast({ title: x?.error || 'Compose failed' });
 return;
 }
 setData(x.checklist);
 setItems(x.items);
 ...
 const r = await fetch(`/api/authoring/checklist/items/${itemId}`, {
 method: 'PATCH',
 headers: { 'Content-Type': 'application/json' },
 ...
 const x = await r.json();
 if (!r.ok) {
 toast({ title: x?.error || 'Update failed' });
 return;
 }
 setItems(prev => prev.map(it => (it.item_id === itemId ? x : it)));
 }
 function jump(citeId) {
 window.dispatchEvent(new CustomEvent('cmc-jump-token', { detail: { citeId } }));
 }

 React.useEffect(() => {
 if (open && docId && sectionId) load();
 }, [open, docId, sectionId, locale]);
 ...
 Reviewer Checklist — {sectionCode} ({region})
 </div>
 <button onClick={onClose} className="text-slate-600 hover:text-black">
 ✕
 </button>
 ...
 {!data ? (
 <button className="border px-3 py-1 rounded text-sm" onClick={compose}>
 Compose from content
 </button>
 ) : (
 <div className="text-xs text-slate-600 mb-2">
 Reviewer: {data.reviewer_email || '—'} • Created:{' '}
 ...
 {it.evidence_cite && (
 <button className="text-xs text-stone-600" onClick={() => jump(it.evidence_cite)}>
 View Evidence
 </button>
 )}
 </div>
 ...
 <input
 type="text"
 className="mt-1 w-full border rounded px-2 py-0.5 text-xs"
 placeholder="Add comment..."
 value={it.comment || ''}
 onChange={e => update(it.item_id, { comment: e.target.value })}
 />
 </li>
 ))}
 </ul>
...
export function ChangeRequestsDrawer({ open, onClose, docId, sectionId, onCreateNew }) {
 ...
 <button
 className="border px-3 py-1 rounded text-sm bg-stone-50"
 onClick={() => onCreateNew()}
 >
 + New CR
 </button>
 ...
 Section: {cr.section_code || cr.section_id} • Type: {cr.apply_kind} • By:{' '}
 {cr.proposer_email}
 </div>
 ...
 <>
 <button
 className="border px-2 py-0.5 rounded text-xs bg-green-50"
 onClick={() => approve(cr.cr_id)}
 >
 Approve
 </button>
 <button
 className="border px-2 py-0.5 rounded text-xs bg-red-50"
 onClick={() => reject(cr.cr_id)}
 >
 Reject
 </button>
 </>
 ...
 {cr.status === 'APPROVED' && (
 <button
 className="border px-2 py-0.5 rounded text-xs bg-stone-50"
 onClick={() => apply(cr.cr_id)}
 >
 Apply
 </button>
 )}
...
export function CreateChangeRequestModal({
 ...
 <button className="border px-4 py-2 rounded bg-stone-50" onClick={submit}>
 Create Request
 </button>
 </div>
 </div>
