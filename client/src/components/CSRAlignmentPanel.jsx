 ...
 */
const CSRAlignmentPanel = ({ protocolData, csrContext }) => {
  ...
  const calculateAlignmentScore = () => {
    ...
    const sampleSizeMatch = compareSampleSize(
      parseInt(protocolData.sample_size || 0),
      parseInt(csrContext.sample_size || 0)
    );
  ...
  const compareEndpoints = (protocolEndpoint, csrEndpoint) => {
  ...
  const comparePopulation = (protocolPop, csrPop) => {
  ...
  const compareSampleSize = (protocolSize, csrSize) => {
  ...
  const compareDosing = (protocolDosing, csrDosing) => {
