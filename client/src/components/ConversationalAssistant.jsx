  ...
  useEffect(() => {
    if (initialPrompt && messages.length === 0) {
      setMessages([
        {
          role: "assistant",
          content: "👋 Hello! I'm the Concept2Cure assistant specializing in clinical trial design and analysis. How can I help you today?"
        }
      ]);
    }
        ...
        {messages.map((msg, idx) => (
          <Card key={idx} className="whitespace-pre-wrap text-sm">
            <CardContent className="py-3">
              <strong>{msg.role === "user" ? "You" : "Concept2Cure"}</strong>
              <div>{msg.content}</div>
            </CardContent>
          </Card>
        ))}
        ...
        {loading && (
          <Card className="whitespace-pre-wrap text-sm">
            <CardContent className="py-3 flex items-center gap-2">
              ...
              <span>Concept2Cure is thinking...</span>
            </CardContent>
          </Card>
        )}
      </div>
