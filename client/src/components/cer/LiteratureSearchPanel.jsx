...
`,
 ...
 type="submit"
 disabled={isLoading}
 className="px-4 py-2 bg-stone-800 text-white rounded hover:bg-stone-900 disabled:opacity-60 flex items-center gap-2"
 >
 {isLoading ? (
 <>
 ...
 className={`cursor-pointer ${searchSources.includes('pubmed') ? 'bg-stone-800' : 'bg-gray-200 text-gray-800'}`}
 >
 PubMed
 </Badge>
 <Badge
 onClick={() => toggleSource('googleScholar')}
 className={`cursor-pointer ${searchSources.includes('googleScholar') ? 'bg-stone-800' : 'bg-gray-200 text-gray-800'}`}
 >
 Google Scholar
 </Badge>
 </div>
 ...
 selectedPapers.some(p => p.id === paper.id) ? 'bg-stone-50' : ''
 } ${activePaper === paper.id ? 'ring-2 ring-stone-400' : ''}`}
 onClick={() => getSummary(paper)}
 >
 <div className="flex justify-between items-start gap-2">
 <h4 className="font-medium text-stone-800">{paper.title}</h4>
 <button
 onClick={e => {
 e.stopPropagation();
 ...
 <Badge
 variant="outline"
 className={`${paper.source === 'PubMed' ? 'border-stone-300 bg-stone-50' : 'border-emerald-300 bg-emerald-50'}`}
 >
 {paper.source}
 </Badge>
 ...
 className="text-stone-600 text-sm hover:underline flex items-center gap-1"
 ...
 className="text-sm text-stone-600 hover:underline flex items-center gap-1"
 >
 ...
 <Badge
 variant="outline"
 className={`${paper.source === 'PubMed' ? 'border-stone-300 bg-stone-50' : paper.source === 'PDF Upload' ? 'border-purple-300 bg-purple-50' : 'border-emerald-300 bg-emerald-50'}`}
 >
 {paper.source}
 </Badge>
 ...
 <Loader2 className="h-10 w-10 animate-spin text-stone-500 mb-4" />
 ...
 <label
 htmlFor="file-upload"
 className="mt-4 px-4 py-2 bg-stone-800 text-white text-sm font-medium rounded hover:bg-stone-900 cursor-pointer"
 >
 Browse Files
 </label>
 ...
 {selectedPapers.length > 0 && (
 <div className="mt-2">
 <Badge className="bg-stone-100 text-stone-800 border border-stone-200">
 ...
 className="w-full bg-stone-800 hover:bg-stone-900 text-white disabled:opacity-60"
 >
 {generatingReview ? (
 <>
 ...
 <h5 className="font-medium line-clamp-2 text-stone-700">{paper.title}</h5>
 <button
 ...
 className="px-3 py-2 bg-stone-800 text-white text-sm rounded hover:bg-stone-900 flex items-center gap-2"
 >
