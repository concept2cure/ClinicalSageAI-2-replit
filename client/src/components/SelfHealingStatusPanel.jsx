    ...
    const removeListener = selfHealingSystem.addListener((event, data) => {
      ...
      setTimeout(() => {
        setStatus(prev => {
          ...
          if (!newStatus.issues) newStatus.issues = [];
          if (!newStatus.fixes) newStatus.fixes = [];
          ...
          switch (event) {
            ...
            case 'fixing':
              newStatus.fixes = [
                ...(prev.fixes || []),
                { ...data, status: 'fixing', timestamp: Date.now() },
              ];
              break;
            case 'fixed':
            ...
            case 'fix-failed':
              ...
              break;
            case 'memory-status':
              newStatus.memoryUsage = data.percentage || 0;
              break;
                ...
                <div className="space-y-1">
                  {(status.issues || []).map((issue, index) => (
                    <div key={index} className="flex items-center justify-between text-xs">
                      <span className="truncate">{issue?.message || 'Unknown issue'}</span>
              ...
              <div>
                ...
                <div className="space-y-1">
                  {(status.fixes || []).slice(-3).map((fix, index) => (
                    <div key={index} className="flex items-center justify-between text-xs">
                      <span className="truncate">{fix?.message || 'Unknown fix'}</span>
