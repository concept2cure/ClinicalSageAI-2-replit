...
const SubmissionTimeline = ({ currentStep = 1, steps = [], onSelectStep = () => {} }) => {
 ...
 <div className="bg-stone-50 p-3 rounded-md border border-stone-100 mb-4">
 <div className="flex">
 <Info className="h-4 w-4 text-stone-600 mt-0.5 mr-2 flex-shrink-0" />
 <p className="text-sm text-stone-700">{activeStep.description}</p>
 </div>
 </div>
 )}
 ...
 {/* Timeline node */}
 <div
 className={`absolute left-5 w-3 h-3 rounded-full border-2 ${isActive ? 'border-stone-700 bg-white' : isPast ? 'border-green-600 bg-green-600' : 'border-gray-400 bg-white'}`}
 ...
 <button
 type="button"
 className={`w-full text-left rounded-md border p-3 transition-colors ${isActive ? 'bg-stone-50 border-stone-200' : 'bg-white hover:bg-gray-50 border-gray-200'}`}
 ...
 <h4
 className={`font-medium ${isActive ? 'text-stone-700' : 'text-gray-800'}`}
 >
 {index + 1}. {step.title}
 </h4>
 ...
 <p
 className={`text-sm mt-1 ${isActive ? 'text-stone-600' : 'text-gray-600'}`}
 >
 {step.shortDescription}
 </p>
 )}
 ...
 <h5 className="text-xs font-medium text-stone-700 mb-2">Next Actions</h5>
