...
export default function DocumentSelector({ onSelectDocument }) {
  ...
  useEffect(() => {
    ...
    const loadDocuments = async () => {
      const response = await fetch('/api/coauthor/documents?limit=6', { headers });
      if (!response.ok) {
    ...
    const loadModules = async () => {
      const response = await fetch('/api/coauthor/ectd-modules/tree-with-counts', { headers });
      if (!response.ok) {
    ...
    const loadComplianceSignals = async docs => {
      ...
      const response = await fetch(`/api/coauthor/validate/latest/${latestDocument.id}`, {
        headers,
      });
      if (!response.ok) {
        return;
    ...
    loadModules().catch(error => console.error(error));
  }, []);
