...
import {
 ...
 FileDown,
 FileUp,
 // Compare and DiffIcon don't exist in lucide-react, removed
 GitCommit,
 Activity,
 TrendingUp,
 TrendingDown,
...
const getStatusBadgeConfig = (status) => {
 ...
 published: { variant: 'default', icon: CheckCircle, className: 'bg-stone-100 text-stone-800 border-stone-300' },
...
export default function VersionHistory({ componentId, documentId, isOpen, onClose }) {
 ...
 selectedVersions.includes(version.id) && "ring-2 ring-stone-400 dark:ring-stone-400"
 )}
 onClick={() => {
 if (compareMode) {
 ...
 <span className="font-mono text-stone-600 dark:text-stone-400">#{cr.id}</span>
 <span>{cr.title}</span>
 <Badge variant="outline" className="text-xs">
 {cr.status}
 </Badge>
