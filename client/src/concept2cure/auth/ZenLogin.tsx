...
const DEMO_PERSONAS = [
  ...
  },
  {
    ...
    email: 'michael.torres@concept2cure.pro',
    role: 'Medical Writer',
    initials: 'MT',
  },
  {
    label: 'Emily Watson',
    email: 'emily.watson@concept2cure.pro',
    role: 'QA Lead',
    initials: 'EW',
  },
  {
    label: 'David Park',
    email: 'david.park@concept2cure.pro',
    role: 'Biostatistics',
    initials: 'DP',
  },
] as const;
...
export const ZenLogin: React.FC = () => {
  ...
  useEffect(() => {
    setError('');
  }, [email, password, mfaCode, newPassword, confirmPassword, suEmail, suPassword, suConfirm]);
  ...
  const handleLogin = useCallback(async () => {
    if (lockedUntil && lockedUntil > Date.now()) {
      setError(
        `Account locked. Try again in ${Math.max(1, getLockoutRemainingSeconds(lockedUntil))}s.`
      );
      return;
    }
  ...
  const handleSignup = useCallback(async () => {
    ...
    if (!suCompany.trim() || suCompany.trim().length < 2) {
    ...
    try {
      ...
      if (data.token) {
        localStorage.setItem('trialsage_access_token', data.token);
    ...
    async (personaEmail: string) => {
      ...
      try {
        ...
        if (data.accessToken || data.token) {
          ...
          // Use authService.setToken to properly set internal state + storage
          ...
          setTimeout(() => {
            globalThis.window.location.href = '/concept2cure';
          }, 300);
        } else {
          setError(data.error?.message || data.message || 'Persona not found.');
        }
      } catch {
        setError('Unable to reach the server.');
      } finally {
  ...
  const handleVerifyMfa = useCallback(async () => {
    ...
    if (!code || (mfaMethod !== 'backup_code' && code.length !== 6)) {
      setError(
        mfaMethod === 'backup_code' ? 'Enter your recovery code.' : 'Enter the 6-digit code.'
      );
      return;
    }
    setLoading(true);
  ...
  const handleForgotPassword = useCallback(async () => {
    ...
    setError('');
    try {
      const r = await authService.requestPasswordReset({ email: norm });
      if (!r.success) {
        setError(r.error?.message || 'Unable to send reset link.');
        return;
      }
      setView('reset-sent');
    } finally {
  ...
  const handleResetPassword = useCallback(async () => {
    ...
    setError('');
    try {
      const r = await authService.confirmPasswordReset({ token: resetToken, newPassword });
      if (!r.success) {
        setError(r.error?.message || 'Reset failed.');
        return;
      }
      setView('success');
      setSuccessMessage('Password updated.');
      setTimeout(() => setLocation('/concept2cure/login'), 500);
    } finally {
      setLoading(false);
    }
  ...
  const handleResendCode = useCallback(async () => {
    ...
    try {
      const r = await authService.resendLoginOtp();
      if (!r.success) {
        setError(r.error?.message || 'Resend failed.');
        return;
      }
      setMaskedEmail(r.data?.maskedEmail || maskedEmail);
    ...
    <>
      ...
      <div data-zen-auth="">
        <div className="za-card">
            ...
            <>
              ...
              >
                <div className="za-field">
                  <label className="za-label" htmlFor="za-email">
                    Email address
                  </label>
                  <input
                    id="za-email"
                ...
                <div className="za-field-last">
                  ...
                  >
                    <label className="za-label" htmlFor="za-pw" style={{ marginBottom: 0 }}>
                      Password
                    </label>
                    <button
                      type="button"
                      className="za-pw-toggle"
                ...
                <div className="za-row">
                  ...
                  <button type="button" className="za-link" onClick={() => setView('sign-up')}>
                    Create account
                  </button>
                </div>
              </form>
              ...
              {isDev && (
                <>
                  ...
                  <div style={{ position: 'relative' }}>
                    <button
                      type="button"
                      className="za-btn za-btn-outline"
            ...
            <>
              ...
              >
                <div className="za-grid-2" style={{ marginBottom: 20 }}>
                  <div>
                    <label className="za-label" htmlFor="za-fn">
                      First name
                    </label>
                    <input
                      id="za-fn"
                  ...
                  <div>
                    <label className="za-label" htmlFor="za-ln">
                      Last name
                    </label>
                    <input
                      id="za-ln"
                ...
                <div className="za-field">
                  <label className="za-label" htmlFor="za-su-email">
                    Email address
                  </label>
                  <input
                    id="za-su-email"
                ...
                <div className="za-field">
                  <label className="za-label" htmlFor="za-co">
                    Company
                  </label>
                  <input
                    id="za-co"
                ...
                <div className="za-field">
                  <label className="za-label" htmlFor="za-ind">
                    Industry
                  </label>
                  <select
                    id="za-ind"
                    className="za-select"
                    value={suIndustry}
                    onChange={e => setSuIndustry(e.target.value)}
                  >
                ...
                <div className="za-field">
                  <label className="za-label" htmlFor="za-su-pw">
                    Password
                  </label>
                  <input
                    id="za-su-pw"
                ...
                <div className="za-field">
                  <label className="za-label" htmlFor="za-su-cpw">
                    Confirm password
                  </label>
                  <input
                    id="za-su-cpw"
                ...
                <div style={{ marginBottom: 24 }}>
                  {suPwRules.map(r => (
                    <div
                      key={r.id}
                      className={`za-pw-rule ${r.met ? 'za-pw-rule-met' : 'za-pw-rule-unmet'}`}
                    >
                      <span>{r.met ? '✓' : '○'}</span>
                      <span>{r.label}</span>
                    </div>
                  ...
                  <div
                    className={`za-pw-rule ${suPwMatch ? 'za-pw-rule-met' : 'za-pw-rule-unmet'}`}
                  >
                    <span>{suPwMatch ? '✓' : '○'}</span>
                ...
                <button
                  type="submit"
                  className="za-btn za-btn-primary"
                  disabled={loading || !suPwOk || !suPwMatch}
                >
                  {loading && <Spin />}Create account
                </button>
                <div style={{ textAlign: 'center', marginTop: 20 }}>
                  <button type="button" className="za-link" onClick={() => setView('sign-in')}>
          ...
          {view === 'mfa' && (
            <>
              <h1 className="za-title" style={{ fontSize: 24 }}>
                Verification
              </h1>
                ...
                <div className="za-mfa-pills">
                  {availableMfaMethods.map(m => (
                    <button
                      ...
                      type="button"
                      className={`za-pill ${
                        mfaMethod === m.type ? 'za-pill-active' : 'za-pill-inactive'
                      }`}
                      onClick={() => {
                        setMfaMethod(m.type);
                        setMfaCode('');
              ...
              {mfaMethod === 'backup_code' ? (
                <div className="za-field-last">
                  <input
                    ...
                    autoFocus
                    style={{ fontFamily: 'monospace', textAlign: 'center', letterSpacing: '0.1em' }}
                  />
                </div>
              ) : (
          ...
          {view === 'forgot-password' && (
            <>
              ...
              <div className="za-field-last">
                <label className="za-label" htmlFor="za-fp-email">
                  Email address
                </label>
                <input
                  id="za-fp-email"
          ...
          {view === 'reset-sent' && (
            <div style={{ textAlign: 'center' }}>
              ...
              <h1 className="za-title" style={{ fontSize: 24, marginBottom: 12 }}>
                Check your email
              </h1>
          ...
          {view === 'reset-password' && (
            <>
              ...
              <div className="za-field">
                <label className="za-label" htmlFor="za-np">
                  New password
                </label>
                <input
                  id="za-np"
              ...
              <div className="za-field">
                <label className="za-label" htmlFor="za-cnp">
                  Confirm password
                </label>
                <input
                  id="za-cnp"
              ...
              <div style={{ marginBottom: 24 }}>
                ...
                <div
                  className={`za-pw-rule ${resetPwMatch ? 'za-pw-rule-met' : 'za-pw-rule-unmet'}`}
                >
                  <span>{resetPwMatch ? '✓' : '○'}</span>
                  <span>Passwords match</span>
                </div>
              </div>
              ...
              <button
                ...
                disabled={loading || !resetPwOk || !resetPwMatch}
              >
