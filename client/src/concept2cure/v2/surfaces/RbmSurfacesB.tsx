...
import { I } from '../icons';
import {
  RBM_VOCAB, RBM_SIGNALS, RBM_SITES, RBM_PATIENTS, RBM_CSM_RUN,
  RBM_OVERSIGHT_COUNTS, RBM_PLAN,
  type RbmSignal, type RbmPlanData,
...
/* 6 -- Signals */
export function RbmSignals({ onTab }: { onTab: (id: string) => void; onAsk?: (t: string) => void; onNav?: (id: string) => void }) {
  const [signals, setSignals] = useState<RbmSignal[]>(RBM_SIGNALS);
  const [filter, setFilter] = useState('open');
  const [openId, setOpenId] = useState<string | null>(null);
  const [run, setRun] = useState<typeof RBM_CSM_RUN | null>(null);
    ...
    <div>
      <div className="rbm-bar">
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <button className="rbm-btn pri" onClick={() => setRun(RBM_CSM_RUN)}>{I.zap}Run central monitoring</button>
        </div>
      </div>
      {run && (
        <div className="rbm-run">
          <div className="rbm-run-h">{I.check}Central monitoring complete -- {run.method}</div>
          <div className="rbm-run-m">{run.cohort} -- {run.findings.length} findings, {run.findings.filter(f => f.sev === 'high').length} raised as signals</div>
          <div className="rbm-run-rows">{run.findings.map((f, i) => (
            <div key={i} className="rbm-run-row">
              <span className="mono">site {f.site}</span><span>{f.metric}</span>
              <span className="mono z" data-hi={Math.abs(f.z) >= 3.5 || undefined}>|z| {Math.abs(f.z)}</span>
              <RbmChip vocab="severity" value={f.sev} /><em>{f.note}</em>
            </div>
          ))}</div>
        </div>
      )}
      <div className="rbm-inbox">{shown.map(s => (
        <div key={s.id} className="rbm-sig" data-sev={s.severity}>
          <button className="rbm-sig-main" onClick={() => setOpenId(openId === s.id ? null : s.id)} aria-expanded={openId === s.id}>
            ...
            <span className="rbm-sig-src" data-src={s.source}>{SRC_LABEL[s.source]}</span>
        ...
        {shown.length === 0 && <div className="rbm-inbox-empty">No {filter === 'all' ? '' : filter + ' '}signals. Run central monitoring to re-scan the site cohort.</div>}
      </div>
...
/* 7 -- Patients */
export function RbmPatients() {
  const [open, setOpen] = useState('1104-014');
  const [scannedAt, setScannedAt] = useState('2026-07-01');
  const P = RBM_PATIENTS;
  const sel = P.find(p => p.sid === open);
  const flagged = P.filter(p => p.status === 'flagged').length, review = P.filter(p => p.status === 'review').length;
  return (
    <div>
      <div className="rbm-bar">
        <span className="rbm-bar-info">scorePatientCohort -- robust modified z-score -- MIN_COHORT 5 -- 412 subjects -- scored {scannedAt}</span>
        <button className="rbm-btn pri" onClick={() => setScannedAt('2026-07-02 (just now)')}>{I.zap}Scan cohort</button>
      </div>
          ...
          <table className="rbm-tbl"><thead><tr><th>Subject</th><th>Site</th><th>Anomaly</th><th>Top dimension</th><th>Status</th></tr></thead>
            <tbody>{P.map(p => (
              <tr key={p.sid} data-on={open === p.sid || undefined} className="rowbtn" onClick={() => setOpen(p.sid)}>
                ...
                <td><span className="rbm-z" data-hi={p.anomaly >= 3.5 ? 2 : p.anomaly >= 3 ? 1 : 0}>{p.anomaly.toFixed(1)}</span></td>
...
/* 8 -- Sites */
export function RbmSites({ onTab }: { onTab: (id: string) => void; onAsk?: (t: string) => void; onNav?: (id: string) => void }) {
  const [at, setAt] = useState('2026-07-01');
  return (
    <div>
      <div className="rbm-bar">
        <span className="rbm-bar-info">site-risk-engine -- composite from Site Intelligence -- scored {at}</span>
        <button className="rbm-btn pri" onClick={() => setAt('2026-07-02 (just now)')}>{I.zap}Recompute site risk</button>
      </div>
      <div className="rbm-card">
        <table className="rbm-tbl"><thead><tr><th>Site</th><th>Composite</th><th style={{ width: 230 }}>Dimensions (of 25)</th><th>Tier</th><th>Drivers -- why this tier</th></tr></thead>
          <tbody>{RBM_SITES.map(s => (
            <tr key={s.n}>
              <td><b className="mono">{s.n}</b> {s.name}<span className="rbm-cty">{s.country}</span></td>
              <td><RbmScore v={Math.round(s.composite)} /> <span className="mono mut">{s.composite}</span></td>
              <td><DimBars enr={s.enr} qual={s.qual} ops={s.ops} /></td>
      ...
      <div className="rbm-note">{I.info}Showing the 8 highest-risk of 24 sites. Tier assignment is monitoringTierFromRisk() on the composite; the enhanced tier drives the monitoring-plan visit cadence.</div>
    </div>
  );
}

/* 9 -- Oversight */
export function RbmOversight({ onTab, onNav }: { onTab: (id: string) => void; onAsk?: (t: string) => void; onNav?: (id: string) => void }) {
  const C = RBM_OVERSIGHT_COUNTS;
  const [visits, setVisits] = useState<Record<string, { when: string; focus: string; monitor: string; type: string }>>({});
  const [schedFor, setSchedFor] = useState<typeof RBM_SITES[number] | null>(null);
  ...
  return (
    <div>
          ...
          <tbody>{RBM_SITES.map(s => {
            ...
            return (
              <tr key={s.n}>
                ...
                <td><RbmScore v={Math.round(s.composite)} /></td>
                <td><RbmChip vocab="tier" value={s.tier} /></td>
      ...
      {schedFor && <RbmFormModal title={`Schedule oversight visit -- site ${schedFor.n}`}
        intro={`${schedFor.name} -- composite ${schedFor.composite} -- ${schedFor.tier} tier. Choose the visit type, focus and monitor. The visit is added to the monitoring plan and the task board.`}
...
/* 10 -- Plan */
export function RbmPlan() {
  const [plan, setPlan] = useState<RbmPlanData>(RBM_PLAN);
  ...
  const generate = () => {
    RBM_STORE.addAction({ type: 'capa', title: 'CAPA for each open critical CtQ without an active mitigation', priority: 'high', owner: 'AnA -> S. Okafor', due: '2026-07-22', ana: true, origin: 'ana' });
    RBM_STORE.addAction({ type: 'site_visit', title: 'Enhanced-tier visit schedule for sites 1104 / 1117 / 1109', priority: 'high', owner: 'AnA -> M. Torres', due: '2026-07-25', ana: true, origin: 'ana' });
    setPlan(p => ({ ...p, anaDraft: true, updated: 'just now' }));
    setGen(false);
  };
  return (
    <div>
      <div className="rbm-asmt">
        <div className="rbm-asmt-l">
          <b>{plan.title}</b>
          <span>v{plan.version} -- strategy <RbmChip vocab="strategy" value={plan.strategy} /> -- updated {plan.updated}</span>
          {plan.anaDraft && plan.status !== 'active' && <span className="rbm-draftflag">{I.sparkles}AnA-drafted -- pending review and approval</span>}
          {plan.approval ? <span className="rbm-audit">{I.check}Approved by {plan.approval.by} -- {plan.approval.when} -- &quot;{plan.approval.reason}&quot; -- <code>{plan.approval.audit}</code></span> : null}
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 7, alignItems: 'flex-end' }}>
          ...
          <button className="rbm-btn" onClick={() => setGen(true)}>{I.sparkles}Generate from RACT</button>
        </div>
      </div>
        ...
        <div key={st} className="rbm-col">
          ...
          {acts.filter(a => a.status === st).map(a => (
            <div key={a.id} className="rbm-act" data-overdue={a.overdue || undefined}>
              <div className="rbm-act-top"><span className="rbm-act-type">{RBM_VOCAB.atype[a.type].l}</span><RbmChip vocab="severity" value={a.priority === 'critical' ? 'critical' : a.priority} /></div>
      ...
      {gen && <RbmFormModal title="Generate plan from RACT (AnA)"
        intro="AnA drafts monitoring actions from the open critical CtQ factors and the current tier assignments. Drafted actions are badged and remain advisory until you approve the plan."
        fields={[{ key: 'confirm', label: 'Type "generate" to draft actions', type: 'text' }]}
        submitLabel="Generate draft actions" onCancel={() => setGen(false)} onSubmit={generate} />}
      {signFor && <GovernedApprovalDialog what={`Monitoring plan v${plan.version}`}
        meaning="The plan becomes the active monitoring commitment; visit cadence and SDV depth follow its tier definitions."
        onCancel={() => setSignFor(false)} onSigned={sig => { setPlan(p => ({ ...p, status: 'active', anaDraft: false, approval: sig })); setSignFor(false); }} />}
    </div>
  );
}
