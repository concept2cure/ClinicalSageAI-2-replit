 ...
 * deficiency: restated ask, editable prose, optional table, discussion,
 * evidence, dossier updates, citations). Top bar carries status + due +
 * reviewer roster; the footer gates Send on internal sign-off.
 *
 * Renders inline inside the pathway surface — when active it takes over the
 * workspace area; Discard / back returns to the correspondence list.
 */
...
function fileIco(_kind: string | undefined): React.ReactNode {
  return I.fileText; // single icon for all kinds — colour tells the rest
}

export interface AnaDrafterProps {
...
export function AnaDrafter({ correspondence, onClose, onOpenSection }: AnaDrafterProps) {
  ...
  const responseScrollRef = React.useRef<HTMLDivElement>(null);
  React.useEffect(() => {
...
function DueChip({ days, overdue, due }: { days: number | null; overdue: boolean; due: string | undefined }) {
      ...
      <div className="drafter-letter-paper">
        ...
        <div className="dl-body">
          {letter.body.map((para, i) => (
            <p key={i}>{enrichParagraph(para, deficiencies, activeDefId, setActiveDefId)}</p>
          ))}
              ...
              >
                ...
                <span className="dl-def-body">
                  ...
                  <span className="dl-def-meta">
                    {d.refs.map((r) => <span key={String(r.section)} className="dl-def-ref">{r.label}</span>)}
...
function enrichParagraph(para: string, deficiencies: Deficiency[], activeDefId: string | undefined, setActiveDefId: (id: string) => void): React.ReactNode[] {
...
function DrafterDraftedBody({ draft, setDraft, deficiencies, activeDefId, setActiveDefId, showCitations, onOpenSection }: {
  ...
  const updateDef = (id: string, patch: Partial<DraftDeficiency>) => {
    setDraft({
      ...draft,
    ...
    <div className="dr-body">
      ...
      <div className="dr-defs">
        {deficiencies.map((def) => {
          const draftDef: DraftDeficiency = (draft.deficiencies || []).find((dd) => dd.id === def.id) || { id: def.id, response: '', evidence: [], updates: [] };
          return (
            <DrafterDefCard
              key={def.id}
              def={def}
...
function DrafterIntro({ draft, setDraft }: { draft: ResponseDraft; setDraft: (d: ResponseDraft) => void }) {
  return (
    <div className="dr-section dr-intro">
      ...
      <DrafterTextarea value={draft.intro} onChange={(v) => setDraft({ ...draft, intro: v })} rows={4} />
    </div>
  );
}
...
function DrafterClosing({ draft, setDraft }: { draft: ResponseDraft; setDraft: (d: ResponseDraft) => void }) {
  return (
    <div className="dr-section dr-closing">
      ...
      <DrafterTextarea value={draft.closing} onChange={(v) => setDraft({ ...draft, closing: v })} rows={3} />
    </div>
  );
}
...
function DrafterDefCard({ def, draftDef, isActive, onActivate, onPatch, showCitations, onOpenSection }: {
  def: Deficiency;
  draftDef: DraftDeficiency;
  isActive: boolean;
  onActivate: () => void;
  onPatch: (p: Partial<DraftDeficiency>) => void;
  showCitations: boolean;
  onOpenSection: (t: SectionTarget) => void;
}) {
  return (
    <div className={`dr-def-card ${isActive ? 'is-active' : ''}`} data-def={def.id} onClick={onActivate}>
      ...
      <div className="dr-def-section">
        ...
        <DrafterTextarea value={draftDef.response} onChange={(v) => onPatch({ response: v })} rows={4} />
      </div>
      ...
      {draftDef.discussion && (
        <div className="dr-def-section">
          ...
          <DrafterTextarea value={draftDef.discussion} onChange={(v) => onPatch({ discussion: v })} rows={3} />
        </div>
      )}
        ...
        <div className="dr-def-section">
          ...
          <div className="dr-updates">
            {draftDef.updates.map((u, i) => (
              <button key={i} className="dr-update" onClick={() => onOpenSection({ id: def.refs[0]?.section ?? '', label: u.section })}>
      ...
      {showCitations && (
        <div className="dr-citations">
          ...
          {def.refs.map((r) => <span key={String(r.section)} className="dr-cit-chip cit-section">{r.label}</span>)}
...
function DrafterReviewers({ draft, setDraft }: { draft: ResponseDraft; setDraft: (d: ResponseDraft) => void }) {
  if (!draft.reviewers) return null;
  const reviewers = draft.reviewers;
  const toggle = (i: number) => {
    const r = reviewers[i];
    const next: DraftReviewer = r.status === 'approved'
      ? { ...r, status: 'pending', signed_at: null }
      : { ...r, status: 'approved', signed_at: new Date().toISOString() };
    setDraft({ ...draft, reviewers: reviewers.map((x, j) => (j === i ? next : x)) });
  };
  return (
      ...
      <div className="dr-rev-grid">
        {reviewers.map((r, i) => (
          <div key={i} className={`dr-rev ${r.status}`}>
            ...
            <button className={`dr-rev-act ${r.status}`} onClick={() => toggle(i)}>
...
/* ── Helpers ── */
function DrafterTextarea({ value, onChange, rows = 3 }: { value: string | undefined; onChange: (v: string) => void; rows?: number }) {
  return (
    <textarea
