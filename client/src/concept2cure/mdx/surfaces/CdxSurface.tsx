/**
 * CdxSurface — Phase 6 doc-first with paired drug-device timeline.
 *
 ...
 * label alignment.
 *
 * The novel piece: the paired timeline. Two parallel tracks (drug ·
 * device) with linked milestones — when one moves, the other must too.
 *
 * Port basis: design-system/ui_kits/mdx/surfaces/Cdx.jsx.
 */
...
export interface CdxSurfaceProps {
  program: Program | null;
  onAskAna: (text: string, opts?: { tool?: string }) => void;
  onOpenEditor?: (docId: string) => void;
}

function tlStatePill(state: string): 'complete' | 'active' | 'review' | 'idle' {
...
function alignmentPill(alignment: string): 'complete' | 'review' | 'draft' {
  if (alignment === 'aligned') return 'complete';
  if (alignment === 'review') return 'review';
  return 'draft';
}

export function CdxSurface({
  program,
  onAskAna,
  onOpenEditor,
}: CdxSurfaceProps) {
  ...
  const programContext = program
    ? `${program.code} ${program.title.split(' ').slice(0, 1).join(' ')} ↔ KEYTRUDA-9`
    : 'IV-415 ↔ KEYTRUDA-9';

  const live = useCdx(program?.id ?? null);
  const timeline = live.timeline ?? CDX_TIMELINE;
  const concordance = live.concordance ?? CDX_CONCORDANCE;
  const xref = live.xref ?? CDX_XREF;
  const kpis = live.kpis ?? CDX_KPIS;
  const documents = CDX_DOCUMENTS as unknown as KitDocument[];
  const frameworks = CDX_DOC_FRAMEWORKS as unknown as KitDocFramework[];
    ...
    <>
      <div className="page-header">
        <div>
          ...
          <div className="page-sub">
            Paired drug-device development. {documents.length} regulatory
            artifacts. FDA + EMA paired filings · concordance with original
            clinical-trial assay · drug-label alignment.
          </div>
        </div>
        <div className="page-actions">
          <button
            ...
            onClick={() =>
              onAskAna(
                'Verify drug label and IVD IFU are aligned across every flagged section. Surface mismatches and propose IFU edits.',
              )
            }
            type="button"
          >
          ...
          <button
            ...
            onClick={() => onOpenEditor?.('doc-cdx-label-align')}
            type="button"
          >
        ...
        {kpis.map((k, i) => (
          <div key={i} className="metric-card" data-tone={k.tone ?? ''}>
            ...
            <div
              ...
              style={{
                fontSize:
                  typeof k.metric === 'string' && k.metric.length > 8 ? 16 : 22,
              }}
            >
              {k.metric}
      ...
      <section className="section">
        <div className="section-head">
          ...
          <span className="section-sub">
            KEYTRUDA-9 NDA + IV-415 PMA · synchronized milestones · FDA + EMA
          </span>
        </div>
          ...
          {timeline.map((m, i) => {
            ...
            return (
              <button
                ...
                className="cdx-ms"
                data-state={m.state}
                data-joint={joint || undefined}
                onClick={() =>
                  onAskAna(
                    `Open paired milestone "${m.ms}" — show me the gate criteria, who owns each side, and the consequence if either side slips.`,
                  )
                }
                type="button"
              >
                  ...
                  <div className="cdx-track-cell cdx-cell-drug">
                    {onDrug && (
                      <>
                        ...
                        <span className="cdx-ms-date mono small">
                          {m.drugDate}
                        </span>
                      </>
                    )}
                ...
                <div className="cdx-ms-label">
                  <span className={`status-pill ${tlStatePill(m.state)}`}>
                    {m.state}
                  </span>
      ...
      <section className="section eng-awareness" data-open={awarenessOpen}>
        ...
        >
          ...
          <span className="section-sub">
            CDx vs CTA biomarker-positive concordance · NDA ↔ PMA document
            ...
            <section>
              <div className="section-head" style={{ marginTop: 0 }}>
                <h2 style={{ fontSize: 14 }}>
                  Concordance · CDx vs original clinical-trial assay
                </h2>
              </div>
              <div className="ctable">
                <div
                  ...
                  style={{
                    gridTemplateColumns: '1.4fr 70px 70px 70px 70px 70px',
                  }}
                >
                  <div>Population</div>
                  <div>CDx+</div>
                ...
                {concordance.map((c, i) => (
                  ...
                  >
                    ...
                    <div
                      ...
                      style={{
                        color: c.ppa >= 95 ? 'var(--success)' : 'var(--warning)',
                      }}
                    >
                      {c.ppa}
                    </div>
                    <div
                      className="mono small"
                      style={{
                        color: c.npa >= 95 ? 'var(--success)' : 'var(--warning)',
                      }}
                    >
                      {c.npa}
                    </div>
                    <div
                      className="mono small"
                      style={{
                        color: c.opa >= 95 ? 'var(--success)' : 'var(--warning)',
                      }}
                    >
                      {c.opa}
                    </div>
            ...
            <section>
              ...
              <div className="ctable">
                <div
                  className="ctable-head"
                  style={{
                    gridTemplateColumns: '1.4fr 1.4fr 100px 80px 80px',
                  }}
                >
                  <div>Drug document</div>
                  <div>Device document</div>
                  <div>Alignment</div>
                  <div>Sync</div>
                  <div>Issues</div>
                </div>
                {xref.map((x, i) => (
                  <button
                    ...
                    onClick={() =>
                      onAskAna(
                        `Reconcile ${x.drug} with ${x.device}. Surface every claim that differs and propose resolution.`,
                      )
                    }
                    type="button"
                  >
                    ...
                    <div>{x.device}</div>
                    <div>
                      <span className={`status-pill ${alignmentPill(x.alignment)}`}>
                        {x.alignment}
                      </span>
                    </div>
                    <div
                    ...
                    <div>
                      {x.issues > 0 ? (
                        <span className="pill-err small">{x.issues}</span>
                      ) : (
