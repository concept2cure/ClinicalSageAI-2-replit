 ...
 *
 * Ported verbatim from design-system/ui_kits/mdx/data-validator.jsx. Drives
 * the Validation Center drawer: every finding carries an FDA reference,
...
export interface EstarSection {
  num: string;
  label: string;
  volume: 'Admin' | 'Device' | 'Performance' | 'Substantial Equivalence';
}

export const ESTAR_SECTIONS: EstarSection[] = [
...
export type FindingSeverity = 'err' | 'warn' | 'ok';
export type SectionStatus = 'err' | 'warn' | 'ok';
export type EvidenceStatus = 'approved' | 'review' | 'draft';

/** Evidence references attached to a finding. Discriminated by `kind`. */
...
/** Concrete actions a reviewer can take to clear the finding. */
export type FixAction =
  | {
      kind: 'open-editor';
      label: string;
      target: { sectionId: number; editorKind?: 'estar-editor' | 'pma-editor' | 'cer-editor' };
    }
  | {
      kind: 'attach';
      label: string;
      target: { fileTypes?: string; sectionId?: number; vaultId?: string };
    }
  | {
      kind: 'esign';
      label: string;
      target: { vaultId: string };
    }
  | {
      kind: 'ask-claude';
      label: string;
  ...
  | {
      kind: 'auto-fill';
      label: string;
      target: { sectionId: number; vaultId: string };
    }
  | {
      kind: 'open-qsub';
      label: string;
      target: { qsubId: string };
    };
...
export interface Finding {
  ...
  since: string;
  owner: string;
  /** When true, this finding gates submission transmit. */
  blockingTransmit?: boolean;
}

export interface SectionDetail {
  status: SectionStatus;
  /** Plain-language summary of what's already done in this section. */
  complete: string;
  findings: Finding[];
}

export interface ProgramOverall {
  complete: number;
  warn: number;
  err: number;
  total: number;
...
export interface ProgramDetail {
  progTitle: string;
  pathway: string;
  overall: ProgramOverall;
  summary: string;
  /** Keyed by section num ('03', '11', …) or by named slot ('Labeling'). */
  sections: Record<string, SectionDetail>;
}

// ─── Per-program · per-section findings ────────────────────────────────────

export const ESTAR_DETAIL: Record<string, ProgramDetail> = {
  ...
  'OR-801': {
    ...
    summary:
      'One blocker in §11 SE discussion gates transmit. Four warnings across §3, §11, §17 do not block but should be cleared before cover-letter sign-off.',
    sections: {
      '03': {
        status: 'warn',
        ...
        findings: [
          {
            ...
            current:
              'Cover letter is 640 words. Missing the regulatory class line ("Class II, 21 CFR 888.3070, product code MNH").',
            expected:
              'Include the standard FDA framing: device class · CFR cite · product code · pathway · indication recap.',
            evidence: [
            ...
            fixes: [
              ...
              {
                kind: 'ask-claude',
                label: 'Draft the framing line',
                prompt:
                  'Add Class II · 21 CFR 888.3070 · product code MNH framing to OR-801 cover letter.',
              },
            ],
            since: '5h ago',
            owner: 'L. Tran',
      ...
      '11': {
        status: 'err',
        complete:
          'Predicate identification (K191822), comparison table, technological characteristics matrix.',
        findings: [
          {
            id: 'CE-01',
            severity: 'err',
            ...
            rule:
              'Every performance claim in the SE discussion must cite an evidence document whose status is "approved" — not "draft" or "review".',
            current:
              'Biocompat equivalence claim cites TR-OR801-014 · status = "review" · approver gate not crossed.',
            expected:
              'TR-OR801-014 must be approved (signed, locked) before §11 can be transmitted.',
            evidence: [
            ...
            fixes: [
              ...
              {
                kind: 'ask-claude',
                label: 'Suggest a workaround',
                prompt:
                  'CE-01 is gating OR-801 §11. The biocompat report is in review. What are options to unblock without waiting for the lock?',
              },
            ],
            since: '2h ago',
            owner: 'L. Tran',
          ...
          },
          {
            ...
            rule:
              'When a performance claim cites a test report, the report must be attached in §17 or §16 and the cross-reference must resolve.',
            current:
              'Pull-out force claim cites TR-OR801-009. Cross-reference points to §17 but no attachment exists at §17.path[3].',
        ...
        findings: [
          {
            ...
            current:
              'Test-article statement names supplier lot 22K but does not confirm equivalence to OR-801 finished device.',
        ...
        findings: [
          {
            ...
            current:
              '3 of 11 rows missing acceptance criterion (rows 4, 7, 9 — torsional yield, axial pull-out, insertion torque).',
        ...
        findings: [
          {
            ...
            fixes: [
              ...
              {
                kind: 'ask-claude',
                label: 'Draft justification to defer',
                prompt: 'Can DX-102 defer thyroxine/cortisol/troponin-I to a post-clearance amendment?',
              },
            ],
            since: '4h ago',
            owner: 'A. Patel',
  ...
  'CV-330': {
    ...
    summary:
      'Clinical evidence package blocks Module 5. SAP signature is the gating item.',
    sections: {
      '17': {
        status: 'err',
        ...
        findings: [
          {
            ...
            fixes: [
              ...
              {
                kind: 'ask-claude',
                label: 'Draft the rationale',
                prompt: 'Draft a clinical-equivalence rationale for BX-204 MARD 8.9 vs predicate 8.3.',
              },
            ],
            since: '2d ago',
            owner: 'J. Chen',
  ...
  'PM-660': {
    ...
    summary:
      'Cybersecurity volume is the long pole. SBOM completeness is being tracked under Q-Sub Q251187.',
    sections: {
      '10': {
        status: 'warn',
        ...
        findings: [
          {
            ...
            current:
              'SBOM lists 47 of 51 components. Missing: openssl 3.2.1, mbedtls 2.28, libcurl 8.4.0, busybox 1.36.',
        ...
        findings: [
          {
            ...
            rule:
              'Each DI-level GUDID record must carry every required PI attribute for the device class.',
            current:
              'PM-660 GUDID record missing 2 attributes: lot number indicator, manufacture date indicator.',
