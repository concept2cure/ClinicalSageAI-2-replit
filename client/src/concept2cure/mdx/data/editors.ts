 ...
 * Ported verbatim from design-system/ui_kits/mdx/data-editors.jsx.
 * Reuses block / comment / validation / message / quick-action types from
 * data/editor.ts (the 510(k) editor) since the shape is identical.
 */
...
export const PMA_EDITOR_PROGRAM: EditorProgram = {
  id: 'cv330',
...
export const PMA_EDITOR_SECTIONS: EditorSectionVolume[] = [
  { volume: 'Module 1 · Administrative', items: [
    { id: 'm1-1' as unknown as number, num: '1.1', label: 'Cover letter',          status: 'complete', required: true,  words: 0 },
    { id: 'm1-2' as unknown as number, num: '1.2', label: 'Table of contents',     status: 'complete', required: true,  words: 0 },
    { id: 'm1-3' as unknown as number, num: '1.3', label: 'Indications for use',   status: 'complete', required: true,  words: 0 },
    { id: 'm1-4' as unknown as number, num: '1.4', label: 'Device description',    status: 'review',   required: true,  words: 0 },
    { id: 'm1-5' as unknown as number, num: '1.5', label: 'Alternative practices', status: 'draft',    required: true,  words: 0 },
  ]},
  { volume: 'Module 2 · Summary of safety and effectiveness', items: [
    { id: 'm2-1' as unknown as number, num: '2.1', label: 'Summary of safety and effectiveness', status: 'draft',  required: true, words: 4810 },
    { id: 'm2-2' as unknown as number, num: '2.2', label: 'Risk analysis',                       status: 'review', required: true, words: 0 },
    { id: 'm2-3' as unknown as number, num: '2.3', label: 'Benefit-risk determination',          status: 'draft',  required: true, words: 0 },
  ]},
  { volume: 'Module 3 · Manufacturing', items: [
    { id: 'm3-1' as unknown as number, num: '3.1', label: 'Manufacturing site information', status: 'complete', required: true, words: 0 },
    { id: 'm3-2' as unknown as number, num: '3.2', label: 'Quality system summary',         status: 'review',   required: true, words: 0, blocker: true },
    { id: 'm3-3' as unknown as number, num: '3.3', label: 'Sterilization validation',       status: 'complete', required: true, words: 0 },
    { id: 'm3-4' as unknown as number, num: '3.4', label: 'Packaging and labeling',         status: 'draft',    required: true, words: 0 },
  ]},
  { volume: 'Module 4 · Non-clinical', items: [
    { id: 'm4-1' as unknown as number, num: '4.1', label: 'Bench performance testing',     status: 'complete', required: true,  words: 0 },
    { id: 'm4-2' as unknown as number, num: '4.2', label: 'Animal studies',                status: 'complete', required: true,  words: 0 },
    { id: 'm4-3' as unknown as number, num: '4.3', label: 'Biocompatibility',              status: 'complete', required: true,  words: 0 },
    { id: 'm4-4' as unknown as number, num: '4.4', label: 'Software verification',         status: 'review',   required: false, words: 0 },
    { id: 'm4-5' as unknown as number, num: '4.5', label: 'Electromagnetic compatibility', status: 'complete', required: false, words: 0 },
  ]},
  { volume: 'Module 5 · Clinical', items: [
    { id: 'm5-1' as unknown as number, num: '5.1', label: 'Clinical investigation summary',   status: 'draft', required: true, words: 6230 },
    { id: 'm5-2' as unknown as number, num: '5.2', label: 'Clinical data — primary endpoint', status: 'draft', required: true, words: 0, blocker: true },
    { id: 'm5-3' as unknown as number, num: '5.3', label: 'Adverse events and SAEs',          status: 'review',required: true, words: 0 },
    { id: 'm5-4' as unknown as number, num: '5.4', label: 'Statistical analysis plan',        status: 'complete', required: true, words: 0 },
    { id: 'm5-5' as unknown as number, num: '5.5', label: 'Conclusions',                      status: 'empty',    required: true, words: 0 },
  ]},
  { volume: 'Module 6 · Post-approval', items: [
    { id: 'm6-1' as unknown as number, num: '6.1', label: 'Post-approval study plan',  status: 'draft', required: true, words: 0 },
    { id: 'm6-2' as unknown as number, num: '6.2', label: 'Post-market surveillance',  status: 'draft', required: true, words: 0 },
    { id: 'm6-3' as unknown as number, num: '6.3', label: 'Tracking and reporting',    status: 'empty', required: true, words: 0 },
  ]},
];

const pmaM5_1Blocks: EditorBlock[] = [
  ...
  { id: 'pb4', kind: 'table', confidence: 0.94,
  ...
  { id: 'pb6', kind: 'p', confidence: 0.71,
    ...
    spans: [{ t: 'A total of 47 device- or procedure-related adverse events were reported across 612 subjects (7.7%). Of these, 18 were serious (2.9%), with the most common being implant-site infection (n = 9) and lead dislodgement (n = 5). Adjudication by an independent committee determined that one death was possibly device-related and 17 were unrelated. All adjudication results are tabulated in §5.3.' }],
  },
  ...
  { id: 'pb8', kind: 'p', confidence: 0.62,
    ...
    spans: [{ t: 'Pre-specified subgroup analysis demonstrated consistent performance across age, sex, and BMI strata, with no subgroup showing detection sensitivity below 92%. Geographic subgroup performance (US vs ex-US) was equivalent within the pre-specified non-inferiority margin.' }],
  },
  ...
  { id: 'pb10', kind: 'p', confidence: 0.89,
    ...
    spans: [{ t: 'The CV-330 pivotal investigation met its primary endpoint with statistical and clinical significance. Safety findings were consistent with the device class and manageable through standard implant practice. The benefit-risk profile supports approval for the proposed indication.' }],
  },
];

export const PMA_EDITOR_CONTENT: Record<string, EditorContent & { wordCount?: number }> = {
  'm5-1': {
    id: 51,
    num: '5.1',
    label: 'Clinical investigation summary',
    status: 'draft',
    masthead: [
    ...
    blocks: pmaM5_1Blocks,
    wordCount: 6230,
  },
};

export const PMA_EDITOR_VALIDATION: Record<string, EditorValidation[]> = {
  'm5-1': [
...
export const PMA_EDITOR_COMMENTS: EditorComment[] = [
...
export const PMA_EDITOR_SEED: EditorMessage[] = [
...
export const PMA_EDITOR_QUICK: EditorQuickAction[] = [
...
export const CER_EDITOR_PROGRAM: EditorProgram = {
  id: 'iv415',
  title: 'IV-415 Companion Diagnostic',
  code: 'IV-415',
  productCode: '',
  regulation: 'EU MDR Annex XIV Part A',
  predicate: '',
  lead: '',
...
export const CER_EDITOR_SECTIONS: EditorSectionVolume[] = [
  { volume: 'Part A · Scope', items: [
    { id: 'cs1' as unknown as number, num: '§1', label: 'Scope and device description',           status: 'complete', required: true, words: 0 },
    { id: 'cs2' as unknown as number, num: '§2', label: 'Intended purpose and target population', status: 'complete', required: true, words: 0 },
    { id: 'cs3' as unknown as number, num: '§3', label: 'Equivalence claim',                      status: 'review',   required: true, words: 0 },
  ]},
  { volume: 'Part B · Clinical evaluation', items: [
    { id: 'cs4' as unknown as number, num: '§4', label: 'State-of-the-art analysis', status: 'complete', required: true, words: 5210 },
    { id: 'cs5' as unknown as number, num: '§5', label: 'Clinical data summary',     status: 'draft',    required: true, words: 7480 },
    { id: 'cs6' as unknown as number, num: '§6', label: 'Safety and risk-benefit',   status: 'draft',    required: true, words: 4360, blocker: true },
    { id: 'cs7' as unknown as number, num: '§7', label: 'Conformity to GSPR',        status: 'review',   required: true, words: 0 },
  ]},
  { volume: 'Part C · Conclusions and PMS', items: [
    { id: 'cs8'  as unknown as number, num: '§8',  label: 'Conclusions',                          status: 'empty', required: true, words: 0 },
    { id: 'cs9'  as unknown as number, num: '§9',  label: 'Post-market surveillance plan',        status: 'draft', required: true, words: 0 },
    { id: 'cs10' as unknown as number, num: '§10', label: 'Post-market clinical follow-up plan',  status: 'draft', required: true, words: 0 },
  ]},
  { volume: 'Annexes', items: [
    { id: 'cs11' as unknown as number, num: 'A', label: 'Literature search protocol',           status: 'complete', required: false, words: 0 },
    { id: 'cs12' as unknown as number, num: 'B', label: 'Excluded literature log',              status: 'complete', required: false, words: 0 },
    { id: 'cs13' as unknown as number, num: 'C', label: 'Risk management file cross-reference', status: 'review',   required: false, words: 0 },
  ]},
];

const cerCs6Blocks: EditorBlock[] = [
  { id: 'cb1', kind: 'p', confidence: 0.91,
    ...
    spans: [
      ...
      { cite: 'FR-8812' }, { t: ', ' }, { cite: 'FR-8802' }, { t: ', ' }, { cite: 'LIT-2241' }, { t: ', ' }, { cite: 'LIT-2203' },
      { t: '), one was excluded as non-representative (' }, { cite: 'FR-8784' },
      { t: '), and two remain under adjudication (' }, { cite: 'FR-8809' }, { t: ', ' }, { cite: 'EU-4412' }, { t: ').' },
    ],
  },
  { id: 'cb2', kind: 'h2', text: 'Included safety signals' },
  { id: 'cb3', kind: 'table', confidence: 0.93,
  ...
  { id: 'cb9', kind: 'p', confidence: 0.84,
    ...
    spans: [{ t: 'On the basis of included signals, equivalence-derived data, and the demonstrated clinical performance, the benefit-risk profile of the IV-415 Companion Diagnostic supports its intended purpose under the conditions of use stated in §2. Residual uncertainty is addressed through the post-market surveillance and PMCF plans in §9 and §10.' }],
  },
];

export const CER_EDITOR_CONTENT: Record<string, EditorContent & { wordCount?: number }> = {
  'cs6': {
    id: 6,
    num: '§6',
    label: 'Safety and risk-benefit',
    status: 'draft',
    masthead: [
...
export const CER_EDITOR_VALIDATION: Record<string, EditorValidation[]> = {
  'cs6': [
...
export const CER_EDITOR_COMMENTS: EditorComment[] = [
...
export const CER_EDITOR_SEED: EditorMessage[] = [
...
export const CER_EDITOR_QUICK: EditorQuickAction[] = [
