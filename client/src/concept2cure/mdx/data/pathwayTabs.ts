/**
 * Pathway sub-tab fixtures — ported verbatim from
 * `design-system/ui_kits/mdx/data-pathway-tabs.jsx`.
 *
 * The four pathway sub-tabs (Workspace · Audit · Correspondence · Approvals)
 * share these fixtures. Pathway-specific where the data is pathway-specific
 * (correspondence kinds: RTA = 510(k), Day-100 = PMA, NB Q&A = CER),
 * pathway-agnostic where the shape is the same (audit log, approvals).
 *
 * All times are ISO 8601; the UI formats them. The `chain` helper synthesizes
 * 8-char SHA-256 prefixes that line up across rows so users can verify the
 * chain visually. Real backend hashes are full SHA-256 over (prev_hash || event_json).
 */
...
import type { ProgramPathway } from './programs';
...
function chain(seed: number, n: number): ChainEntry[] {
  const out: ChainEntry[] = [];
  let prev = seed;
  for (let i = 0; i < n; i++) {
    const cur = (Math.imul(prev, 2654435761) + 0x9e3779b9) >>> 0;
    out.push({
      hash:
        cur.toString(16).padStart(8, '0') +
        '…' +
        (cur ^ 0xa5a5a5a5).toString(16).padStart(4, '0'),
      prev: (prev >>> 0).toString(16).padStart(8, '0'),
    });
    prev = cur;
  }
  return out;
...
const K510_AUDIT_SEED: AuditSeed[] = [
...
const PMA_AUDIT_SEED: AuditSeed[] = [
...
const CER_AUDIT_SEED: AuditSeed[] = [
...
function attachChain(seed: AuditSeed[], hashes: ChainEntry[]): AuditEvent[] {
  return seed.map((row, i) => ({ ...row, hash: hashes[i].hash, prev: hashes[i].prev }));
}

export const K510_AUDIT: AuditEvent[] = attachChain(K510_AUDIT_SEED, K510_AUDIT_HASHES);
export const PMA_AUDIT: AuditEvent[]  = attachChain(PMA_AUDIT_SEED,  PMA_AUDIT_HASHES);
export const CER_AUDIT: AuditEvent[]  = attachChain(CER_AUDIT_SEED,  CER_AUDIT_HASHES);

/* Pathway-specific correspondence kinds:
...
export const K510_CORRESP: Correspondence[] = [
...
export const PMA_CORRESP: Correspondence[] = [
...
export const CER_CORRESP: Correspondence[] = [
...
/* Approvals — workflow tasks awaiting Part 11 e-sign. Pathway-agnostic shape. */

export const K510_APPROVALS: Approval[] = [
...
export const PMA_APPROVALS: Approval[] = [
...
export const CER_APPROVALS: Approval[] = [
...
export const PATHWAY_TABS_DATA: Record<ProgramPathway, PathwayTabsBundle> = {
