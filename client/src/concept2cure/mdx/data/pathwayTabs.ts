/**
 * Pathway sub-tab data — Audit · Correspondence · Approvals.
 * Ported from `design-system/ui_kits/mdx/data-pathway-tabs.jsx`.
 *
 * Consumed by `surfaces/pathway/PathwayPanes.tsx` (audit / correspondence /
...
function chain(seed: number, n: number): Array<{ hash: string; prev: string }> {
  const out: Array<{ hash: string; prev: string }> = [];
  let prev = seed;
  for (let i = 0; i < n; i++) {
...
const K510_AUDIT_BASE: AuditSeed[] = [
...
const PMA_AUDIT_BASE: AuditSeed[] = [
...
const CER_AUDIT_BASE: AuditSeed[] = [
