...
type TabId = 'about' | 'platform' | 'intelligence' | 'training' | 'dr-sage';
  ...
  return (
    <div className="space-y-8">
      <div className="text-center max-w-2xl mx-auto mb-2">
        ...
        <p className="text-sm text-zinc-500">
          Master every capability of the platform. From your first conversation with AnA
          to advanced submission operations — learn at your own pace.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {filtered.map(mod => {
          ...
          return (
            <div
              key={mod.id}
              className="bg-white rounded-xl border border-zinc-200 p-5 hover:shadow-md hover:border-zinc-300 transition-all cursor-pointer group"
            >
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <h4 className="text-sm font-semibold text-zinc-900 truncate">{mod.title}</h4>
                  </div>
                  <p className="text-xs text-zinc-500 leading-relaxed mb-3">{mod.description}</p>
                ...
                <div className="flex-shrink-0 self-center">
                  <div className="w-8 h-8 rounded-full bg-zinc-50 flex items-center justify-center group-hover:bg-violet-100 transition-colors">
                    <Play className="w-4 h-4 text-zinc-400 group-hover:text-violet-600 transition-colors" />
                  </div>
                </div>
              </div>
            </div>
      ...
      <div className="bg-white rounded-2xl border border-zinc-200 p-8">
        ...
        <p className="text-sm text-zinc-500 mb-5">
          AnA can answer any question about the platform, your current project, or regulatory strategy.
          Just go to any chat and ask.
        </p>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          {[
            'How do I start a new IND submission?',
            'What does Snow Globe predict?',
            'How do I configure Sentinel scanning?',
            'What eCTD sections does Co-Author support?',
            'How do I link evidence to claims?',
...
function DrSageTab() {
  ...
  const handleSend = useCallback((text: string) => {
    ...
    // Simulate Dr. Sage thinking and responding
    setTimeout(() => {
      const sageMsg: SageMessage = {
        id: `sage-${Date.now()}`,
        role: 'sage',
        content: `That's a great question. Based on my experience reviewing thousands of submissions at FDA, here's my perspective:\n\n${text.toLowerCase().includes('review') ? 'From a reviewer standpoint, the key things we look for are completeness, consistency, and scientific rigor. ' : ''}${text.toLowerCase().includes('train') ? 'Training is critical — I\'ve seen too many submissions fail because teams didn\'t understand the regulatory expectations. ' : ''}${text.toLowerCase().includes('fix') || text.toLowerCase().includes('issue') ? 'Let me work with AnA to identify the root cause. Often what looks like a documentation issue is actually a data gap that needs to be addressed upstream. ' : ''}I can provide more specific guidance if you tell me about your product type and target indication. Or use one of the quick actions below to get started with a specific workflow.\n\nRemember — I'm here to give you the reviewer's perspective. AnA handles the writing, I handle the "would this pass review?" question.`,
        agentAction: text.toLowerCase().includes('fix') || text.toLowerCase().includes('ana') ? {
          type: 'consult-ana',
          label: 'Consulting with AnA...',
          status: 'complete',
        } : undefined,
        timestamp: new Date(),
      };
