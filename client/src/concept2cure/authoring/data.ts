/**
 * Phase 9 — Universal Authoring · data + types.
 *
 * Ported verbatim from ui_kits/authoring/data.jsx. One canonical document
 * model + (doc_type × agency) rule packs. The fixture shapes are the
 * live-data contract for the Phase 9 backend (c2c_documents family +
 * c2c_rule_packs); the surface renders these until those routes land.
 *
 * @module client/src/concept2cure/authoring/data
 */

import type { AuthIconKey } from './icons';

/* ───────── Types ───────── */
export interface AuthAgency { id: string; label: string; full: string; region: string; esubmit: string; }
export interface AuthDocType { id: string; label: string; family: string; defaultAgencies: string[]; icon: AuthIconKey; }
...
export interface Prov { source: string; model: string; confidence: number; auditId: string; foot: string; }
export interface Citation { ev: string; span?: string; }
export interface Paragraph { id: string; text: string; tag?: string; prov?: Prov; citations?: Citation[]; }
export interface SectionBody { heading: string; subheading?: string; paragraphs: Paragraph[]; }

export interface Evidence { id: string; kind: string; title: string; meta: string; tag: string; conf: number; }
export interface Reviewer { id: string; name: string; role: string; tone: string; }
export interface Program { code: string; title: string; indication: string; sponsor: string; }

export type GateSeverity = 'err' | 'warn' | 'info';
export interface Gate {
  id: string; match: string; sev: GateSeverity; category: string;
  title: string; desc: string; fix: string | null; apply: string | null;
  paragraphId?: string;
}
export interface Skill { id: string; label: string; hint: string; }
export interface SlashCommand { id: string; label: string; hint: string; }

export type Block =
  | { kind: 'p'; text: string }
  | { kind: 'tool'; label: string; value: string }
  | { kind: 'stream'; target: string; hint: string }
  | { kind: 'chip'; title: string; meta: string };

export type AuthMessage =
  | { role: 'user'; text: string; pre?: string }
  | { role: 'ai'; blocks: Block[] };

/* ───────── Agencies — first-class rule packs at launch ───────── */
export const AUTH_AGENCIES: AuthAgency[] = [
...
/* ───────── Document types ───────── */
export const AUTH_DOC_TYPES: AuthDocType[] = [
...
export const AUTH_OUTLINES: Record<string, AuthSection[]> = {
  'ind:fda': [
...
/* ───────── Reviewers / owners ───────── */
export const AUTH_REVIEWERS: Reviewer[] = [
...
/* ───────── Programs ───────── */
export const AUTH_PROGRAMS: Program[] = [
...
/* ───────── Sample sections ───────── */
export const AUTH_SECTION_BODY: Record<string, SectionBody> = {
  'm2.5': {
    ...
    paragraphs: [
      ...
      { id: 'p2', text: 'BX-204 is a humanized IgG1κ monoclonal antibody targeting the extracellular domain of receptor tyrosine kinase X (RTK-X), engineered with Fc-silencing mutations (L234A/L235A) to abrogate effector function and developed for adults with advanced or metastatic RTK-X–overexpressing solid tumors.',
        prov: { source: 'CSR-099, IB v3.1, IND module 2.7.4', model: 'c2c-Opus 4.7', confidence: 0.94, auditId: 'aud_8b21f', foot: 'Drafted from IND §2.7.4 · 2 sources · CFR-11 chain verified' },
      ...
      { id: 'p4', text: 'Efficacy was evaluated in BX204-201, a pivotal, multicenter, open-label, single-arm Phase II trial enrolling 184 adults with RTK-X-positive advanced solid tumors refractory to ≥1 prior systemic therapy. The primary endpoint was confirmed objective response rate (ORR) assessed by blinded independent central review per RECIST v1.1.',
        prov: { source: 'CSR-201, SAP v3, Protocol BX204-201', model: 'c2c-Opus 4.7', confidence: 0.95, auditId: 'aud_8b22a', foot: 'Drafted from CSR-201 §6.1 · 3 sources · 1 reviewer edit' },
      ...
      { id: 'p5', text: 'The confirmed ORR of 38.6% (95% CI 31.5–46.0) exceeded the pre-specified threshold of 25% derived from an indication-matched pooled historical control of 412 patients across five sponsor-independent datasets (2019–2023). The magnitude and consistency of effect across pre-specified subgroups, together with the FDA 2023 oncology bridging guidance and precedent EMEA/H/C/005612, support the benefit-risk case for accelerated approval.',
        prov: { source: 'CSR-201, FDA 2023 bridging guidance, EMA precedent', model: 'c2c-Opus 4.7', confidence: 0.92, auditId: 'aud_8b22b', foot: 'Strengthened against RIM precedent 0.89 · 2025-04-12' },
      ...
      { id: 'p7', text: 'Across the 184-subject pivotal cohort and the supportive Phase I population (n=42), the most common treatment-emergent adverse events were grade 1–2 infusion reactions (24.5%), fatigue (18.5%) and rash (11.4%). Grade ≥3 events related to study drug occurred in 6.5% of subjects; one treatment-related grade 5 event (immune-mediated pneumonitis) was reported.',
        prov: { source: 'CSR-201 §10.4, IB v3.1', model: 'c2c-Opus 4.7', confidence: 0.88, auditId: 'aud_8b22c', foot: 'Needs reviewer attention · pneumonitis disclosure pending' },
      ...
      { id: 'p9', text: 'The clinical evidence supports a favorable benefit-risk profile for BX-204 in the proposed indication. The accelerated approval pathway is recommended on the basis of a confirmed ORR that meets the pre-specified threshold, a safety profile consistent with mechanism, and durable responses in the post-platinum subset.',
        prov: { source: 'CSR-201 §13, RIM precedent', model: 'c2c-Opus 4.7', confidence: 0.86, auditId: 'aud_8b22d', foot: 'Author-edited · last reviewed 11 minutes ago' },
  ...
  'c2': {
    ...
    paragraphs: [
      ...
      { id: 'p2', text: 'The proposed BX-D2 cardiac monitor is substantially equivalent to the legally marketed predicate K221456 (PrediCare CM-100, cleared 2022-08-19), and to the reference device K201123 for software class B claims. Both share the intended use of continuous ambulatory single-lead ECG monitoring in adults.',
        prov: { source: 'K221456 510(k) summary, FDA product code DSI', model: 'c2c-Opus 4.7', confidence: 0.93, auditId: 'aud_d29a', foot: 'Drafted from predicate match · RIM 0.91' } },
      { id: 'p3', text: 'Technological characteristics are equivalent in indications for use, anatomical site, energy source (rechargeable Li-ion), sampling rate (250 Hz), and sensor topology. The only meaningful difference is the on-device arrhythmia detection algorithm — addressed in Section C2 of this submission via bench data and the special controls invoked by IEC 62304 Class B.',
        prov: { source: 'Bench protocol BP-D2-001, IEC 62304 §5', model: 'c2c-Opus 4.7', confidence: 0.9, auditId: 'aud_d29b', foot: 'Cross-checks IEC 62304 Class B controls' },
...
/* ───────── Seed conversation ───────── */
export const AUTH_SEED_THREAD: AuthMessage[] = [
...
/* ───────── Compliance gates ───────── */
export const AUTH_GATES: Record<string, Gate[]> = {
  p2: [
    { id: 'g1', match: 'L234A/L235A', sev: 'info', category: 'consistency',
      ...
      fix: 'Standardize on "L234A/L235A (LALA)" across both sections', apply: 'L234A/L235A (LALA)' },
  ],
  p4: [
    { id: 'g2', match: '184 adults', sev: 'err', category: 'number',
      ...
      fix: 'Update to "186 adults dosed (184 efficacy-evaluable per protocol)"', apply: '186 adults dosed (184 efficacy-evaluable per protocol)' },
    { id: 'g3', match: '≥1 prior', sev: 'warn', category: 'consistency',
      ...
      fix: 'Add "platinum-based" qualifier', apply: '≥1 prior platinum-based systemic therapy' },
  ],
  p5: [
    { id: 'g4', match: '38.6%', sev: 'info', category: 'evidence',
      title: 'Source anchored',
      desc: 'ORR 38.6% verified against CSR-201 §6.2 Table 14.2.1 and the matched SAP §9.4 analysis plan.',
      fix: null, apply: null },
    { id: 'g5', match: 'pre-specified threshold of 25%', sev: 'err', category: 'citation',
      ...
      fix: 'Add citation to FDA 2024 §V.B alongside FDA 2023', apply: 'pre-specified threshold of 25% (FDA 2024 oncology accelerated-approval guidance §V.B, consistent with FDA 2023 bridging guidance §IV.C)' },
    { id: 'g6', match: 'EMA precedent EMEA/H/C/005612', sev: 'warn', category: 'evidence',
      ...
      fix: 'Add PMDA Y-203 as supporting precedent', apply: 'EMA precedent EMEA/H/C/005612 and PMDA consultation Y-203' },
  ],
  p7: [
    { id: 'g7', match: 'grade 5 event', sev: 'err', category: 'consistency',
      ...
      fix: 'Mark as "pending adjudication" until PV signal s2 closes', apply: 'one treatment-related event with a fatal outcome (Grade 5, pending final adjudication)' },
  ],
};

export const AUTH_GATE_CATEGORIES: Record<string, string> = {
  number:      'Number check',
...
/* ───────── Slash commands ───────── */
export const AUTH_SLASH_COMMANDS: SlashCommand[] = [
...
/* ───────── Skills / tool palette ───────── */
export const AUTH_SKILLS: Skill[] = [
...
/* ───────── Outline helpers ───────── */
export function resolveOutline(docType: string, agency: string): AuthSection[] {
...
export function findNode(outline: AuthSection[], id: string): AuthSection | null {
...
export function firstLeaf(outline: AuthSection[]): AuthSection | null {
  for (const n of outline) {
    if (n.children) {
...
export type StatusCounts = { todo: number; drafted: number; review: number; approved: number; locked: number; fail: number };
export function countLeaves(nodes: AuthSection[]): StatusCounts {
  const counts: StatusCounts = { todo: 0, drafted: 0, review: 0, approved: 0, locked: 0, fail: 0 };
  const walk = (arr: AuthSection[]) => {
    for (const n of arr) {
      if (n.children) walk(n.children);
      else {
        const s = (n.status || 'todo') as keyof StatusCounts;
...
export interface LeafWithParent extends AuthSection { parent?: AuthSection | null; }
export function flattenLeaves(nodes: AuthSection[], parent: AuthSection | null = null, out: LeafWithParent[] = []): LeafWithParent[] {
...
export const evMap = (): Record<string, Evidence> =>
  Object.fromEntries(AUTH_EVIDENCE.map((e) => [e.id, e]));
