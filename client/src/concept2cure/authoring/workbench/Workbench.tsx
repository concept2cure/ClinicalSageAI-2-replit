/**
 * Universal Authoring — workbench surface (section table + inspector).
 * Port of ui_kits/authoring/Workbench.jsx.
 *
 * @module client/src/concept2cure/authoring/workbench/Workbench
 */

import * as React from 'react';
import { I } from '../icons';
import {
  AUTH_REVIEWERS, flattenLeaves,
  type AuthSection, type AuthDocType, type Evidence, type Reviewer,
} from '../data';
import { Chat, type ChatProps } from '../conversation/Conversation';
...
const STATUS_UPDATED: Record<string, string> = {
  approved: '3 days ago',
  review: 'Today',
  drafted: 'Yesterday',
  locked: '6 days ago',
...
const reviewerById = (id?: string): Reviewer | undefined => AUTH_REVIEWERS.find((r) => r.id === id);

function ownerCell(ownerId?: string) {
  if (!ownerId) {
    return (
...
export function WorkbenchTable({ outline, docType, agencyLabel, activeId, onPick, onDraft }: WorkbenchTableProps) {
  const leaves = React.useMemo(() => flattenLeaves(outline), [outline]);
  ...
  return (
    <div className="au-wb">
      ...
      <div className="au-wb-table">
        ...
        {leaves.map((n) => (
          <div key={n.id} className="au-wb-row" data-active={n.id === activeId || undefined} onClick={() => onPick(n.id)}>
...
function EvidencePane({ items, sectionMeta }: { items: Evidence[]; sectionMeta: AuthSection }) {
  return (
    <div>
...
function ReviewersPane({ reviewers, sectionMeta }: { reviewers: Reviewer[]; sectionMeta: AuthSection }) {
  const owner = sectionMeta.owner ? reviewerById(sectionMeta.owner) : undefined;
  return (
    <div>
      ...
      {sectionMeta.owner
        ? (
          <div className="au-rev-row">
            ...
            <div>
              <div className="name">{owner ? owner.name : sectionMeta.owner}</div>
              <div className="role">{owner ? owner.role : '—'}</div>
            </div>
...
export function Inspector({ tab, setTab, sectionMeta, evidence, reviewers, chatProps }: InspectorProps) {
  return (
    <aside className="au-inspector">
      ...
      <div className="au-insp-body" style={tab === 'ana' ? { padding: 0, display: 'flex', flexDirection: 'column' } : undefined}>
        {tab === 'ana' && <Chat {...chatProps} hint="Ask AnA · scoped to this section" />}
