    ...
    <div className="flex-1 min-h-0 flex flex-col">
      <div className="shrink-0 border-b border-zinc-200 bg-zinc-50/70 px-3 py-2">
        <div className="flex items-center justify-between gap-2">
          <div>
            <h3 className="text-xs font-semibold text-zinc-800">{LAYER_META[osLayer].title}</h3>
            <p className="text-[11px] text-zinc-500">{LAYER_META[osLayer].subtitle}</p>
          </div>
...
function WorkbenchButton({
  ...
  return (
    <button
      onClick={onClick}
      className={cn(
        'flex items-center gap-1 rounded-md border border-zinc-200 bg-white px-2 py-1 text-[11px] text-zinc-600',
        'hover:text-zinc-800 hover:border-zinc-300 hover:bg-zinc-50 transition-colors'
      )}
    >
      <Icon className="w-3 h-3" />
      {label}
      <FileStack className="w-3 h-3 text-zinc-400" />
    </button>
  );
}
