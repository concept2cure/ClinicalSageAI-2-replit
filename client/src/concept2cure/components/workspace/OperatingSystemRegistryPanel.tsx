...
export function OperatingSystemRegistryPanel({
  ...
  return (
    <div className="flex-1 min-h-0 flex flex-col">
      <div className="border-b border-zinc-200 bg-zinc-50/60 px-2 py-2 space-y-2">
              ...
              <button
                ...
                className={cn(
                  ...
                  activeRegistry === kind
                    ...
                    : 'bg-transparent border-transparent text-zinc-500 hover:text-zinc-700 hover:bg-white/70'
                )}
              >
        ...
        <label className="flex items-center gap-1.5 px-2 py-1.5 rounded-md bg-white border border-zinc-200">
      ...
      <div className="px-2 py-1.5 border-b border-zinc-200 text-[11px] text-zinc-500 bg-white">
        <div className="font-medium text-zinc-700">{REGISTRY_META[activeRegistry].label}</div>
      ...
      <div className="flex-1 overflow-y-auto p-1.5 space-y-1">
        {registryRows.map(row => (
          <div key={row.id} className="rounded-md border border-zinc-200 bg-white px-2 py-1.5">
            <div className="text-xs font-medium text-zinc-800 truncate">{row.title}</div>
        ...
        {registryRows.length === 0 && (
          <div className="text-xs text-zinc-400 p-3 text-center">No records match this registry filter.</div>
        )}
      </div>
    </div>
  );
