      ...
      <div className="px-4 py-3 border-b border-stone-200 bg-stone-50">
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="flex items-center gap-1.5">
            ...
            {onClose && (
              <button onClick={onClose} aria-label="Close memory panel" title="Close" className="p-1.5 rounded-md hover:bg-white/60 transition-colors duration-150 focus-visible:ring-2 focus-visible:ring-stone-400 outline-none">
      ...
      <div className="flex-1 overflow-y-auto">
        {filteredEntries.length === 0 ? (
          <div className="text-center py-12">
            ...
            <h3 className="text-sm font-semibold text-stone-600 mb-1">No memories yet</h3>
          ...
          Object.entries(grouped).map(([category, items]) => {
            ...
            return (
              <div key={category} className="border-b border-stone-100">
                <button
                  ...
                  <div className="px-4 pb-2 space-y-1.5">
                    {items.map(entry => (
                      <div
                        key={entry.id}
                        className={cn(
                          'p-2.5 rounded-lg border border-stone-100 text-xs group hover:border-stone-200 transition-colors duration-150',
                          entry.source === 'auto' ? 'bg-stone-50/50' : 'bg-white',
                        )}
                      >
                        {editingId === entry.id ? (
                          /* Edit mode */
                          ...
                          /* Display mode */
                          <>
                            <p className="text-stone-700 leading-relaxed">{entry.content}</p>
                            <div className="flex items-center justify-between mt-1.5">
                              <div className="flex items-center gap-1.5">
                                ...
                                {entry.source === 'auto' && (
                                  <span className="px-1 py-0.5 rounded bg-stone-100 text-[10px] text-stone-400">
                                    auto
                                  </span>
                                )}
                                {entry.tags.map(tag => (
