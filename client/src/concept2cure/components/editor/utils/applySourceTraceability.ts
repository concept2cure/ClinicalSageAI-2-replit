...
function processTextWithSources(
  ...
  return segments
    .map((seg) => {
      ...
      return `<span data-traceability="true" data-source-id="${escapeAttr(sourceId)}" data-source-hash="${escapeAttr(sourceHash)}" data-link-id="${escapeAttr(linkId)}" class="traceability-link bg-blue-100 border-b-2 border-blue-500 cursor-pointer hover:bg-blue-200" title="Source: ${escapeAttr(seg.sourceRef.name || seg.sourceRef.title || sourceId)}">${seg.text}</span>`;
    })
    .join('');
}
