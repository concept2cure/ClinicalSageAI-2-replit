  ...
  return (
    <div className="h-full overflow-y-auto p-4 space-y-4 bg-white">
      ...
      <section className="space-y-2">
        <h4 className="text-xs font-semibold uppercase tracking-wide text-stone-500">Integration checks</h4>
        {checks.map(check => (
          <div
            key={check.id}
        ...
        {capabilities.map(item => (
          <Card key={item.id}>
            <CardContent className="pt-4 space-y-2">
              <div className="text-xs font-semibold text-stone-900">{item.capability}</div>
              <div className="grid grid-cols-3 gap-2 text-[11px]">
                <div>
                  <div className="text-stone-500 mb-1">Weave Bio</div>
                  {competitorBadge(item.weaveBio)}
                </div>
                <div>
                  <div className="text-stone-500 mb-1">Artos</div>
                  {competitorBadge(item.artos)}
                </div>
                <div>
                  <div className="text-stone-500 mb-1">Concept2Cure</div>
                  {competitorBadge(item.c2c)}
                </div>
              </div>
              {item.gapSummary && <p className="text-[11px] text-stone-500">{item.gapSummary}</p>}
            ...
            <Card key={item.id}>
              <CardContent className="pt-4 space-y-2">
                <div className="flex items-center justify-between">
                  <div className="text-xs font-semibold text-stone-900">{item.title}</div>
      ...
      <section className="space-y-2">
        <h4 className="text-xs font-semibold uppercase tracking-wide text-stone-500">Use-case models</h4>
        {models.map(model => (
          <div key={model.name} className="rounded-lg border border-stone-200 p-3 bg-stone-50/60">
            <div className="flex items-center gap-2">
              <GitBranch className="w-3.5 h-3.5 text-stone-500" />
              <span className="text-xs font-semibold text-stone-900">{model.name}</span>
            </div>
            <p className="text-xs text-stone-600 mt-1">{model.objective}</p>
            <p className="text-[11px] text-stone-500 mt-1">Owner: {model.owner}</p>
            <div className="mt-2 flex items-center gap-1.5 flex-wrap">
              {model.path.map((step, idx) => (
                <React.Fragment key={`${model.name}-${step}`}>
                  <span className="px-2 py-0.5 rounded bg-white border border-stone-200 text-[11px] text-stone-700">
                    {step}
                  </span>
                  {idx < model.path.length - 1 && <Link2 className="w-3 h-3 text-stone-300" />}
                </React.Fragment>
              ))}
            </div>
          </div>
