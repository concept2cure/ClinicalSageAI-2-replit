...
export const ToolExecutionBlock: React.FC<ToolExecutionBlockProps> = ({
  ...
  return (
    <div className="my-2">
      ...
      >
        ...
        {hasContent && (
          <ChevronRight className={cn(
            'w-3.5 h-3.5 text-zinc-300 transition-transform flex-shrink-0',
            expanded && 'rotate-90'
          )} />
        )}
      </button>
      ...
      {/* Expanded content */}
      {expanded && hasContent && (
        <div className="ml-8 mt-1 mb-2 px-3 py-2 bg-zinc-50 rounded-lg text-xs text-zinc-600 font-mono whitespace-pre-wrap max-h-48 overflow-y-auto">
          {input && (
            <div className="mb-2">
              <span className="text-zinc-400 font-sans text-[10px] uppercase tracking-wider">Input</span>
              <div className="mt-0.5">{input}</div>
            </div>
          )}
          {result && (
            <div>
              <span className="text-zinc-400 font-sans text-[10px] uppercase tracking-wider">Result</span>
...
export const ThinkingBlock: React.FC<ThinkingBlockProps> = ({
  ...
  return (
    <div className="mb-3">
      ...
      {expanded && (
        <div className="mt-2 pl-4 border-l-2 border-zinc-200 text-sm text-zinc-500 leading-relaxed whitespace-pre-wrap max-h-64 overflow-y-auto">
          {content}
        </div>
      )}
    </div>
