    ...
    <div className="space-y-4">
      ...
      <div className="flex items-start justify-between">
        ...
        <button
          ...
          className="shrink-0 text-xs px-2 py-1 rounded border border-stone-300 hover:bg-stone-50 disabled:opacity-50"
        >
      ...
      <div className="grid grid-cols-4 gap-2">
        {[
          ...
          <div
            key={m.label}
            className="rounded-lg bg-stone-50 p-2 text-center"
          >
      ...
      {snapshot.changes.length > 0 && (
        <div>
          <h4 className="text-xs font-medium uppercase text-stone-500 mb-2">
            Recent Changes
          </h4>
          <div className="space-y-1">
            {snapshot.changes.slice(0, 5).map((change, i) => (
              <div key={i} className="flex items-center gap-2 text-sm">
                ...
                <span className="text-stone-700">{change.description}</span>
      ...
      {snapshot.newlyReady.length > 0 && (
        <div>
          <h4 className="text-xs font-medium uppercase text-stone-700 mb-2">
            Newly Ready
          </h4>
          <div className="space-y-1">
            {snapshot.newlyReady.map((item) => (
              <div key={`${item.type}-${item.id}`} className="flex items-center gap-2 text-sm text-stone-800">
      ...
      {snapshot.needsAttention.length > 0 && (
        <div>
          <h4 className="text-xs font-medium uppercase text-stone-600 mb-2">
            Needs Attention
          </h4>
          <div className="space-y-1">
          ...
          <div className="space-y-1">
            {snapshot.nextActions.slice(0, 3).map((action) => (
              <div key={action.id} className="flex items-center gap-2 text-sm">
                ...
                <span className="text-stone-700">{action.suggestedAction}</span>
              </div>
            ))}
          </div>
        </div>
