...
const CLAIM_PATTERNS: Array<{
  ...
  // Efficacy claims
  { pattern: /\b(demonstrated?|showed?|achieved?)\s+\w+\s+(efficacy|effectiveness|performance)/gi, type: 'efficacy', confidence: 90 },
  { pattern: /\b(\d+\.?\d*)\s*%\s*(efficacy|success\s+rate|response\s+rate)/gi, type: 'efficacy', confidence: 95 },
  
  // Safety claims
  { pattern: /\b(no|zero|minimal|low)\s+(adverse|side)\s+(events?|effects?|reactions?)/gi, type: 'safety', confidence: 90 },
  { pattern: /\b(safe|well-tolerated|tolerable)\s+(for|in|when)/gi, type: 'safety', confidence: 85 },
  
  // Design claims
  { pattern: /\b(designed?|manufactured?|constructed?)\s+(with|using|from)\s+/gi, type: 'design', confidence: 80 },
  { pattern: /\b(titanium|stainless\s+steel|polymer|silicone)\s+(housing|body|component)/gi, type: 'design', confidence: 85 },
  
  // Regulatory claims
  { pattern: /\b(FDA\s*cleared?|510\(k\)|CE\s+marked?|approved?)/gi, type: 'regulatory', confidence: 95 },
  { pattern: /\bpredicate\s+device/gi, type: 'regulatory', confidence: 90 },
  
  // Clinical claims
  { pattern: /\b(clinical\s+)?(trial|study|investigation)\s+(demonstrated?|showed?|found)/gi, type: 'clinical', confidence: 90 },
  { pattern: /\b(patients?|subjects?|participants?)\s+(showed?|demonstrated?|experienced?)/gi, type: 'clinical', confidence: 85 },
  
  // Statistical claims
  { pattern: /\bp\s*[<>=]\s*0\.\d+/gi, type: 'statistical', confidence: 95 },
  { pattern: /\b(\d+\.?\d*)\s*%\s*(accuracy|sensitivity|specificity|precision)/gi, type: 'statistical', confidence: 95 },
  { pattern: /\b(statistically\s+significant|CI\s+\d+%)/gi, type: 'statistical', confidence: 90 },
  
  // Manufacturing claims
  { pattern: /\b(GMP|cGMP|ISO\s*\d+|Good\s+Manufacturing)/gi, type: 'manufacturing', confidence: 90 },
  { pattern: /\bmanufactured?\s+(per|according\s+to|in\s+accordance)/gi, type: 'manufacturing', confidence: 85 },
  
  // Comparison claims
  { pattern: /\b(superior|better|improved|enhanced|comparable)\s+(to|than|with)/gi, type: 'comparison', confidence: 85 },
  { pattern: /\b(non-inferior|equivalent|similar)\s+(to|efficacy|safety)/gi, type: 'comparison', confidence: 90 },
];
...
function matchClaimToSources(
  ...
  const scored = candidates.map(source => {
    ...
    if (['efficacy', 'safety', 'clinical'].includes(claim.claimType) && 
...
const ClaimCard: React.FC<ClaimCardProps> = ({
  ...
  const colorClasses = {
    ...
    amber: 'bg-amber-100 dark:bg-amber-900/20 border-amber-200 dark:border-amber-800',
    rose: 'bg-rose-100 dark:bg-rose-900/20 border-rose-200 dark:border-rose-800',
  };
      ...
      <div className="p-4">
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-start gap-3 flex-1">
            <div className={`p-2 rounded-lg bg-white dark:bg-slate-800 ${iconColorClasses[config.color as keyof typeof iconColorClasses]}`}>
          ...
          <div className="mt-4 p-3 bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700">
            ...
            <div className="flex items-center gap-2 mt-3">
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 text-slate-600 dark:text-slate-400 text-sm hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg transition-colors"
              ...
              {suggestedSources.length > 1 && (
                <button
                  ...
                  className="flex items-center gap-1 text-xs text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 ml-auto"
          ...
          {suggestedSources.slice(1).map(source => (
            ...
            >
              <div className="flex-1 min-w-0 mr-3">
                ...
                <p className="text-xs text-slate-500">{source.relevanceScore}% match</p>
              </div>
...
const BulkLinkPanel: React.FC<BulkLinkPanelProps> = ({
  pendingLinks,
  onConfirmAll,
  onClear,
}) => {
  ...
  return (
    <div className="p-4 bg-green-50 dark:bg-green-900/20 rounded-xl border-2 border-green-200 dark:border-green-700">
      ...
      <p className="text-sm text-green-700 dark:text-green-300 mb-4">
        The AI has found matching sources for these claims. Click below to link them all at once.
      </p>
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          onClick={onClear}
          className="px-4 py-2 text-green-700 dark:text-green-300 hover:bg-green-100 dark:hover:bg-green-800/30 rounded-lg transition-colors"
...
export const AutoTraceabilityEngine: React.FC<AutoTraceabilityEngineProps> = ({
  ...
  const [claimSuggestions, setClaimSuggestions] = useState<Map<string, SourceSuggestion[]>>(new Map());
  ...
  const visibleClaims = useMemo(() => 
    detectedClaims.filter(c => 
      !dismissedClaims.has(c.id) && c.sourceStatus !== 'supported'
    ),
  ...
  // Handle linking
  const handleLink = useCallback((claim: SmartClaim, source: SourceSuggestion) => {
    setDetectedClaims(prev => prev.map(c =>
      c.id === claim.id
  ...
  return (
    <div className={`space-y-4 ${className}`}>
      {/* Header */}
      <div className="flex items-center justify-between p-4 rounded-2xl border border-slate-200/70 dark:border-slate-700/70 bg-white dark:bg-slate-800 shadow-sm">
        <div>
          <div className="flex items-center gap-2">
            <Sparkles className="w-5 h-5 text-blue-500" />
            <h3 className="font-semibold text-slate-800 dark:text-slate-200">Auto-Traceability</h3>
          </div>
      ...
      {visibleClaims.length === 0 ? (
        <div className="text-center py-10 text-slate-500 bg-white dark:bg-slate-800 rounded-2xl border border-slate-200/70 dark:border-slate-700/70 shadow-sm">
          <CheckCircle className="w-12 h-12 mx-auto mb-3 text-green-500" />
          <p className="font-medium text-slate-700 dark:text-slate-300">All claims sourced!</p>
