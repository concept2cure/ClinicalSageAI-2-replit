...
const MockCard: React.FC<{
  ...
  <div className={cn(
    'rounded-xl border border-zinc-200/60 bg-white/90 backdrop-blur-sm p-4 shadow-sm',
    className,
  )}>
  ...
  <div className="flex flex-col items-center justify-center h-full text-center px-8">
    ...
    <div className="grid grid-cols-4 gap-3 mt-8 w-full max-w-lg">
      {['FDA', 'EMA', 'PMDA', 'NMPA'].map((a) => (
        <div key={a} className="bg-zinc-50 rounded-lg py-2 px-3 text-xs font-medium text-zinc-700 flex items-center justify-center gap-1">
    ...
    <div className="grid grid-cols-3 gap-2 mt-4 w-full max-w-lg">
      {['15+ Submission Types', '12+ AI Agents', '102 CMC Endpoints'].map((s) => (
        <div key={s} className="bg-violet-50 rounded-lg py-2 px-3 text-[10px] font-medium text-violet-700 text-center">{s}</div>
      ))}
    </div>
  </div>
);
      ...
      <div className="overflow-hidden rounded-lg border border-zinc-100">
        <table className="w-full text-left">
          <thead><tr className="bg-zinc-50">
            <th className="px-3 py-2 text-[10px] font-medium text-zinc-500">Submission</th>
            <th className="px-3 py-2 text-[10px] font-medium text-zinc-500">Type</th>
            <th className="px-3 py-2 text-[10px] font-medium text-zinc-500">Agency</th>
            <th className="px-3 py-2 text-[10px] font-medium text-zinc-500">Status</th>
            <th className="px-3 py-2 text-[10px] font-medium text-zinc-500">Readiness</th>
          </tr></thead>
          <tbody>
...
const Sub510kPreview: React.FC = () => (
  <div className="space-y-4">
    <MockCard title="510(k) Predicate Research" icon={<Search className="w-4 h-4 text-blue-500" />}>
      <div className="space-y-2 mb-3">
        <div className="flex items-center gap-2 px-3 py-2 bg-zinc-50 rounded-lg text-xs">
          <Search className="w-3 h-3 text-zinc-400" />
          <span className="text-zinc-500">Search: "transcutaneous electrical nerve stimulator"</span>
        </div>
      </div>
        ...
        {[
          ...
          <div key={p.name} className="flex items-center justify-between p-3 bg-white border border-zinc-100 rounded-lg">
            <div>
              ...
              <p className="text-[10px] text-zinc-500">{p.manufacturer} — Product Code: {p.code}</p>
            </div>
    ...
    <MockCard title="SE Comparison Matrix" icon={<Layers className="w-4 h-4 text-violet-500" />}>
      <div className="grid grid-cols-3 gap-2">
        {['Intended Use', 'Technology', 'Performance', 'Biocompat.', 'Software', 'Labeling'].map((d) => (
          <div key={d} className="flex items-center gap-1 text-[10px] text-zinc-600">
...
const SubINDPreview: React.FC = () => (
  <div className="space-y-4">
    <MockCard title="IND Workspace — eCTD Modules" icon={<FolderOpen className="w-4 h-4 text-blue-500" />}>
      {[
        ...
        <div key={m.mod} className="mb-3">
          ...
          <p className="text-[10px] text-zinc-400 mt-0.5 pl-1">{m.items}</p>
        </div>
      ))}
    </MockCard>
  </div>
...
const SubGlobalPreview: React.FC = () => (
  <div className="space-y-4">
    <MockCard title="Multi-Agency Filing Status" icon={<Globe className="w-4 h-4 text-blue-500" />}>
      <div className="grid grid-cols-2 gap-3">
        {[
          ...
          <div key={a.agency} className="p-3 bg-zinc-50 rounded-lg">
            ...
            <p className="text-[10px] text-zinc-500">{a.type} — Region-specific Module 1</p>
          </div>
        ))}
      </div>
    </MockCard>
  ...
  <div className="space-y-4">
    <MockCard title="Structured Trial Results" icon={<Database className="w-4 h-4 text-blue-500" />}>
      <div className="overflow-hidden rounded-lg border border-zinc-100">
        <table className="w-full text-left">
          <thead><tr className="bg-zinc-50">
            <th className="px-3 py-2 text-[10px] font-medium text-zinc-500">Trial</th>
            <th className="px-3 py-2 text-[10px] font-medium text-zinc-500">Phase</th>
            <th className="px-3 py-2 text-[10px] font-medium text-zinc-500">Enrollment</th>
            <th className="px-3 py-2 text-[10px] font-medium text-zinc-500">Primary Endpoint</th>
          </tr></thead>
          <tbody>
    ...
    <MockCard title="Precedent Finder" icon={<Target className="w-4 h-4 text-violet-500" />}>
      {[
        ...
        <div key={p.name} className="flex items-center justify-between p-3 border border-zinc-100 rounded-lg mb-2">
          <div>
            <p className="text-xs font-medium text-zinc-800">{p.name}</p>
            <p className="text-[10px] text-zinc-500">{p.pathway} — {p.timeline} review</p>
          </div>
...
const ResEvidencePreview: React.FC = () => (
  <div className="space-y-4">
    <MockCard title="Evidence Manager" icon={<Database className="w-4 h-4 text-emerald-500" />}>
      ...
      <div className="space-y-2">
        {['Module 2.5 — Clinical Overview', 'Module 2.7 — Clinical Summary', 'Module 5.3 — Study Reports'].map((s) => (
          <div key={s} className="flex items-center justify-between p-2 bg-zinc-50 rounded-lg text-xs">
  ...
  <div className="space-y-4">
    <MockCard title="eCTD Co-Author" icon={<FileText className="w-4 h-4 text-blue-500" />}>
      ...
      <div className="flex gap-2">
        ...
        <StatusBadge status="v3.2" color="bg-zinc-100 text-zinc-600" />
      </div>
    </MockCard>
  </div>
);
...
const DocCSRPreview: React.FC = () => (
  <div className="space-y-4">
    <MockCard title="CSR Builder — ICH E3" icon={<FileCheck className="w-4 h-4 text-emerald-500" />}>
      {[
      ...
      <div className="grid grid-cols-2 gap-2">
        {[
          ...
          <div key={t.agency} className="p-3 bg-zinc-50 rounded-lg">
            ...
            <p className="text-[10px] text-zinc-500">{t.types}</p>
          </div>
        ))}
      </div>
    </MockCard>
...
const DocVaultPreview: React.FC = () => (
  <div className="space-y-4">
    <MockCard title="Document Vault" icon={<FolderOpen className="w-4 h-4 text-blue-500" />}>
      <div className="grid grid-cols-3 gap-3 mb-3">
      ...
      <div className="space-y-1">
        {[
          ...
          <div key={d.name} className="flex items-center justify-between p-2 bg-zinc-50 rounded-lg text-xs">
            <div>
              <span className="text-zinc-700">{d.name}</span>
              <span className="text-zinc-400 ml-2">{d.date}</span>
            </div>
            <StatusBadge status={d.status} color={d.status === 'Approved' ? 'bg-emerald-50 text-emerald-700' : d.status === 'In Review' ? 'bg-amber-50 text-amber-700' : 'bg-zinc-100 text-zinc-600'} />
          </div>
        ))}
      </div>
    </MockCard>
    ...
    <MockCard title="Document Sherpa" icon={<Eye className="w-4 h-4 text-violet-500" />}>
      <div className="space-y-2">
        {[
          ...
          { check: 'IFU consistency', status: 'Passed', icon: <CheckCircle className="w-3 h-3 text-emerald-500" /> },
          { check: 'Labeling compliance', status: 'Passed', icon: <CheckCircle className="w-3 h-3 text-emerald-500" /> },
          { check: 'Formatting (agency specs)', status: 'Passed', icon: <CheckCircle className="w-3 h-3 text-emerald-500" /> },
          { check: 'eCTD structure valid', status: 'Passed', icon: <CheckCircle className="w-3 h-3 text-emerald-500" /> },
        ].map((c) => (
          <div key={c.check} className="flex items-center justify-between p-2 bg-zinc-50 rounded-lg text-xs">
            <div className="flex items-center gap-2">
              {c.icon}
              <span className="text-zinc-700">{c.check}</span>
            </div>
...
const CMCBlueprintPreview: React.FC = () => (
  <div className="space-y-4">
    <MockCard title="CMC Platform" icon={<FlaskConical className="w-4 h-4 text-emerald-500" />}>
      <div className="grid grid-cols-3 gap-3 mb-3">
  ...
  <div className="space-y-4">
    <MockCard title="Analytical Methods & Validation" icon={<Microscope className="w-4 h-4 text-blue-500" />}>
      <div className="overflow-hidden rounded-lg border border-zinc-100">
        <table className="w-full text-left">
          <thead><tr className="bg-zinc-50">
            <th className="px-3 py-2 text-[10px] font-medium text-zinc-500">Method</th>
            <th className="px-3 py-2 text-[10px] font-medium text-zinc-500">Status</th>
            <th className="px-3 py-2 text-[10px] font-medium text-zinc-500">ICH Q2</th>
          </tr></thead>
          <tbody>
      ...
      {[
        ...
        <div key={s.step} className="flex items-center justify-between p-2 border-b border-zinc-50 last:border-0">
          <div>
            ...
            <p className="text-[10px] text-zinc-500">Scale: {s.scale}</p>
          </div>
      ...
      <div className="space-y-2">
        {[
          ...
          <div key={g.gap} className="flex items-start gap-2 p-2 bg-zinc-50 rounded-lg">
            ...
            <span className="text-[10px] text-zinc-700">{g.gap}</span>
          </div>
        ))}
      </div>
    </MockCard>
      ...
      <div className="grid grid-cols-3 gap-2">
        {[
          ...
          <div key={a.name} className="p-2 bg-zinc-50 rounded-lg text-center">
            ...
            <p className="text-[10px] font-medium text-zinc-700 truncate">{a.name}</p>
      ...
      <div className="grid grid-cols-2 gap-3">
        {[
          ...
          <div key={s.scenario} className="p-3 border border-zinc-200 rounded-lg">
            ...
            <div className="space-y-1">
              <p className="text-[10px] text-zinc-600">Approval probability: <span className="font-bold text-violet-700">{s.probability}</span></p>
              <p className="text-[10px] text-zinc-600">Est. timeline: {s.timeline}</p>
              <p className="text-[10px] text-zinc-600">Risk: {s.risk}</p>
            </div>
          </div>
        ))}
      </div>
...
const AIReviewPulsePreview: React.FC = () => (
  <div className="space-y-4">
    <MockCard title="Review Pulse" icon={<Activity className="w-4 h-4 text-emerald-500" />}>
      <div className="grid grid-cols-3 gap-3 mb-3">
      ...
      <div className="space-y-1">
        {[
          ...
          <div key={f.finding} className="flex items-center gap-2 p-2 bg-zinc-50 rounded-lg text-[10px]">
      ...
      <MockCard title="AnA 1.0" icon={<Sparkles className="w-4 h-4 text-violet-500" />}>
        ...
        <div className="space-y-1">
          {['Submission strategy', 'Document co-writing', 'Regulatory Q&A', 'Compliance checking'].map((f) => (
            <div key={f} className="flex items-center gap-1 text-[10px] text-zinc-600">
              <CheckCircle className="w-3 h-3 text-violet-500" /> {f}
            </div>
          ))}
        </div>
      </MockCard>
      <MockCard title="Dr. Sage" icon={<BookOpen className="w-4 h-4 text-emerald-500" />}>
    ...
    <MockCard title="Mission Control" icon={<LayoutDashboard className="w-4 h-4 text-violet-500" />}>
      ...
      <div className="space-y-1">
        {[
          ...
          <div key={p.name} className="flex items-center justify-between p-2 bg-zinc-50 rounded-lg text-xs">
            <span className="text-zinc-700">{p.name} ({p.projects} projects)</span>
            <StatusBadge status={p.status} color={p.status === 'On Track' ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-700'} />
          </div>
        ))}
      </div>
    </MockCard>
...
const PMTimelinesPreview: React.FC = () => (
  <div className="space-y-4">
    <MockCard title="Project Timeline" icon={<Clock className="w-4 h-4 text-blue-500" />}>
      {[
...
const PMTasksPreview: React.FC = () => (
  <div className="space-y-4">
    <MockCard title="Task Board" icon={<BarChart3 className="w-4 h-4 text-blue-500" />}>
      <div className="grid grid-cols-3 gap-2">
        {[
          ...
          <div key={c.col} className="bg-zinc-50 rounded-lg p-2">
            <p className="text-[10px] font-semibold text-zinc-600 mb-2">{c.col}</p>
            {c.items.map((item) => (
              <div key={item} className="bg-white rounded p-1.5 mb-1 text-[10px] text-zinc-700 border border-zinc-100 shadow-sm">{item}</div>
            ))}
          </div>
        ))}
      </div>
...
const PMDecisionsPreview: React.FC = () => (
  <div className="space-y-4">
    <MockCard title="Decision Log" icon={<Star className="w-4 h-4 text-amber-500" />}>
      {[
        ...
        <div key={d.decision} className="p-2 border-b border-zinc-50 last:border-0">
          ...
          <div className="flex gap-2 mt-1">
            ...
            <span className="text-[10px] text-emerald-600">{d.signers} sign-offs</span>
          </div>
        </div>
      ))}
    </MockCard>
...
const PMGapsPreview: React.FC = () => (
  <div className="space-y-4">
    <MockCard title="Submission Gap Analysis" icon={<AlertTriangle className="w-4 h-4 text-amber-500" />}>
      ...
      {[
        ...
        <div key={g.gap} className="flex items-start gap-2 p-2 bg-zinc-50 rounded-lg mb-1">
          ...
          <div>
            <p className="text-[10px] font-medium text-zinc-700">{g.module}: {g.gap}</p>
  ...
  <div className="space-y-4">
    <MockCard title="21 CFR Part 11 — Audit Trail" icon={<Shield className="w-4 h-4 text-blue-500" />}>
      <div className="overflow-hidden rounded-lg border border-zinc-100">
        <table className="w-full text-left">
          <thead><tr className="bg-zinc-50">
            <th className="px-3 py-2 text-[10px] font-medium text-zinc-500">Action</th>
            <th className="px-3 py-2 text-[10px] font-medium text-zinc-500">User</th>
            <th className="px-3 py-2 text-[10px] font-medium text-zinc-500">Timestamp</th>
            <th className="px-3 py-2 text-[10px] font-medium text-zinc-500">Hash</th>
          </tr></thead>
          <tbody>
        ...
        {[
          ...
          <div key={r.role} className="flex items-center justify-between p-2 bg-zinc-50 rounded-lg">
            <div>
              ...
              <p className="text-[10px] text-zinc-500">{r.access}</p>
            </div>
            <span className="text-[10px] text-zinc-400">{r.count} users</span>
          </div>
        ))}
      </div>
    </MockCard>
...
const SecInspectionPreview: React.FC = () => (
  <div className="space-y-4">
    <MockCard title="Inspection Readiness" icon={<Eye className="w-4 h-4 text-emerald-500" />}>
      <div className="grid grid-cols-2 gap-3 mb-3">
        <MetricBox label="Readiness Score" value="94%" trend="Inspection-ready" />
        <MetricBox label="Open Items" value="2" />
      </div>
      {[
        { item: 'Audit trail export', status: 'Ready', icon: <CheckCircle className="w-3 h-3 text-emerald-500" /> },
        { item: 'Document staging', status: 'Ready', icon: <CheckCircle className="w-3 h-3 text-emerald-500" /> },
        { item: 'E-signature log', status: 'Ready', icon: <CheckCircle className="w-3 h-3 text-emerald-500" /> },
        { item: 'Training records', status: 'Pending', icon: <Clock className="w-3 h-3 text-amber-500" /> },
      ].map((i) => (
        <div key={i.item} className="flex items-center gap-2 p-2 bg-zinc-50 rounded-lg mb-1 text-xs">
          {i.icon}
          <span className="text-zinc-700 flex-1">{i.item}</span>
...
const CollabHubPreview: React.FC = () => (
  <div className="space-y-4">
    <MockCard title="Collaboration Hub" icon={<Users className="w-4 h-4 text-blue-500" />}>
      {[
        ...
        <div key={t.thread} className="flex items-center justify-between p-2 border-b border-zinc-50 last:border-0">
          <div className="flex items-center gap-2">
            {t.unread && <div className="w-2 h-2 rounded-full bg-violet-500" />}
            <div>
              <p className="text-xs text-zinc-800">{t.thread}</p>
              <p className="text-[10px] text-zinc-400">{t.replies} replies — {t.last}</p>
            </div>
          </div>
...
const CollabReviewPreview: React.FC = () => (
  <div className="space-y-4">
    <MockCard title="Review Queue" icon={<FileCheck className="w-4 h-4 text-emerald-500" />}>
      {[
        ...
        <div key={r.doc} className="flex items-center justify-between p-2 border-b border-zinc-50 last:border-0">
          <div>
            ...
            <p className="text-[10px] text-zinc-500">{r.reviewer} — Due: {r.due}</p>
          </div>
    ...
    <MockCard title="Report Center" icon={<BarChart3 className="w-4 h-4 text-violet-500" />}>
      <div className="grid grid-cols-3 gap-2 mb-3">
        {['PDF', 'Word', 'eCTD XML'].map((f) => (
          <div key={f} className="bg-zinc-50 rounded-lg py-2 text-center text-[10px] font-medium text-zinc-600">{f} Export</div>
        ))}
      </div>
      <div className="space-y-1">
        {[
          ...
          <div key={r} className="flex items-center gap-2 p-2 bg-zinc-50 rounded-lg text-[10px] text-zinc-700">
...
const CollabPostmarketPreview: React.FC = () => (
  <div className="space-y-4">
    <MockCard title="Post-Market Surveillance" icon={<Activity className="w-4 h-4 text-red-500" />}>
      <div className="grid grid-cols-3 gap-3 mb-3">
      ...
      <div className="space-y-1">
        {[
          ...
          <div key={i.item} className="flex items-center justify-between p-2 bg-zinc-50 rounded-lg text-xs">
            <span className="text-zinc-700">{i.item}</span>
...
const CollabAcademyPreview: React.FC = () => (
  <div className="space-y-4">
    <MockCard title="Academy & Enablement" icon={<BookOpen className="w-4 h-4 text-emerald-500" />}>
      <div className="grid grid-cols-2 gap-3 mb-3">
