...
import { 
  Eye, EyeOff, Mail, Lock, Building2, ChevronRight, Shield, 
  CheckCircle2, Fingerprint, X, Search, Clock, AlertCircle,
  Loader2, Smartphone, Key, ExternalLink, Users, Globe
} from 'lucide-react';
 ...
 */
const DNAHelix = () => (
  <svg viewBox="0 0 100 200" className="w-32 h-64 opacity-20 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
    <defs>
    ...
    {[0, 1, 2, 3, 4, 5, 6, 7].map((i) => (
      <g key={i} style={{ animation: `pulse 2s ease-in-out ${i * 0.2}s infinite` }}>
        <ellipse cx="50" cy={20 + i * 22} rx="35" ry="8" fill="none" stroke="url(#dnaGradient)" strokeWidth="2" />
        <line x1={50 - 30 * Math.cos(i * 0.8)} y1={20 + i * 22 - 6} 
              x2={50 + 30 * Math.cos(i * 0.8)} y2={20 + i * 22 + 6} 
              stroke="url(#dnaGradient)" strokeWidth="1.5" />
      </g>
    ))}
    <style>{`
      @keyframes pulse {
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mt-1 flex-wrap">
            <span className={`text-xs px-2 py-0.5 rounded-full ${typeColors[type] || 'bg-gray-100 text-gray-700'}`}>
              {type}
            </span>
            <span className="text-xs bg-slate-100 text-slate-600 px-2 py-0.5 rounded-full">{role}</span>
          </div>
          {lastAccessed && (
...
export default function TrialSageSignOn() {
  ...
  const checkSSODomain = useCallback(async (emailAddress) => {
    ...
    try {
      ...
      const response = await fetch(`/api/auth/enterprise/check-sso-domain?domain=${encodeURIComponent(domain)}`, {
        method: 'GET',
        credentials: 'include',
      });
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 429 || data.lockoutSeconds) {
          ...
          throw new Error(`Account temporarily locked. Try again in ${Math.ceil((data.lockoutSeconds || 900) / 60)} minutes.`);
        }
        
        // Handle remaining attempts warning
        if (data.remainingAttempts !== undefined && data.remainingAttempts <= 2) {
          throw new Error(`Invalid credentials. ${data.remainingAttempts} attempts remaining before lockout.`);
        }
  ...
  const filteredOrgs = (organizations.length > 0 ? organizations : fallbackOrganizations)
    .filter(org => 
  ...
  const renderPasswordStep = () => (
    <form onSubmit={handlePasswordSubmit} className="space-y-6">
      ...
      <div className="flex items-center gap-2 text-sm text-gray-600 bg-gray-50 rounded-xl p-3.5 border border-gray-100">
        ...
        <button 
          type="button" 
          onClick={() => { setStep(AUTH_STEPS.EMAIL); setPassword(''); setError(''); }}
  ...
  const renderMFAStep = () => (
    <form onSubmit={handleMFASubmit} className="space-y-6">
      ...
      <div className="text-center">
        ...
        <h3 className="text-lg font-semibold text-[#1a2e4a] mb-1">
  ...
  const renderSSORedirect = () => (
    <div className="text-center py-8">
      ...
      <button
        type="button"
        onClick={() => { setStep(AUTH_STEPS.EMAIL); setSsoProvider(null); }}
      ...
      <div className="hidden lg:flex lg:w-[42%] bg-gradient-to-br from-[#1a2e4a] via-[#1a2e4a] to-[#0f1a2a] relative overflow-hidden flex-col justify-between p-10">
        ...
        <div className="relative z-10 flex-1 flex flex-col justify-center max-w-lg">
          ...
          <p className="text-white/70 text-base leading-relaxed">
            The enterprise platform for regulatory authoring, impact analysis, and submission intelligence.
          </p>
        ...
        <div className="w-full max-w-md">
          ...
          <div className="mb-8">
            <h2 className="text-2xl font-bold text-[#1a2e4a] mb-2">
              {step === AUTH_STEPS.SUCCESS ? 'Welcome back!' : 
               step === AUTH_STEPS.MFA ? 'Verify your identity' :
              ...
              {filteredOrgs.some(org => org.lastAccessed) && (
                <>
                  ...
                  <div className="space-y-3 mb-6">
                    {filteredOrgs.filter(org => org.lastAccessed).map((org) => (
                      <OrgCard
                        key={org.id}
                        {...org}
