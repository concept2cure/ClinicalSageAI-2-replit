  ...
  return (
    <Card>
      ...
      <CardContent className="text-sm space-y-2 max-h-80 overflow-y-auto">
        {logs.length === 0 ? (
          <div className="text-stone-600">No decisions logged yet</div>
        ) : (
          logs.map((log: any, i: number) => (
            <div key={i} className="p-2 bg-stone-50 rounded border-l-4 border-stone-900">
              ...
              {log.explain && <div className="text-xs text-stone-600 mb-1">{log.explain}</div>}
              <details className="text-xs">
                <summary className="cursor-pointer text-stone-600">View details</summary>
                <pre className="mt-1 p-1 bg-stone-100 rounded text-xs overflow-x-auto">
