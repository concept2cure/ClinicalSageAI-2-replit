...
export async function initializeProofDatabasePersistence(): Promise<void> {
  ...
  auditService.setDatabasePersister(async (entry: ProofAuditEntry) => {
    try {
      await database.insert(proofAuditLogs).values({
        entryId: entry.id,
  ...
  logger.info('proof.system.initialized', {
    message: 'Database persistence configured for proof audit system',
  });
}
...
export async function verifyDatabaseAuditChain(organizationId?: number): Promise<{
  ...
  try {
    ...
    const entries = await db
      .select()
      .from(proofAuditLogs)
      .where(organizationId ? eq(proofAuditLogs.organizationId, organizationId) : undefined)
