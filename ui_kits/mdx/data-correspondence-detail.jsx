/* data-correspondence-detail.jsx
   ─────────────────────────────────────────────────────────────
   Full letter bodies, deficiency decomposition, and the AnA-
   drafted response *as the user would actually receive them*
   from the agency / NB. These attach to the existing K510_CORRESP
   / PMA_CORRESP / CER_CORRESP rows by id, so the AnA Drafter has
   real structured input.

   Shape per item:
     letter:   { from_name, from_title, from_office, ref, dated,
                 our_ref, body: [paragraph...], signature: {...} }
     deficiencies: [
       { id, n, title, body, refs: [{ section, label }],
         regs: [{ ref, title }], severity, complete }
     ]
     draft: {
       status: 'unstarted' | 'drafted' | 'in_review' | 'approved' | 'sent',
       generated_at, generated_by: 'AnA',
       intro, deficiencies: [{ id, response, evidence: [...], updates: [...] }],
       closing,
       reviewers: [{ role, name, status, signed_at? }],
     }
   ───────────────────────────────────────────────────────────── */

const CORRESP_DETAIL = {
...
window.CORRESP_DETAIL = CORRESP_DETAIL;
