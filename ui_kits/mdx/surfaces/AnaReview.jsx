...
const { I } = window;
const { ARQ_KPIS, ARQ_FILTERS, ARQ_DRAFTS } = window;

function AnaReviewSurface({ program, onAskAna }) {
    ...
    <>
      <div className="page-header">
        ...
        <div className="page-actions">
          <button className="btn ghost small" disabled={selected.size === 0} onClick={() => onAskAna(`Refine ${selected.size} selected drafts — apply consistent reviewer comments to each.`)}>
            {I.pencil} Refine selected ({selected.size})
          </button>
          <button className="btn primary small" disabled={selected.size === 0} onClick={() => onAskAna(`Accept ${selected.size} selected drafts. Open the e-signature flow once per draft.`)}>
...
window.AnaReviewSurface = AnaReviewSurface;
