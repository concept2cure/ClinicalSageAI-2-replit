...
/* ═════════════════════════════════════════════════════════════════
   ...
   FDA Q-Submission program (Pre-Sub, Submission Issue Request,
   Study Risk Determination, Agreement, Informational meetings).
   Mirrors the conversation-with-FDA before a 510(k) gets filed.
   ═════════════════════════════════════════════════════════════════ */
...
/* Q-Sub types per CDRH guidance "Requests for Feedback…" Sep 2023 rev. */
window.PRESUB_TYPES = [
...
/* Stages a Q-Sub moves through. The list rendering uses these to colour the strip. */
window.PRESUB_STAGES = [
...
/* Active Q-Subs across the portfolio. Anchored to the existing programs
   in data.jsx (BX-204, OR-801, RX-340, PM-660, NM-512) so the live-dossier
   linking actually resolves. */
window.PRESUB_LIST = [
  {
...
/* Per-Q-Sub detail — questions, FDA written response, meeting log,
   commitments. Keyed by Q-Sub id. */
window.PRESUB_DETAIL = {

  /* Q251142 — BX-204 predicate / AI scope. The flagship: meeting completed,
     all questions answered, 3 commitments captured, 2 already rolled into
     the 510(k) dossier. */
  'qs-2025-1142': {
    summary: 'BX-204 sought CDRH agreement on predicate K212284 (Dexcom G6) and locking the AI/ML algorithm scope ahead of a Q3 510(k) filing. Meeting June 3 — written minutes received June 12.',
    questions: [
      {
        n: 1,
        q: 'Does FDA agree K212284 is a suitable primary predicate, given the AI-derived trend arrows added to BX-204?',
        ourPosition: 'K212284 same intended use, technology, target population. AI-derived features are an output enhancement and do not change indications.',
        fdaResponse: 'FDA agrees K212284 is appropriate as the primary predicate. Recommend including K223341 as a reference predicate to address the trend-arrow technology. Sponsor should provide a Technology Difference Justification per §6 of the eSTAR.',
        status: 'answered',
        commitment: { id: 'cm-1', text: 'Add K223341 as reference predicate; draft Technology Difference Justification', dossierLink: { kind: 'k510-section', label: 'eSTAR §6.1', sectionId: 6 }, rolledIn: true }
      },
      {
        n: 2,
        ...
        fdaResponse: 'Locked algorithm acceptable. Provide an algorithm-change protocol in the device description that defines what triggers a new submission. Note: any model retraining post-clearance requires new 510(k).',
        status: 'answered',
        commitment: { id: 'cm-2', text: 'Author algorithm-change protocol (device description §4.3)', dossierLink: { kind: 'k510-section', label: 'eSTAR §4.3', sectionId: 4 }, rolledIn: true }
      },
      {
        n: 3,
        ...
        fdaResponse: 'Sample size adequate for primary endpoint. Recommend pre-specifying subgroup analysis for hypoglycemia (<70 mg/dL) — current SAP only includes overall MARD. FDA will not accept post-hoc subgroup claims.',
        status: 'answered',
        commitment: { id: 'cm-3', text: 'Amend SAP to pre-specify hypo-MARD subgroup', dossierLink: { kind: 'k510-section', label: 'eSTAR §11 — Performance', sectionId: 11 }, rolledIn: false, blocker: true }
      },
      {
        n: 4,
        ...
        fdaResponse: 'HF validation is required per FDA HF guidance (Feb 2016). Sample size and methodology proposed are acceptable. Confirm task list maps to the use-related risk analysis from §10.',
        status: 'answered',
        commitment: null
      },
      {
        ...
        fdaResponse: 'FDA cautions that "complementary to insulin therapy decisions" approaches a dosing claim and would require a separate IDE-supported study. Recommend "trend monitoring and pattern recognition; not for use in insulin dosing decisions" as labeled indication.',
        status: 'answered',
        commitment: { id: 'cm-5', text: 'Restrict labeled indication per FDA recommendation', dossierLink: { kind: 'k510-section', label: 'eSTAR §3 — Indications for Use', sectionId: 3 }, rolledIn: false }
      },
    ],
    timeline: [
  ...
  'qs-2025-0987': {
    summary: 'OR-801 requested a fatigue testing waiver based on equivalence to predicate K191822 — same alloy, identical thread geometry, validated dynamic compression bending data on file. Filed Feb 4, 75-day target Apr 20.',
    questions: [
      {
        n: 1,
        ...
        ourPosition: 'Identical alloy spec ASTM F136. Thread geometry from K191822 reused. Dynamic compression bending data on file from supplier qualification.',
        fdaResponse: null, status: 'awaiting', commitment: null
      },
      {
        n: 2,
  ...
  'qs-2025-0844': {
    summary: 'Mid-review Submission Issue Request raised after FDA AI letter on the OR-801 510(k). FDA asked for extractables and leachables under ISO 10993-18. Response received, supplier-tested data accepted with a documentation update.',
    questions: [
      {
        n: 1,
        ...
        fdaResponse: 'Supplier extractables data is acceptable for the OR-801 device given identical alloy and surface treatment. Sponsor must include the supplier qualification audit trail and confirm lot-traceability in the test article statement.',
        status: 'answered',
        commitment: { id: 'cm-844-1', text: 'Add lot-traceability statement and supplier audit trail to §15 — Biocompatibility', dossierLink: { kind: 'k510-section', label: 'eSTAR §15', sectionId: 15 }, rolledIn: true }
      },
      {
        n: 2,
        ...
        fdaResponse: '24-hour extraction acceptable. Confirm clinical exposure duration in §3 matches the biocompatibility test article statement.',
        status: 'answered',
        commitment: null
      },
    ],
