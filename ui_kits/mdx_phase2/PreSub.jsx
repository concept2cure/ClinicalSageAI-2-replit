/* global React, I, PRESUB_KPIS, PRESUB_TYPES, PRESUB_STAGES, PRESUB_LIST, PRESUB_DETAIL */
/* ═════════════════════════════════════════════════════════════════
   Pre-Sub manager — 510(k) Q-Submission tracker.

   Three rows:
   1. Page header — eyebrow + title + actions (New Q-Sub)
   2. Metric strip — KPIs
   3. Filter row + list/detail two-pane

   The detail pane uses tabs: Questions · Timeline · Commitments. Each
   commitment links to the live 510(k) dossier (eSTAR section) — that's
   the cross-cutting "live dossier" promise (Gap 6 partial).
   ═════════════════════════════════════════════════════════════════ */

var { useState: usePS } = React;

function PreSubTypeChip({ type }) {
  const t = PRESUB_TYPES.find(x => x.id === type) || PRESUB_TYPES[0];
  return <span className={`ps-type ps-type-${t.id}`}>{t.label}</span>;
}

function PreSubStageStrip({ stage }) {
...
function PreSubRow({ row, selected, onPick }) {
...
function PreSubQuestion({ q, onJumpToDossier }) {
  return (
    <div className="ps-q" data-status={q.status}>
      ...
      {q.commitment && (
        <div className="ps-q-commit" data-rolled={q.commitment.rolledIn || undefined} data-blocker={q.commitment.blocker || undefined}>
          ...
          <div className="ps-q-commit-r">
            {q.commitment.dossierLink && (
              <button className="ps-q-commit-link" onClick={() => onJumpToDossier && onJumpToDossier(q.commitment.dossierLink)}>
                <span>{q.commitment.dossierLink.label}</span>
                <span className="ico">{I.right}</span>
              </button>
            )}
            <span className={`ps-q-commit-state ${q.commitment.rolledIn ? 'ok' : q.commitment.blocker ? 'err' : 'warn'}`}>
...
function PreSubDetail({ row, onJumpToDossier, onAskAna }) {
  const detail = PRESUB_DETAIL[row.id];
  const [tab, setTab] = usePS('questions');
    ...
    <div className="ps-detail">
      <div className="ps-detail-head">
        <div>
          ...
          <h3 className="ps-detail-title">{row.title}</h3>
          <div className="ps-detail-target">FDA team · {row.fdaTeam}{row.filed && ` · Filed ${row.filed}`}{row.targetDate && ` · Target ${row.targetDate}`}</div>
        </div>
        <div className="ps-detail-actions">
          <button className="btn ghost small" onClick={() => onAskAna && onAskAna(`Summarize ${row.qNumber} commitments`)}>
      ...
      <div className="ps-tabs">
        ...
        <button className="ps-tab" data-on={tab === 'commitments'} onClick={() => setTab('commitments')}>
          Commitments <span className="ps-tab-n">
            {detail.questions.filter(q => q.commitment).length}
          </span>
        </button>
      </div>
      ...
      {tab === 'questions' && (
        <div className="ps-q-list">
          {detail.questions.map(q => (
            <PreSubQuestion key={q.n} q={q} onJumpToDossier={onJumpToDossier}/>
          ))}
        </div>
      )}
      ...
      {tab === 'commitments' && (
        <div className="ps-commit-table">
          {detail.questions.filter(q => q.commitment).length === 0 && (
            <div className="ps-empty">
          ...
          {detail.questions.filter(q => q.commitment).map(q => (
            <div key={q.commitment.id} className="ps-commit-row" data-rolled={q.commitment.rolledIn || undefined} data-blocker={q.commitment.blocker || undefined}>
              <div className="ps-commit-id mono">{q.commitment.id}</div>
              <div className="ps-commit-text">{q.commitment.text}</div>
              <div className="ps-commit-from">From Q{q.n}</div>
              <button className="ps-commit-jump" onClick={() => onJumpToDossier && onJumpToDossier(q.commitment.dossierLink)}>
                {q.commitment.dossierLink.label} {I.right}
              </button>
              <span className={`ps-commit-state ${q.commitment.rolledIn ? 'ok' : q.commitment.blocker ? 'err' : 'warn'}`}>
                {q.commitment.rolledIn ? 'Rolled in' : q.commitment.blocker ? 'Blocker' : 'Pending'}
              </span>
            </div>
          ))}
...
function PreSubSurface({ onAskAna, onJumpToDossier }) {
  const [filter, setFilter] = usePS('all');
  const [stageFilter, setStageFilter] = usePS('all');
  const [selected, setSelected] = usePS('qs-2025-1142');

  const filtered = PRESUB_LIST.filter(r => {
    if (filter === 'mine') return ['BX-204','OR-801'].includes(r.prog); /* demo — assigned to JC */
    if (filter !== 'all' && filter !== 'mine') return r.type === filter;
    return true;
    ...
    <>
      <div className="page-header">
        <div>
          ...
          <div className="page-sub">FDA Q-Submission program — Pre-Sub, SIR, Study Risk, Agreement. Track questions, FDA responses, and commitments back to the live dossier.</div>
        </div>
      ...
      <section className="section">
        <div className="section-head">
          ...
          <span className="section-sub">{PRESUB_LIST.length} total · {PRESUB_LIST.filter(r => r.stage === 'await').length} awaiting FDA · {PRESUB_LIST.filter(r => r.stage === 'feedback').length} feedback received</span>
        </div>
        ...
        <div className="ps-filter-row">
          <div className="seg small">
            ...
            {PRESUB_TYPES.map(t => (
              <button key={t.id} className="seg-btn" data-on={filter === t.id} onClick={() => setFilter(t.id)}>{t.label}</button>
            ))}
          </div>
          <div className="ps-filter-spacer"/>
          <div className="seg small">
            <button className="seg-btn" data-on={stageFilter === 'all'} onClick={() => setStageFilter('all')}>Any stage</button>
            {PRESUB_STAGES.map(s => (
              <button key={s.id} className="seg-btn" data-on={stageFilter === s.id} onClick={() => setStageFilter(s.id)}>{s.label}</button>
            ))}
          </div>
        </div>
...
Object.assign(window, { PreSubSurface });
