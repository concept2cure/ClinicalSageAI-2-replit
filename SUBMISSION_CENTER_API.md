...
| POST | `/api/submissions/sequences/:seqId/dispatch` | `GovernedTransitionRequest` → `EctdSequence` | Same governance as freeze, applying `frozen→dispatched` (sets `dispatchStatus='pending'`). Records dispatch intent; the actual transmission to the agency gateway stays behind the governed `transmit_submission` tool. |
