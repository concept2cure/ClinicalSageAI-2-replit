...
// Mount Enhanced 510(k) Workflow routes
try {
  const enhanced510kModule = await import('./routes/enhanced-510k-routes.js');
  const enhanced510kRoutes = enhanced510kModule.default;
  app.use('/api/510k', enhanced510kRoutes);
  console.log('✅ Enhanced 510(k) Workflow API routes mounted successfully');
} catch (error) {
  console.error('❌ Failed to mount Enhanced 510(k) routes:', error);
}
...
// Mount Demo Data routes for client presentations
try {
  const demoDataModule = await import('./routes/demoDataRoutes.js');
  const demoDataRoutes = demoDataModule.default;
  app.use(demoDataRoutes);
  console.log('✅ Demo Data API routes mounted successfully (client presentations ready)');
} catch (error) {
  console.error('❌ Failed to mount Demo Data routes:', error);
}
...
async function startServer() {
  ...
  // Redirect root to the client portal; Vite middleware (dev) or static serving (prod) will render the client
  ...
  server.listen(PORT, '0.0.0.0', () => {
    ...
    // PROXY DISABLED TO PREVENT CRASHES
    /*
    const proxyPort = 2000;
    try {
      // Create proxy server instance but don't listen yet
      const proxy = createServer((req, res) => {
        ...
        proxyReq.on('error', (err) => {
          // Silent fail on proxy connection errors
          try { res.writeHead(502).end('Bad Gateway'); } catch (e) {}
        });
      });

      // Handle server-level errors (like EADDRINUSE) before listening
      proxy.on('error', (err) => {
        console.warn(`Could not start preview proxy on port ${proxyPort}:`, err.message || err);
      });

      proxy.listen(proxyPort, '0.0.0.0', () => {
        console.log(`✅ Proxy listening on port ${proxyPort} and forwarding to ${PORT}`);
      });
    } catch (err) {
      console.warn('Preview proxy startup skipped:', err?.message || err);
    }
    */
  });
