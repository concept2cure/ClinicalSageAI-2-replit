...
router.get('/', async (req, res) => {
  try {
    // Try the full select first (newer schema)
    let result;
    ...
    try {
      ...
      // If the 'slug' column is missing (older schema), retry without it and add a null slug
      if (innerErr && innerErr.code === '42703') {
        console.warn('Database missing column "slug". Retrying tenants query without slug.');
        slugMissing = true;
        try {
          result = await sql`
            SELECT id, name, domain, logo, tier, max_users as "maxUsers", 
                   max_projects as "maxProjects", max_storage as "maxStorage", 
                   status, created_at as "createdAt", updated_at as "updatedAt"
            FROM organizations 
            ORDER BY created_at DESC
          `;
        } catch (innerErrDomain) {
          if (innerErrDomain && innerErrDomain.code === '42703') {
            console.warn('Database missing column "domain". Retrying tenants query without domain.');
            domainMissing = true;
            result = await sql`
              SELECT id, name, logo, tier, max_users as "maxUsers", 
                     max_projects as "maxProjects", max_storage as "maxStorage", 
                     status, created_at as "createdAt", updated_at as "updatedAt"
              FROM organizations 
              ORDER BY created_at DESC
    ...
    const normalizedResult = result.map(r => ({
      ...r,
      slug: slugMissing ? generateSlug(r.name) : r.slug ?? generateSlug(r.name),
      domain: domainMissing ? null : 'domain' in r ? (r as any).domain ?? null : null,
    }));
